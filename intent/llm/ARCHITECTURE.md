...
| `lib/templates/`                 | Template source. `llm/` and `prj/` are also embedded in the binary for `init`; at runtime `claude upgrade`, `agents sync` and the hooks read every template, `llm/` included, from the install.                                                                                                                                                          |
...
| `intent/.canon/`                 | The committed extract of the store (`st/`, `issues/`, `events/`, `project.json`).                                                                                                                                                                                                                                                                        |
...
| `intent/llm/`                    | LLM-facing project documents: `MODULES.md` (searched with `intent modules find <name>`, never read whole), `DECISION_TREE.md`, `RULES.md`, `ARCHITECTURE.md`. Consulted on demand; nothing loads them into a session.                                                                                                                                    |
| `native/macos/`                  | The macOS menubar app (a Swift / Xcode project); `intent app` starts, stops and reports it (`intentsvcs::macapp`).                                                                                                                                                                                                                                       |
...
All generated content -- AGENTS.md, CLAUDE.md, settings.json, hook scripts -- comes from `lib/templates/`: `init` renders from the copies of `llm/` and `prj/` embedded at build time, and `canon.rs` / `rootfiles.rs` read templates, `llm/` included, from the install at runtime, expanding placeholders in Rust. There are no parallel copies of template content. This is `IN-AG-HIGHLANDER-001` (Highlander) concretised: one source, many use sites.

### Canon-apply via the canon-installer

`intent claude upgrade` reports what canon would change; `--apply` writes it (`--force` overwrites user-edited canon: `CLAUDE.md` without its marker, `.claude/settings.json` without Intent's hook door, and `.intent_critic.yml`; `--skip-settings` leaves `.claude/settings.json` and `.mcp.json` alone). The artefacts it owns: `.claude/settings.json`, `CLAUDE.md` / `AGENTS.md` (rendered from `lib/templates/llm/_*`), `usage-rules.md`, `.mcp.json` and `.intent_critic.yml` (seeded only when absent), and, in the hooks directory `git rev-parse --git-path hooks` names, the `pre-commit.intent` carrier with the `pre-commit` chain block and the `post-merge`, `post-checkout` and `post-rewrite` carriers with theirs (`canon.rs`).

Auto-insert mechanisms (the chain blocks) use marker pairs (`intent-chain-block:start` / `:end`): re-application rewrites only the lines between them to canon's block, writes nothing when the block is already canon's, and passes every line outside the markers through byte for byte; a hook whose block is doubled, unclosed or carries the retired marker is held and named.
...
`intent critic <lang>` is the headless runner (`intentsvcs::critic`, no LLM); `intent critic --languages` lists the languages it serves. Exit codes: 0 clean, 1 findings, 2 the runner could not answer (a usage error, an unreadable file, an empty rule library -- the gate fails open), 3 an armed rule's tool is absent (blocks). The LLM critics are subagents (`critic-<lang>`, `critic-prose`), invoked as `Task(subagent_type="critic-<lang>", prompt="review <paths>")`; each reads the rule library at invocation and applies the Detection heuristics.

The pre-commit gate is `lib/templates/hooks/pre-commit.sh`, run live from the Intent install and never copied into a project: `intent claude upgrade --apply` writes the shim `lib/templates/hooks/pre-commit-shim.sh` to `pre-commit.intent` in the git hooks directory and inserts a chain block into that directory's `pre-commit`, and the shim reads the install root from `~/.local/share/intent/home` and execs the gate (`intentsvcs/src/canon.rs` `install_carrier`). The gate runs the guard roster (`lib/templates/hooks/pre-commit-guards.sh`, read from this checkout when it is an Intent source tree and from the install otherwise), then `intent critic <lang> --staged --severity-min <threshold>` for each declared language, then `intent doctor`. It blocks on a failing guard, on critic exit 1 or 3 and on doctor exit 1, and fails open on any other code. The threshold is `.intent_critic.yml`'s `severity_min` (default `warning`; `style` when only `show_all: true` is set).
