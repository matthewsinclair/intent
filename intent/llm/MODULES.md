...
| Acceptance AC/AT mgmt   | `bin/intent_acceptance` | ac list/status/satisfy, at list/red/green/na (+ done/notdone aliases); close-gate precondition for st/wp done; reads/writes acceptance.md only (ST0044). THE field seam: `extract_field` (partial extraction -- prints nothing + returns 1 on non-match; every AC/AT field reader is one line over it) and `assert_written` (post-state write verification, reusing those same readers) -- issues 0006 + 0007                                                                                                                                                                                                                                        |
