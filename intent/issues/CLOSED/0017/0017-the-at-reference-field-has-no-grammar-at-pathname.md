...
### The home estate was swept with the pre-fix migrator, measured, and restored (vc F6; hv-ratified)

The `f28938c` sweep of this repo's own 116 rows ran the THEN-LOSSY `--fix`: 87 AT rows lost the test name riding their reference span (`::name`, a trailing quoted name, or a parenthesised one). For completed threads the loss was permanent by our own ruling -- the L3 id-backfill is archaeology-not-done, so the stripped name was the only link those rows would ever have. Measured after the fact: with the L3 exemption lifted, 66 of 104 completed-thread green rows had no link, 62 of them bare. The multi-citation damage class did NOT occur here: the one true `+` row was hand-split losslessly, and the 13 hand-fixed rows preserved their names in notes -- the exact pattern the corrected diagnostic now recommends.

Restored 2026-08-14 from git (`f28938c^`), mechanically: every destroyed name carried verbatim into its row's trailing `-- test:` note (never parsed, grammar-legal). Plan-then-apply with the plan reviewed; idempotent (a second plan pass finds 0 to restore, 87 present); estate lint clean after; the bare link-less count fell 62 -> 8, and those 8 never carried a name in the pre-sweep record -- nothing was lost on them, and nothing was invented for them. The directive to strip (`at lint --fix strip ::name suffixes`) was the vc work order's own delta 1a, passed again by the vc U1-U5 audit, and caught by a consumer measuring a real sweep: the reviewer's defect survived two layers of the reviewer's review.
