...
## Resolution -- CLOSED 2026-08-17, VERIFIED BY RUNNING v2's OWN LINTER OVER v3's OWN OUTPUT

**The pairing this issue exists to force has now been run, and it had never been run before.** cc landed the fix at `d14cd0b5`; the check below is vc's, measured at `55e540df`, and it is the first time v3's generated bytes have been handed to the tool that rejects them.

### The measurement

A v2.19.0 project (`intent init`, `st new`, `wp new`) whose `acceptance.md` was **authored in v2's spelling** and lints clean before anything else happens:

```
$ intent at lint ST0001
lint: ST0001 ok -- 4 AT row(s) conform          # exit 0 -- the baseline
```

All four `AtStatus` variants are seeded, deliberately. Three of the four coincide between the wire and display vocabularies, and **that coincidence is this issue's hiding mechanism** -- a fixture carrying only `Na` would prove the fix without proving the other three still agree, and a fixture carrying only the coincident three would prove nothing at all.

The same four rows were then rendered through `views::acceptance` -- the function this issue names -- from an out-of-tree harness linked against `intentsvcs` by path, so nothing in the repository was touched and the bytes compared are the bytes the renderer produces.

**Byte-equality, which is what this issue's own closing section asked for and not a resemblance judgement:**

```
$ diff v2-authored-rows v3-generated-rows      # 4 rows
$ shasum -a 256 v2rows.txt v3rows.txt
034cb38164ba8b3613d77371ff9d4b367efdf297f83a9532c3aeceba8d42c7d0  v2rows.txt
034cb38164ba8b3613d77371ff9d4b367efdf297f83a9532c3aeceba8d42c7d0  v3rows.txt
```

**And v2's verdict on the generated file, in place, by the tool that scores this repository:**

```
$ intent at lint ST0001
lint: ST0001 ok -- 4 AT row(s) conform          # exit 0
```

**v2's SEMANTIC readers agree too, which is more than the grammar check asked for.** The projection writes the AC lines in the same file, and v2 reads the whole contract back correctly -- coverage, computed satisfaction from a green AT, and evidence-based satisfaction on the non-test row:

```
$ intent ac list ST0001
ac: AC-01.1  covered-by: AT-01.1  satisfied: no
ac: AC-01.3  covered-by: AT-01.3  satisfied: yes
ac: AC-01.4  covered-by: AT-01.4  satisfied: yes
$ intent ac status ST0001
ac: 2/4 satisfied -- BLOCKED
$ intent at list ST0001
at: AT-01.4  the projected row is re-read by v2's own linter  n/a
```

### The green is attributable, because the instrument was made to go red

**An expected green is exactly the condition under which an instrument has to prove it is still looking.** The same file, same linter, same invocation, with the single token mutated back to the pre-fix form:

```
$ intent at lint ST0001
lint: L1 AT-01.4 -- unreadable AT status: 'n-a' -- vocabulary is to-write | red | green | n/a
lint: ST0001 FAILED -- 1 finding(s) over 4 AT row(s)     # exit 1
```

So the pass and the fail are separated by the four characters this issue is about, and nothing else. **Without that arm the green would be a member of the sibling class** -- true, and unable to distinguish a fixed generator from a linter that never read the file.

### Four call sites, and the wire spelling correctly left alone

Read at `55e540df`, all four clean against HEAD:

| site                            | now                                | why                                    |
| ------------------------------- | ---------------------------------- | -------------------------------------- |
| `views.rs` (the generated view) | `t.status.display()`               | the write path -- the migration hazard |
| `render.rs` `at list`           | `t.status.display()`               | the stdout parity break                |
| `render.rs` movement arm        | `format!("-> {}", status.display())` | takes the word from the ENUM, and restores v2's arrow |
| `facade.rs` `at_set` self-loop  | `from.display()`                   | `Outcome::AlreadyThere`'s state name   |

**The movement arm is the one ic's third-spelling section demanded, and `display()` alone would not have reached it** -- it echoed the subcommand name. **And the self-loop is reached through a different parameter than the movement phrase**: `reported()` discards `moved` on the `AlreadyThere` branch and prints `Outcome::already()`, so a fix applied only in the renderer would have left the no-op line unchanged. It was fixed at the facade instead, which is why both lines now take their state from the same place and are structurally unable to disagree.

`legacy.rs:608` (the liberal reader), `model.rs:754` (`#[serde(rename = "n-a")]`), `transitions.rs:518` and the event payload's `enum_str` all correctly keep the wire form. **The two spellings are supposed to be separate; `display()` is what keeps them apart.**

### The enum-set property re-measured -- and my first instrument had the wrong subject

This issue's general rule is a property over the ENUM SET: _the serde spelling and the displayed spelling must be the same string, either because they coincide or because a `display()` maps one to the other._ Swept over all nine enums in `model.rs`, derived from the source rather than from a list:

| enum                                  | verdict                                       |
| ------------------------------------- | --------------------------------------------- |
| `AtStatus`, `ThreadStatus`, `WpStatus`, `IssueStatus` | `display()` maps wire to human |
| `TShirt`                              | coincide -- no `rename_all`, variants verbatim |
| `AcKind`, `AtKind`, `AcState`, `AcceptanceMode` | coincide -- the wire form IS the authored form |

**The last row is a correction to my own check.** The first pass flagged those four as satisfying neither arm, because it compared the wire string to the **Rust variant identifier** (`NonTest` vs `non-test`) instead of to the **authored spelling**. That is this thread's own class inside the instrument written to verify it: a well-formed answer to a question adjacent to the one asked. Re-derived from the corpus rather than from memory -- ic's rule -- `(non-test)` appears **110 times** in authored `acceptance.md` files, and `descoped` / `withdrawn` likewise, all byte-identical to the wire form. **They satisfy the property for the right reason, not by exemption.**

### The in-tree regression test is self-referential on the vocabulary, and this is recorded rather than fixed

`view_determinism.rs::the_view_writes_every_at_status_in_the_authored_spelling` asserts `view.contains(format!("status: {}", status.display()))` -- ie it compares the renderer to `display()`, **which is the function that defines the renderer's spelling.** Change `display()` to return `n@a` and that assertion still passes. Its only external anchor is the hardcoded `!view.contains("status: n-a")`, which pins one wrong value rather than the right one.

**That is a true assertion that has stopped discriminating** -- the sibling class, in the guard written for this issue. It is a real improvement over what preceded it (nothing) and it is not what this issue asked for, which was byte-equality against a corpus v3 did not produce. The measurement above supplies that once; **making it permanent means a fixture in v2's spelling checked into the suite, and that is cc's to place, not vc's to write.** Flagged, not filed: the test is not wrong, it is narrower than its name.

### The residue, which is left OPEN and filed separately

**One authored site still spells this state in the wire form on a human surface**, and it is the `at na` help string itself:
...
Its home is `surface/dispatch-table.json`'s `help` field, regenerated into both faces of `dispatch-table.md`. **And the same row's `target.no_op` still records `ok: <AT> already n-a` at `d0f345b5`**, which HEAD no longer prints -- against the register's own stated invariant that every `target` value describes HEAD.

Both are ic's fields and neither is this issue's mechanism -- an authored literal rather than a rendering through `enum_str` -- so they are filed as their own issue rather than held over this one. **The row's own `observed.notes` states the rule they break**, written by ic on this very row hours earlier:

> A correction has to reach every field that repeats the value, not the field that was reported.

### Provenance

Measured at `55e540df`. `git diff --stat ffcc8764..55e540df -- native/ surface/` is empty and all five measured files are clean against HEAD, so the release binary and the harness are attributable to HEAD rather than to a worktree. The v2 side is v2.19.0 on PATH.
