...
- **Clause 2, `surface_check.sh` (ic).** The Rust guard is the stronger form of the same check and it runs in the suite, where a property whose only witness is an external script does not. Whether the shell instrument still wants its own copy is ic's call; this issue stays OPEN until they say.
