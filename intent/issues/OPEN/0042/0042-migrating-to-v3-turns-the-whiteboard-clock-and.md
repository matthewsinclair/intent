...
### THE REGRESSION IS FIXED, 2026-08-17 (dc) -- and repair 1 was DELIBERATELY NOT TAKEN

**Landed with the repair recommended second rather than first, and this section exists so that choice is visible as a choice.** A disagreement resolved quietly in code is indistinguishable from an oversight, so the departure is recorded where the proposal lives.

**What shipped.** The branch gates on `[ ! -d "$INTENT_HOME_RESOLVED" ]` -- repair 2. `wb_info_rc` is now consumed, but it **reports and does not gate**: a non-zero code prints a line of its own and the guards still run if they can be located. The `sed` also strips trailing whitespace as well as leading (vc's measured hardening: the line is column-padded on the LEFT today, so a renderer that ever padded the right would hand `-d` a path with trailing spaces and turn a working resolver into a loud false block).

**Why repair 1 was not taken, which is the part worth reading.** Gating on the exit code makes every whiteboard guard conditional on a number whose meanings are still being settled -- **vc's own 0045 measured that `Facade::open` gates EVERY command and the migration refusal returns 1**. The day `info` inherits that, `rc` is non-zero in every unmigrated project (ie every consumer, in the window immediately before it upgrades) while `INTENT_HOME` resolves perfectly, and gating there would silently stop the guards **estate-wide**. That is the exact class this issue exists to prevent, delivered by the fix for it. **An unreachable branch under-enforces once; a gate keyed to a moving code under-enforces everywhere.** So: if the guards can be located, they run, and a failing resolver is said out loud instead of being acted on.

**vc's argument for repair 2 is better than the one originally given for it, and it is the one to keep.** The coupling is not gone -- the `sed` still parses a rendering -- but **the direction of its worst case has flipped**. Before: the rendering moves, the parse yields non-empty text, the branch becomes unreachable, and non-enforcement is SILENT. After: the rendering moves, the answer is not a place, and the branch FIRES. You cannot stop a rendering from moving again; you can decide whether its failure is loud. That holds for renderings nobody has thought of, which is more than "it catches `<not set>` without naming it" buys.

**Proven by mutation rather than by the tests passing**, each mutant asserted to differ from the original before anything was run against it, and each still passing the baseline canary so the reds are about the branch and not a broken file: gating back on emptiness reds the new non-place test; putting `rc` back into the gate reds the new live-directory test; deleting the `rc` report reds it differently. Two new arms in `tests/unit/pre_commit_hook.bats` pin both halves, and the `notset` shim reproduces a **measured** run of the real v3 binary staged outside its own tree (`INTENT_HOME:     <not set>`, exit 1) rather than an imagined one.

**The coupling set is FIVE files, not the one this fix's author first named (vc, measured).** Three EXECUTE the shipped hook (`exit_codes.rs`, `critic_dispatch.bats`, `critic_config.bats`) alongside `pre_commit_hook.bats`; one ASSERTS ON ITS TEXT (`whiteboard_header_guard.bats` greps the literals `WB_BLOCKED=1` and `[ "$WB_BLOCKED" -eq 0 ] || exit 1`, so that file reds on FORMATTING of this block, not on behaviour, and nothing on the hook's side shows it exists). All five verified green after this change. **Recorded because a shell edit here can red a rust test, and that direction is not obvious from either end.**

**STILL OPEN, and this fix does not touch it: item 2.** The hook still resolves an executable path by parsing another command's display output. It now fails loudly instead of silently, which is a real improvement and is not a fix. The structural answer is a command that ANSWERS the question -- an install-root query with an exit-code contract and no prose -- and that is not this change.

**Not bundled, deliberately:** the `case "$rc"` dispatch forty lines below is 0045's polarity conflict (`1)` blocks, `*)` fails open, and every non-zero code blocks exactly one of the two consumers). It is before hv and it crosses cc's `critic` exit code, so it is not settled inside an 0042 commit.
