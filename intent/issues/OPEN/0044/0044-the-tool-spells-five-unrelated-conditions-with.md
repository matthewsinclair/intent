...
**DENOMINATOR CORRECTED 2026-08-17 (vc): the 104 above is what was RUN, and the population that should have been run is 103.** The sweep excluded `daemon`, `mcp` and `claude start` from `lib_surface.sh`'s exclusion list -- **which was short by one.** `implemented_check.sh` also excludes `claude upgrade`, because it installs into the user's real `~/.claude`, and my constant was named `SURFACE_NONRETURNING` so the one member that returns fine (it just returns after writing to a home directory) fell out of a list the name was never entitled to define. **So this sweep probed `claude upgrade` three times.** It was inert -- `claude upgrade` is unimplemented in v3, exit 2, writes nothing, verified in a sandboxed `HOME` -- so **neither arm's finding changes and the `1` is still `info` alone**; only the denominator is wrong. Corrected in the table's own terms rather than silently: **104 ran, 103 should have.** Full mechanism on issue 0037 (fifth instance and its correction); the block now shipping in `surface/dispatch-table.json` is `not_probed` with four members, each carrying its own `why`.

**AND A CITATION HAZARD THAT IS NOW IN THE CANON, NAMED HERE BECAUSE THIS IS THE DOCUMENT PEOPLE CITE.** There are two different `103`s in this thread and they are **not** the same quantity:

- **`flag refused: 103, pos refused: 103` is the RETRACTED FIRST RUN. DO NOT CITE IT.** It is the dead instrument -- the one whose hash included `intent info`'s own `Location: <cwd>` line, so no two runs could compare equal and every command read as "refused". It appears in this issue **only** as the worked example of what a clean sweep from a broken instrument looks like.
- **`103` as the corrected probeable population** (107 shipped minus the four not-probed) is a real figure and is the one to cite.

**vc cited the first of these to ic on 2026-08-17 as "the differential's population", and it was neither the population nor a live figure.** ic caught it by going to the artefact. **The rule this earns, in ic's sharpening of mine: a sentence citing a NUMBER names where it was read, or it is a memory wearing a citation's clothes -- and a citation naming a FILE is not a citation, it is a promise that a file exists.** Mine named the right document and the wrong sentence in it.
