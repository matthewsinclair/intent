...
0. Stop the daemon: `intent daemon stop`. Step 3's stamp step renders the views in-process with a freshly built CLI and runs `sync --to-disk`, which refuses beside an intentd watching the same tree — and the driver reports that refusal as `intent sync --to-disk failed` and nothing else (issues 0468 and 0469). Nothing relaunches the daemon on this host, so `intent daemon start` is the last line of step 4 rather than something that happens by itself.
