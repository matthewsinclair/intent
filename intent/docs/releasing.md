...
3. Run `bin/devbin build release --patch` or `... vX.Y.Z`, ideally `--dry-run` first. It makes the version-bump commit, tags it — so the tag now contains step 2 — asks for a confirmation, and pushes. hv fires this. **When it refuses after stamping**, its own remedy names `git checkout -- <paths>`, and that restores the STAMPED faces, because the schema faces are staged at that point (issue 0467); the restore that works is `git restore --source=HEAD --staged --worktree -- VERSION CHANGELOG.md AGENTS.md CLAUDE.md intent/.config/config.json native/rust/Cargo.toml native/rust/Cargo.lock schema`. A red in the cargo gate on intentd's watcher arms right after the workspace build is the gate's known shape (issue 0481): the driver re-runs that family once by itself, and a red inside that re-run with the host at its floor means one more run, not a finding. The gate's cargo log is kept and named only when the gate is red; a green run deletes it, so a red you want to read again is in the log the refusal named and nowhere else.
4. AT THE TAG, `bin/devbin build all` first: `int macos prepare` stages whatever `native/rust/target/release` holds and refuses when the staged CLI's version differs from the app's — it builds nothing. Then `int macos prepare`, `formula`, `publish`, `smoke --reinstall`. The smoke leaves brew LINKED and the running intentd becomes the keg's, while the delivery on this machine is the dev tree, so finish with `brew unlink intent && brew pin intent && intent daemon restart` and read `intent daemon status` for the image it names.
