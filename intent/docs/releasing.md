...
2. Regenerate both halves at `HEAD` against `--baseline "$(git describe --tags --abbrev=0)"`, read the diff, and commit them together. Do this BEFORE step 3 or the cut ships without them.
   ...
   - **The CHANGELOG, in the same pass.** Every user-facing issue closed since the previous tag has an entry in the release's CHANGELOG section, BEFORE step 3 stamps it. List the closed issues from the committed extract:
     ...
     Then read each one's landing commit, not its title. A change a user can see gets an entry: a verb, a refusal or its remedy, a shipped guard's output, intentd, an MCP tool, or the Homebrew formula and its caveat. A change to CI, to tests or to `bin/.devbin`'s development tooling does not, but the formula and caveat `bin/.devbin/cmd/macos` renders are read by every brew user at install and upgrade, so a change to them does. An issue closed with no change (by design, or with nothing in this repository to fix) does not. The list starts at the previous tag's DAY, so an issue closed that day may have landed before the tag and belong to the previous section: `git merge-base --is-ancestor <commit> "$(git describe --tags --abbrev=0)"` says which side it is on. **Found at the 3.2.1 pre-cut pass**: a run of user-facing fixes had landed since v3.2.0 with no entry in any section, because an entry was written only when the landing author remembered to write one.
3. Run `bin/devbin build release --patch` or `... vX.Y.Z`, ideally `--dry-run` first. It makes the version-bump commit, tags it — so the tag now contains step 2 — builds the pair at the tag with `bin/devbin build all`, asks for a confirmation, and pushes. The build is there because the version bump changes `native/rust/Cargo.toml` and `Cargo.lock`, and the pre-push check refuses any push while the pair under `native/rust/target/release` is behind HEAD in the build-input scope (issue 0518); before issue 0546 the cut stopped at its own push with the commit and the tag made. It takes that check's own currency verdict over the pair it built and refuses before the confirmation when the push would be refused, with the finish-by-hand list; `build all` exits 0 into `target/private/release` on a dirty tree, so its exit code alone is not the answer. `--dry-run` prints the build instead of running it. hv fires this. **When it refuses after stamping**, its own remedy names `git checkout -- <paths>`, and that restores the STAMPED faces, because the schema faces are staged at that point (issue 0467); the restore that works is `git restore --source=HEAD --staged --worktree -- VERSION CHANGELOG.md AGENTS.md CLAUDE.md intent/.config/config.json native/rust/Cargo.toml native/rust/Cargo.lock schema`. A red in the cargo gate on intentd's watcher arms right after the workspace build is the gate's known shape (issue 0481): the driver re-runs that family once by itself, and a red inside that re-run with the host at its floor means one more run, not a finding. The gate's cargo log is kept and named only when the gate is red; a green run deletes it, so a red you want to read again is in the log the refusal named and nowhere else.
4. AT THE TAG, the pair is already built: step 3 ran `bin/devbin build all` there. `int macos prepare` stages whatever `native/rust/target/release` holds and refuses when the staged CLI's version differs from the app's — it builds nothing — so if anything was built since, run `bin/devbin build all` at the tag again first. Then `int macos prepare`, `formula`, `publish`, `smoke --reinstall`. The smoke leaves brew LINKED and the running intentd becomes the keg's, while the delivery on this machine is the dev tree, so finish with `brew unlink intent && brew pin intent && intent daemon restart` and read `intent daemon status` for the image it names.
