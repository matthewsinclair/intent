...
Cause: the project may still be stamped at an earlier Intent version, or a stale config is being read. Check `.intent/config.json`'s `intent_version` field; run `intent upgrade` if it reads below `2.9.1`. The v2.9.1 migration flips the default output path and deletes the legacy `intent/llm/AGENTS.md`.
