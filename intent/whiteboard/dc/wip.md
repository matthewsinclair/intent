...
heartbeat_at: 2026-09-11 18:12Z
status: active
focus: "THE 3.0.1 CUT, vc holds the pen. dc's two steps are ON MAIN: the AC-00.6 prune at d5998ac3 and the whole docs/reference at 28ac7a01 (2026-09-11 18:12Z). Holding for vc. NO FIGURE HERE IS EVIDENCE; RUN THE VERBS."
...
**Nothing in flight; holding for vc's next word (2026-09-11 18:12Z).** My two cut steps are both on main and vc has both shas:

- **The AC-00.6 prune + decision 6: `d5998ac3`** -- 86 paths, gated in `wtd6` at `4942340b` and landed byte-identical. The release dry-run passed preflight up to the suite. Its 50 not-ok were all in the 16 mixed files, and cc's and ic's work has since landed on them.
- **The WHOLE `docs/reference` at the cut: `28ac7a01`** (ST0068 AC-04.2) -- 19 files regenerated at `61ebcd6a` (register identical to vc's step-4 green `1409aff7`): 133 commands documented of 133 shipped; cut surface 133 commands, 58 refusals. The whole regen caught `index.md` stale since 09-09, still counting decision 3's retired doors. Completeness: a second regen gives an empty `diff -r`, ignoring only the wall-clock `Generated by ... at` line; that check is controlled both ways.
- **Found while, told to vc:** doctor in a FRESH clone refuses on `backup-stale` until one `intent backup`. `bin/.devbin/cmd/suite --with-build` builds DEBUG while the bats suite drives the release binary.
...
**Nothing.** vc holds the pen for the rest of the cut (hv's release, artefacts, close-out) and corrals the nodes. Do not invent work.
