...
status: active
focus: "**0133 IS BUILT AND NOT LANDED -- branch `dc/0133-unsatisfied-note`, held ONLY because `facade.rs` is cc's mid-edit and the widening cannot compile without two one-line hunks there.** hv ruled both halves: `Unsatisfied` gets its OWN payload `note`, NOT `evidence`; `ac unsatisfy` KEEPS CLEARING, and the separate field is what leaves nothing to orphan. **THE STRICTNESS ARM FOUND A SHIPPED DEFECT BEFORE THE TYPE MOVED: `deny_unknown_fields` is a property of the variant SHAPE, not of the enum -- both UNIT variants swallow unknown fields and they are the two ENTRY states.** Mine to file, and to price HONESTLY before escalating `computed`. Generator fix landed `14879fc5` -- a hole in my own fiat work, driven to both verdicts at 342/2000. **THIS FOLD CUT 36% (105251->67760) AFTER THE LAST ONE CUT 2% AND I CALLED IT AGGRESSIVE: standing means NOT ARCHIVED, it never meant NOT REWRITTEN.** Owed: the two coverage gaps, the arm message, two stale docs my own commit made false, the composer as a GUARD."
...
**NOTHING IN FLIGHT. Tree clean, index clear, nothing staged, nothing owed.** Two days verbatim in `.history/20260828/` and `.history/20260829/`; NOT restated here.

### ST0066 -- `intent fc`. AC half landed `b7a3e771`. **0133 BUILT AND NOT LANDED: branch `dc/0133-unsatisfied-note`**

- **0133 IS COMPLETE AND HELD OFF MAIN BECAUSE `facade.rs` IS cc's RIGHT NOW** (102 uncommitted insertions, calling a method in neither tree nor HEAD -- cc mid-edit, not a defect). My two hunks there are one line each. **cc has been asked for a ping when they commit; re-applying is trivial and the ONLY reason for the hold is that the widening cannot compile without them.**
- **hv RULED BOTH HALVES 2026-08-29.** `Unsatisfied` gets its OWN payload, `note`, **NOT `evidence`** -- evidence means proof a criterion was MET, on `Satisfied` and nowhere else. And **`ac unsatisfy` KEEPS CLEARING**. Ruling 1 is what made ruling 2 cheap: **nothing to orphan**, because the note is a different field from the one the verb clears.
- **INGEST: FUSE, DO NOT SPLIT** -- the satisfied path already fuses `evidence` and `note` identically, so splitting only the unsatisfied path would make two ingest paths disagree with nothing detecting it. vc accepted this over their own flag.
- **A SHIPPED DEFECT THE STRICTNESS ARM FOUND BEFORE THE TYPE MOVED, AND IT IS NOT MINE TO BUNDLE: `deny_unknown_fields` IS A PROPERTY OF THE VARIANT'S SHAPE, NOT OF THE ENUM.** Both UNIT variants (`computed`, `unsatisfied`) silently swallow unknown fields; all four payload-carrying variants refuse. **Those are the two ENTRY states.** The widening closes `unsatisfied` incidentally; **`computed` stays open.** vc measured Intent at 416 `AcState` objects / 333 unit / ZERO stray keys -- **and Intent is the estate LEAST likely to carry it; the fleet is UNMEASURED and a row accepted today starts REFUSING after the widening.**
- **STILL OWED ON THIS: file that as an issue MYSELF** (vc: if they file it, they author my measurement), and **before escalating `computed` to hv, MEASURE the price I quoted** -- does `Computed {}` serialise identically to the unit variant, and does `schemars` render it identically? **If both hold, nothing published moves and it is my implementation detail rather than hv's ruling.** Today the escalation would carry an unverified cost.
- **ALSO STILL OWED IN 0133's OWN SCOPE:** the two hand-enumerated coverage gaps (GraphQL projection test, kind/state sample table) and making the strictness arm's failure message say **what it guards**, or the next reader deletes it as a duplicate of serde behaviour they assume is automatic. **It is not automatic; that is the whole finding.**
- **TWO STALE DOCS BORN IN MY OWN `b7a3e771`, BOTH vc's find, MINE TO TAKE:** `transitions.rs:562` still says _"waiting on the ruling for whether a fiat close is reversible"_ -- added in the same commit as the two edges implementing that ruling, twenty lines above. And `AcState::enum_str`'s doc says **three** variants carry payloads; there are **four** since `Fiat`. **With `model_laws`'s exhaustiveness comment that is three prose claims falsified by one commit, none of which failed anything.**
- **STILL UNBUILT AND IT IS THE PIECE vc MOST WANTS REPORTED ON: the composer as a GUARD.** `fiat_marker` is the one rendering; **"one composer" and "one composer nobody can go around" are different deliverables.** Owed as a CENSUS over the render sites with a two-sided drive on planted fixtures -- `shared-artefact-guard` arms 10 and 11 are the shape, and arm 11 is why a census alone is decoration.
- **ALSO UNBUILT: `ac.fc` itself, the cascade, doctor.** `fiat` is still a declared ORPHAN because no verb reaches it. **AC-00.4 is being REWRITTEN by vc -- do not build to its letter.** **Minutia 3: no structured half-field**, it is a sequencing question with `0116`.
- **CLOSED THIS SESSION: the Machine 3 divergence is vc's and DONE (`6d985366`).** The counting rule if anyone builds the detector: the table has 14 rows, `transitions.rs` has 10 `Edge::` declarations expanding to 14 pairs, because one `Edge::` carries a LIST of from-states. **8 vs 10 vs 12 were all correct and never equal -- expand before comparing or the check reds forever.**
- **AC-00.6 drafted and routed, not built** (`450dc02a`) -- ic's surface. It surfaced a discrepancy for vc: the rule schema says `concretised_by` is required on agnostic rules, 4 of 5 carry it, `IN-AG-RED-CONTROL-001` does not, and `rules validate` is unimplemented so nothing detects which it is.
