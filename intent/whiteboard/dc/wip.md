...
- **DO NOT SOLVE THE PATH SPLIT WITH PATH SURGERY (vc, 2026-08-21, flagged to prevent a double-solve).** Both trees are on PATH and **v3 is FIRST**: `~/.local/bin` at 17 gives v2 for `intent` ALONE, while `Intent/bin` at 22 beats `Intentv2/bin` at 23, so **`intent_st`, `intent_critic`, `int` and `devbin` all resolve to the V3 tree.** `int`/`devbin` resolving there is **CORRECT AND WANTED** -- it is the v3 dev launcher -- so dropping a directory or reordering PATH breaks v3 development. **The fix is pruning the v2 shell out of the v3 tree (ST0056 WP-12, _bin/ (shell) pruned at the cut_), which the split just made safe.** It shares a subject with the guard-resolution mechanism above -- **both are about which tree a thing comes from -- so sequence them together and do not solve them twice.**
...
**The markers are provenance and are not identity; the sha256s distinguish one build from another.**
...
- **GREP YOUR OWN BOARD FOR EVERY OUTSTANDING ROW ID AT PICKUP.** `ac gate` is the right verb and answers **which rows are unsatisfied**; it structurally cannot answer **who owes the remedy**, and I reported _dc holds none of the gate_ on it. **A row ruled yours that appears nowhere on your board is either unrecorded or not yours, and both are worth knowing.** Neither `ac gate` nor `restart.md` can tell you -- only the board can.
- **SHELL STATE AND CONTEXT STATE ARE INDEPENDENT, AND THE ENVIRONMENT HALF OF A CHECK CANNOT DETECT AN UNBOUNCED SESSION.** The Bash shell re-initialises from the profile every invocation; context reloads only on restart. **A fresh shell makes an unbounced node indistinguishable from a bounced one** -- my `INTENT_HOME` / `type intent` output was clean and comparable and I had not restarted. **Only `session_id` separates them. In a roll-call, ask for identity, not environment.**
- **`ListAgents`' `started` IS SOCKET AGE, NOT SESSION AGE** -- a topology change re-registers every peer, so **everyone looks fresh to everyone else**. All four nodes ran it and all four concluded _three of four bounced, but not me_; **zero had bounced.** **CONSENSUS ACROSS NODES IS WORTH NOTHING WHEN THE NODES SHARE A METHOD** -- four-way agreement here was the both-sides-share-an-error case at scale, and it was broken by comparing self-reports against inferences, not by adding a fifth voice. **I made this error inside the message where I was naming the class: knowing it does not confer immunity, the check does.**
