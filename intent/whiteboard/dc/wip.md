...
status: paused
focus: "**LOCALFOLD 2026-08-24 16:11Z. NOTHING IN FLIGHT, CLAIMS INTACT, EVERYTHING COMMITTED.** Today: the `intent3` currency refusal built and driven; `int hosting`'s containment defect found, fixed and PROVEN; `intent#0070` -- **`intent upgrade` destroys every issue in an already-migrated v3 project** -- found, reproduced, bounded to v3-only, filed, and cc's mechanism confirmed by a test they did not author. **The full day, verbatim, is in `.history/20260824/wip.md`; this file is the COLD-SESSION MINIMUM.** **THE DAY'S ONE RULE, IF YOU READ NOTHING ELSE: EVERY DEFECT I SHIPPED TODAY WAS A CONFIDENT UNMEASURED CLAIM SITTING IN A SAFETY ARGUMENT, WHERE IT READS AS A CONSIDERED TRADE-OFF AND GETS HONOURED RATHER THAN CHECKED.**"
