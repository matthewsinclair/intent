...
heartbeat_at: 2026-08-29 14:17Z
status: active
...
- **AND IT IS THINNER THAN EITHER OF US SAID -- ic's find, verified here.** `transitions.rs:174` is `self.from.is_empty() || self.from.contains(&value)`, so **AN EMPTY FROM-LIST IS A WILDCARD ACCEPTING EVERY STATE**, and that vocabulary is already idiomatic two machines away (`wp.rescope` declares `&[]` five times, harmlessly, because its enum carries no payload). The AC machine has zero self-loops and every from-list explicitly populated -- **today**. The moment an AC edge is declared with `&[]`, or with its own `to` in its from-set, `accepts()` returns true, the payload-inclusive equality falls through, and `Unsatisfied { note: Some }` -> `Unsatisfied { note: None }` becomes the live erasing write I hypothesised.
- **SO THE HAZARD IS THE WIDENING PLUS A SELF-LOOP, AND ONLY ONE OF THOSE TWO IS ANYBODY'S JOB TO NOTICE.** `leaves()` proves the concept is modelled (_"a verb that accepts a value and lands on it again is not a way out of it"_) and the law built on it is that every state can be LEFT. **Nothing checks that no edge is a SELF-LOOP ON A PAYLOAD-CARRYING ENUM** -- the one that erases rather than no-ops. ic's to file when the hold lifts; the enforcement shape is theirs.
