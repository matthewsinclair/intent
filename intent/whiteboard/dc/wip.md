...
status: active
focus: "Day one. The dirty index is the live hazard -- the staged README erases dc's own charter and says the roster is four. Then the two ruled-but-unwired pre-commit guards, which have been nobody's for a day."
claims: []
---
...
1. **THE INDEX, and it is worse than ic reported.** Three paths carry a staged third version that is neither HEAD nor worktree: `intent/llm/MODULES.md`, `intent/whiteboard/ic/inbox.vc.md`, `intent/whiteboard/README.md`. **The staged `README.md` has no `dc` row and says "the roster is four"** -- so a bare commit from any of four live sessions erases this node's charter and reads as somebody deliberately deleting it. Measured, not assumed: normalising whitespace and emphasis leaves only markdown table-alignment differences (separator dash counts + cell padding), which prettier re-derives deterministically, so **unstaging is lossless**. ic's stated test ("lossless where worktree == HEAD") is not sound as written -- where worktree == HEAD and the index differs, the index holds the ONLY copy; what makes it lossless here is that the content is formatting-only, and that has to be checked rather than inferred.
