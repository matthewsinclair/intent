...
heartbeat_at: 2026-09-12 06:58Z
status: active
...
- **The second rehearsal is held for batch 4 as well as for the hydrate.** Condition: hv's new gate lands -- every code path that removes or overwrites bytes without naming them before it acts, triggered by `organise --apply` naming its removals only afterwards. vc is sweeping and may assign me part of it. The venue and the deliberate `intent backup` stand as ruled.
...
- **`cargo test -p intent-cli` ALONE reds a couple of dozen daemon-dependent tests that `--workspace` passes** (cc, 2026-09-12): a single-package run does not build the `intentd` binary they need. Triaging with it reads as a catastrophe of which almost all is the invocation. The release gate runs `cargo test --workspace --no-fail-fast`; match the gate, not a narrower spelling.
- **A red that MOVES between tests across runs of the same bytes is timing, not state.** cc's daemon red landed on three different ingest-timing tests across five `--workspace` runs and vanished under `-p intentd`. Elimination reaches "setup"; only repetition reaches which.
...
- **`--only` is not ceremony: a peer's work can arrive INSIDE your commit window.** Two files were staged in the shared index between my `git add` and the commit landing, and they stayed out of it. A plain `git commit` there sweeps them in silently.
- **A green at a commit that is no longer the subject is a claim about a tree nobody is releasing.** HEAD moved while I waited for the tree to go clean; re-drive at the new HEAD before committing, however unrelated the intervening commit looks.
- **A working-tree deletion under `intent/st/` is not data loss on its own.** The canon carries each attachment's full text and hash, so check the canon and the event log (`disk.organize`) before calling it one.
