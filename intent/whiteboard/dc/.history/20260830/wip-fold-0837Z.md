...
heartbeat_at: 2026-08-30 01:25Z
status: active
focus: "ST0066's ST/WP HALF IS BUILT AND GREEN (145 groups in intentsvcs) AND IS HELD UNCOMMITTED BY A GATE DEFECT, NOT BY ME: the new rustfmt arm probes a staged blob in `mktemp -d`, so any file declaring `mod common;` fails to RESOLVE and is reported as unformatted. cc has the diagnosis and the fix (probe beside the original, as their own Elixir arm already does). THE SHARED STORE IS AT SCHEMA 16 AND EVERY PEER CLI REFUSES until my commit lands and a release build follows -- the artefact guard refuses a release build while native/rust is dirty, so that order is forced. NOT STARTED: the CLI arm for `fc` on ST/WP, because `render.rs` is ic's in-flight."
...
**`st.fc`/`wp.fc` ARE BUILT AND GREEN, AWAITING ONLY THE GATE.** Model fields on both kinds, DDL columns, rung 15->16 rebuilding `threads` AND `wps` in one rung (one ruling, so a store cannot sit at a version where a thread is fiat-closable and its cascade has nowhere to land), `Facade::st_fc`/`wp_fc`, the two edges, Machines 1 and 2 rows, `KNOWN_OPS`, `st.fc` in `declared_list_edit`'s Remove arm, D7's `put` refusal, the `unsettable` arms, faces **DDL 13 / SDL 13 / JSON 15**, schema pins re-cut. **THE INVARIANT IS STRUCTURAL: `set_thread_status`/`set_wp_status` are now one-line wrappers over one implementation that takes what happens to the fiat record, and the clear is unconditional in the one place every status write passes** -- vc ratified the shape. A verb added later inherits it.

**WHAT REMAINS (superseded description of the build kept only as the checklist):** MEASURED 2026-08-30: `grep "pub fiat" model.rs` returns ONE hit (`AcceptanceTest`); there is no `st_fc`/`wp_fc` in `facade.rs` and no `st.fc`/`wp.fc` in `transitions.rs`. So it is the AT half again across ST and WP: model field, DDL column, a rung and **SCHEMA 16**, the two facade verbs, the two edges, Machine 1 and Machine 2 rows, `RATIFIED_THREAD`/`RATIFIED_WP` plus driver arms in `mutation_completeness.rs`, CLI wiring, faces bump. **Guarded `ReasonRecorded` and NEVER `GatePass` -- a fiat close is BY DEFINITION the case where the gate does not pass, and relaxing `st.done` to accept either would make `st done` silently fiat-capable.**
...
- **F21 -- NEW. A CHECK THAT CANNOT DISTINGUISH _WRONG_ FROM _COULD NOT MEASURE_ NAMES INNOCENT FILES, AND ITS REMEDY THEN RUNS CLEAN.** The new rustfmt arm copies the staged blob to `mktemp -d`; a lone file there cannot resolve `mod common;`, rustfmt exits non-zero on the RESOLUTION error, and the arm reads any non-zero as unformatted. **The remedy it prints changes nothing, so you run it, retry, and get the identical refusal** -- I did that once before diagnosing it. Positive control: all 7 named files declare a module, and mod-free `faces.rs` passes the identical probe. **The fix was already in the same file one arm above** -- the Elixir arm puts its probe BESIDE the original so config resolution is the file's own. Third instance today of one message covering two conditions.
- **F22 -- NEW. `HOME` ISOLATES USER STATE AND NOTHING ELSE; THE STORE IS PROJECT-RELATIVE.** I isolated a suite run under a fake `$HOME` on a correct, measured basis -- `userstate::home()` reads it, `every_path_descends_from_the_one_home` holds the rest to descending from it -- and the store still migrated to 16. `Project::db_path()` is `intent_dir()/.cache/intent.db`, resolved by walking up from CWD, so **a spawned binary with no `current_dir` finds the REAL project** (cc measured; four tests spawn it that way). **A correct instrument aimed one directory tree away from the damage**, and the first measurement is what licensed the confidence the second betrayed. `current_dir` into a fixture project is the isolation; the two are independent and a test touching both needs both.
...
- **2026-08-30 -- WHERE AN INVARIANT LIVES IS A DESIGN DECISION, AND STRUCTURAL BEATS REMEMBERED.** `fiat` sits BESIDE the status on all three kinds because hv ruled the status value must not move, so no type holds the pair in agreement. Putting the clear in the ONE door every status write passes -- rather than in each verb, or as a parameter at fourteen call sites -- means a verb added later inherits it. **The same field's `status_reason` takes the OPPOSITE default deliberately**: a reason is written only when the caller says something, because a stale reason is confusing; a fiat record is cleared unless told otherwise, because a stale fiat record is a false claim about a person.
