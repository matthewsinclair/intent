...
3. **AC-04.6's SECOND CONDITION IS NOT SATISFIED AND THE ARM IS MINE** (vc caught it; I verified it at the source). The criterion has two clauses. The ratified-machine match holds -- `mutation_completeness` 16/16, the whole matrix walked with a floor assertion. **The second clause is "for each `Unbuilt` field, assert no entity can hold a non-initial value by ANY path INCLUDING INGEST", and `an_unbuilt_field_is_one_no_service_call_can_set` asserts the narrower claim.** Its exemption at `:434` reads `("Criterion","kind") | ("AcceptanceTest","kind") | ("Issue","status") => false` on the ground that they "arrive only as authored canon" -- **and arriving as authored canon IS the path the clause says to check.** `legacy.rs:574` returns `AcKind::Test` on the way in, so a criterion holds a non-initial `kind` no service call can change: an entity in a state nothing can leave, which is the trap the AC forbids. **WP-04's gate is 5/6 and the remaining 1 is real work.**
4. **AC-06.1's surface tail** (NOT the installer/canon block).
5. **WP-10 PHASE B** -- fixtures and a sacrificial copy only. **Not against this estate, and v3 does not go on PATH.**
6. **`intent at` must stop destroying the row's note** (issue 0033). It is v3 and therefore mine, and it is the only fix that retires the standing "commit first" ritual.
...
- **COMMIT BEFORE ANY `intent at` STATUS CHANGE.** Issue 0033: `at red|green|na` DESTROYS the row's note and `at lint` reports `ok` immediately after, so **the contract's own linter cannot see it leave.** **SEVEN instances now, and the last two are the largest: AT-04.1 729 -> 99 and AT-10.9 5316 -> 97, 5,849 characters in one sitting** (vc -- survived only because they committed first and had captured text). 14,253 characters across 34 noted rows. `to-write -> green` is refused, so recording a passing test costs TWO rewrites. A committed note is recoverable with `git show`; one written and moved in the same session is gone.
...
- **AT-10.9 AND AT-04.1 ARE BOTH GREEN** (vc, `e827bd54`), mutation-proven independently and **read from a `git archive` extract at `29d32e71` so no peer in-flight work was in the measurement** -- which is the discipline that answers dc's build-artefact trap, one node over. Nothing owed on either.
...
- **THREE INSTANCES TODAY, IN THREE MATERIALS, AND THE THIRD WAS CAUGHT BY A PEER RATHER THAN BY ME.** The measurement is sound every time and **the sentence reporting it is wider than the measurement** -- which is where it can be caught, because the tell is in the reporting step. **The third: AC-04.6 has TWO clauses, I measured the first and reported the criterion** (vc). First written as "the two-writers rule applies to arguments", which reads as being about failure modes; it is about any claim of the form _I checked this_, where the check has a SCOPE the claim then drops. vc's own version is a CLASSIFICATION standing in for a measurement, so the class is **"the thing I have is narrower than the thing I said"**. The first two: 0046: I would have inferred v3's behaviour from a `keep`/`as-observed` classification -- **a classification is a claim ABOUT code and only code answers for code.** TornRollback: my own earlier note enumerated four constructions, found none reached the variant, and concluded it was unreachable -- **all four were about `WriteSet::commit`'s unwind, and `Applied::rollback` is the other producer.** Both times the enumeration was sound and the conclusion was scoped to whatever was in view. **Ask what your checks have in common before believing they are independent, and ask which of the producers you enumerated.**
...
- **A FILE MATCHING HEAD IS NOT PROOF YOUR WRITE WAS LOST -- THE RECIPIENT MAY HAVE CONSUMED IT.** My reply landed in `dc/inbox.cc.md`, dc read it, acted on it and CLEARED it in their own fold, so by the time I committed the worktree matched HEAD and `--only` had nothing to add. **My leftovers check flagged it correctly and my first inference from it was wrong.** In a single-writer protocol the reader owns the lifecycle; check the peer's commits before concluding a message vanished.
