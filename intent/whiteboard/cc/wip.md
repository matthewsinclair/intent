...
heartbeat_at: 2026-08-19 07:34Z
status: active
focus: "**THE LIVE MOVE LANDED AT `16048f82` AND THE TREE IS GREEN.** 97 canon files into `intent/.canon/` -- 57 flat threads + 40 zero-padded issues, byte-identical, verified at HEAD by `git ls-tree`; vc re-verified independently across every rename BLOB SHA, 97 of 97 identical, and fired the of-N trigger to dc and ic. Suite **648/0/0 across 88 suites**, fmt clean. **AT-01.7 crossed the move red 1-of-8 to green 8-of-8** -- its red-first arm had been silently consumed by `f41d6760` landing the code first, and the window was closing rather than opening. **The pinned `f2e4d1f9` is BLIND to `.canon` and reports the estate unmigrated: judge an instrument by CONTENT, never by build time.** Next: AC-01.5s commit guard, then AC-01.1..01.4 measurable for the first time."
claims: [ST0056/10]
---
...
1. **AC-01.5's commit guard** -- `lib/templates/hooks/canon-ignore-guard.sh`, refuse at COMMIT the edit adding `intent/.*/` to `.gitignore`. **Every other `intent/.<x>/` IS gitignored, so the convention reads "a dot directory under intent/ is local".** `.canon/` is the single deliberate exception and a tidy-up would silently un-commit the estate. AC-01.2 checks the STATE by cloning; AC-01.5 refuses the EDIT. **vc's baseline at `8bb47e49` is the attributability anchor: check-ignore rc=1 on both samples, and the root `.gitignore` names the other three dot-dirs LITERALLY and individually, with no `intent/.*/` pattern anywhere.** Run only post-hoc, the check cannot tell a rule someone ADDED from one that was always there.
...
5. **ST0056 AC-03.16 / AT-03.17** -- `views.rs:329` + `:653` emit "_ACs and ATs live in `acceptance.md` -- the single source of truth_" and `acceptance.md` IS A GENERATED VIEW. A **WORK-LOSS INSTRUCTION**. Assert the property, print the denominator; read EMITTED VIEWS. **`views.rs:382` must STILL PASS.**
