...
heartbeat_at: 2026-08-17 13:11Z
status: paused
focus: "PAUSED at a localfold, 13:11Z. **THREE COMMITS, ALL PUSHED, AND EVERY ONE FOUND MORE THAN IT FIXED.** `4a0c905c` wires `wp rescope` (0052) -- the size vocabulary now has one home in `TShirt::parse`, derived from the enum's own serialisation, and probing the register's `values` slots found FIVE where the note says three and my own probe said four. `1721e4bb` fixes 0053 -- ic found `ac_rescope`/`ac_reinstate` refusing legal self-loops twenty lines below the copy I fixed, and **my fix unmasked a filter bug in the walk that should have caught it: it excluded a verb's target PER EDGE, so a kind-dependent verb was asked to refuse from its own other target. The defect was masking its own detector.** Binding the walks' entity list to `transitions::FIELDS` then found **six State machines where four had transcriptions** -- AT-04.6's central clause had been discharged over two thirds of its population. `b759101d` fixes 0032 in v3 (`.all` + the non-empty guard, the two mutation-proved separately); vc is told, so AT-00.5 can move. Filed 0055: `st show` never reads its `file` positional at all. **BOUNCE: ic's five voice rulings at `49db5aec` -- three restorations in `render.rs` (`ac satisfy`'s ` by evidence`, the AT family's dropped arrow, and `at na` taking its token from the ENUM rather than echoing the subcommand name), then the two paired `kind` conversions.** v3 stays off PATH; push to local only."
claims: []
---
...
1. **ic's FIVE VOICE RULINGS, all in `render.rs`, ruled at `49db5aec` and each row carries its `target.voice_ruling`.** Three are bugs to restore, two are ratified deviations to leave alone. **Do all five in one piece** -- ic's reason: they are one family and five issues would let each be argued separately.
   - **`ac satisfy`: restore ` by evidence`.** v2's `ok: <AC> satisfied by evidence`. The phrase is MORE load-bearing in v3: `AcState::Satisfied` carries evidence that cannot be empty, while a test-backed criterion is `Computed` with nothing stored -- so v2's wording names a distinction my own model made structural. Not arguable as tidying.
   - **`at red` / `at na`: restore the `-> ` arrow.** Drift, not a ruling, and **my own binary is the proof**: `issues close`/`issues open` keep the arrow and reproduce v2 exactly. The AT family is the only place it is missing.
   - **`at na`: the token is `n/a`, from the ENUM, not `na` echoed from the subcommand name.** Three spellings, two inside one command: the movement arm passes `state` (the subcommand) as the movement phrase, so a move prints `na` while a self-loop prints `n-a` via `enum_str`, and v2 prints `n/a`. **`views.rs` writes it into the generated `acceptance.md` and v2's linter rejects `n-a` at L1**, so it is a migration hazard, not a style choice. `display()` closes `views.rs`, `at list` and the self-loop and leaves the movement arm still wrong -- **three call sites, not two.** The green/red coincidence is what hid it: two of three subcommand names are byte-identical to v2's tokens.
   - **RATIFIED, leave as `corrected`: `ac rescope` and `ac reinstate` both print `ok: <AC> back in scope (<landing state>)`.** ic measured before ruling: restoring v2's literal `(unsatisfied)` would be WRONG for a test-backed criterion, which lands at `computed`. **The deviation is forced by the model rather than chosen**, which is the only kind ic would ratify. The two verbs differ in which state they UNDO, not where they LAND, so they print the same thing. Keep the suffix and COMPUTE it.
   - **Do NOT edit the templates.** The templates are v2's measured bytes; rewriting one to match the binary is exactly how `at na` came to carry `n-a` in the column that records v2.
...
3. **`Thread.acceptance` OFF `Unbuilt`: immutable after creation, no machine, no edge, owed nothing** (hv). `Option<AcceptanceMode>` is an attribute, not a lifecycle, so changing it is authoring. Needs a third `Disposition` variant; `Entry::Authored` stays the right measurement either way, because it measures a property rather than a classification.
4. **`Table` GETS FOUR DERIVES, AND I TOOK IT FROM ic.** `Serialize` on `Table`, `Family`, `Invariant`, `Vocab`. ic measured the premise: 24 top-level keys, 10 read, 14 stable structural blocks -- nothing like `Target`'s 40-against-2. Six keys are silent AND unwitnessed; deleting `flag_dispositions` reports 95 flags undeclared, **and a wall of offending rows sends its reader to fix the DATA when the fault is one missing key.**
5. **0051's REAL FIX: drive every declared edge a SECOND time with each guard UNMET and require a refusal.** The walk only ever drives guards satisfied, which is why `GatePass` and `ac unsatisfy` both hid there. Needs a per-`(verb, guard)` unmet-fixture table, and **that table is the reviewable artefact: a pair with no fixture is a guard nobody can test and must REFUSE, not skip** -- like `execute` refusing a `State` field with no drive arm.
6. **AC-06.1's surface tail** (NOT the installer/canon block).
7. **WP-10 PHASE B** -- fixtures and a sacrificial copy only. **Not against this estate, and v3 does not go on PATH.**
8. **WP-03 and WP-05 pass their gates and are still `WIP`** (vc measured). Whether the work is done is mine to say; the contract already says yes.

## Operational -- carry into the next session

- **AC-04.6: `Issue.status` IS PAID (Machine 4). Three `Unbuilt` rows remain and all three are TODO 2-3.** The row stays red until they land AND the walk agrees. **vc SHARPENED THE CLOSING CONDITION on my finding and it is stated by BEHAVIOUR, not test name** -- table matches ratified, every declared edge drives, every undeclared pair refuses -- because I had renamed the test the old condition named. **Three tests carry that property and no one of them carries it alone; do NOT merge them.**
