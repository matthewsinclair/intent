...
status: active
focus: "FIVE LANDED TODAY: a2a51938 AC-10.8, a9a4c070 AT-10.12, 1e1cef8c the daemon-status pair, dad94ce9 AT-06.11, plus boards. THE `DOUBLED 0` FINDING IS RESOLVED BY ic: predicate asked the REALISED tree, which realises one flat info.md, so the counter could not fire. Corrected on the same tree: DOUBLED 194 + RELOCATED 192 = 386 exactly, STRANDED 0. ARTEFACT CONSERVATION IS CLEAN; AT-10.5's live residue is the prose arm's ALTERED 114. Rows for AT-06.11 / AT-10.12 are vc's."
...
- **AT-10.5 -- THE `DOUBLED 0` FINDING IS RESOLVED AND THE RESIDUE IS SMALLER THAN EITHER FIGURE.** I drove ic's relocation fix, reproduced all eleven figures exactly, and refused to hand anyone `STRANDED 386` because its partition counterpart `DOUBLED 0` is impossible on a tree that migrated 54 bucketed threads. Measured per-thread: 50 of 52 bucketed `info.md` had their body in their OWN thread's canon. **ic found the cause and it is bigger than my 50: the DOUBLED / RELOCATED / STRANDED predicates asked the REALISED tree for `st/<ID>/<base>`, and the migrated canary realises exactly ONE flat info.md, so the counter could not fire at all.** Canon holds the counterparts -- the thread record for the decomposed names, attachments with sha256 for the authored ones. **Corrected on the same tree: DOUBLED 194, RELOCATED 192, ALTERED-ATTACHMENT 0, STRANDED 0 -- and 194 + 192 = 386 exactly, with HEAD's tool still saying 0/0/386 as the control.** My split reproduces to the file: 181 info.md + 13 acceptance.md = 194; 54x3 + 30 one-offs = 192. **MY HYPOTHESIS WAS RIGHT IN SUBSTANCE AND WRONG IN MECHANISM** -- those files are not the only copy, but the cause was the predicate's subject, not the bucketed-attachment fix I proposed. **ARTEFACT CONSERVATION IS CLEAN. The live residue is the prose arm's ALTERED 114**, untouched by this, and that is what AC-10.5 now has to account for.
