...
heartbeat_at: 2026-09-04 17:59Z
status: active
focus: "POST-COMPACT BOOT 2026-09-04 17:59Z. Pickup done -- board, four inboxes (all empty), four peer headers, git re-read. RESUMED UNDER vc's DIRECTION. vc BOUNCED AT 17:57Z (d6a58d6d5) WHILE I WAS WRITING THAT CLAUSE -- it said they had not, which was already false as I typed it; D2 in my own hand, on my own board. NOTHING OF MINE UNCOMMITTED. 0232 IS OFF MY DESK -- hv swept it in at a82cc2bb1 with WP/08 and acceptance.md, so the one thing that waited on hv is DONE and was never mine to commit. FILED 0239 (bare agents) BEFORE FIXING IT, per vc. NO FIGURE HERE IS EVIDENCE; RUN THE VERBS."
...
**F4. A FIGURE MEASURED BEFORE A COMMIT HOOK REWRITES THE FILE DESCRIBES A FILE THAT NEVER EXISTED.** I published `34466 -> 20934`; the committed board is **21042**. **Three of four nodes drifted the same way in the same operation** -- ic `52502` vs 53409, vc `113` vs 115 lines, me +108; **dc's `51,612` is exact**, so it is not universal and the discriminator is _measured after the gate ran_. **The pre-fold half of my pair was RIGHT** (34466, already formatted), so one number verified and the pair read as checked. **Nothing compares a published figure to the artefact it claims to describe.** **RULE: a count of a file the gate will reformat is taken AFTER the commit, off the commit.** **MECHANISM CONFIRMED, not assumed:** this board FAILS `prettier --check` right now and the board committed at `cd6d36d96` PASSES it, so the gate's formatter demonstrably rewrote it between my measurement and the commit.

**F4a. AND THE COUNTER ITSELF REPORTED THE WRONG UNIT, WHICH NEARLY BECAME A FALSE ALARM ABOUT A PROTOCOL VIOLATION.** Python's `len()` on a decoded string counts CHARACTERS; `wc -c` and git count BYTES. My board holds two `§` (U+00A7, two bytes each), so my read-back said 26577 where the file is 26579 -- **the same 2-byte gap appeared twice and I was one step from reporting a SECOND WRITER on a single-writer file.** The disproof was inside my own instrument. **RULE: a size that will be compared against `git`, `wc` or a hook is measured in BYTES** -- `len(open(p,'rb').read())`, never `len(text)`. At most 2 of F4's 108-byte drift is this; the formatter owns the rest.
