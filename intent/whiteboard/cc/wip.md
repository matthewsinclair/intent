...
status: active
focus: "**BOOTED ON THE BOUNCE 2026-08-25 22:35Z AFTER THE COMPACT; HOLDING FOR INSTRUCTIONS. STATUS STAYS `active` -- a compact does not end a session (invariant 6).** Day folded at `97b8dbcf`, verbatim in `.history/20260825/wip-fold-2224Z.md`. **WP-06 with vc's pen: four landings; `lang` AND `modules` ARE BOTH FULLY RULED, SEQUENCED AND NOT STARTED** -- pick either up cold from DOING. **ONE CLAIM EXPIRED WHILE I WAS COMPACTING and hv hit it: the `CLAUDE.md:52` edit is DISCHARGED, not with hv. Corrected here at `:29` and `:66`, verified on disk rather than taken on report.** AC-06.1's remainder is 36 of 105 -- re-drive it, do not read it from here. Tree clean on my paths, four inboxes at the sentinel, nothing tagged, pushed or published."
...
**A CLAIM ABOUT A FILE ANOTHER NODE WRITES HAS A SHELF LIFE OF ZERO.** Re-read a peer-written file at the moment you assert its state. **And a peer's correction can be wrong in the OTHER direction**: ic retired a TRUE row believing it had expired, which is strictly harder to see than an expiring claim because a row that disappears leaves nothing to re-read. **And the asymmetry under both directions: A DEFECT ANNOUNCES ITSELF AND A DEFECT'S DISAPPEARANCE DOES NOT.** Nothing pushes a ruling back to the board that recorded the question, so **a resolved item stays open on your board BY DEFAULT** -- two instances tonight, mine at `:29`/`:66` and dc's `AC-07.2`. **The fix in both was ANNOTATE DISCHARGED, never delete: the SUBJECT expired and the LESSON did not.**
