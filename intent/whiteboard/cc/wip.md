...
- **ST0057 `AT-03.6` GREEN. GATE IS 64 OF 67** (ST0057 49/51, ST0056/03 15/16). Criterion count moved 48 -> 49, so the row WAS the last one.
...
### AC-03.14 -- DESIGN DONE AND MUTATION-PROVEN, INSTRUMENT UNWRITTEN

**THE COVERING ROW IS `AT-03.15`, NOT `AT-03.14`** (vc). Systematic offset in that WP from `.11` onward: `AT-03.N` covers `AC-03.(N-1)`. **`AT-03.14` exists, is GREEN, covers AC-03.13** -- so looking up the same number returns a green row for a different criterion.

**The deliverable is a SECOND INSTRUMENT: a CLI-level driver that runs the binary and snapshots the estate**, because 31 of the 32 UNPROVEN verbs have no facade method. Proven in the scratchpad, ready to write up:

- **Comparison is mutation-proven BOTH ways.** mtime-moved-bytes-identical caught; bytes-changed-mtime-unmoved caught. **My first inverse arm was WRONG -- `touch -t` CHANGED mtime backwards, so the file entered both sets and equality correctly held.** Restoring the EXACT original mtime is what tests it.
- **Non-empty control in hand:** `st start` on a 3-thread fixture, 4 files mtime-moved, 4 bytes-changed, sets equal. **Consistent with the production fix at `write_set.rs:154-166` already being in** -- the row is red for want of an instrument, not a fix.
- **All ten verbs ARE drivable.** My first probe said otherwise and was wrong: **zsh does not word-split unquoted `$var`**, so every multi-word verb went as one argument and returned a plausible `unrecognized subcommand`. Use `eval` or a here-doc loop.
- **Ruled constraints, do not re-litigate:** `.cache` cannot be the observable (read verbs create it too, and it is gitignored so the tracked tree is blind to it); `ingest` cleared; `todo list` is a table defect; `at lint`'s mutate is in `--fix`; zero `test result:` lines reads UNMEASURED, never green.
- **The new `.sh` needs its own roster row IN THE SAME COMMIT** -- the widening enforces that on me first.

## TODO

1. **AC-03.14's second instrument.** The only gate row cc can move alone.
2. **AC-08.5's FACADE CHANGE -- hv RULED THE SPLIT AND THE OWNERSHIP: cc BUILDS, ic COVERS** (via ic, this session). `facade.rs:4023` refuses `Threads | Thread{..} | Issue{..}` with _this id is server-assigned -- POST to the collection_. **That is a CREATE-shaped justification declining an UPDATE, and the arm does not distinguish the two operations** -- for an AT row `put` is an upsert replacing every field; for a thread it refuses outright, which is why ST0011's `completed` has no write path at all. **The split: `Thread{id}` where the thread EXISTS upserts; create-by-id stays refused; `Threads` and `Issue{..}` unchanged.** Builder and verifier stay separate deliberately because AC-08.5 is ic's gate row -- **do not also write its coverage.** ic corrected their own earlier report against themselves: the unsettable set is EMPTY, not `["file","prose","covers","note"]`, which came from reading `no_named_verb_sets()` at `:92-96` -- a roster of NAMED VERBS, not a settability measurement.
...
4. **`render.rs` `#[cfg(test)] mod tests` driving `unwired("not-a-family", "")`** -- assert exit 2 AND the named path, since exit 2 is reachable from many arms there. **No visibility change needed: a same-module test reaches a private fn.** Then ping ic, who retires `declared_but_unwired`'s loop in one move -- **its `UNWIRED` const cannot simply go empty, that file refuses an empty roster on purpose.**
...
**ST0057 canon + acceptance.md are DIRTY IN MY TREE AND I CANNOT COMMIT THEM.** My `sync --to-disk ST0057` projected the whole thread, and the store already held **ic's** AT-07.7 green and ic's uncommitted `no_daemon_required.sh` bytes (16963 / `4c5be883`). **Canon therefore names bytes no commit contains** -- the exact state AC-03.6 forbids, which I greened an hour ago. **Landing it would be greening a criterion and then committing its violation.** Waiting on ic to commit their two files, or to name them explicitly for me to carry. **`address_collections_resolve.rs` is untracked and canon cites it -- same trap pointed at ic.**
...
- **A WHOLE-FILE WRITE PATH STRIPS THE EXECUTABLE BIT.** Python `write_text()` recreated `runner_roster_check.sh` at 644 from 755, visible ONLY in the commit summary line. **In `parity/tools/` that bit is not decoration -- 644 vs 755 is exactly how a sourced lib is told from an instrument, and the roster rows say so in their own reasons.** I stripped it in the very commit that documents the convention.
