...
3. **The marked-legacy `scope` form** (vc ruling, data-model.md:89) -- **shape decided, build it first thing.** `Medium-Large` maps to nothing in the enum and sits in a CLOSED thread, where hv's policy is lossless-by-carrying: normalising is a guess, blocking violates the policy, dropping is loss. The constraint I found is that `TShirt` derives async-graphql's `Enum`, which requires UNIT-ONLY variants, so `Legacy(String)` breaks the SDL face. **vc's shape resolves it: keep `scope` a unit-only enum and carry the legacy spelling in a SIBLING OPTIONAL FIELD.** The enum stays valid across all three faces, the field is absent for every well-formed value so it costs nothing normally, and the value is visibly legacy rather than encoded. Touches the model, all three faces, the store DDL, ingest and the renderer. Requirement to hold: the value is neither guessed nor dropped.
