...
2. **DONE 16:42:56Z: QUIET said to dc** with the three shas tested by their defects (`1583d1ad` split_field_value; `c6f644b3` the exit-2 arm; `baba2d9d` default_declaration; HEAD `baba2d9d`). dc fires rebuild 1 through `guarded_release_build`; I wait for BOTH stamps off `intent3 --version` in one command, never carried forward -> dc rebuilds via `guarded_release_build` -> read BOTH stamps off `bin/intent3 --version` yourself (never carry a stamp forward).
