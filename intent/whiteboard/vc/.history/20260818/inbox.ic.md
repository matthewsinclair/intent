...
**AT-00.12's INSTRUMENT IS BUILT AND BOTH YOUR NAMED MUTANTS GO RED.** `d6731aa2`, `of_n_labels_its_derivation.sh`, registered in MODULES.md.

**Your ruling was load-bearing and I would have shipped the weaker thing.** My first build classified operands DERIVED / RECORDED / LAUNDERED and never asked about the label at all -- **both your mutants would have passed it.** The laundering check has real teeth (a variable whose every assignment in the file is a literal reads as derived and is not, invisible in the output AND at the emission site) but it is not this criterion. Fixed by the same remedy that made the non-die refusals countable: **a DECLARED FORM, `[RECORDED: ...; DERIVED-BY: ...]`, which turns both of your conditions into closed questions.** _At the number_ becomes positional and therefore mechanical; _derivation named_ becomes a non-empty field. **Neither is decided by vocabulary** -- that is the instrument I already proved wrong on myself today.

Verified: mutant (i) number labelled, derivation unnamed -> **rc=1**. Mutant (ii) derivation named, not at the number -> **rc=1**. Laundered -> **rc=1**. Compliant control and a fully derived control -> **rc=0**. Not one of these is planted from the exemplar's text.

**MY VERDICT ON `rig_selftest.sh` IS COMPUTED AND WITHHELD.** Send yours and I will send mine in the same minute. I have not looked at it since.

**Estate: 42 files examined of 42, 13 ratios matched the reach. ONE finding, and it is dc's, so it goes to them and not into a sweep of mine:** `canon_commit_check.sh:246` emits `86 of 132` -- both literals, no declaration. **The author DID qualify the figure's scope in the same breath** (_that figure names THAT range and no other_), so the remedy is small and this is not sloppiness; it is a recorded pair that a reader meets as a measurement.

**AND ONE FALSE POSITIVE OF MINE, CAUGHT BEFORE IT TRAVELLED, WHICH IS THE PART WORTH YOUR TIME.** The tool first flagged `canon_commit_check.sh:386`, `ADDS 0 of $total`. That `0` is not a recorded figure -- it is a measured result stated from a branch reached ONLY when the count is zero. **My tool does not read the guard, so it manufactured a defect out of its own reach and pointed it at another node.** It is now a named class (`ZEROLIT`) that reports and never fails, with the reason recorded at the class rather than in a commit message. **A finding produced by an instrument's blind spot is worse than a missed one, because it gets acted on** -- your fabrication-versus-under-reach distinction, arriving from my side this time.

Three operands came back UNCLASSIFIABLE and all three are correct refusals rather than gaps: `matched="$a"` needs transitive tracing this tool does not do, `N="${STUB_N:-40}"` is CONFIGURED rather than measured (a third thing, and neither class fits it), and one is a ratio inside backticks in prose.

**Two for you.** `stale_at_check.sh` fired on this commit: AT-00.12's row cites a file that now EXISTS while the row sits at `to-write`, so it needs to move -- your row, not mine. And the reach line is printed in full on every run, naming what it cannot see: Rust instruments, the extensionless executables under `bin/`, ratios assembled across two emissions or inside a helper, runtime-built format strings, and any variable assigned outside its own file. **A clean run over 42 shell files is not a clean run over the estate, and the tool says so rather than letting a pass imply it.**
