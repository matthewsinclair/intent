...
cc is back on the bounce -- localfold committed at `0cff6a54`, board and all four inboxes re-read cold, status active, heartbeat fresh. Holding for your sequencing.

One measurement you would otherwise have to run yourself, because it changed while I was folded.

**Item 2 is still parked, and I measured the park rather than assuming it.** `surface/dispatch-table.json` is still dirty -- ic is holding it. Note the path has NO crate prefix; I guessed `native/rust/crates/intent-cli/surface/...` and `git status -- <bad path>` answered with a warning and no output, which is byte-identical to clean. Worth knowing if you or ic script anything against it.

**Both free arms you authorised would fire TODAY, on a table that has MOVED two rows since I profiled it.** `shipped n retired` = [organize], non-empty. `declared` = 129 entries / 128 distinct, still double-counting `organize`. And `shipped` + `retired` = `declared` still balances exactly -- for precisely that reason, two errors cancelling. The finding surviving the table growing underneath it is worth more than the reading I originally banked, so I have taken the counts OFF my board and left the probe.

**ic has independently landed the same finding as a comment in `spine.rs`** -- that `table.retired()` selects on `!is_shipped()`, that `organize` is two rows, and that publishing the unfiltered list would delete a working verb from a caller. Two nodes, opposite ends, same defect. The gate arm is the only half of it still unwritten.

**One thing for your sequencing, not a complaint:** my lane is not mine at the moment. `native/**` carries dc ST0066 Fiat cascade and the ic surface-retired door, both uncommitted, and ic says the next ST0056 commit is a joint act. If you hand me WP-08 I will be greenfield in `intentd` and clear of both; if you hand me item 2 it needs ic to put the table down first.

FYI only -- no response needed beyond the sequencing itself.
