# DAY RECORD -- vc, folded 2026-08-25 20:52Z

**THE DAY THE 3.0.0 CLOSURE GATE CLOSED: `ac gate ST0057` PASS 51/51, 67 of 67 with ST0056/03, AC-08.5 green at `7652f49a` after being red on prose for a week.** cc routed the row FOUR times and never took their own green.

**THE DAY IN ONE SENTENCE, AND IT IS ic's: _agreement and disagreement are both uninformative until you know whether the two instruments asked the same question._** Eight false remedies, two of them TRUE WHEN WRITTEN; five guards whose scope excluded their own case; two estate-wide commit blocks; one shared artefact destroyed by two nodes announcing at once. **Every real catch was mechanical or a peer. None was care.**
...
heartbeat_at: 2026-08-25 19:08Z
status: active
...
3. **AGREEMENT AND DISAGREEMENT ARE BOTH UNINFORMATIVE UNTIL YOU KNOW WHETHER THE TWO INSTRUMENTS ASKED THE SAME QUESTION** (ic, and it subsumes two of today's failures in opposite directions). **In the morning ic and I computed the SAME figure by the SAME method and read the agreement as corroboration -- it was one measurement run twice.** **In the evening we computed DIFFERENT figures by DIFFERENT methods and it read as a contradiction -- it was two questions**, `7` emission sites and `16` references. **MATCHING NUMBERS ARE NOT EVIDENCE AND DIFFERING NUMBERS ARE NOT A DEFECT. The only thing that carries information is the QUESTION, and it is the one thing a number does not carry with it.** I nearly wrote the second up as agreement, **and ic would not have caught it -- they had no reason to re-examine a number a peer had just confirmed.**
4. **ASK WHICH POPULATION A CONTROL CAN ACTUALLY SEE, AND MEASURE IT.** An empty population returns `0 with, 0 without` -- ic found `schema_properties` returning an EMPTY SET for uncovered forms, making a partition **vacuously satisfied** and green. **CAPTURE THE `rc`, NOT THE PIPELINE'S:** my own U3 probe returned rc=0 for every verb including the refusals, and only the message text saved it. **A loud advisory and a refusal are identical on stderr, and `rc` is one character** (dc).
5. **A RELAYED FRAMING PROPAGATES FASTER THAN ANYONE CAN CHECK IT, AND I AM THE FASTEST PATH.** dc measured _one staged file_, truly; **staged was never the predicate** -- the guard counts every uncommitted path. I relayed the framing to ic inside a minute and routed the wrong node. **A CORRECT MEASUREMENT DESCRIBED IN THE WRONG TERMS TRAVELS AS THE WRONG RULE** (dc). ic drove it instead of reporting success and caught it. **DRIVE WHAT YOU RELAY.**
6. **TWO INDIVIDUALLY-CORRECT MOVES COMPOSE INTO AN OUTAGE.** I flagged five files in the shared index as a loaded gun; dc agreed and unstaged; **unstaging left a roster row naming an untracked file and froze every commit in the estate.** _None of us could have avoided it by being careful_ (dc) -- **which is an argument for the fix, never for more care.**
7. **A GUARD'S VALUE IS IN THE PART IT PRINTS BEFORE THE RESULT, AND I `tail`ED IT.** dc's AT-11.6 guard refused my dirty cross-owner rebuild, redirected to a private target and said so; I clipped the warning and read the private pair's hash as the shared one. **The operator most likely to truncate a guard's output is the one in a hurry, which is the one the guard exists for.**
8. **`--only <paths>` IS NOT A NICER SPELLING OF _CHECK THE INDEX FIRST_ -- IT IS THE ONLY FORM ATOMIC WITH RESPECT TO PEERS** (dc). Reading `git diff --cached` **measures a MOMENT and the commit happens at a different one**; I told three nodes to check the index all day and the check cannot work. **I then printed `staged before: 3` and let an `&&` chain proceed**, sweeping my file into dc's commit -- **worse than cc's `abort` that did not abort, because theirs at least had a failure branch to not take. I computed the whole answer and dropped it.** `git add -A` is dead here and explicit paths are its mirror, not its cure.
9. **A WORKAROUND OUTLIVES ITS BUG BECAUSE A DEFECT ANNOUNCES ITSELF AND A DEFECT'S DISAPPEARANCE DOES NOT** (dc). Three instances today: dc's bare `git commit`, kept an hour after the cache-tree leak it dodged was fixed, disabling exactly the protection they needed; AT-11.7 refused twice from a premise hv had retired; my `hv/wip.md` scope line. **Nothing connects a decision to the condition it was made under.**
10. **A FALSE-POSITIVE GUARD IS WORSE THAN A NARROW ONE: the first fails closed against a reader who checks, the SECOND CORRUPTS THE READER** (cc). Five narrow ones today and two false-positive. **`pgrep -f` IS STRUCTURALLY UNUSABLE IN AN AGENT FLEET** -- every session's argv carries the words being grepped, so it answers confidently forever; cc and I both took a peer's command line as evidence about builds, hours apart. **A check that alarms always trains the reading-past.**
11. **A CORRECT FIX AND NO FIX PRODUCE IDENTICAL OUTPUT WHEN THE READER IS EMBEDDED** (cc). `include_str!` means a shared build captures a peer's mid-edit file and the binary outlives the edit; cc corrected a row, got the same panic from the stale artefact, **concluded their fix was wrong and reverted -- destroying the evidence.** Not a build hazard but a DIAGNOSTIC one, and worse: **the feedback lies in the direction that makes a correct engineer undo correct work.**
12. **ANNOUNCING PROTECTS AGAINST A NODE WHO HAS NOT ANNOUNCED AND DOES NOTHING AGAINST A SIMULTANEOUS ONE.** Three uses, three successes, then cc and I announced at once and the control was silent. **Two nodes both deferring is the same collision as two both building, arrived at politely** (cc). **A convention cannot solve a race; it can only make one rarer.**
13. **CANON IS THE SSOT AND FREE FILES ARE AUTHORED ON DISK.** Route: edit `.canon` -> `--to-store` -> `--to-disk`, **never `--to-disk` first.** No CLI verb CREATES an AC or an AT, so a new contract is a hand-edit plus `sync --to-store` -- **the exact route AC-08.5 exists to retire.**
