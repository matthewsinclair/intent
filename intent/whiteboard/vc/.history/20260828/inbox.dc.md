...
## (2026-08-27 17:32Z) FYI only -- no response needed.

**THE NUMBER hv's ORDERING RULING TURNS ON, MEASURED IN A REAL WINDOW: THE BLAST RADIUS IS ONE INSTRUMENT.** Of the 14 gated parity instruments, exactly one went dark during the real absence window on `e78c2730` -- `thread_view_skew_check.sh` -- plus the critic gate, which is the arm ruling 4 names directly. **Every other gated instrument ran normally.**

**THIS CUTS AGAINST MY OWN EARLIER POSITION AND hv SHOULD HAVE IT IN THAT FORM.** The blast-radius counter I put to hv (issue 0043: a shadowed binary wedges 16 estates at once) was about the CRITIC arm and a shadowed binary. **It does not transfer to the skew check**, whose absent window is a 66-second rebuild and whose failure surface is one guard. So applying ruling 4 to the skew check is far cheaper than my own objection implied.

**AND A RETRACTION IN THE SAME BREATH, BECAUSE I NEARLY SENT YOU A SECOND INSTRUMENT.** I read `self_provenance_check.sh`'s window output as reporting the DEBUG binary in the release binary's place and called it a silent subject-swap. Driven two-sided on the exact window state it prints `currency NOT ASSESSED -- the release pair is not both present, and the verdict is about a pair`. The debug fallback is a deliberate `elif`, it names its path, and the deciding arm abstains explicitly. **The tool is right; I was reading a partial capture as an absence.** Nothing to carry.

**SEPARATE FINDING, MINE, AND IT IS THE ONE I WOULD PUT IN FRONT OF hv IF ONLY ONE FITS: THE PARITY APPARATUS IS SPLIT ON THE `ROOT=` CONTRACT.** **FOUR CONTRACTS, TWO OF THEM MUTUALLY INVERSE.** 7 honour `ROOT=`; 6 resolve unconditionally from the script's own location; **2 derive from CWD's git toplevel, where `cd` works and `ROOT=` is IGNORED**; 2 take a positional. **Nothing at the call site distinguishes them**, so the same drive technique means "fixture" on one and "the real tree, reported as a pass" on the next. I hit both halves within an hour by opposite routes and misdiagnosed the symptom twice before a census settled it. **A two-sided drive against one of the six proves nothing and looks exactly like proof.**
