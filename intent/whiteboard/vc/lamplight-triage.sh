...
set -uo pipefail
P=~/Devel/prj/Lamplight; cd "$P" || exit 1; I=${VC_INTENT:?set VC_INTENT to a PINNED copy of the pair -- native/rust/target/release is replaced by any build under a live run (2026-08-26: 252 refusals); gate coherence with `int cli --version` first}; L=${VC_SCRATCH:-/tmp/vc-scratch}; mkdir -p "$L"
COMMIT=0; [ "${1:-}" = --commit ] && COMMIT=1
# AN ARRAY, NOT A SPACE-SEPARATED STRING (IN-SH-CODE-001). The string form only
# works by word-splitting at every use site, which is the exact expansion the
# rule forbids -- and it reads as safe because these ids never contain a space.
...
dirty_before=$(git status --porcelain | grep -vE '^.. (mix.lock|intent/todo.md|intent/whiteboard/|apps/|intent/.canon/issues/0005.json)' | wc -l | tr -d ' '); [ "$dirty_before" -eq 0 ] || { echo "tree dirty outside the known peer-owned paths ($dirty_before) -- refusing"; git status --porcelain | head -5; exit 3; }
...
ok=0; for st in "${T[@]}"; do grep -q '^status: Triage' "intent/st/$st/info.md" && ok=$((ok+1)); done; echo "views reading Triage: $ok of ${#T[@]}"
changed=$(git status --porcelain | grep -vE '^.. (mix.lock|intent/todo.md|intent/whiteboard/|apps/|intent/.canon/issues/0005.json)' | awk '{print $2}'); echo "changed paths: $(printf '%s\n' "$changed" | grep -c .)"; printf '%s\n' "$changed" | grep -vE "^intent/(st/(ST0339|ST0340|ST0341|ST0342)/|\.canon/st/(ST0339|ST0340|ST0341|ST0342)\.json|todo\.md|st/steel_threads\.md)" | sed 's/^/UNEXPECTED: /' | head -5
[ "$ok" -eq "${#T[@]}" ] || { echo "NOT DONE: views do not all read Triage"; exit 5; }
# `paths` IS AN ARRAY read line-by-line, not a string split by the shell: a git
# pathspec is exactly the place where an unquoted expansion turns one path with a
# space into two paths that do not exist, and `git add` would take both.
if [ "$COMMIT" -eq 1 ]; then paths=(); while IFS= read -r p; do [ -n "$p" ] && paths+=("$p"); done < <(printf '%s\n' "$changed" | grep -E "^intent/(st/(ST0339|ST0340|ST0341|ST0342)/|\.canon/st/(ST0339|ST0340|ST0341|ST0342)\.json|todo\.md|st/steel_threads\.md)"); git add -- "${paths[@]}" && git commit -q --only "${paths[@]}" -m "intent: the four ex-_inbox threads are Triage (hv, 2026-08-26, via lamplight/vc)
