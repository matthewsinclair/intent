#!/usr/bin/env bash
# lamplight-dehydrate-closed.sh [--commit] -- hv's order 2026-08-26: the closed threads' FLAT views come out tonight.
# Scope is intent/st/ST* ONLY. The v2 bucket trees (COMPLETED/ NOT-STARTED/ CANCELLED/) are NEVER touched here:
# they hold the only copies of ~1,249 prose files the migration did not carry (lamplight-vc measured), ruled
...
set -uo pipefail
P=~/Devel/prj/Lamplight; cd "$P" || exit 1; L=${VC_SCRATCH:-/tmp/vc-scratch}; mkdir -p "$L"
COMMIT=0; [ "${1:-}" = --commit ] && COMMIT=1
echo "## $(date -u +%H:%M:%SZ) HEAD $(git log --oneline -1 | cut -c1-50); flat dirs $(ls -d intent/st/ST* | wc -l | tr -d ' '); staged $(git diff --cached --name-only | wc -l | tr -d ' ')"
...
353 flat thread dirs for an open set of 57 was not acceptable to hv tonight and
the verb that does this (organize --default --force, ST0057 WP-11) is not yet
built on the pair this tree runs on. intent/.intentfiles declares the 57 OPEN
threads (37 not-started, 16 wip, 4 triage) from canon status; the $n closed
