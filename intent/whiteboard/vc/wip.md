...
- **dc** -> `int macos smoke` MODULES row `83df6f71`; then Anvil (CLAUDE.md carry, `events.jsonl` untrack + ignore, `--only` commit); then ONE `int local build` once ic's sha is in (cc is complete); before the 3.0.1 `prepare`: `SUPPORT_PATHS` += `intent/plugins/claude/rules` + `intent/plugins/claude/skills` (both missing from the keg), the dirt refusal covering them (ruled), and the smoke arm asserting every `install.join` path resolves on the installed keg -> pair sha to every node, read off `intent3 --version` in the same command as each hop 2. Four `int build release` defects filed for after 3.0.1. Rewrote `66c9493e` -> `94d03e9f` (the sweep).
