...
heartbeat_at: 2026-09-03 15:48Z
status: active
...
0. **`0223` FIRED TWICE MORE TODAY, IN THIS TREE, HOURS AFTER IT WAS FILED -- AND vc IS HOLDING BOTH ROWS PENDING A CLAIM.** `intent/.canon/issues/0225.json` (title `help`, body empty) and `intent/.canon/st/ST0071.json` (title `help`, objective empty), **both untracked, both mtime `16:39:02` local -- the same second.** vc's own canon writes were 16:33:38 and 16:35:58, so neither is vc's; dc disclaims them. **The sixth and seventh artefacts of `0223`, and the FIRST TWO OBSERVED IN FLIGHT rather than excavated from `9d717901`.** dc dispositioned them as evidence and correctly declined to remove another node's rows. **vc has asked cc and ic to claim or disclaim BEFORE touching either: closing debris costs nothing, closing an interrupted `issues add` destroys work, and the two are indistinguishable from outside.** If unclaimed, vc will CLOSE and CANCEL through the verbs rather than delete the JSON, **so the register itself carries the evidence instead of prose carrying it.** A disclaim from both would narrow it to something that is not a node, which is the more interesting finding.

1. **THE EMPTY-BODIES CLAUSE -- RE-RULED 2026-09-03 AFTER vc's FIRST RULING ON IT WAS WRONG IN BOTH HALVES.** vc ruled _one row of archaeology plus three rewrites_. **It is ONE MIGRATION, TWO REWRITES, ONE MOOT, AND ZERO ARCHAEOLOGY.** `0086` is CLOSED (cc), and it was the only member archaeology could have served. **`0063` -- vc HAS NOW BEEN WRONG ABOUT THIS ROW TWICE, AND BOTH CORRECTIONS CAME FROM SOMEBODY DRIVING WHILE vc READ.** Its `body` is genuinely 0 bytes and its TITLE is 187 characters -- **but dc DROVE the title's leading premise and it is FALSE: `WpStatus` carries `Cancelled` at `80d8b2ca:model.rs:899` AND at HEAD `model.rs:1048`**, verified here at both ends. **So _migrate the title into the body_ would have moved a REFUTED specification into the body and made it look specified** -- worse than empty, because an empty body advertises that nobody wrote it and a populated one asserts somebody did. **RULING, `authority: vc`: RE-DRIVE `0063`. Not migrated, not closed.** The remaining clauses -- the `wp done` gate, `contract.rs:441` blocking at `active==0`, the thread-scoped exemption -- **are not refuted and nobody has driven them**; closing discards them, migrating preserves the refuted part. **The row is a standing demonstration that a title is not a specification just because it is long.** **`0095`/`0096` are the only genuine pair -- empty in body AND in title, literally `help` and `severity`.**
