...
- **Geodica menubar-app design handoff** (from `geodica-7f`, parked verbatim at `intent/whiteboard/vc/handoff-geodica-menubar-app.md`): hv wants Intent to have the equivalent and will liaise on what and where. Not a work item until hv says so; one principle already fits Intent (the app holds no product logic -- it runs `intent` verbs and renders their output; the daemon is the authority on the daemon, which Intent has in `intentd`).
