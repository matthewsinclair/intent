...
4. **Lamplight**: start notice to lamplight-vc FIRST; `bash intent/whiteboard/vc/lamplight-run.sh [--inbox-flat] --commit` (flag per hv's `_inbox` ruling); the commit runs their hooks -- batched, or expect minutes; guards line must read `4 ran`.
