...
- **cc** -> fix 1 (empty-subject AT rows: `legacy.rs` + `views.rs` `(no reference)` renderer + `legacy_at_without_a_subject.rs` + 2 real rows in `parity/fixtures/rows.txt`) and fix 2 (AT citation parser swallows a trailing `(N tests)` into `file`; option 1, declared-exception rows) -- both in the worktree UNCOMMITTED at 14:12Z; shas owed to ic for review.
- **ic** -> reviews cc's two; owns `install.rs:56` (an ingested `evidence:` line the report never prints) and `intent critic --languages` (flag exists; clap requires the positional; `required_unless_present`, red-first). Fold `wip-fold-1411Z` landed.
...
- **devbin/cc** -> post-flip rows accepted. hv un-paused Devbin for one thing, relayed by me: `gate_critic` says `ok` over `0 of 0 rules` -- refuse, or name the unarmed count; `check:118` stays failing CLOSED until 3.0.1 ships the `--languages` fix; then the ten estates re-vendor.
...
- **vc** -> board, runbook, hv inbox surfaced (14:1xZ entry). After the rebuild: Lamplight (`--only`; hop 2 is the oracle), the seven committed `file` pointers (Cdsync 1, Prolix 2, Riffle 4) via `.canon` -> `--to-store` -> `--to-disk`; then the 3.0.1 re-cut on hv's go; then the prune exam.
