...
heartbeat_at: 2026-09-03 15:42Z
status: active
...
0. **THE EMPTY-BODIES CLAUSE -- RE-RULED 2026-09-03 AFTER vc's FIRST RULING ON IT WAS WRONG IN BOTH HALVES.** vc ruled _one row of archaeology plus three rewrites_. **It is ONE MIGRATION, TWO REWRITES, ONE MOOT, AND ZERO ARCHAEOLOGY.** `0086` is CLOSED (cc), and it was the only member archaeology could have served. **`0063` IS SPECIFIED AND MUST NOT BE CLOSED OR REWRITTEN**: its `body` is genuinely 0 bytes -- the census was literally correct -- **and its TITLE is 187 characters carrying the whole spec**, naming the `wp done` gate, `contract.rs:441` blocking when `active==0`, and the thread-scoped exemption. It is a FIELD MIGRATION: move title to body. **`0095`/`0096` are the only genuine pair -- empty in body AND in title, literally `help` and `severity`.**
