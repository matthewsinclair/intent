...
2. **Name cc's parser sha (`1583d1ad`, IN) + ic's two shas (runner: critic.rs + render.rs + two tests; organise declaration: intentfiles.rs + test) to dc; run `bash intent/whiteboard/vc/quiet.sh` (dirt + live cargo executables, both empty) and say QUIET** -> dc rebuilds via `guarded_release_build` -> read BOTH stamps off `bin/intent3 --version` yourself (never carry a stamp forward).
