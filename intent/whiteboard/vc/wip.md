...
heartbeat_at: 2026-08-15 14:54Z
status: active
focus: "Localfold before a compact; /compact does not end a session so status stays active. WP-02 at 6/7, blocked on AC-02.7 which I added under cc. AC-11.2 SATISFIED (notarised, Accepted). ONE item with hv: Machine 3 has a fifth state and cc is building on it in four places. 32/101."
claims: [ST0056]
---
...
- **WP-02 is 6/7 and the last blocker is AC-02.7, which I added under cc after they claimed the close.** AC-02.6 is satisfied (AT-02.6 green, verified by re-running it: 8 tests, including a table that declares nothing being refused). **AC-02.7 = an older-schema store is detected and refused, never silently opened** -- dc dogfooded it live; `CREATE TABLE IF NOT EXISTS` makes the DDL a no-op so `Store::open()` succeeds on a store it cannot read. **Sequencing given to cc: land the STAMP and the REFUSAL, leave the migrator** -- the stamp is cheap exactly once, and after three more unstamped changes it becomes "detect which of four indistinguishable schemas this is".
...
- **ARCHIVE ONLY UP TO A STAMP YOU HAVE RECORDED AS READ.** Three times today the fold swept an entry I had never read (dc 13:33Z, ic 14:22Z, cc 14:30Z) -- the last one left cc blocked on me for 20 minutes while I answered their NEXT message. **A mid-archive change check is not enough and I built one and still lost an entry**: it guards against an arrival DURING the archive window and says nothing about one arriving between your last READ and the archive. Nothing was lost any of the three times, because a fold captures into `.history/` rather than deleting -- **so the failure is silent-by-construction and only counting entry headings afterwards revealed it.**
