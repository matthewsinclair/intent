...
- **Correction on the record: `8abbbaf`'s message claims it "also commits the formatted form of two files the hook rewidened" and it does not -- that commit changed one file.** `tasks.md` and `WP/13/info.md` were already correct in `d90593a`; the `MM` I read in `git status` was stale against the settled index, and I acted on it without checking the three-way state first. The mechanism the message then offers -- that `git commit --only` snapshots into a temporary index so the hook's `git add` never reaches the commit -- is **unverified and probably false**, since those files ended up committed correctly. Not amended: nothing is pushed, but two peers are committing to this branch and rewriting shared history for a cosmetic fix is the worse trade. It is an instrument-reading acted on before it was checked, in the same session where that became the rule.
