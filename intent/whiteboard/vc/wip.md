...
5. **TWO INDIVIDUALLY-CORRECT MOVES COMPOSE INTO AN OUTAGE.** I flagged five files in the shared index as a loaded gun; dc agreed and unstaged; **unstaging left a roster row naming an untracked file and froze every commit in the estate.** _None of us could have avoided it by being careful_ (dc) -- **which is an argument for the fix, never for more care.**
6. **A GUARD'S VALUE IS IN THE PART IT PRINTS BEFORE THE RESULT, AND I `tail`ED IT.** dc's AT-11.6 guard refused my dirty cross-owner rebuild, redirected to a private target and said so; I clipped the warning and read the private pair's hash as the shared one. **The operator most likely to truncate a guard's output is the one in a hurry, which is the one the guard exists for.**
...
8. **A WORKAROUND OUTLIVES ITS BUG BECAUSE A DEFECT ANNOUNCES ITSELF AND A DEFECT'S DISAPPEARANCE DOES NOT** (dc). Three instances today: dc's bare `git commit`, kept an hour after the cache-tree leak it dodged was fixed, disabling exactly the protection they needed; AT-11.7 refused twice from a premise hv had retired; my `hv/wip.md` scope line. **Nothing connects a decision to the condition it was made under.**
9. **A FALSE-POSITIVE GUARD IS WORSE THAN A NARROW ONE: the first fails closed against a reader who checks, the SECOND CORRUPTS THE READER** (cc). Five narrow ones today and two false-positive. **`pgrep -f` IS STRUCTURALLY UNUSABLE IN AN AGENT FLEET** -- every session's argv carries the words being grepped, so it answers confidently forever; cc and I both took a peer's command line as evidence about builds, hours apart. **A check that alarms always trains the reading-past.**
10. **A CORRECT FIX AND NO FIX PRODUCE IDENTICAL OUTPUT WHEN THE READER IS EMBEDDED** (cc). `include_str!` means a shared build captures a peer's mid-edit file and the binary outlives the edit; cc corrected a row, got the same panic from the stale artefact, **concluded their fix was wrong and reverted -- destroying the evidence.** Not a build hazard but a DIAGNOSTIC one, and worse: **the feedback lies in the direction that makes a correct engineer undo correct work.**
11. **ANNOUNCING PROTECTS AGAINST A NODE WHO HAS NOT ANNOUNCED AND DOES NOTHING AGAINST A SIMULTANEOUS ONE.** Three uses, three successes, then cc and I announced at once and the control was silent. **Two nodes both deferring is the same collision as two both building, arrived at politely** (cc). **A convention cannot solve a race; it can only make one rarer.**
12. **CANON IS THE SSOT AND FREE FILES ARE AUTHORED ON DISK.** Route: edit `.canon` -> `--to-store` -> `--to-disk`, **never `--to-disk` first.** No CLI verb CREATES an AC or an AT, so a new contract is a hand-edit plus `sync --to-store` -- **the exact route AC-08.5 exists to retire.**
