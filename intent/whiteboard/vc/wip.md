...
- **cc** -> fix 1 (empty-subject AT rows: `legacy.rs` + `views.rs` `(no reference)` renderer + `legacy_at_without_a_subject.rs` + 2 real rows in `parity/fixtures/rows.txt`) and fix 2 (AT citation parser swallows a trailing `(N tests)` into `file`; option 1, declared-exception rows) -- fix 1 IN `e696de15` (five files `--only`; crate suite 906 passed; the corpus is a registered attachment so canon was synced in the same commit). Fix 2 IN `80cb8509` (913 passed; the staleness arm mutation-proved both ways; the corpus appends four rows and moves none). cc's batch contribution COMPLETE. Next for cc on their bounce, same subject: TODO 7 (one canon emitter, attachments) and ST0057/11 (WIP-only default disposition; hv's ruling recorded in the WP).
- **ic** -> `intent critic --languages` FIXED `e2a01fd1` (1185 passed, clippy -D warnings rc=0, five drives incl. the leak control); `install.rs:56` -- my board's wording was a paraphrase through a compact and ic has read the file (it is `InstallError::NotFound` with a wrong remedy); ic builds it only on hv's confirmation, 3.0.1. Reviews cc's two (written, red-first honoured, green pending the shared target cc holds; its ONLY consumer is devbin's `check:118`, which fails closed -- the shipped hook dispatches the positional at `pre-commit.sh:368` and was never affected; vc's fail-open claim corrected in the runbook and hv's inbox) Fold `wip-fold-1411Z` landed.
- **dc** -> Anvil DONE `b63ba9c`; the split pair (`int build all` was a bare cargo build) rebuilt as a set `74bb3aea`; then the build entrances unified `7456a158` (one guarded release path behind every verb; `cli`/`daemon` refuse release into the shared target; `int cli` freshest-wins) -- item (1) COMPLETE; waits on cc's parser sha + ic's runner sha for the ONE rebuild that carries 3.0.1; `SUPPORT_PATHS` + `support_paths_coverage` (every `install.join` chain) + `int macos smoke` DONE `0112b8c1`, driven red on the keg and green on a fixture keg (`total: 6 rule(s)`); smoke's default asserts the keg on disk with its reduced claim printed, cold `--reinstall` opt-in (ruled) -> pair sha to every node, read off `intent3 --version` in the same command as each hop 2. Four `int build release` defects filed for after 3.0.1. Rewrote `66c9493e` -> `94d03e9f` (the sweep).
...
- **vc** -> board, runbook, hv inbox surfaced (14:1xZ entry). The seven committed `file` pointers DONE via the canon route (Cdsync `41fd2fb`, Riffle `5982ff7`, Prolix `1298531`); three tool findings on the way (Finder droppings refuse the sync; two `.canon` emitters disagree on `attachments`; empty-title trailing space), in the runbook and filed for cc. Lamplight MIGRATED THEN REVERTED: hop 2 refused on a duplicate AT id (two threads, demoted), then said ok over four `_inbox` threads the binary refused (moved flat, re-converted: 353 threads), hop 3 clean, verifier 0 failed -- and the AT accounting found 25 rows lost across ten threads, so nothing was committed and the tree is back on v2 (two demotions kept). Arca: arca_cli `33e3c2d` LOSSY (26 of 55 ATs), kept unpushed as evidence; arca_config untouched. ALL MIGRATION STOPPED until cc's parser fix; re-conversions on the fixed pair tonight. Then the 3.0.1 re-cut on hv's go; then the prune exam, MEASURED under step 8. `IN-AG-RED-CONTROL-001` landed (hv's ruling via devbin/vc).

## CRITICAL PATH

1-6. **DONE.** Tag `v3.0.0` -> `80d8b2ca` (pushed to both remotes after `main`, CI green macOS + Ubuntu); fold spliced; dc's build on the tag and the pair sha at every hop 2; ST0057 canon flip applied, gate PASS 53/53 (`5c3b1967`); prepare -> formula -> `int macos publish` under hv's grant -> tap install over the network (644 defect: generator `ded669c2`, formula `revision 1`, republished) -> arms a/b/c; **THE FLIP 2026-08-26 14:00:46Z**: `brew link intent` rc=0, `brew test intent` rc=0, `command -v intent` -> `/opt/homebrew/Cellar/intent/3.0.0_1/libexec/bin/intent` (`-rwxr-xr-x`, `intent 3.0.0 (80d8b2ca...)`); post-flip verifier: sixteen at 3.0.0, bare `intent st list` rc=0 in all; `use dev|prod` driven for real by dc.

7. **Batch window CLOSED -- COMPLETE:** cc's two IN (`e696de15`, `80cb8509`), ic's one IN (`e2a01fd1`); dc's ONE rebuild waits only on hv's go; `converge_gitignore` (`.backup/`, `intent/.backup/`, `intent/events.jsonl`), the template's "Preserved across regeneration." sentence -> dc rebuilds once -> stragglers on the new pair (arca_cli, arca_config, Lamplight, Anvil, seven canon pointers) -> 3.0.1 re-cut on hv's go. **QUEUED BEHIND IT, ON hv's LAKSA FINDING (tool work, no sweep until it lands):** one `.canon` emitter so canon carries attachments; migration + upgrade write `.intentfiles` = open threads; dehydration preconditions in the tool, not per-project ACs; `st dehydrate` (ST0061); then dc's one `organize --apply` sweep. ALSO 3.0.1: the keg ships NO rule library (`SUPPORT_PATHS` stages `lib/templates` only), so every critic gate on it passes over 0 armed rules -- dc packages the rules, ic makes the runner/hook refuse on 0 armed; the AT `file` root; the clippy socket (five to dc).

8. **hv's final exam:** prune `Intent/bin` of every v2 script except what devbin needs (`bin/int`, `bin/.devbin/`, whatever devbin's `cmd/*` still `source` -- MEASURE it, do not guess); `tests/run_tests.sh` retires with its subject except what devbin keeps; the three v2 `intent` links on PATH go with it; then the tests run. Only then is it known to have worked. **Measured, read-only:** devbin's only hard v2 dependency is ONE file -- `cmd/build.d/release` sources `bin/intent_helpers` (:67) and calls `bin/intent` for `doctor` (:373), `agents sync` (:663) and `claude upgrade --apply` (:690); `tests/run_tests.sh` sources `bin/intent_helpers` too; `config.yaml:24` already plans `link_alias` to put `bin/intent` back as a link once the v2 file is gone. The bats estate: 60 files reach the CLI through `run_intent` -> `$INTENT_BIN` (default `bin/intent`; the override exists and is what ST0056 parity used) and flip with it; 30 call `bin/intent_*` directly and retire with their subject; 19 exercise no binary (skills, guards, templates) and stay; 13 name devbin.

## HELD FOR hv

- **Geodica menubar-app design handoff** (from `geodica-7f`; now `ST0064/design-menubar-app.md`, WP `ST0064/01`, on hv's instruction): hv wants Intent to have the equivalent and will liaise on what and where. Not a work item until hv says so; one principle already fits Intent (the app holds no product logic -- it runs `intent` verbs and renders their output; the daemon is the authority on the daemon, which Intent has in `intentd`).
