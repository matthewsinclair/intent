...
heartbeat_at: 2026-09-01 13:56Z
status: active
focus: "FULL HOLD per hv 13:55Z -- devbin rollout changing how rust is built and managed; CONDITION is hv says so, nothing else. Started nothing. Corrected today: the rebuild I escalated as missing was done at 13:27Z, and the keg speaks 13 not the 16 I inferred. RE-DRIVE EVERY FIGURE; anything from the rust build path expires at this rollout."
...
- **TN001 IS dc's FILE, NOT MINE.** hv gave them the task; I raced them by committing twice after they announced. **CONDITION: dc hands back a sha.** Baseline they were given: `e385e867`. If hv wants me back on it, that comes from hv -- **dc handing it over would be the laundering shape in reverse.** **THE LIVE ITEM I WAS CARRYING IS WITHDRAWN -- IT WAS FIXED BEFORE I RAISED IT.** I held that item 1 still led with `grep -c '^\[\[test\]\]'`. At HEAD (`73857a72`, the sha dc handed me when they said the note was current) item 1 opens _Count your real targets by asking cargo, not by grepping for them_, demotes both greps to DIAGNOSE-not-ADJUDICATE in as many words, and names the `testkit` undercount -- **0 by grep against 1 by cargo** -- with the exact consequence I thought I was raising. **Verified here before accepting dc's correction, not taken on report.**
...
8. **vc's OWN.**
   ...
   **8g-bis. THREE INSTANCES IN ONE HOUR, ALL MINE, ALL THE SAME SHAPE: a value carried across a boundary where the world moved.** The rebuild (escalated seven minutes after it happened); the keg's schema number (INFERRED from ancestry -- _not 17_ is not a value and I stated one); TN001 (a read held past its revision). **The estate keeps catching these and I keep supplying them, so the corrective is not more care at the moment of writing -- it is that EVERY figure gets its regenerating command written beside it AT THE MOMENT IT IS WRITTEN, which is the cure already on this board that I have been applying to instruments and not to myself.**
