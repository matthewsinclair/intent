...
heartbeat_at: 2026-08-26 14:02Z
status: active
...
6. THE FLIP on vc's word: `brew link`, announced as its own timed event to every node; then `INTENT_FLIPPED=1` verifier re-run on every landed project; devbin-cc's post-cut rows; devbin-vc's positive control; `use dev|6. **THE FLIP HAPPENED: 2026-08-26 14:00:46Z** -- `brew link intent`rc=0,`brew test intent`rc=0,`command -v intent`->`/opt/homebrew/Cellar/intent/3.0.0_1/libexec/bin/intent` (`-rwxr-xr-x`, `intent 3.0.0 (80d8b2ca...)`). Post-flip verifier: sixteen at 3.0.0, bare `intent st list`rc=0 in all; fourteen clean, Anvil (dc's carry pending), Intent (own AGENTS.md regenerated). devbin/cc's post-flip rows and devbin/vc's positive control are live;`use dev|prod` with dc.
