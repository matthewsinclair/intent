...
- **dc DELIVERED: CI un-swallowed and green THROUGH a workflow that can now fail, guards wired, `*.bak` class closed, pre-push clone-build built.** Two corrections taken: the `$?` half of my tests.yml claim was wrong (one defect, not two -- verified at 42/7), and `rust-toolchain.toml` is REFUSED not deferred, because rustup is not installed so the pin binds CI alone while reading as a project guarantee. **Next, ruled FIRST above everything: `int install hooks`** -- `.git/hooks/` is never tracked, so a fresh clone gets every guard and nothing invoking them. The guards are in the half-move's exact state: correct in the working copy, absent from what anyone else receives. The CHECK matters more than the installer, and it must report UNWIRED (a hook present but calling something else) rather than merely MISSING.
...
- **THIS REPOSITORY IS PUBLIC** (`gh repo view` -> `isPrivate: false`; dc found it, I re-ran it). The machine's environment brief says "assume private", which is wrong in the dangerous direction. **60 whiteboard files are tracked**, so every board and inbox is world-readable at push to `upstream` (`local` is Dropbox and private). Not changing how we write -- the candour is the value -- but the `-A` hazard is now a PUBLICATION hazard into a history that cannot be rewritten. Check `git status` for untracked strays before any commit, not only the paths you name.
...
- (2026-08-15) **A rule true in its own scope is the easiest kind to over-apply, because it keeps being true wherever you check it.** Three instances in one day: ic's unstaging rule (right conclusion, backwards reasoning), cc's `unbuilt_fields` test (right about the set it looked at, wrong set), and mine -- I applied the pipe `$?` trap to `A || echo "$?"`, where `$?` correctly reports A. dc measured it at 42 and 7. **Check the SHAPE matches the shape the rule was measured on**, not just that the rule is sound.
