...
**THE INVERSION IS BUILT AND GREEN, AND TWO THINGS IT SURFACED ARE YOURS.** On the file as well as in the channel because both are rulings that must survive a bounce on either side.

**WHAT LANDED.** Routing is opt-in behind a global `--daemon` (hv's own spelling, verbatim from the ruling). `SERVED_BY_DAEMON` is now a CAPABILITY list rather than a routing policy -- `routed_paths()` became `daemon_servable_paths()`, same data, honest name. `daemon_fallback.rs` became `routing_is_opt_in.rs` because that file's subject stopped existing: with the default local there is no fallthrough to have an opinion about. **Every property the old file asserted still holds somewhere in the new one**, including your sync carve-out and its no-daemon control, verbatim in substance.

**THE INVERSION MINTED A HOLE THAT YOUR COUNTER USED TO CLOSE FOR FREE.** Under mandatory routing, a path declared servable whose renderer arm forgot to call `served()` showed as a zero delta on the daemon's dispatch counter. **With the default local, the conformance harness never asks a daemon anything, so the declaration lost its witness** -- and I would not have noticed for weeks. `every_servable_path_asked_with_daemon_actually_leaves_this_process` replaces it and is COMPLETE over the declared set: against a probe-only fixture an invocation that really left the process cannot succeed, so **rc=0 is the tell that the arm never routed**.

**(1) RULING NEEDED: THE SYNC CARVE-OUT IS NOW THE ONLY PLACE A DAEMON'S PRESENCE CHANGES A LOCAL COMMAND, AND I KEPT IT UNCHANGED ON PURPOSE.** hv's reversal is about ROUTING; mutual exclusion is a different question and hv did not answer it. Your parenthetical is literally true of this family -- a daemon watching a project (`AC-08.5`) while `intent sync` runs in it really would watch and ingest twice -- so **opt-in routing makes the carve-out MORE load-bearing, not less**: it used to be one refusal among many on a daemon machine and it is now the only thing between a watching daemon and a second engine. **But the predicate is WIDER than the hazard**: it fires on any answering daemon, not on one watching THIS project. The right predicate is knowable -- the daemon has a registry and `AC-08.5` will have a watch set -- and until `AC-08.5` exists the honest answer to _is a daemon watching this project_ is always no. **I have not narrowed it. Refusing preserves and running does not, and it is your row.**

**(2) `design.md:22` NOW CONTRADICTS THE SHIPPED BINARY, AND THAT IS WORSE THAN IT WAS THIS MORNING.** Until today the line and the code agreed, so a reader could take either and be right. Now the ratified design says the CLI MUST route to an answering daemon and the build says it must not. You declined to edit it under the pen and flagged it to hv; **hv has since ruled the opposite of what the line says.** A design line disagreeing with the binary is the many-homes defect in the most authoritative home there is, and **the next node to read it will build to the line.** Yours or hv's hand, never mine.

**NEXT FROM ME.** `Route::Daemon` in the conformance harness now means _driven with `--daemon` against a real intentd_, which is the shape `AC-08.2` actually needs, and then the `AC-08.4`-`.9` block.

## (2026-08-30 16:29Z)

**BOUNCE HANDOVER, AND THE FIRST ITEM IS A RULING I NEED FROM YOU.**

**`intent init` NEVER MINTS A `project_id`, AND D20's BOTH SUBSCRIPTION EVENTS ARE KEYED ON ONE.** Measured, not read: `intentsvcs::init::init` writes a config with no such field; `grep -rn 'project_id: Some|project_id = Some|set_project_id'` over `intentsvcs/src/` returns NOTHING; `Config::project_id` is `Option<String>` reached through `unwrap_or_default()` at seven sites, so the absence becomes an empty string rather than an error; `uuid` is pinned in the workspace with a rationale naming _(project_id, natural_id)_ as the global identity and its only use in the estate is `facade.rs:294`, for something else. **This repository has one only because the v2 migration wrote it**, which is why nobody has hit it. D15 ratifies the seam. **So it is a seam with no minting, not a design choice** -- and every project v3 creates today emits events naming no project, with two such projects indistinguishable to a subscriber. **I did not change `init`**: minting an identity on a ratified seam is a design act and it is yours. If you rule `init` should mint one it is small; if you rule the seam stays optional, `AC-08.6` needs to say what an event names when there is none, because `""` is not an answer.

**AND IT WAS DEGRADING FOUR OF FIVE ARMS BEFORE ANYTHING NOTICED.** Three assert `event.project_id == feed.project_id` and both sides were `""`. They passed. The only thing that caught it was a guard written reflexively into the fourth -- _the fixture project has no project_id, so this arm cannot discriminate_. **Without that one line the row would have gone green over four vacuous assertions.**

=== WHAT LANDED, AND WHERE THE RECORD IS ===

`dfbe68a7` routing is opt-in (hv's reversal). `1d7bcdb2` `AC-08.2` driven against a real `intentd`. `24f317c5` your three stale homes plus a fourth. `5579a7dc` `AC-08.5`, the watcher. **`AC-08.6` IS IN `5b76a49e`, WHICH IS ic's COMMIT UNDER ic's MESSAGE** -- their `--amend` with no pathspec took seven of my staged files. Bytes correct, tests green (`-p intentd -p intentsvcs`: 162 targets, 1260 tests, 0 failures on exactly that content), and **no verb can edit a commit message**, so this entry is the record instead. Nobody is rewriting a shared HEAD to fix a message.

**`AC-08.6` IN SUBSTANCE:** D20's ceiling is now held by the COMPILER -- `the_subscriptions_are_exactly_d20s_two` matches exhaustively over `Event`, so a third variant fails to BUILD rather than fails review. The two events are two because of which LAYER moved: `fileChanged` from the watcher, the only thing that knows the paths; `projectChanged` from the store thread after the re-read, the only thing that knows it finished -- collapsing them would tell a subscriber the model changed before it had. `every_subscriber_receives_every_event` is the arm I would not skip: an mpsc fan-out gives each event to exactly ONE listener, indistinguishable from a broadcast with one subscriber and catastrophic with two. A lagged subscriber is DISCONNECTED rather than skipped, because a feed with a hole in it is indistinguishable from one without. `Op::Subscribe` joins `Op::Registry` in `wire::UNCOUNTED` -- it binds a connection and never reaches a store, so it must not move your `AC-08.2` discriminator.
