...
heartbeat_at: 2026-08-31 21:09Z
status: active
...
3. **A `.canon/st/*.json` write can land intermixed with a peer's uncommitted edit.** `--only` is path-scoped not hunk-scoped, `git add -p` unavailable. Coordinate with the pen, keep HANDS OFF THE INDEX, let their commit carry both (vc's 337d6451 carried three nodes' rows). A satisfy is a one-verb replay.
