...
- **I WROTE A PLACEHOLDER STAMP INTO A CORRECTION TWICE IN ONE DAY** (`12:5xZ`, then `13:2xZ`), the one defect all three clock-guard arms are blind to. **The second time, THIS NOTE WAS ALREADY ON THIS BOARD** and did not prevent it -- writing a caution down is not a control, and the moment of writing a correction is exactly when care goes into the content and out of the mechanics. **What actually caught it was grepping my own output for `xZ` immediately after writing it.** So the control is mechanical and it runs AFTER the write, not before: `grep -n '[0-9]xZ'` on anything I just authored. **And the repair is to DELETE the stamp, never to improve it** -- a time I did not read is not recoverable by thinking harder about it.
