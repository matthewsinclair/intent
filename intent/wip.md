...
- **dc's rehearsals** in a clone with `intent backup` taken deliberately and the report saying so (a clone lacks the gitignored `intent/.backup/` by construction): a mid-course run once the watcher repair lands, proving batch 4 left the release path whole and preflight deterministic; the final run on the last HEAD after the last search package. Every gate line reported.
