...
- The design and build plan are `intent/st/ST0069/design.md`; the packages are WP-17 to WP-24 with their criteria. Ships in 3.0.2 (hv). Each package lands on main as it closes; the second dry-run rehearsal and the cut follow the last package. Closed and verified: WP-17, WP-18, WP-19, WP-21. In flight: cc on WP-23's seams (the embedder trait, the refusing Null and the HTTP embedder behind a stub, the vector rung, the semantic group proven through a test-only embedder) and the whole-scope refresh door dc needs; dc on WP-22 (the roster becomes a constructor, `--no-reconcile` on the search row, the daemonless path reconciles the whole scope before it queries, one parity arm) then WP-24's PostToolUse hook with the PreToolUse redirect specified not built; ic on the AT-24.5 row then the AC-24.7 traceability spec beside the design. WP-20 and WP-24 stay WIP on hv's halves. Then the quiet window and the final rehearsal. hv rules the design's listed decisions as each comes due.
