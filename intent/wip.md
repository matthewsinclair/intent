...
- **Five decisions from ST0069, each ready for a one-line answer:** (1) the grammar line, recommended four on by default (rust, elixir, swift, lua) with bash declared and off because its grammar ships no tags query; the measured table is in the design; (2) the omnibox for the explorer, kept out of 3.0.2 as a front-door change; (3) `intent modules find` retirement (AC-20.6), the canon now names the index first and keeps the registry lookup; (4) the Local embedder runtime (AC-23.4), once dc measures the candidate shapes; (5) the go on re-keying ST0073's AC-05.1 through the CLI on the closed thread. AC-20.4 and AC-20.6 stay open on (1) and (3).
