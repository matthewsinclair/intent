...
- The design and build plan are `intent/st/ST0069/design.md`; the packages are WP-17 to WP-24 with their criteria. Ships in 3.0.2 (hv). Each package lands on main as it closes; the second dry-run rehearsal and the cut follow the last package. Closed and verified: WP-17, WP-18, WP-19, WP-21; WP-23 landed and closing on AC-23.4's Local-runtime record; WP-20 and WP-24 open only on hv's halves. The QUIET WINDOW is open (2026-09-12 14:14Z): cc and ic hold, no builds or tests on the box until vc lifts it; dc runs serially: WP-22 finished and landed (the roster becomes a constructor, `--no-reconcile`, the daemonless reconcile-then-query through `index_refresh(None)`, one parity arm), the PostToolUse hook with AT-24.1 cited and AC-24.6 reviewed in one message, the two Local-runtime shapes measured (fastembed on ONNX, candle) sizes only, then the final rehearsal on the last HEAD. hv rules the design's listed decisions as each comes due.
