...
- **2026-04-15**: v2.8.2 released -- ST0033 (cwd-resilient dispatch) and ST0032 (Credo check wiring) complete. `intent` subcommands now work from any subdirectory of a project; outside any project, fail cleanly without creating stray `.intent/` or `intent/`. Credo custom checks wired into `.credo.exs` via `lib/scripts/configure_credo.exs`.
