...
- **From the mid-course rehearsal (2026-09-12), what its now-reporting test gate named:** the swallowed-output defect is fixed (c837cf4ad) and the shim-wording test is fixed (b4cbda03e); ic's stale label assertion is fixed (d52d21135); **open: `rules_path_guard.bats`, five propagated artefacts cite the local rules path `intent/plugins/claude/rules/` (the in-review skill and four critic subagents), real and on main since the doc audit, ic rewords them to the install's library, the guard is the red.** The final rehearsal needs it closed.
