...
| #   | id     | sev    | lane  | defect                                                                                                                                                                                                                                                                                               |
| --- | ------ | ------ | ----- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 64  | `0100` | high   | cc    | An unmappable WP status is defaulted silently. hv said go (decision 2). LANDED at 9046156b (store rung 17 -> 18); vc to drive.                                                                                                                                                                       |
| 65  | `0084` | medium | cc    | A retired refusal left two survivors in ingest. hv said go (decision 11). LANDED at d4b31f5c (byte writes for opaque attachments); vc to drive, with the Latin-1 migrator case.                                                                                                                      |
| 66  | `0066` | medium | cc    | `_inbox/` is invisible to st show/list, ac gate, at lint, todo.                                                                                                                                                                                                                                      |
| 68  | `0065` | medium | dc    | hv said go (decision 14): dc's S design as written. The acknowledgement lives in project config under `doctor.acknowledged`, and an acknowledged class drops OUT of the finding count and the exit code. After 0259 half 2.                                                                          |
| 70  | `0283` | medium | dc+cc | `doctor` and `organize` never consider the store being newer. RULED A+B (vc, 2026-09-11): dc makes both messages true; cc makes the projection refresh an undeclared on-disk view that is exactly the pre-change render. Closes when both land. HALF B VERIFIED (vc, fe8775ed); half A (dc) pending. |
...
| 72  | `0145` | medium | cc    | `st edit` writes on its refusal, and the remedy names an empty list.                                                                                                                                                                                                                                 |
| 73  | `0146` | medium | cc    | `at edit` is kind-blind and its remedy produces a row lint cannot judge.                                                                                                                                                                                                                             |
| 74  | `0153` | medium | cc    | `intent edit` refuses the address form its own remedy recommends.                                                                                                                                                                                                                                    |
| 75  | `0150` | medium | dc    | `skills list` cannot see an orphaned skill directory.                                                                                                                                                                                                                                                |
| 77  | `0176` | high   | cc    | `todo notdone`/`toggle` mutate around the Facade.                                                                                                                                                                                                                                                    |
...
| 84  | `0231` | medium | cc    | TUI repaints can tear on real terminals. LANDED at ea06d7c3; vc to drive.                                                                                                                                                                                                                            |
