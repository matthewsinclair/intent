...
- The design and build plan are `intent/st/ST0069/design.md`; the packages are WP-17 to WP-24 with their criteria. Ships in 3.0.2 (hv). Each package lands on main as it closes; the second dry-run rehearsal and the cut follow the last package. Every package of the 3.0.2 scope, WP-17 to WP-24, is closed and verified; the thread stays open on its other two legs. The QUIET WINDOW is open (2026-09-12 14:19Z): ic holds, and cc holds but for one measurement build, no tests or drives on the box until vc lifts it. cc's fix for issue 0304 is landed (a document the store carries and the disk holds answered a search twice; the disk corpus now excludes what the store carries, one arm driven to both verdicts). The final rehearsal on 25af41fba, the WP-24 close, is green through the workspace suite at a measured load of 21 after two reds at 82 and 84; dc now runs the previews after preflight in the same clone, isolated, with gh's config dir pointed at the real one. The cut waits on hv's specific go. The wide red population cc met on a clean HEAD was an absent sibling intentd in a per-package target dir, cleared by building it, so the rehearsal's workspace run does not meet it. Four Laksa nodes now share the box, so the load floor is above what dc measured. hv rules the design's listed decisions as each comes due.
