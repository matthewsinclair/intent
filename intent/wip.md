---
verblock: "29 Jun 2026:v0.94: matts - v2.13.1 SHIPPED (ST0048 close-gate + ST0049 release notes)"
...
**v2.13.1 is SHIPPED** (ST0048 close-gate fail-by-default + ST0049 release notes; the `cp -R` backup fix `e36d3b3` rode along). Fleet picks up v2.13.1 on each member's next `intent upgrade` -- and hits the close-gate change (un-authored in-flight units stop closing until they author ACs or mark `acceptance: exempt`). Open: push the local wrap commit `c0eeefe`. Standing backlog:
