...
- The design and build plan are `intent/st/ST0069/design.md`; the packages are WP-17 to WP-24 with their criteria. Ships in 3.0.2 (hv). Each package lands on main as it closes; the second dry-run rehearsal and the cut follow the last package. Lanes: cc the engine (WP-18's writer, facade operations and the engine half of its watcher, all landed; next the source FTS table for ic's recall fixture, then WP-20's integration half, the symbols table and `--kind def` from the store, then WP-23); dc the daemon and install (the grammar-size measurement, done; WP-24's `.mcp.json` canon; the watcher's daemon half in `watch.rs`, registering the source directories and dispatching into cc's reconcile; WP-22; WP-24's PostToolUse hook, the PreToolUse redirect specified not built); ic the surfaces (WP-19's last two criteria once cc's corpus lands; WP-20's pure half, `index::symbols` over each grammar's own tags query behind per-language features; WP-24's `--outline` and `--context` flags on the `search` row, the register-generated descriptions, the canon sweep with its file list first). Ruled today: D29 as the landed test holds it; two tables, one writer each; hits are button rows; the editor opens the file and the row names the line; `intent modules find` stays until hv rules its retirement; the omnibox stays out pending hv. hv rules the design's listed decisions as each comes due.
