...
**Standing:** BATCH 1 LANDED and verified by vc: items 2 and 3 (`597a9f26f`, `9173bbb63`; tap formula local at `9987a93`, unpushed) and item 1 (`2f90fd283`: preflight refuses faces that disagree with VERSION, and the stamp regenerates and reads them back). CHANGELOG retitled at `b9a8deb5e`. BATCH 2 LANDED and verified by vc at `4699fb500`: both uninstall verbs hold a unit edited since install at rc 1, and `--force` removes it and names the discarded checksum. Batch 3 is in progress (cc). dc's first `--dry-run` rehearsal ran on a clone of HEAD with the gates live and REFUSED at the doctor gate; the second run, on the final HEAD, is owed once the blockers below clear.

**BLOCKING THE CUT, surfaced to hv at 2026-09-12 06:36Z:**

1. **Doctor refuses preflight on view-banner skew.** The generated views of ST0046, ST0059, ST0061, ST0062, ST0063, ST0065, ST0066, ST0072 and ST0073 still carry the v3.0.0 banner while the renderer is 3.0.1; `intent doctor` names each file. dc measured every one equal-length on disk and rendered, banner-only, an exact correspondence. The remedy is doctor's own, `intent st hydrate <ID>` per thread, then one commit of the views and `.intentfiles`. **vc attempted it under the pen and the session's tooling refused the write as destructive; it is not delegated to a peer. hv runs it or authorises it.** Structural, hv's to rule as a batch-4 candidate: doctor compares the generator banner, so every cut leaves every untouched view skewed until something re-renders it, and the next preflight refuses again. Either doctor ignores the version stamp when comparing, or the stamp step re-renders every realised view, the schema-faces shape.
2. **The workspace suite is red at HEAD, and every red is new since the tag** (checked at `a8942aead`): `exit_code_consumers::every_shipped_consumer_is_declared`, the shim's `intent bootstrap` from batch 1 item 3 has no CONSUMERS row (dc, own commit); `no_pm_state_in_output::no_installed_payload_file_cites_intents_own_tracker`, the doc audit put ST0052 into `in-review/SKILL.md` and ST0001 into the intent subagent (ic, own commit); intentd's `a_change_outside_the_sync_scope_delivers_nothing` (dc drives, content or setup, then ruled). Preflight runs the suite, so each is a blocker.
3. **A fresh clone can never pass doctor's backup gate** (`intent/.backup/` is gitignored). Ruled: dc takes `intent backup` in the clone before the second run and records that it did.
4. **The reference regeneration moves to AFTER the tag**, both halves keyed to v3.0.2, one commit, the precedent being `97e127339`. Before the cut, ic lands one register edit: the one `note` field in `surface/dispatch-table.json` whose emphasis markers prettier rewrites on every commit, so the generator's pages can be byte-stable.

**Found by hv at 06:39Z (`disk.organize` in the event log), restored by vc at 2026-09-12 06:42Z, unruled:** `intent organise --apply` in this tree removed the realised files of every thread `.intentfiles` does not list (ST0058, ST0064 and ST0068 whole; ST0046's design, impl and tasks; ST0065's `_sources`, catalogue, draft and proposal attachments; ST0073's impl) and refused the views of the nine banner-skewed threads with one identical paragraph each. By ST0057's model that is dehydration and the canon carries every attachment's text and hash, so no byte was lost; vc restored the working tree from HEAD and nothing was committed. Three defects for hv's list: `--apply` dehydrates committed, hand-authored attachments without naming that it will before it does; a refusal that is the same paragraph many times over is one finding printed many times; and the tree was left half-dehydrated, views kept and attachments gone. The coherent end state is the hydrate in item 1 above, extended to all twelve unlisted realised threads (add ST0058, ST0064, ST0068), so that `.intentfiles` lists what is on disk and organize has nothing to remove.
