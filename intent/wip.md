...
- The design and build plan are `intent/st/ST0069/design.md`; the packages are WP-17 to WP-24 with their criteria. Ships in 3.0.2 (hv). Each package lands on main as it closes; the second dry-run rehearsal and the cut follow the last package. Closed and verified: WP-17, WP-18, WP-19, WP-21; WP-23 landed and closing on AC-23.4's Local-runtime record; WP-20 and WP-24 open only on hv's halves. The QUIET WINDOW is open (2026-09-12 14:19Z): ic holds, and cc holds but for one measurement build, no tests or drives on the box until vc lifts it. cc measures the two Local-runtime shapes (fastembed on ONNX, candle) sizes only, a build not a test, in their worktree. dc runs serially: WP-22 finished and landed (the roster becomes a constructor, `--no-reconcile`, the daemonless reconcile-then-query through `index_refresh(None)`, one parity arm), the PostToolUse hook with AT-24.1 cited and AC-24.6 reviewed in one message, then the final rehearsal on the last HEAD once cc's build is done. Four Laksa nodes now share the box, so the load floor is above what dc measured. hv rules the design's listed decisions as each comes due.

## TODO: waiting on hv

- **Six decisions from ST0069, taken under the pen on 2026-09-12 when hv handed it over without ruling them; each overrulable with a line:** (1) the grammar line: rust, elixir, swift and lua ship on by default, bash declared and off because its grammar ships no tags query, the measured table in the design; cc lands the default line and AC-20.4 closes on it; (2) the explorer omnibox stays out of 3.0.2; (3) `intent modules find` stays as the registry fallback with the canon naming the index first, and AC-20.6 is withdrawn naming AC-24.5, which carries the sweep; (4) the Local embedder runtime is ruled once cc's size table is in, and the lean is no new runtime in 3.0.2, the seams and the Null and HTTP embedders shipping as built; (5) ST0073's AC-05.1 is re-keyed to AC-00.1 with its text and evidence carried forward; (6) the HTTP embedder gets no TLS in 3.0.2: it speaks `http://` only and refuses `https://` by name, because one POST of one JSON document needs no HTTP crate and a TLS stack is a dependency with a rationale. AC-23.4 closes on (4)'s record.
