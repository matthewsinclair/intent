...
- The design and build plan are `intent/st/ST0069/design.md`; the packages are WP-17 to WP-24 with their criteria. Ships in 3.0.2 (hv). Each package lands on main as it closes; the second dry-run rehearsal and the cut follow the last package. Closed and verified: WP-17 to WP-23; WP-24 closes on `wp done`, every criterion satisfied. The QUIET WINDOW is open (2026-09-12 14:19Z): ic holds, and cc holds but for one measurement build, no tests or drives on the box until vc lifts it. cc's fix for issue 0304 is landed (a document the store carries and the disk holds answered a search twice; the disk corpus now excludes what the store carries, one arm driven to both verdicts). dc's hook is landed (bd79cf605) and dc prepares the final rehearsal, which starts on the HEAD carrying ic's release-note follow-up when vc names the sha. The wide red population cc met on a clean HEAD was an absent sibling intentd in a per-package target dir, cleared by building it, so the rehearsal's workspace run does not meet it. Four Laksa nodes now share the box, so the load floor is above what dc measured. hv rules the design's listed decisions as each comes due.
