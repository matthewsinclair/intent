...
- The design and build plan are `intent/st/ST0069/design.md`; the packages are WP-17 to WP-24 with their criteria. Ships in 3.0.2 (hv). Each package lands on main as it closes; the second dry-run rehearsal and the cut follow the last package. Closed and verified: WP-17, WP-18, WP-19, WP-20, WP-21, WP-23; WP-22 landed and closing on its AT rows; WP-24 on dc's hook. The QUIET WINDOW is open (2026-09-12 14:19Z): ic holds, and cc holds but for one measurement build, no tests or drives on the box until vc lifts it. cc fixes issue 0304 before the tag (a document the store carries and the disk holds answered a search twice; ruled: the disk corpus excludes what the store carries, the views rule widened to carried documents, one arm). dc runs serially: WP-22's close (AT rows, CHANGELOG line, `wp done`), the PostToolUse hook with AT-24.1 cited and AC-24.6 reviewed in one message, then the final rehearsal on the HEAD that carries cc's 0304 fix, dc's hook and ic's release-note follow-up. Four Laksa nodes now share the box, so the load floor is above what dc measured. hv rules the design's listed decisions as each comes due.
