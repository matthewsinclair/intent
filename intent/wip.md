...
hv ruled the release on 2026-09-12: a patch, vc directing. Rules for every item: own commit, red before green, a private worktree under an isolated HOME with its own CARGO_TARGET_DIR, no release, no push, no hardcoded counts in anything a reader reads.
...
- **Batch 4, silent deletion (hv ruled 2026-09-12: _"this cannot be released publicly with that kind of bug"_):** every path that removes or overwrites bytes on disk without naming them before it acts is identified, triaged and fixed before the cut. vc's sweep of the code is running; fixes are assigned on its result. The trigger: `organise --apply` removed the realised files of every thread `.intentfiles` does not list, hand-authored attachments included, and named them only after.
...
- **intentd's `a_change_outside_the_sync_scope_delivers_nothing`:** green for dc in isolation, red for cc; cc owes dc its invocation, working directory, HOME and whether the directory sits under the live daemon's watch. Setup on the evidence so far.
- **The view regeneration that clears doctor's refusal, HELD FOR hv:** `intent st hydrate` on ST0046, ST0059, ST0061, ST0062, ST0063, ST0065, ST0066, ST0072 and ST0073 (banner-skewed) and on ST0058, ST0064 and ST0068 (unlisted), so that `.intentfiles` lists every realised thread and organize has nothing to remove; then one commit of the views and `.intentfiles`. vc's session tooling refused the write as destructive; it is not delegated to a peer.
...
- **`organise --apply`, three defects found by hv on 2026-09-12:** it dehydrates committed, hand-authored attachments without naming that it will before it does (batch 4's class); a refusal that is the same paragraph many times over is one finding printed many times; the tree is left half-dehydrated, views kept and attachments gone.
...
- **ST0069, project search:** vc's review and re-elaboration of its design and work packages, asked by hv on 2026-09-12; no work starts on it.
