...
- **2026-04-23**: **v2.9.0 released + fleet rollout complete.** ST0034 (Agentic Software Engineering Suite) closed. Release commit `d1b0fe1`; tag `v2.9.0` on `local` + `upstream`; GitHub release published. Canary (Anvil, Arca/arca_cli, Arca/arca_config, Arca/arca_notionex) + batch 2 (Laksa, MeetZaya, MicroGPTEx, Molt, Molt-matts) + batch 3 (Multiplyer, Prolix, Utilz, Courses/Agentic Coding) — 13/13 active projects upgraded clean. `intent st done` empty-`completed:` field bug fixed in the release commit (sed pattern required a literal space; removed it). CI workflow retry-fixed after a transient GitHub HTTP 500 broke the macOS bats-library clone on the release push.
...
Open queue is empty. Pick from Parked or initiate a new ST when work surfaces.
