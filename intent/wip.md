...
- **The release's test gate names what refused (dc, from the mid-course rehearsal of 2026-09-12):** `release` runs the BATS suite with its output swallowed and aborts before `cargo test`, so a refusal costs a rebuild to diagnose; on failure it prints the failing test lines and the log's path. The BATS test that failed at that HEAD is named by dc's kept-output run and goes to its owner: a v2-era assertion is deleted, named first; a regression is a batch item, red first.
