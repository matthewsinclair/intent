...
- **The preflight gate is non-deterministic (dc, 2026-09-12):** two of five runs of preflight's own invocation at the same commit red the daemon pair `a_change_outside_the_sync_scope_delivers_nothing` and `every_subscriber_receives_every_event`, a negative assertion and its positive twin exposed to ingest timing. dc fixes the synchronisation after its batch-4 items: each test waits on the event it asserts about, not a window. vc's ruling under the pen, hv to overrule: the cut does not depend on re-running a gate until it passes.
