...
**Current as at 2026-09-14 22:09Z: WRAPPED FOR THE DAY, EVERY NODE HOLDING (hv, 2026-09-14).** v3.0.3 is shipped (tag 46599d145); the keg is unlinked and not pinned; `intent` on PATH is the dev tree; the live intentd runs the 3.0.3 pair. The delivered pair predates today's fixes: schema rung 27 migrates the live store on the next `bin/devbin build all` and a 3.0.3 binary then refuses it, so the build, `intent daemon restart`, `bin/int macos app-install` and one mechanical view re-render commit come in a quiet window on hv's word. Open issues: `intent issues list`. Banked, not landed: cc's 0338 (ii) as a patch on d2491639b, landing on the matched control of the index event-wait arm (cc's first act); ic's 0334, researched and ruled, no code. Next order, one lane on the tree AND the store at a time on vc's word: cc (the control, 0338 (ii), 0338 (i), 0377, 0331's comments, 0375), then ic (0334, 0339, 0396), then the quiet-window build. hv's items are in `intent/wip.md`. Landing rules: rebased onto the last hash vc sends; full intentsvcs and intent-cli suites plus workspace clippy on every judging run; an event-wait family red re-runs once and is judged by mechanism; each landing closes its issues with `organize --apply`; a register row with no canon behind it stops every landing; a landing turn ends with its report SENT. NO RELEASE, NO PUSH without hv at the terminal; never `--no-confirm`.
...
- Every suite runs from a private worktree's own build, with an in-tree `CARGO_TARGET_DIR`, under an isolated HOME written as a command (`HOME=<dir>`), after `cargo build -p intentd`; a test run writes the estate it lives in, so check `~/.intent/home` after any peer run.
...
- Worktree results come back as a patch, never a whole-file copy; an attachment edited in a worktree goes through `st attach` with the worktree's own build after an unscoped `sync --to-store`.
