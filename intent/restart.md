...
- **A positive control proves the instrument on the corpus you controlled against and says NOTHING about the corpus you point it at next** (dc, 2026-09-17). Draw the control from the target corpus and check it shares the target's SHAPE: a fence walk validated against a twelve-fence document reported clean over three documents with zero fences and forty inline commands.
- **An instrument must fail toward a warning a reader discards, never toward an all-clear a reader believes** (ic, 2026-09-17). Prefer a structural test that over-reports to a filter typed from what you expect to find; three instruments in one evening were narrowed by an alternation their author guessed.
...
- **A COMMAND IN A DOCUMENT IS A CLAIM AND BEATS A SENTENCE ONLY ONCE SOMEBODY HAS RUN IT** (ic and dc, 2026-09-17, four of five commands in a landed release page wrong). Read-only ones get RUN; ones nobody can safely run -- `git tag`, a push, a publish -- get READ as control flow. **Say which way each was verified.** Enumerate them structurally in BOTH homes, fenced blocks and inline backticks: the three documents every node reads at boot carry forty commands and no fenced block.
