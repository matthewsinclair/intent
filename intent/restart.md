...
- **A board commit also carries the event files its own acts wrote, selected by SUBJECT NODE and never by principal** (cc, 2026-09-18, the first board commit on the rebuilt pair): every node here acts under one git identity, so the principal on an event file says nothing about which node's chain wrote it; the untracked files under `intent/.canon/events/` whose subject is your node, written since your CHAIN START, are yours to name in the commit, and a peer's are left for the peer's chain.
