...
**A NEW MACHINE IS PART OF THE ROW AND NOT AN INTENSIFIER**: the ambient `INTENT_HOME`, an installed v2, a `~/.intent/`, a `core.hooksPath` this clone did not inherit -- each of these makes a broken page work for the person checking it. **`bin/int hooks` is the standing instance**: a fresh clone runs no hooks and nothing announces it, so the estate already knows this class by name. -- satisfied: no
