---
wp_id: WP-01
title: Self-upgrade Intent to v2.10.0 and cancel ST0010 and ST0015
scope: XS
status: Done
---

# WP-01: Self-upgrade Intent to v2.10.0 and cancel ST0010 and ST0015

## Objective

Bump Intent's own `.intent/config.json` from `2.8.2` to `2.9.1`, add the v2.9.0 → v2.10.0 migration stub in `bin/intent_helpers`, and cancel the two stale NOT-STARTED steel threads (ST0010, ST0015) via `intent st` — moving them to `intent/st/CANCELLED/` with a deprecation one-liner at the top of each `info.md`.

## Context

Intent v2.9.0 shipped 2026-04-23 but Intent's own version stamp in `.intent/config.json` never bumped past `2.8.2` (audit confirmed). The generated `intent/llm/AGENTS.md` footer correctly reads `Intent v2.9.0`, so the content is current — it's just the stamp that lags. WP01 fixes that and adds the migration entry point for v2.10.0 before any later WP touches project state.
...
4. **CHANGELOG entry**: stub entry for `v2.10.0` added — title only, body fills out across the ST.
...
7. Update `CHANGELOG.md` v2.10.0 stub.
8. Commit: `chore: bump to v2.10.0 and cancel ST0010/ST0015`.

## Acceptance Criteria

- [ ] `cat .intent/config.json | jq .intent_version` returns `"2.10.0"`.
...
- [ ] `CHANGELOG.md` has a `v2.10.0` heading (body can be empty at this point).
...
- Existing `migrate_v2_8_2_to_v2_9_0` is at `bin/intent_helpers`. Read it as a structural template. Key concerns it handles: stamp, bootstrap of `~/.intent/ext/`, seed worker-bee, prune deprecated agents. The v2.9.0 → v2.10.0 stub just needs the stamp bump at this WP — other concerns are later WPs (canon artefacts in WP11).
...
- **Edge**: `CHANGELOG.md` v2.10.0 entry with an empty body may trip over any markdown linter that requires content. Add a placeholder line like `- In progress (ST0035).` to keep linters happy.
...
7. Run a no-op test of migration idempotence: `bin/intent upgrade --dry-run` should report "already at v2.10.0" (or equivalent).
...
- [ ] Migration stub tested: running `intent upgrade` on a v2.9.0 project bumps to v2.10.0 without error.
- [ ] Both cancelled STs visible in `bin/intent st list --status cancelled`.
- [ ] `tests/run_tests.sh` passes at baseline.
- [ ] CHANGELOG.md v2.10.0 stub present.
- [ ] Committed and branch clean.

## Acceptance

Acceptance Criteria for this work package are RENDERED into `ST0035/acceptance.md`, under the `WP-01` heading. THAT FILE IS A GENERATED VIEW -- a row authored there is discarded by the next sync. The contract is canon in the thread's model: change a state with the `intent ac` / `intent at` verbs, and mint or reword a row in `.canon/st/ST0035.json`, then `intent sync --to-store`. This cover never restates them.
