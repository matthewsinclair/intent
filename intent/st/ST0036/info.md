---
st_id: ST0036
title: Directory relocation: `.intent/` → `intent/.config/`
status: Completed
created: 2026-04-24
...
| ----- | --------------------------------------------------------------- | ---- | ------ |
| WP-01 | Migration function: atomic .intent to intent/.config relocation | M    | Done   |
| WP-02 | Path probes for intent/.config root                             | S    | Done   |
| WP-03 | Literal sweep: replace .intent/ with intent/.config/            | M    | Done   |
| WP-04 | Template and generator updates                                  | S    | Done   |
| WP-05 | BATS fixtures and create_test_project flip                      | M    | Done   |
| WP-06 | Ignore patterns: gitignore and treeindexignore                  | XS   | Done   |
| WP-07 | Migration guide: intent/docs/migration-v2.10.0.md               | XS   | Done   |
| WP-08 | Intent self-apply directory relocation                          | S    | Done   |
| WP-09 | Merge with ST0035 fleet rollout (coord)                         | XS   | Done   |

## Acceptance

Acceptance Criteria and Acceptance Tests are RENDERED into `acceptance.md`, which is a GENERATED VIEW -- a row authored there is discarded by the next sync. The contract is canon in this thread's model: change a state with the `intent ac` / `intent at` verbs, and mint or reword a row in `.canon/st/ST0036.json`, then `intent sync --to-store`. This cover never restates them.
