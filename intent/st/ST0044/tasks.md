...
- [ ] WP-04 (M) Close-gate -- `intent st done` / `intent wp done` refuse to close until coverage is complete (green AT or non-test evidence) and sign-off is recorded; verdict computed, not hand-ticked. [depends on WP-03]
- [ ] WP-05 (S) Template references + show/edit -- update `info.md` and `WP/info.md` templates to reference `acceptance.md` and restate no ACs (Highlander); extend `st show` / `st edit` to know the `acceptance` file type.
...
- [ ] WP-07 (S) Dogfood -- `acceptance.md` for ST0043 + ST0044 (done); run ST0044's own build through the five-step with matts as verifier (in progress -- WP-01 lap complete).
