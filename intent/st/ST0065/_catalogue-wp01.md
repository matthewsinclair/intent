# WP-01 catalogue: the Intent-canon .md set

**Read-and-catalogue leg. NOTHING IN THIS SET HAS BEEN EDITED.** Every verdict below is a recommendation to hv, per vc's assignment (14:32Z): no rewrites before hv sees the catalogue.

Verdict vocabulary is hv's: **keep** / **correct** (wrong, fixable) / **retire** (stale) / **MISGUIDED** (leads users and LLMs down the wrong path -- the worst class, because it is followed).

Measured 2026-08-28 against `intent 3.0.0 (4479264f)` on this checkout, with a fresh `intent init` driven in a scratch dir as the control for anything claimed about new projects.

---

## 0. The number hv asked for: what a session actually costs

hv's brief says "we're carrying a lot of baggage". Quantified, and the vendored companion asks for exactly this measurement -- _"nobody owns the total... budget the context the way you would budget a build time"_:

| Loaded automatically, every session | bytes                    |
| ----------------------------------- | ------------------------ |
| `in-whiteboard` SKILL.md            | 30284                    |
| `MEMORY.md`                         | 16298                    |
| `in-session` SKILL.md               | 6808                     |
| project `CLAUDE.md`                 | 6492                     |
| `in-content-essentials`             | 4900                     |
| `in-author-essentials`              | 4522                     |
| `in-essentials`                     | 4017                     |
| `in-elixir-testing`                 | 3964                     |
| `in-elixir-essentials`              | 3808                     |
| `in-standards`                      | 3791                     |
| `~/.claude/CLAUDE.md` (global)      | 2050                     |
| **TOTAL**                           | **~87 KB (~22k tokens)** |

Top three are 59% of it. Against Cherny's rule for the one file he discusses -- _"if it gets too long, it's just going to use up a bunch of context and it's usually not that useful"_ -- the canon set has never been measured as a total, and the total is where the cost lives.

**A further 27.6 KB is referenced by the canon and injected by nothing:** `AGENTS.md` (6673) + `usage-rules.md` (20973). See finding 1.

---

## 1. `AGENTS.md` -- the primary contract never arrives. **MISGUIDED**

**What it is:** the tool-agnostic contract. Generated by `intent agents sync`; `NotByInit` (correctly -- seeding a generated file teaches the operator it is theirs to edit).

**The defect.** `CLAUDE.md` line 3: _"The primary config file for AI coding agents is `AGENTS.md` at the project root -- **read that first**."_ Nothing executes that instruction. No `@`-import exists in `CLAUDE.md`, `~/.claude/CLAUDE.md` or `AGENTS.md`. `/in-session` -> `/in-standards` step 1 names CLAUDE.md, MODULES.md and DECISION_TREE.md, and **does not name AGENTS.md**. This session booted by the canonical path and never read it.

**Scope, stated precisely because the obvious reading is wrong.** AGENTS.md is the cross-tool convention and other agents do load it automatically; it is not dead weight. The finding is narrower: **within Claude Code, the tool-agnostic contract is the one document that reliably does not arrive**, and the agent is told it is holding the secondary overlay with no way to notice the primary never came.

**Compounding it -- the four rules.** `_CLAUDE.md` deliberately omits Highlander / PFIC / Thin Coordinator / No Silent Errors, and says why: _"a second copy would be a Highlander violation in the document that defines the rule."_ The reasoning is impeccable as document design and wrong as context design -- **Highlander governs a single source of truth, not a single source of delivery.** Measured: `IN-AG-` appears 4x in AGENTS.md (never loaded), 8x in `in-standards` (loaded every session), 2x in CLAUDE.md. So the rules do arrive -- via the skill, not the canon -- and **the duplication the comment was avoiding already exists between AGENTS.md and in-standards.** The document reasoned itself out of carrying them on grounds that do not describe where the copies actually are.

**Verdict: MISGUIDED.** Not because the file is wrong -- it is the best-organised document in the set -- but because the canon asserts a loading order that no mechanism implements, and an agent that trusts CLAUDE.md line 3 believes it has read a contract it has not.

**Coordination:** ST0067 AC-00.4 (cc, hv-ruled 14:38Z) adds a pointer line here to `intent llm guide`. Correct and worth building; it closes the loop for tools that load AGENTS.md and not for Claude Code. Flagged to cc 14:43Z. **I have not touched the template.**

---

## 2. `intent/llm/MODULES.md` -- the retirement reached the file and not the instruction. **MISGUIDED**

> **WORK ORDER FOR ISSUE 0122: THIS LIST, NOT THE ISSUE BODY.** 0122's body names `module_check_hook.json` as a site to fix. **It is not one -- the hook is guarded and correct** (see the 17/3 split below). The issue is left standing unpatched on vc's ruling (15:40Z) because `issues` has no post-creation write path, and the standing mis-scope is cited live as evidence in the write-path menu to hv. **Anyone acting on 0122 takes the corrected list from this document.** When the edit verb is built, 0122's scope correction is its first drive.
>
> **AND THE LOOP IS OPEN AT THE ISSUE END.** This document points at 0122; 0122 cannot point back, because nothing can write to it. **That is structurally the same defect as finding 1** -- AGENTS.md and `intent llm guide` each pointing at a document that never arrives -- and it means a reader who lands on 0122 alone gets the superseded scope with nothing indicating so. vc carries that as a coordination guarantee at assignment time; **it is not a mechanical one, and the gap is the point of the evidence.**

**What it is:** the Highlander registry. `init.rs` retired it on 2026-08-24 -- `NotByInit("a hand-maintained index of a tree the store already indexes")` -- with the reason stated in the source comment: _"an instruction nobody can follow."_

**Driven, not inferred.** A fresh `intent init` in a scratch dir: **`intent/llm/MODULES.md` ABSENT**, and that same fresh project's generated **`CLAUDE.md:21` still says "Highlander registry; check before creating new modules."**

**TWENTY mentions across TWELVE files still issue the instruction.** This count was wrong in the first version of this catalogue -- I grepped `llm/MODULES.md`, which is path-qualified, and **most references are the bare `MODULES.md`**, so I reported four when there were twenty. Positive-controlled after correction: the old pattern returns 0 on a bare mention, the new one returns 1. **The same failure class as the rest of this document -- a pattern fitted to one surface form -- committed while cataloguing it.**

| File                                                  | mentions |
| ----------------------------------------------------- | -------- |
| `in-plan` SKILL.md                                    | 4        |
| `lib/templates/prime/operational-knowledge.md`        | 3        |
| `lib/templates/llm/_DECISION_TREE.md`                 | 3        |
| `in-standards` SKILL.md                               | 2        |
| `_CLAUDE.md`, `_AGENTS.md`, `_ARCHETYPES.md`          | 1 each   |
| `lib/templates/hooks/module_check_hook.json`          | 1        |
| `lib/templates/archetypes/elixir/service.ex.eex`      | 1        |
| `lib/templates/archetypes/elixir/ash_resource.ex.eex` | 1        |
| `in-start` SKILL.md, `in-review` SKILL.md             | 1 each   |

**THE 20 SPLIT 17/3 ONCE READ RATHER THAN COUNTED, AND dc's 0113 SWEEP IS WHY I CHECKED.** Their run turned up a false positive where the hit was _prose describing_ a guard rather than the guard itself. Applied to my own figure, three of the twenty do not survive:

- **`module_check_hook.json:9` DOES NOT MISFIRE.** Its command is guarded -- `if [ -f intent/llm/MODULES.md ] && ...` -- so on a project without the registry it short-circuits and says nothing. When the file _does_ exist, `intent modules check` works. **It is correct as written**, and I had it listed as a site that routes operators into the defect.
- **`_ARCHETYPES.md:26` is a DESCRIPTION, not an instruction** -- _"Templates include 'Check MODULES.md' reminders"_ is documentation of what the archetype templates contain. Counting it is the same error as scanning prose for the defect it describes.
- **`_AGENTS.md:264` is a file-map row**, not an imperative. It asserts the file exists rather than telling anyone to read it -- still wrong on a fresh project, but a different defect.

**So: 17 imperative instructions that a fresh project actually meets, one false existence claim, and two that are fine.** The 17 is the number to act on.

**It reaches the generated CODE.** `_ARCHETYPES.md` states that its templates "include 'Check MODULES.md' reminders" -- literally true, and it means **every module generated from an archetype carries a comment pointing at a registry `init` refuses to create.**

The four highest-reach sites remain:

| Site                                                  | Reaches                                 |
| ----------------------------------------------------- | --------------------------------------- |
| `lib/templates/llm/_CLAUDE.md:21`                     | every project's CLAUDE.md (auto-loaded) |
| `lib/templates/llm/_AGENTS.md:262`                    | every generated AGENTS.md               |
| `in-standards` SKILL.md:14 -- "Re-read project rules" | **auto-loaded into every session**      |
| `in-standards` SKILL.md:56 -- Red Flags row           | same                                    |

**Size.** Intent's own copy is **571769 bytes (~143k tokens)**. `CLAUDE.md` states "~354KB across ~367 rows" -- **stale by 62%**, and it is the figure the file uses to argue the read is impossible.

**Verdict: MISGUIDED.** A fresh v3 project receives four instructions to consult a registry its own `init` deliberately refused to create. The fix landed on the laydown path and not on the instruction path -- the same shape as the shim-carrier class (a repair aimed at one layer while another still carries the old contract).

---

## 3. `intent/llm/DECISION_TREE.md` -- the one the 08-24 ruling missed. **MISGUIDED**

**What it is:** code-placement flow chart. Still seeded **unconditionally** by `init` -- the only member of the Elixir-flavoured group that was not retired.

**Driven.** A fresh project with `languages: []` receives a 3056-byte file headed **`## Elixir/Phoenix Decision Tree`**, carrying 13 Ash / Phoenix / LiveView / Oban / GenServer markers, whose line 3 reads _"Always cross-reference MODULES.md"_ -- the file `init` had refused to create moments earlier. `in-standards` then tells every session to re-read it.

**Verdict: MISGUIDED.** A Rust, Swift or prose-only project is handed an Elixir architecture as its code-placement authority, pointing at a registry that does not exist. `_ARCHETYPES.md` and `_DEPENDENCY_GRAPH.md` were correctly moved behind `intent lang init` on 08-24; this is the same class and was left seeded.

---

## 4. `usage-rules.md` -- three checkable claims, two false. **CORRECT**

**What it is:** the terse DO / NEVER contract. `NotByInit` (user-owned; the canon installer seeds it). 20973 bytes live -- the largest single document in the canon set, and **never injected** (see finding 1).

| Claim                                                                                                                   | Measured                                                                                                                                      |
| ----------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| _"`.git/hooks/pre-commit` runs `intent critic <lang>`"_                                                                 | **OVERSTATED BY ME -- see the correction below.** TRUE for a default project; false only where `core.hooksPath` is set, as in this repo.      |
| _"Skills (**auto-loaded every Claude Code session**)"_ then lists `/in-review`, `/in-verify`, `/in-debug`, `/in-finish` | **FALSE for the list given.** Those four are on-demand. Only `in-session`, `in-essentials`, `in-standards` and the language skills auto-load. |
| the 8 named subagents exist                                                                                             | **TRUE.** All 8 resolve. Verified negative -- do not re-check.                                                                                |

**CORRECTION TO MY OWN FINDING, MADE WHILE FIXING IT.** I catalogued the hook path as flatly FALSE. It is not. **The canon installer writes to `.git/hooks/pre-commit`, so for a default project the original text was RIGHT.** It is wrong only where `core.hooksPath` is set -- which is the case in **this** repo, the self-hosted one I measured on. I generalised a single estate's configuration into a claim about a template that ships everywhere, **and hv adopted the verdict in that form.**

**Same class as the count errors in finding 2**: a reading taken from one population and reported as a property of the corpus. The fix I made is still the right fix and is better than either wording -- it names no path and points at `git rev-parse --git-path hooks`, which holds under both configurations and is what the v3 code already does for exactly this reason (`canon.rs:343` asks `--git-path` rather than assuming `.git/hooks`). **But the finding as catalogued was stronger than its evidence.**

**Verdict: CORRECT, on two of three claims.** The auto-load claim was plainly false and is fixed. The hook path was configuration-dependent and is now stated in a form that holds either way. Nothing here sent a reader down a wrong path; it told _some_ readers the wrong filename.

---

## 5. `intent/wip.md` template -- 2025 chat-era advice. **RETIRE (body) / MISGUIDED (the LLM section)**

**What it is:** `lib/templates/prj/_wip.md`, 1090 bytes, `verblock: "06 Mar 2025:v0.1: Matthew Sinclair - Initial version"` -- the oldest thing in the set, and the only canon template that carries a `verblock:` frontmatter field at all. (`_AGENTS.md` matches a grep for the word because it _prescribes_ verblocks at line 145; it does not carry one. Checked, because the two are different claims.)

**Contents:** placeholder scaffolding (`**002: [Brief description of the current development focus 2]**`, listed above `001`), a `## Notes` stub, then a `## Context for LLM` section instructing: _"When beginning work with an LLM assistant, **start by sharing this document**"_, followed by `### How to use this document`, four numbered steps addressed to the human.

**Two defects.**

**(a) It is the exact anti-pattern the good templates name.** `_RULES.md` and `_ARCHITECTURE.md` both ship deliberately empty and both argue the case: _"a heading waiting to be filled reads as a commitment the project has not made"_; _"a skeleton of headings and placeholder trees reads as a specification the project never committed to, which is worse than nothing."_ `_wip.md` is that skeleton. **Two templates in one directory holding opposite philosophies, and the better one has already written the argument against the worse.**

**(b) `## Context for LLM` teaches a workflow the tool replaced.** "Start by sharing this document" is March-2025 copy-paste-into-chat practice. Under Claude Code the equivalent is a session hook and `/in-session`, both of which this project ships. An operator following it does something unnecessary; an LLM reading it infers a mode of use that is not how it is being run.

**Verdict: RETIRE the placeholder body; MISGUIDED for `## Context for LLM`.** Strongest candidate in the set for deletion rather than repair.

---

## 6. Verified negative -- do not re-find these

- **`_ARCHETYPES.md`** -- all nine `.eex` templates it names exist in `lib/templates/archetypes/elixir/`. Correctly `NotByInit` (a language pack's, via `intent lang init`).
- **`_DEPENDENCY_GRAPH.md`** -- declares _"Enforced by the `Mix.Checks.DependencyGraph` Credo check (D11)"_; that enforcer exists nowhere but this template and one archived ST. **Latent, not live**, because it is correctly `NotByInit`. Fix if the file is ever revived; not shipping today.
- **`_RULES.md` (701) and `_ARCHITECTURE.md` (744)** -- **KEEP, unchanged.** Ship deliberately empty, each with a stated rationale for why. The two best-written documents in the set and the source of the principle finding 5 is judged against.
- **`intent modules find`** -- answered at rc=0 here. Evidence against issue **0067** as currently worded. Flagged, not resolved; not WP-01's to close.
- **The four `intent/docs/*.md` pointers** in CLAUDE.md/usage-rules all resolve in this checkout and are honestly qualified "at the Intent install". They do not resolve project-locally in a fresh project, which the wording already admits. **No verdict** -- usability question for hv, not a defect.

---

## 7. Already settled 2026-08-24 -- folded in, not re-derived

`init.rs` carries rulings with full rationale for `_MODULES.md`, `_AGENTS.md`, `_usage-rules.md`, `_ARCHETYPES.md`, `_DEPENDENCY_GRAPH.md` (all `NotByInit`) and for the ST templates (`NotByInit`, "a generated view (D02)"), plus the deliberate rewrite of the `_RULES.md` / `_ARCHITECTURE.md` pair. Per vc's brief these are folded in as settled. **What that ruling did not reach is findings 2 and 3: the instructions that survive the retired file, and the one Elixir-flavoured template still seeded unconditionally.**

---

## Summary

| #   | Item                                                            | Verdict                                  |
| --- | --------------------------------------------------------------- | ---------------------------------------- |
| 1   | `AGENTS.md` -- primary contract never injected into Claude Code | **MISGUIDED**                            |
| 2   | `MODULES.md` -- retired file, four surviving instructions       | **MISGUIDED**                            |
| 3   | `DECISION_TREE.md` -- Elixir tree seeded to every project       | **MISGUIDED**                            |
| 4   | `usage-rules.md` -- wrong hook path (x2), wrong auto-load claim | **CORRECT**                              |
| 5   | `_wip.md` -- placeholder skeleton + chat-era LLM section        | **RETIRE / MISGUIDED**                   |
| 6   | `_RULES.md`, `_ARCHITECTURE.md`                                 | **KEEP**                                 |
| 7   | `_ARCHETYPES.md`, `_DEPENDENCY_GRAPH.md`                        | keep as-is (latent, correctly unshipped) |
| 8.1 | prime payload -- documents two flags the shipped binary refuses | **MISGUIDED**                            |
| 8.2 | prime payload -- skills roster contradicts its own workflow     | **CORRECT**                              |
| 8.3 | prime generator -- unconditional checklist re-issues MODULES.md | **MISGUIDED**                            |
| 8.4 | prime generator -- `## Rules` extraction is silent dead code    | **MISGUIDED**                            |
| 8.5 | two templates, two delivery paths (embedded vs read-from-disk)  | not a doc defect                         |

**One theme runs through 1, 2 and 3, and it is the same defect three times: a canon document names a file or a rule, and the mechanism that would deliver it does not exist or was retired underneath it.** The canon describes a system that is one revision ahead of, or behind, the one that runs.

**WP-02 (the 25 `/in-*` skills) is DELIVERED** at `_catalogue-wp02.md`, and the budget prediction held: `in-whiteboard` alone is 30 KB of the 87 KB, and `in-standards` was implicated in findings 2 and 3 as expected. **Section 8 below is a later addendum** -- the `intent claude prime` payload, authorised by vc as in-passing, no new WP.

---

## 8. ADDENDUM -- the `intent claude prime` payload. **MISGUIDED**

vc authorised this as an in-passing addendum to WP-01, no new WP. It is here because it is the **third injected surface and nobody had audited it**: WP-01 costed what a session loads from canon and WP-02 costed the skills, and neither of them is what `intent claude prime` writes.

### What the payload actually is

Not one file, which is the first thing worth recording. `intent claude prime` is a **bash plugin** -- `intent/plugins/claude/bin/intent_claude_prime`, 253 lines -- and `prime` appears nowhere in the Rust crates. Its `build_memory()` assembles `MEMORY.md` from **seven sources**: the bundled template `lib/templates/prime/operational-knowledge.md`, `CLAUDE.md`'s `## Rules` section, `MODULES.md`, `DECISION_TREE.md`, `ARCHETYPES.md`, `intent/.config/learnings.md`, and a **hardcoded session checklist**. The output is written to `~/.claude/projects/<dashed-path>/memory/MEMORY.md`, which Claude Code auto-loads into **every conversation**.

**So an error here is injected everywhere, permanently, without anyone invoking anything.** That is the reason this addendum ranks MISGUIDED rather than stale.

### Instrument, and its control

The template was swept **exhaustively rather than by hand**: all 43 backticked spans extracted mechanically, then each classified and driven. Hand-listing is the census that misses one, which is the day's class and also the stated design reason behind `flag_reachability.rs`.

Both instruments were positive-controlled **two-sided before use**, not after: known-good verb `rc=0`, known-bad verb `rc=1`, known-bad flag on a good verb `rc=1`; known-good skill found, known-absent skill not found. A green from an instrument that has not been seen to fire is not evidence.

### 8.1 The payload teaches two flags the shipped binary refuses. **MISGUIDED**

`operational-knowledge.md:12` reads ``- `intent claude prime [--refresh] [--dry-run]` -- refresh this memory file``.

Measured against the shipped v3 binary: **both return `rc=1`, `error: unexpected argument found`.** The verb itself resolves; the flags do not exist on it.

This is not drift that someone forgot to ratify. `surface/dispatch-table.json` declares `claude prime` with **`"flags": []`**, disposition `keep` -- **dropping the three flags was the ratified v3 decision**, so the payload is the half that is out of step.

**Why nothing caught it, and this is the load-bearing part.** `tests/unit/claude_prime.bats:22` asserts exactly this flag: _"claude prime --dry-run previews MEMORY.md without writing"_. It drives `run_intent`, which resolves `INTENT_BIN` to `bin/intent` -- **the v2 bash binary, where the flag genuinely works (`rc=0`)**. The test is green and **structurally cannot observe v3's refusal**. And `flag_reachability.rs` (AC-06.8) tests the _opposite_ direction -- every flag the surface DECLARES is READ -- so declared-but-inert is covered and **implemented-but-undeclared is not tested by anything**.

A test passing about a binary nobody runs is the same class as a green from an uncontrolled instrument.

### 8.2 The payload's skills roster contradicts its own workflow, four lines later. **CORRECT**

Lines 18 and 21 list `/in-start` and `/in-next` -- **both retired by hv's 2026-08-28 ruling** -- and the roster **omits `/in-session` entirely**. Line 32 then names `/in-session` as step 1, _"the entry point, enforced by the `UserPromptSubmit` gate"_.

So one 43-line file advertises a skill roster whose first entry is retired, and asserts the correct entry point four lines below without listing it. **My own partial edit at `ba3c3ec1` is half of this**: I corrected the workflow half and left the roster. Recorded rather than quietly repaired, because the deferral is deliberate -- **it resolves with the retirement-mechanics draft**, and I will not edit lines in files I am drafting the deletion of.

### 8.3 The generator re-issues the retired instruction the template no longer carries. **MISGUIDED**

`build_memory()` ends with an **unconditional** echo block -- no guard, no `[ -f ]` -- emitting:

```
- CLAUDE.md
- intent/llm/MODULES.md
- intent/llm/DECISION_TREE.md
- intent/wip.md
- intent/restart.md (if exists)
```

Measured on a real `intent init` in a clean directory: **`MODULES.md` ABSENT, `DECISION_TREE.md` PRESENT.** So the `MODULES.md` line is wrong today for every fresh project, and the `DECISION_TREE.md` line **becomes wrong the moment cc lands hv's gating ruling** -- it is correct now only because that ruling has not been executed yet.

**This is finding 2's class exactly, one layer down.** The retirement reached the template because that is where the audit was aimed, and **the generator kept emitting the instruction underneath it**. A retirement retires its instructions, and a generator is an instruction.

### 8.4 The "Project Rules" section is dead code, and it fails silently. **MISGUIDED**

`build_memory()` extracts project rules with `sed -n '/^## Rules$/,/^## /{...}'` from `CLAUDE.md`.

**No shipped `CLAUDE.md` has a `## Rules` heading.** The template's heading is `## Rules of the road`, and `/^## Rules$/` is anchored, so it cannot match.

Measured, with a two-sided control: the script's exact sed yields **0 bytes** against both `lib/templates/llm/_CLAUDE.md` and this project's `CLAUDE.md`; the **same sed shape** aimed at `## Rules of the road` -- the heading that does exist -- yields **501 bytes**. The instrument works; the heading it names does not exist.

The result is swallowed by `if [ -n "$rules" ]`: **no error, no warning, no empty section**. The payload promises project rules and delivers none, in silence. `IN-AG-NO-SILENT-001` in spirit.

**Checked as a population, not assumed:** this is the _only_ extraction in the script naming a literal heading. The other four (`MODULES`, `DECISION_TREE`, `ARCHETYPES`, learnings) match generically and are sound.

### 8.5 Two templates, two delivery paths -- and "landed" means different things. **NOT A DOC DEFECT**

Recorded because it changes what today's ST0065 rewrite leg actually delivered, and because assuming one path would have been an overgeneralisation from one estate.

Templates do **not** all reach a project the same way:

- `lib/templates/llm/_CLAUDE.md` is **`include_str!`'d into the binary**. The shipped binary carries the **old** bytes and **zero** occurrences of my new wording, and a fresh `intent init` emitted the old file-map row. **That fix is source-only until the rebuild.**
- `lib/templates/prime/operational-knowledge.md` is **not embedded** -- the bash plugin `cat`s it from disk at runtime. **That fix is live now.**

So the ST0065 edits land per-file, not as a set, and the embedded half rides cc's rebuild batch. **"Eleven sites landed and verified" is true of the source; it is true of delivery only for the files that are not compiled in.**

### 8.6 Verified negative -- do not re-find these

- **All 22 verbs** named in the payload resolve against the shipped v3 binary (`st`/`wp` x5 each, `claude skills` x4, `claude subagents` x3, `plugin list`, `doctor`, `modules find`, `init`). Only the two flags in 8.1 fail.
- **Every skill named exists on disk.** Existence is not currency -- `in-start` and `in-next` are retired by ruling and still present, pending the retirement draft.
- `show_help()`'s _"Source files (all optional, skipped if missing)"_ is **accurate**; each of those five reads is genuinely `[ -f ]`-guarded. It is the hardcoded checklist in 8.3, not the documented source list, that is unconditional.
- `compute_memory_path()` is correct: the dashed-path transform matches the real memory path in use.
- The issue-0025 comment at the top (`resolve_project_root` before any read) is sound and load-bearing; leave it.
- Line 42's conditional MODULES.md wording (landed `ba3c3ec1`) is correct as written.

**Adjacent, and NOT mine:** the dispatch table records `"v2": "bin/intent claude arm"` for `claude prime`, and declares stdout as _"the primed memory content"_ while the implementation writes a file and prompts. Both are surface-declaration questions for the surface owner, not payload defects. Noted, not ruled on.

---

## 9. The `NotByInit` reasons are ALLOCATIONS, and nothing checks the allocatee acts. **MISGUIDED**

**`Destination` has two variants and the second carries a REASON string.** Five of them are not justifications for declining; they are **allocations** -- each names a different mechanism that will lay the file down instead. **An allocation is a claim about another party's behaviour, and nothing in the estate checks any of them.** Driven one at a time, with the search positive-controlled first (`_CLAUDE.md`, a template known to be consumed, returns 6 hits in `native/rust/crates/`).

| `NotByInit` reason                                                                          | shape         | does the named party act?                                               |
| ------------------------------------------------------------------------------------------- | ------------- | ----------------------------------------------------------------------- |
| `_MODULES.md` -- _a hand-maintained index of a tree the store already indexes_              | justification | **n/a -- nothing is owed.** hv's 2026-08-24 ruling. Correct.            |
| `_usage-rules.md` -- _user-owned; the canon installer seeds it_                             | allocation    | **YES.** `canon.rs:424` references the template.                        |
| `_AGENTS.md` -- _generated by `intent agents sync`_                                         | allocation    | see finding 1; hv has since ruled `init` should lay it down, unexecuted |
| `_ARCHETYPES.md` -- _archetypes are a language pack's, laid down by `intent lang init`_     | allocation    | **NO.**                                                                 |
| `_DEPENDENCY_GRAPH.md` -- _written when a project first declares dependencies, not at init_ | allocation    | **NO.**                                                                 |

**`intent lang init` EXISTS AND DOES NOT DO THIS.** The verb is real -- `intent lang` offers `list`, `show`, `init`, `remove` -- so this is not a reference to an unbuilt command, which would be the easier defect to spot. **The string `archetype` appears exactly ONCE in the whole of `native/rust/crates/**/*.rs`: at `init.rs:127`, inside the reason itself.** The named party does not so much as mention the artefact it is said to own.

**`_DEPENDENCY_GRAPH.md` is the same, allocated to a MOMENT rather than a verb** -- _when a project first declares dependencies_ -- and nothing in v3 references the template at all.

**THE ONLY WRITER OF EITHER IS v2, AND IT IS SCHEDULED FOR DELETION.** `bin/intent_st_zero:474` and `:540` both `install_from_template` these two files. That is the v2 bash line, which `AC-00.6` removes **at the cut**. **So the last writer of both templates disappears at the cut, and `init.rs` goes on carrying a comment saying `intent lang init` handles it.** The reason will then be false in a way no test can see, because no test reads reasons.

**THIS DOES NOT OVERTURN ROW 7, IT TESTS THE OTHER HALF OF IT.** The Summary's row 7 verdict -- _keep as-is (latent, correctly unshipped)_ -- is about the FILES and it stands: nothing in v3 ships them, and nothing should. **What it does not test is the REASON**, and the reason is a live claim about `intent lang init` that is false today and will be false differently after the cut.

**THE CLASS, AND IT IS THE THIRD INSTANCE OF ONE SHAPE IN ONE EVENING.** `0281` records `IntentCLI.swift:131` assigning the console tail's lifecycle to a verb that does not exist, in the present tense, as a settled division of responsibility. These two assign a template's laydown to a verb that does exist and does not do it. **In every case the sentence does not read as a plan, so the next reader builds on it** -- and in every case the allocation is in a comment or a reason string, which is precisely the surface nothing executes and therefore nothing checks.

**THE INSTRUMENT GAP IS NAMED RATHER THAN LEFT TO BE FOUND, and it is dc's `B2` seam exactly.** `implemented_check.sh` probes what the dispatch table DECLARES; `canon_mandated_verbs_check.sh` probes what canon NAMES. **Both populations are verbs. A `NotByInit` reason is neither**, so an allocation to a real verb that ignores the artefact is invisible to both, and no better predicate inside either instrument reaches it. The remedy is a static comparison of two populations -- reasons that name a party, against that party's references to the artefact -- not a sharper grep in an existing tool.

### 9.1 The size figure in finding 2 has itself rotted

Finding 2 prices Intent's own `MODULES.md` at **571,769 bytes** and calls `CLAUDE.md`'s _"~354KB across ~367 rows"_ **stale by 62%**. Measured 2026-09-09: **697,867 bytes over 326 table rows.**

**So the catalogue's own corrective figure is now 18% low, and `CLAUDE.md`'s is stale by ~97% rather than 62%.** **And the two halves have moved in OPPOSITE directions** -- bytes nearly doubled while rows went DOWN, 367 to 326 -- so a reader checking only one half would conclude the file had shrunk. **A finding about a stale figure, carrying a figure that went stale inside a month, is the strongest available argument that the number does not belong in the prose at all**: the argument finding 2 actually needs is _too large to read_, which is true at every one of these values and needs none of them.

**Driven that the figure is NOT in the template** -- `grep '354KB' lib/templates/llm/_CLAUDE.md` returns 0 against a positive control of 4 on `Highlander` -- so it is this project's `CLAUDE.md` overlay only and seeds into no other project. **Project-level, and therefore vc's rather than this WP's.**

---

## 10. `.prettierignore` -- **CORRECT**, and its own stated criterion names a path it does not cover

**THE SEEDING HALF IS RIGHT AND IS THE PART WORTH RECORDING AS A VERIFIED NEGATIVE.** `init` calls `facade::converge_formatter_exclusion` (`init.rs:405-407`), which writes the generated-view patterns from `Project::generated_view_patterns` (`project.rs:1359`) -- **one roster, beside the methods that produce the real paths, so the function holds no copy of its own to drift.** It is additive and never authoritative: missing patterns are appended, an operator's tuning is left alone, and it writes five patterns and never `*`. **So a fresh consumer project DOES get the protection**, which is `AC-07.6`'s closing clause and the exact gap the doc comment says the estate could not see from inside once it had fixed itself by hand. **Do not re-find this.**

**THE `.history/` HALF IS NOT A JUDGEMENT CALL, IT IS THE FILE'S OWN RULE APPLIED TO A PATH IT DOES NOT LIST.** `.prettierignore:65-69` states the criterion so the next person widening it has one rather than a list: **_a path belongs here when it has a single writer AND its existing content is evidence. Both halves are required._** It then applies that to archived inboxes, and its comment at `:78` says the earlier pattern _misses by one directory level_.

**A BOARD ARCHIVE MISSES BY FILENAME INSTEAD, AND MEETS BOTH HALVES.** `<node>/.history/<date>/wip-prefold-*Z.md` and its siblings are written once by the folding node and never rewritten -- **ZERO writers after creation, which the file itself notes is stronger than one** -- and their content is evidence: a fold records the archive's sha as its verification handle.

**ASKED OF PRETTIER RATHER THAN REASONED ABOUT, two-sided on real files:**

| file                                          | `prettier --file-info` |
| --------------------------------------------- | ---------------------- |
| `ic/.history/20260909/doing-prefold-2003Z.md` | `"ignored": false`     |
| `vc/.history/20260820/inbox.ic.md`            | `"ignored": true`      |

**POPULATION: 567 `.md` files under `intent/whiteboard/*/.history/`. 142 match `inbox.*.md` and are exempt; 425 are exposed** -- 162 `wip-fold-*Z`, 86 `wip-prefold-*Z`, 41 `wip.md`, 16 `wip-*Z`, plus the `doing-folded`, `watch-outs-full`, `day-narrative` and `decisions` families.

**AND THE EXPOSURE IS BOUNDED, WHICH MAKES THIS SMALLER AND SHARPER THAN THE COUNT SUGGESTS.** `prettier --check` over all 425 returns **_All matched files use Prettier code style!_** -- positive-controlled against a planted non-conformant file, which it warns on. **Nothing is pending a bulk rewrite; every archive was already normalised on the way in.** So the remedy must NOT rewrite the 425, and the harm is entirely at WRITE time.

**THE REALISED COST, MEASURED ON MY OWN FOLD TONIGHT.** The gate refuses unformatted markdown, so an archive is formatted between being taken and being committed. `doing-prefold-2003Z.md` was `437aeb7a531fa038` when taken and `3a410bee87283436` when it landed -- **one trailing blank line** -- and the fold's recorded sha had to be corrected in the same commit. **An archive's sha is therefore a property of the formatter rather than of the board it archives, and a fold that records the sha it took records one that will never reproduce.** That is the same class as the inbox case: running the protocol's own control forces the precise harm the exemption exists to prevent.

**REMEDY IS ONE PATTERN AND IT IS FORWARD-LOOKING**: widen the archive exemption from `inbox.*.md` to every `.md` under `intent/whiteboard/*/.history/`. The 425 already conform, so nothing is rewritten; future archives simply land verbatim. **The consequence to accept, stated rather than discovered: `.history/` will then hold a mix of formatted older files and verbatim newer ones -- which is already true of the inbox archives and is the intended outcome, not a wart.**

**NOT TAKEN.** `.prettierignore` is a shared root file on a five-writer tree and the whiteboard block in it is hand-written rather than generated from `generated_view_patterns`, so the edit is not this WP's to make unilaterally. Routed.

---

## 11. Currency re-drive, 2026-09-09 21:04Z -- findings 1, 2 and 3 all still hold at HEAD

**A CATALOGUE'S FINDINGS ROT EXACTLY LIKE A GATE FIGURE, so this section carries the commands rather than a reassurance.** Driven on a genuinely fresh project -- `intent init` in a tempdir under a scratch `HOME`, so nothing reaches the operator's estate.

**WHAT `init` ACTUALLY LAYS DOWN, ENUMERATED RATHER THAN RECALLED** -- nine files: `CLAUDE.md`, `.prettierignore`, `intent/wip.md`, `intent/llm/{ARCHITECTURE,DECISION_TREE,RULES}.md`, `intent/.intentfiles`, `intent/.config/config.json`, `intent/.cache/intent.db`. It reports **11 embedded templates deliberately not written**.

| finding                                                    | claim                                     | re-driven                                                               |
| ---------------------------------------------------------- | ----------------------------------------- | ----------------------------------------------------------------------- |
| **1** `AGENTS.md` never arrives                            | absent at init; `CLAUDE.md` points at it  | **HOLDS.** `AGENTS.md` ABSENT; `CLAUDE.md` references it **4** times    |
| **1** the estate's own validator fails a fresh init        | `agents validate` non-zero                | **HOLDS.** `rc=1`, _error: AGENTS.md not found at project root_         |
| **2** the retirement reached the file, not the instruction | `MODULES.md` absent, instructions survive | **HOLDS.** `MODULES.md` ABSENT; `DECISION_TREE.md` cites it **3** times |
| **3** an Elixir decision tree seeded to every project      | written unconditionally                   | **HOLDS.** written into a project with **no declared language**         |
| **AC-11.3** `.intentfiles` present, header only            | header, no `STEELTHREAD:` line            | **HOLDS.** present, 29 lines, **0** `STEELTHREAD:` lines                |

**hv's RULING THAT `init` LAYS `AGENTS.md` DOWN IS THEREFORE STILL UNEXECUTED**, and `init.rs:115-116` still reads `NotByInit("generated by \`intent agents sync\`")`. **That reason is section 9's class exactly**: an allocation to a party that does act, but only when a human runs it, so the file never exists for the reader `CLAUDE.md` sends to it.

**AC-11.3 IS A VERIFIED NEGATIVE AND ITS REASONING IS THE MODEL FOR THE REST OF THIS SET.** `init.rs:352-374` records why the manifest comes from `default_declaration(&[])` rather than a template: **one home for the header, shared by init, the migration and `upgrade`, so a change moves all three at once**, and it says outright that honouring the `lib/templates/` project rule here **would break the criterion it looks like it serves** -- durable state derived from status is not generated content. **A rule correctly declined, with the decline reasoned at the site.** Nothing to do.
