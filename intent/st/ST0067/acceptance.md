...
- AC-00.1 (non-test) Bare `intent llm` serves the agent guide rather than refusing as not-implemented, and `intent llm guide` continues to serve it under its own name. -- evidence: vc validation drive 2026-08-28: llm_serves_the_guide_and_the_rules 4/4 (identity assertion with length-and-content control), dispatch_ssot 18/18; built be3edf70 -- satisfied: yes
- AC-00.2 (non-test) The guide's 'Workflows, methodology and conventions' section is written -- the ST/WP/AC/AT model, the canon-vs-view contract, the sync discipline, gate semantics -- compiled into the binary so it versions with the tool, and the whole guide output stays free of Intent PM identifiers beyond neutral placeholders. -- evidence: vc re-read 2026-08-28 after cc's 1f89b3e9: three accuracy fixes verified in source (st list in-progress + --status all pointer; bare-vs-scoped --to-store; markdown has NO store path, ingest --from-md parses and writes nothing) with the new absence+presence arm the_guide_does_not_claim_markdown_reaches_the_store; llm_serves 5/5 in validator's hand; mechanical sweep clean, ST0000 only -- satisfied: yes
- AC-00.3 (non-test) `intent llm usage_rules` prints the project's root usage-rules.md, and refuses plainly on stderr with a nonzero exit when the file is absent. -- evidence: vc validation drive 2026-08-28: llm_serves_the_guide_and_the_rules covers the print + exit-1 refusal; built be3edf70 with the init-remedy correction cc disclosed -- satisfied: yes
- AC-00.4 (non-test) AGENTS.md as generated by `intent agents sync` routes an arriving LLM to `intent llm guide` -- one pointer line in the template, landed once, in coordination with ST0065 WP-01. -- evidence: vc validation drive 2026-08-28: root_files_generated 12/12 incl. the four-language-set sweep proving the pointer sits outside every lang block; built 8a997c1e; this repo's AGENTS.md regenerated two lines only -- satisfied: yes
