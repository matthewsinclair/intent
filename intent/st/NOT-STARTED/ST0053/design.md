...
- **D5 -- release framing (OPEN -- hv sequencing call).** The pack is a minor (new project-type surface), so it cannot fold into the 2.15.1 _patch_. Open: cut 2.15.1 (the pending fixes) first and ship the pack as 2.16.0, OR hold 2.15.1 and roll its fixes + the pack into a single 2.16.0. Awaiting hv.
