...
The scope is the gitignore-aware repository, and D29 stands whole: git's rules, nested files, negations, `.git/info/exclude` and the global excludes file decide membership, as git on this machine sees it. What changes is the instrument: today `Ignored::for_root` derives the ignored set as the difference between two walks, and its own note records that on this tree the unfiltered walk visited the whole of `native/rust/target/` and cost doctor ten seconds; widening that technique to the repository rebuilds the defect unbounded. So `Ignored` becomes a matcher, built once per root and asked per path, enumerating nothing, with a test that a large ignored directory is never visited. `sync` does not widen with the scope: `scan` filters to the canon corpus explicitly, so no source file is ever ingested as canon. Inside the scope:
...
- **Every exclusion is a row.** A binary (NUL in the first block, as grep decides it), a file over the size cap (a config value with a measured default), an unreadable file and a symlink each get a `file_index` row with `skipped_reason`. `intent index status` lists them. A file the index does not hold is never an absence.
...
| Table          | Purpose                                                                                                             |
| -------------- | ------------------------------------------------------------------------------------------------------------------- |
| `file_index`   | Widened: `corpus`, `lang`, `indexed_sha256`, `skipped_reason`; one row per in-scope path, skipped ones included     |
| `src_sections` | FTS5 over code, `unicode61` without stemming: `path, seq, start_line, end_line, kind, name, name_parts, body`       |
| `symbols`      | Definitions: `path, lang, kind, name, name_parts, qualified, parent_id, start_line, end_line, start_byte, end_byte` |
| `symbol_refs`  | Name-matched references: `path, lang, name, start_line, start_byte`                                                 |
