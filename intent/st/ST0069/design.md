...
The scope is the gitignore-aware repository, defined by the existing `Ignored::for_root` walk so git's rules, nested files, negations and the global excludes file decide membership. Inside that scope:

- **Corpus assignment is by kind, not by directory.** The store's entities stay the canon corpus, unchanged. A markdown or text file on disk (`README.md`, `docs/**`, `intent/wip.md`, a whiteboard board) joins the prose table through the existing heading splitter, with `owner_type = file`. A code file joins the source table. The extension-to-language map has one home, the one `critic::applies_to_file` reads.
