...
- **Two corpora, two staleness policies** (AC-13.5, the D24 interaction): canon hashes always; source is stat-then-hash. The asymmetry is justified by consequence, a missed same-size same-mtime edit in source costs one stale hit, and each policy is asserted against its own missed-edit case. As built (2026-09-13), `intent index rebuild` replaces the whole index and `Facade::index_refresh` reconciles one path's stat rows against `index_file`, re-reading what moved and removing what has gone (`facade.rs:7169-7219`).
- **One scope object, two consumers.** The scanner and the watcher read one `Scanned`, which answers the sync's question (`includes`, unchanged) and the index's (`in_repository`) from one matcher. As built (vc, 2026-09-12), the watcher is two registrations rather than one widened one: the canon watch is unchanged, and a second recursive watch from the root reaches only `Work::IndexRefresh` (`intentd/src/watch.rs:124-170`). An event naming the bare project root reconciles the root's own files and never descends (`index::reconcile::changed_under`). It is one recursive watch on every platform; the macOS volume was measured, and the Linux leg is neither built per directory nor measured.

## The tiers

### T1, lexical, widened

The prose table keeps `porter unicode61`. The source table is `unicode61` without stemming: stemming mangles identifiers, and unicode61's default token characters split `snake_case` into its words, so a search for `disabled` finds `parse_disabled`, while `CamelCase` stays one token and is reached by the prefix form `Removal*` that the escaper already preserves. One row per file for T1, with a `name_parts` column so the words inside a camel-cased name are searchable too; T2's symbol rows went to a table of their own, `symbols` (as built, 2026-09-13, `store.rs:469-480`).
...
The deltas are additive (the five singles sum to within linker alignment of the all-five build), so any subset can be priced from the table; the runtime is paid once. Swift alone is close to half the whole grammar cost. Node-kind count does not predict size (elixir has fewer kinds than rust and costs more), so a sixth language is measured, never estimated. **The line is hv's.** The crates entered the tree through WP-20's symbols module behind per-language features, all off by default until the line was ruled; as built, `lang-rust`, `lang-elixir`, `lang-swift` and `lang-lua` are on by default and `lang-bash` is declared and off (`intentsvcs/Cargo.toml:62-69`). One fact found there bears on the line: **`tree-sitter-bash` 0.25.1 ships no tags query** (a highlight query only), so `shell` parses and names no symbols, and the bash row above buys nothing for its bytes. Ruled 2026-09-12: `lang-bash` stays declared and off until upstream ships a tags query, and nobody writes one here, because a hand-written query is the per-language extractor AC-20.2 exists to avoid. The other four went on by default (ruled 2026-09-12, decision 2).
...
- **HTTP**: an OpenAI-compatible embeddings endpoint from `config.json`, which covers a local Ollama; as built (2026-09-13) it speaks `http://` only and refuses an `https://` endpoint by name, so no hosted provider is reachable yet (`embed.rs:116-157`). Small, no runtime in the binary, opt-in. The posture cost is the operator's choice, stated in the docs: a hosted endpoint sends the corpus to it.
...
**Recommendation: Null and HTTP in this thread, Local after hv rules on the runtime and the binary-size line.** Storage is an `embeddings` table keyed by `(chunk_id, model)`, with brute-force cosine ranking in Rust (`embed::cosine`); as built (2026-09-13) nothing chunks or writes a row yet (`store.rs:481-507`); `sqlite-vec` is the recorded upgrade when a measured corpus outgrows brute force, which the estate's corpora do not.
...
```json
{
  ...
  "index": {
    ...
    "corpora": {
      ...
      "prose": { "policy": "stat-then-hash", "files": 0 },
      "code": { "policy": "stat-then-hash", "files": 0 }
    },
...
```
intent search <query> [--tier lexical|structural|semantic]... [--kind def|ref|file|thread|wp|issue|attachment|project]...
                      [--lang <lang>]... [--path <glob>] [--limit <n>] [--no-reconcile] [--json] [--daemon]
intent search --outline <path> [--json]
intent search --context <name> [--json]
intent search --sql <statement> [--limit <n>] [--json]
intent index status [--json]
intent index rebuild [--json]
```

`--json` is terminal-channel only and not exposed on MCP, which always answers the structured form. A bare query is text; each structured door is a flag and nothing is auto-detected, because a query that begins with `select` is also a thing someone searches for. `query` is therefore optional in the register and the verb refuses more than one of a query, `--sql`, `--outline` and `--context`, or none, as a usage error; only the text query crosses to the daemon (as built, 2026-09-13, `render.rs:3574-3606`). `index rebuild --corpus` does not ship: the register holds it pending. The door's bounds are defaults, named in its refusals: a default row cap, a hard ceiling, and a bound on SQLite work per statement.

### The structured query door, `--sql`

hv's `<sqlite-query>` half. One statement, run on a second connection opened read-only with `query_only` set and an authorizer that permits reads alone; a second statement, a write, or a pragma that changes state is refused with the remedy naming the read-only contract. Rows come back as objects under `--json` and as an aligned table otherwise, capped by `--limit` with both denominators. Every `--json` and MCP response carries the store's schema version, and the schema itself is already published by `intent schema`, versioned per release since batch 1 of 3.0.2 made the release regenerate its faces. That published face is the contract the agent writes against.
...
Generated from the register row as today. `search` gains the flags above as parameters, except the terminal-channel `--json` and `--no-reconcile`, and answers the envelope; `outline` and `context` are parameters of the same row under the same one-question rule; the structured door is the same row's `--sql` flag, not a second row, because the tool list is generated from the register and a `search sql` leaf would parse a text query whose first word is `sql` as the leaf (ic, WP-17's shape, 2026-09-12). The tool's description is the row's `help`, which states the modes, followed by the safety sentence and the row's `when_to_use` (as built, AC-24.2, `mcp.rs:140-155`). Nothing in `mcp.rs` beyond the serve arms. The `index` family is split on the tool tier: `index status` is served and `index rebuild` is withheld, because it rewrites every row while another surface may be reading them.

### The explorer's `/search`

`/search <query>` in the palette opens a resident pane, `nav::View::Search` (as built, 2026-09-13, `tui/views.rs:143-240`): each hit is a `button` row named by its bare path, titled `path:line` where it has a span and valued with its kind and snippet (the row's `kind` field is the mode machine's widget discriminator and not a taxonomy, found by ic when a `thread` kind opened an editor over a hit); the freshness note goes to INFO, and a rule row separates the tier groups. Enter on a thread or issue hit lands on its view through the existing navigation; every other hit opens its file through the lent terminal, the way `edit` already borrows it, without positioning the editor at the line (AC-21.2). The pane calls the same facade method as the CLI; the TUI's pure layers stay pure and testable without a terminal. An option for hv: the omnibox treats input that matches no entity as a search, which makes the search box the home screen.

### The daemon

`Op::Search` crosses the wire with the envelope. `intentd` keeps the index warm through its own index watch (the corpus section), incrementally: a changed file is re-tokenised and re-parsed and its rows replaced. A daemonless query reconciles the whole index scope first (canon by hash, prose and source by a stat and then a hash for what the stat moved), so its answer is identical to the daemon's for the same tree state; the difference is when reconcile ran, and `--no-reconcile` trades that for a named staleness. The MCP face stays in-process and correct without the daemon, because it reconciles too. As built (2026-09-13), a daemon-served query does not reconcile: `Op::Search` calls `search_all` and nothing before it (`intentd/src/store.rs:896-909`). Because a watch names only what changes after it starts, a daemon opening a project surveys with `Facade::index_stale_roots` and refreshes each stale top-level directory in turn between client ops, a waiting op first (issue 0366, AC-22.4).

### The agent canon

Every skill, template and rule that tells the model how to check for prior art names `intent search --kind def <name>` first, with grep as the fallback when the envelope says the index is not complete for the paths that matter, and the agent guide regenerates from the register (as built, AC-24.5, `lib/templates/llm/_CLAUDE.md:21`). `intent modules find` was planned to retire here and was kept (decision 5): it stays in 3.0.2 as the lookup for a project that keeps a registry, named after the index.
...
- **inotify on Linux** takes a watch per directory; as built (2026-09-13) the index registration is one recursive watch on every platform; the macOS volume was measured and the Linux rate is not measured and not claimed (`intentd/src/watch.rs:136-150`).
...
The safety condition is one sentence and it is the whole specification: **the redirect fires only when the envelope says the index is COMPLETE for the paths the pattern would have reached, and never otherwise.** Not complete in general -- complete for those paths -- because a redirect that answered from a stale index would hand the model a confident wrong answer in place of a fresh right one, and the model has no way to tell it happened. **That condition is implemented as ONE shell function, `intent_index_answers_for` in `lib/templates/.claude/scripts/index-freshness.bash`, reading the envelope's `skipped` and `stale` lists -- complete for these paths means no entry in either list beneath them, with the envelope's own `complete` as a fast path and never as the rule -- called by the PostToolUse hook (AC-24.4) first and by this redirect when it is built, because `index.complete` is the WHOLE index's claim and one unreadable file anywhere makes it false for a path nowhere near it** (dc's review of this spec; vc, 2026-09-12). **It takes the path prefix as an ARGUMENT rather than deciding the subject for its callers, because the two callers have different subjects**: the hook's paths involved are the ones its appended ANSWER names, and this redirect's are the ones the pattern would have reached -- dc's first build of the hook read the grep's own scope and appended hits from a file that had moved underneath the index.
