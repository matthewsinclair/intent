...
- **Attachments.** ~~not modelled yet~~ **LANDED `36bc02c5`** -- `Attachment{path,text,bytes,sha256}`, one constructor, `text` carried with NO trim so a round trip cannot cost a byte. **110 carried; 198 uncarried, being 196 `.tap` plus a 2-file `.tsv` tail.** **STILL BLOCKING, and it is the bigger half: 163 `design.md`/`impl.md`/`tasks.md` are classified TYPED and the model has no field for any of them -- open question 7.**
- **Issue bodies.** ~~trailing newline normalised, so 5.1 refuses every issue until the render re-emits it~~ **NO LONGER BLOCKING.** cc made `body` VERBATIM rather than accept the precondition -- _a normalisation that requires a future component to compensate is a scheduled defect_ -- so frontmatter plus `body` reproduces the source exactly and these bucket as `conserved byte-identical`. **This entry stood stale for an hour after 5.1b was corrected, and cc caught it: the correction landed in the section that ARGUES the point and not in the section a reader consults for a VERDICT.** That is this programme's own class, in a document rather than an instrument, in the preconditions list for the only irreversible act in the design.
...
6. **THE BIGGEST ONE, AND IT IS A MODEL DECISION RATHER THAN A REALISATION ONE: 163 files are classified as TYPED DOCUMENTS and the model has no field for any of them.** `design.md`, `impl.md` and `tasks.md` are three of the canonical five, `Project::classify` counts 163 of them, and `thread.json` carries no `design`, `impl` or `tasks` field. **Measured on the live estate, all 163 rather than a sample: longest distinctive line probed against its own thread's canon, 0 found. 748 authored sections whose content exists nowhere but those files.**

**Under rule 1 of the attachment spec a file is a typed doc OR an attachment, never both -- so being classified typed is what EXCLUDES them from being carried.** Two ways out:
...
8. **Issues have no rendered-view path in v3, and `intent edit <ISSUE>` therefore has nowhere to write.** (The trailing-newline precondition this originally carried is retired -- cc made `body` verbatim rather than ask the renderer to compensate.) `intent/issues/NNNN.json` is canon and now carries `body`; the only rendered issue markdown in the estate is v2's `issues/<BUCKET>/NNNN/NNNN-slug.md`, which is precisely the residue this design retires. A path has to be declared -- `intent/issues/NNNN/NNNN.md` is the obvious shape and matches the thread arrangement. **Found by the plan tool rather than by writing this document**, which is the argument for building the second derivation: a probe looking only at the v3 path reported all 40 issues absent, and "absent" reads as nothing-to-do.
