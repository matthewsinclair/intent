...
- **13 is numbered last but is NOT sequenced last.** Its retirement half (T0: treeindex + `in-handoff`) must land BEFORE WP-06 ports commands that are about to be deleted -- 762 lines of bash and two `/in-essentials` rules leave the port list, so WP-13 shrinks WP-06 rather than adding to it. Its search half depends on 03/08/09. WP numbers are creation order; the Depends column is the sequence.
