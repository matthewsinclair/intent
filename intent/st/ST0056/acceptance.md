...
- AC-05.3 (non-test) Every file in the on-disk `tests/**` estate **at WP close** is classified in the keep/retire/deviate register; no unclassified rows. The corpus is the estate at the close, not at an earlier sweep -- a guard landing after a measurement moves the corpus out from under it, so the register names the revision it covers and is brought current at close. A file that **never invokes the CLI** is classified by inspection as out-of-scope, not by burn-in: the estate-wide sweep is the cost of a file that DOES reach the binary, never of one that cannot -- evidence: parity register at WP close -- satisfied: no
