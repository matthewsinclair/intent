...
- AT-00.8 `native/rust/crates/intent-cli/tests/no_pm_state_in_output.rs` -- covers AC-00.9 -- status: to-write -- **the discriminating case is a test that greps only the ERROR paths, which passes on the defect**: three of the six measured leaks are not errors at all (a startup banner, a scaffolding refusal, a remedy's worked example), so the check must cover every emitted-string surface of every shipped binary. Assert on the STRING-LITERAL surface rather than on captured output, since a leak on a rare branch is never captured; and it must go red on `owed_by`-style structured fields reaching a renderer, not only on inline literals, because that is the shape the leak actually took. Comments and Intent's own test fixtures are deliberately exempt and the test must prove it tolerates them, or it will be disabled the first time it fires on a doc comment
