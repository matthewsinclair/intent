...
- AC-02.3 Deleting the DB and rebuilding from the files yields identical queryable content -- **the disk-to-db sync direction**. (Rationale corrected 2026-08-15: this was written as "the D01 disposability invariant" and D01 is reversed -- the DB is now the SSOT. **The AC and its test are unchanged and WP-02 does NOT reopen**, because the property is still required and is now load-bearing for a different reason: it is what lets a colleague pull the files and reconstitute state, and it is the condition under which "no DB migrations, ever" can survive the reversal. What changed is why we want it, not whether. **Its scope is now a live question**: it can only hold for state that HAS a file form, and `event_log` currently has none -- see the open question on D01)
