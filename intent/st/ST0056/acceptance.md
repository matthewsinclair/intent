...
- AC-10.8 **Egest symmetry**: deleting the file estate and egesting from the DB reproduces it, and the only difference is the out-of-model set enumerated at `data-model.md`, **named in the output rather than silently absent**. The second half of ic's proposal, accepted 2026-08-15 as hv's own stated condition -- `rm intent.db` is safe only if files remain to ingest back from, and vice versa. The safety was always a property of **git holding the canon**, not of the DB, and stating it that way is what makes the untested half visible: the fleet acceptance fixture tests ingest forward only, and explicitly on semantic completeness rather than byte round-trip. Accepted on the failure shape rather than the feature: this is exactly the class that failed on ic's `burn-baseline.tsv`, where the header asserted the artefact reproduced the register, everyone believed it, nothing checked, and it drifted three rows in silence -- with the difference that an untested egest guarantee only fails on the day someone has already lost the files. D30 shrank this materially by moving the whiteboard, which was the largest entry in the out-of-model set, into the model
