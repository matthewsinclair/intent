...
**RULED BY hv 2026-09-13, after typing `/issues` and getting `intent issues` printed over the screen:** _both /threads and /issues SHOULD be natively handled by the TUI._ This section had kept navigation out of the palette on the ground that `thread` typed into the omnibox already reaches the collection. The reach is real and the conclusion did not survive the operator: someone who types `/issues` has told us where they look for the issues, which is the lesson `/quit` taught.

**The two collections hv named are acts, and nothing else moves.** A work package is reached through its thread, and every other place is still one word in the omnibox. `/threads` and `/issues` open their lists; `/threads` on the threads list moves nothing rather than stacking the view on itself. **`/issues <args>` still runs `intent issues <args>`**, because `issues` was a roster verb and taking its name for the view must not take the command away; `/threads` has no `intent threads` to run, so an argument there is refused on the info row.

### `/projects` is a list, and choosing a project switches to it
...
**RULED BY hv 2026-09-16 (issue 0418):** _it seems odd to have this as a standalone thing that is so different to the rest of the app ... when run out of an intent project, or when the users types /projects in the omnibox in the TUI, then we list projects just like we list issues and threads and selecting one changes to that project ... I'd retire the current project selector in favour of doing it from within the TUI main._

**The projects list is a view in the stack, not a screen of its own.** `/projects` pushes it like `/threads` and `/issues` push theirs, and it is moved through, searched and left the same way: Up and Down, the omnibox, Backspace or `/back`. **Choosing a project is arriving at `/projects/<root>`**, whichever door reached it -- Enter on a row, the omnibox, or a typed path -- and the run loop answers that arrival by ending this project's loop with the root to switch to, because the rest of the stack reads one project's store and a view inside it cannot change which store that is. The chosen project opens at the threads list; the view stack of the project left is not kept. Every registered project is also in the omnibox's index, so typing a project's name from anywhere offers it.

**`intent explore` outside a project opens the explorer with no project**, rooted at the projects list. Help and settings work there, since neither reads a store; every other view says that no project is open and names `/projects`. Quitting, with `/quit` or Ctrl-C, returns to the shell; walking back never does. Starting inside a project is the same as choosing it.

**It lists the project registry, read through `intentsvcs::projects`, rather than asking intentd over the wire.** intentd lists exactly that file and never writes it, so the file is the daemon's answer and it is still there when no daemon is running. An unreadable registry is an error row, never an empty list. A registered root that is no longer a project is listed as missing, and choosing it is refused with the reason on the info row, leaving the operator on the list. Choosing a project moves the working directory, which is what a project is to every other door: the palette's `/{cmd}` runs the CLI's dispatch in this process and acts on the project chosen. `intent explore` adds the project it opens to the registry, and says so on the info row when the registry cannot be written; an empty registry names `intent discover <dir>`.
