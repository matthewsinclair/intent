...
| `IN-SH-CODE-005` no-silent-exit-codes        | critical       | **DECLARE none**          | **REVERSED 2026-08-19 from GREP AT A STATED COST, on dc's measurement after arming it blocked their own commit with 13 findings on one file, every one a deliberate `2>/dev/null` on a git or jq probe.** The structural reason and not the effort one: **the rule's content IS its qualifier -- _without an adjacent comment_ is a predicate over the NEIGHBOURING LINE, and a proxy may carry no `-A`/`-B`, so it is unreachable BY CONSTRUCTION.** The proxy grammar cannot state this rule; no amount of grep quality reaches it. Cost, measured across the estate: `2>/dev/null` appears 395 times in 87 of 128 shell files (68%), so arming it refuses two thirds of the project's shell files on a CRITICAL rule over correct code, and a gate that must be bypassed is one nobody keeps. **NOT a retreat to silence: yesterday this rule was UNDECLARED and skipped without a word; a declare-none is named and counted on every run.** The named tools also answer ADJACENT propositions -- SC2015 is `&&`/`\|\|` mixing, SC2164 is `cd` without a check; shellcheck lints neither `\|\| true` nor `2>/dev/null` nor `set +e`. **THE CELL ALREADY CARRIED THIS ARGUMENT AND CHOSE GREP ANYWAY**: its previous text said it _cannot evaluate the rule's own "without an adjacent comment" qualifier_ and _fires on every documented, correct use_. The reasoning and the disposition disagreed inside one cell, and only arming it surfaced that. |
