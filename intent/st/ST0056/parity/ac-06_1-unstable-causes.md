...
**Status, checked against the build on 2026-09-15: a dated read, kept as the record.** `AC-06.1` was withdrawn on 2026-09-11, and `AC-06.3`, which this read fed, is satisfied. `coverage_map.sh` runs only from a worktree at the measured v2 revision and refuses at HEAD by design (issue 0330). Causes B, C and D were filed as issues 0200, 0201 and 0202, all closed: C was `intent_critic` calling `find_project_root` without sourcing `intent_helpers`, so `disabled:` was dropped, and D was not a defect, because the test invoked the pruned v2 `intent_claude_upgrade`. The citations below describe the tree at `e0cdc389`: `tests/lib/test_helper.bash` now defaults `INTENT_BIN` to the v3 build, and `77b724c55` removed `intent_claude_hook` and `rules_lib.sh`.
