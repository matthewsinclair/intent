...
hoist|.|9b73e98f11e0cd52cc3523f13993e82e5db728f4|HISTORICAL PIN -- NEVER RE-PIN. The tree the hoist ACTUALLY MIGRATED (precedes 0ec2ac79 by 4 commits); the migration is BEHIND, not ahead. Originally pinned 2026-08-18 on hv's directive to self-host ASAP, as Intent's own estate at the HEAD the migration would be run against. NOT a substitute for `canary` and it does not replace it -- canary is pinned at 42fb5269 because that is the last revision where `intent/issues/` populates BOTH arms, so it is the member that exercises the migrator's BLOCK path, and at HEAD the OPEN arm is empty. **This row exists because a verdict about the canary pin is not a verdict about the tree the hoist migrates**, and those had been read as one. **THIS PIN IS HISTORICAL AND MUST NEVER BE RE-PINNED.** The earlier instruction here read "re-pin it whenever HEAD moves; a stale pin here is a claim about a tree nobody will migrate", and **that rule EXPIRED when the hoist ran -- it was written while the migration was still AHEAD.** Measured: this revision PRECEDES the hoist at `0ec2ac79` by 4 commits, so it IS the tree the hoist migrated; the migration is BEHIND, not ahead. **Following the old rule now would do the exact harm it was written to prevent: re-pinning to HEAD would point this row at a tree with no v2 source, making STRANDED a claim about nothing.** THE RULE WAS PER-MEMBER, AND AS OF 2026-09-01 IT HAS EXPIRED FOR EVERY MEMBER. When vc ruled this on 2026-08-18 only `hoist` had crossed over, so re-pin-on-HEAD-move still governed `canary`, `lamplight`, `utilz` and `baize`. Measured 2026-09-01: all four now declare `intent_version: 3.0.0` at HEAD (`canary` IS this repo; the other three read from their `intent/.config/config.json`, NOT their product `VERSION` files, which say 0.5.0/2.5.0/0.1.0 and are unrelated), so their migrations have run and re-pinning ANY of them to HEAD would point the row at a v3 tree with no v2 source -- the exact harm this row records for `hoist`. Every pin in this table is now HISTORICAL; none is re-pinned. **This is the class where a refusal's REASON expires and the refusal must be retired by the change that expires it, or it survives as a rule nobody can re-derive** (vc ruled 2026-08-18 on ic's question; extended to all members by ic on measurement 2026-09-01, `parity/tools/` being ic's harness)
