...
printf '  3. THE ROSTER LISTS EVERY *.sh UNDER parity/tools (runner_roster_check.sh reads\n'
printf '     `git ls-files` for them), whatever the name, so a tool lands in `unrostered`\n'
printf '     only when it has no roster row. `unrostered` here means UNCLASSIFIED, not\n'
printf '     `not gated`.\n'
