...
thread_view_skew_check.sh  manual  new 2026-08-20, awaiting admission by dc -- cc built it, dc rosters. 130-150ms measured at f0c2805c ON ONE MACHINE over 268 views, which would make it the CHEAPEST gated instrument here and ~20x faster than view_skew_check.sh at 2860-2940ms; stated with subject and revision because the bare figures in the rows above cannot be compared against anything. IT COVERS THE POPULATION THE SIBLING NEVER DID: the CHECKABLE in view_skew_check.sh is ONE triple under surface/, so gated skew coverage is 1 of 269 and the missing 268 are the thread covers, acceptance contracts and WP covers. It forms no verdict -- views::skew is the single home for the question and this parses one answer rather than computing a second. UNGATEABLE BEFORE b082b488, which is why it is new rather than late: doctor reported every dehydrated view as MISSING, 235 findings at rc=1 on a healthy tree, so wiring it then would have delivered a permanently-red gate. Driven ten arms on a purpose-built rig: clean, planted skew with and without --changed, inherited-only, absent binary, changed output shape, doctor exiting 2, a 0-view denominator, and a MENTION/SUBJECT pair carrying identical decoy lines to opposite verdicts. Refuses at exit 2 rather than passing when it cannot read the summary line, because a text-reading gate whose needle stops matching goes green forever and nothing says so. Needs the v3 binary at native/rust/target/release/intent (v3 is off PATH by ruling); absent, it names what goes unchecked and exits 0
