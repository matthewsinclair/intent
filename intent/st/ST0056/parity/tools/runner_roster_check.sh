...
canon_commit_check.sh      manual  new 2026-08-18, awaiting roster admission -- reads git only, no worktree, no binary, no clock. Driven four ways on real history (vacuous exit 2, adds exit 1, inherited-not-failed exit 0, clean exit 0) and caught an unplanted divergence on its first whole-tree run
