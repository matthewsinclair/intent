...
inert_arg_check.sh         manual  new 2026-08-30 (dc), covering AC-05.7 via AT-05.7. MANUAL BECAUSE IT DRIVES THE BINARY AGAINST THE LIVE PROJECT and because its subject FAILS TODAY -- precedent provenance_fields_check.sh rather than guard_home_check.sh: a gated row would deliver a permanently-red gate on every node commit for a defect whose fix is a WP-05 surface call, and AC-05.7 states in its own text that it REPORTS rather than refuses, on the AC-05.5 reasoning that families land through the ladder. POPULATION DERIVED FROM THE TABLE, NOT DECLARED HERE: the 10 entries carrying an argument with two or more `values`, which is 10 of 121 entries today against the 10 of 105 the criterion recorded -- THE DENOMINATOR MOVED AND THE NUMERATOR DID NOT. METHOD: drive the entry twice differing only in that argument, a DECLARED value against one the enum does not contain, and compare stdout+stderr+exit. The bogus value is the discriminator because two legal values that legitimately agree are byte-identical for the same reason an unread argument is. THE TWO-SIDED CONTROL IS FREE AND LIVES IN THE POPULATION: `st show` DISCARDS and `st edit` READS -- same family, same argument name, opposite verdicts -- so a run in which neither verdict fires measured nothing, and the tool refuses rather than reporting clean. DRIVEN TO THREE CONTROLS, each rc=2, tool restored and hash verified IDENTICAL after every plant: a broken partition sum (9 against 10), a vacuity plant making all 10 undrivable, and an empty population. ITS FIRST DRIVE FOUND A DEFECT IN ITSELF: `agents template` classified as reads because clap rejects a bogus subcommand BEFORE the unwired arm runs, so the pair disagrees and an UNWIRED row read as a healthy one -- the unwired test now keys on the marker in stderr rather than on agreement between the two probes. AND THE CRITERION PREDICTION HAS COME TRUE UNNOTICED: AC-05.7 recorded 8 of the 10 behind unwired(), and 3 are today (`agents template`, `claude subagents`, `claude prime`) -- five families were wired since, which is exactly why the criterion argued for building the instrument BEFORE the wiring rather than after. DRIVES A THROWAWAY FIXTURE WITH ITS OWN PROJECT AND ITS OWN HOME, never the live one. The first version drove the real project and HYDRATED ST0001 into .intentfiles on every run -- a check that mutates tracked project state is a defect in the check, and it was found by reading git status after a clean run rather than by any arm. THE FIXTURE ALSO TRIPLED THE REACH: 7 rows driven where 4 were before, because a fixture makes the mutating verbs safe to drive (wp rescope, claude skills, claude ws), and claude subagents moved from undrivable to correctly UNWIRED. Now 1 discards + 6 reads + 2 unwired + 1 undrivable. AND THE FIXTURE INTRODUCED A FALSE POSITIVE THAT IS NOW GUARDED: if BOTH probes fail identically the pair agrees because the verb never ran, so reporting it as a discarded argument would be a finding manufactured by the harness -- the discriminator requires the DECLARED value to SUCCEED. REACH: the population is DECLARED, so an argument with an enumerable domain the table does not declare as `values` is invisible -- the same blindness the criterion was written about. The one remaining UNDRIVABLE row is `claude`, a bare dispatch whose values ARE the sub-families, each rostered here in its own right; undrivable is a bucket rather than a skip, since a skipped row is indistinguishable from a clean one. THE ROW IS APOSTROPHE-FREE BECAUSE ROSTER IS A SINGLE-QUOTED SHELL STRING -- one apostrophe terminates it, which this row did on its first attempt and broke the parse fourteen rows further down.
...
suite_path_check.sh        gated   new 2026-09-14 (dc), issue 0348. Reads the INDEX, never the working tree: every #[path] a staged native/rust/crates/*/tests/suite.rs registers must be a file the commit carries, because the node that wrote the registration has the file on disk and nowhere else does (d4d7aec2b, which HEAD could not build until 5746e02e). Static and fast. RUNS A RED AND A GREEN CONTROL IN A SCRATCH REPOSITORY BEFORE ITS VERDICT, the red being a file on disk and not staged, and refuses at 2 if either does not fire.
...
provenance_fields_check.sh     manual            new 2026-08-21 (dc), covering AC-11.7 via AT-11.7. STILL MANUAL, AND THE WRITER FIX IS DONE. The stated release condition is DISCHARGED 2026-09-05 (dc): int macos stage now emits the labelled partition (artefact_sha256 answering IDENTITY with one line per staged artefact, commit answering CURRENCY, drift HELD with its release condition), FIELDS reports ok, and the self-test drives all seven controls. commit and checkout_clean keep their spellings because int macos publish parses both out of this record. BUT THE CONDITION NAMED A PROXY RATHER THAN THE PROPERTY IT WAS PROTECTING, and that is why this row stays manual. It was minted to avoid a PERMANENTLY-RED gate and it named only the FIELDS cause; the SET arm is red independently, because the delivered pair is 92e4d914 while the record names HEAD. Gating today would still block every commit in the repository -- the unbounded blast radius vc ruled on. REVISED CONDITION, CORRECTED THE SAME DAY AFTER DRIVING IT -- and the first revision was wrong, which is the point. I wrote green on both arms against a FRESHLY STAGED PAIR, then rebuilt at 0196 first run and staged immediately: SET was still red, because HEAD had moved between the build and the stage. A freshly staged pair is not sufficient and never was. THE MECHANISM IS THAT commit: HAS TWO READERS WITH TWO DIFFERENT SUBJECTS. int macos publish reads it as the CHECKOUT at stage time and requires it to equal the TAG; this tool SET arm reads it as a claim about the ARTEFACTS and compares it to their embedded markers. Those coincide at a release -- clean checkout, tag at HEAD, artefacts built there -- and diverge on a live five-node checkout every time anything lands between build and stage. SO SET IS RED HERE FOR A NON-DEFECT AND GATING NOW WOULD BLOCK EVERY COMMIT IN THE REPOSITORY. HONEST CONDITION: the promotion point is a RELEASE, where both subjects coincide by construction. CHECK IT: bash intent/st/ST0056/parity/tools/provenance_fields_check.sh at a tag, with the artefacts built from that tag. The two-subject collision is reported to vc as a finding in its own right and is NOT fixed here: publish depends on the commit spelling, so restructuring the record is a design call on ST0056 rather than a roster edit. TWO ARMS, DIFFERENT SUBJECTS, REPORTED SEPARATELY: FIELDS asks whether the record is well-formed, SET asks whether the record describes the BYTES beside it and whether those bytes agree with EACH OTHER. A PER-RECORD CHECK CANNOT SEE A PROPERTY OF THE SET -- found on real bytes, two binaries naming two different commits beside a record naming a third, every file individually well-formed. NOT A COPY of artefact_commit_blockers in cmd/macos: that pivots on the TAG at release time only, this pivots on the RECORD at any time. COST 53-74ms across two sittings, strings over a 9.5MB and a 373KB binary, timed in ONE python3 process with perf_counter around subprocess.run, harness floor 4.9ms, at loadavg 28 over 16 cores. A RANGE ACROSS SITTINGS RATHER THAN A POINT, because the point moved 15 percent in twenty minutes with nothing about the tool changing. AN EARLIER 82-88ms IS WITHDRAWN AS AN ARTEFACT OF ITS OWN HARNESS: it spawned two clock processes per iteration, so roughly 50ms was the instrument measuring itself. AND A FIGURE NAMING A REVISION AND A MACHINE BUT NO LOAD STATE IS STILL UNCOMPARABLE WHEN FOUR SESSIONS SHARE THE MACHINE ALL DAY (cc) -- a defect in every timing row here, and only naming the condition fixes it. SELF-TEST drives SEVEN controls, each shown able to fire alone, including an empty set that must report NOT EXAMINED rather than pass silently. REACH: FIELDS checks fields EXIST and are LABELLED, SET checks that record and artefacts name ONE commit between them, and NEITHER can check that an embedded marker is HONEST about the bytes carrying it -- that is the drift field and drift is held
...
declared_kind_check.sh     gated   new 2026-08-30 (vc), covering AC-00.15 via AT-00.16. GATED ON DAY ONE, precedent guard_home_check.sh and shared_artefact_build_guard.sh rather than provenance_fields_check.sh: those two went manual because their SUBJECT fails today, and this subject is GREEN on the live estate -- 334 cited rows, 59 absent, 5 unjudged, 269 agreeing, 0 disagreeing. WHOLE-SET AND NEVER PATH-TRIGGERED, for the machine_table_check.sh reason: the failure is a DISAGREEMENT between a row and an artefact, so a path trigger would have to fire on whichever of the two the commit does not carry. READS THE CANON EXTRACT ON DISK RATHER THAN THE STORE, and not for convenience: canon_commit_check.sh already gates the two agreeing, so either is sound, and the extract is the one a FRESH CLONE has -- a tool needing a machine-local .cache/intent.db would refuse in CI and in every clone, which is the population most likely to carry a divergence nobody has looked at. THE WORD COMMITTED WAS DOING TWO JOBS HERE AND IS REMOVED 2026-09-02 (vc): it named the ARTEFACT CLASS, extract against store, and four readers took it as COMMIT STATE. DRIVEN BY dc TWO-SIDED ON A SCRATCH REPO WHOSE COMMITTED AND WORKING-TREE EXTRACTS DISAGREE: the verdict tracks the WORKING TREE in both directions and the committed content moves nothing. That is deliberate and it stays -- a create only has to touch disk, so a committed-only reading would go blind to the class the tool exists for. DRIVEN NINE WAYS AGAINST A PREDICTION WRITTEN FIRST, all nine matching: five mutants each producing a DISTINCT reason (no test attribute in an integration test, no at-test in a bats file, a rostered not-an-instrument, an unrostered parity tool, and a source file cited as a colocated test module with no test in it), a NEGATIVE CONTROL of known-good rows at rc=0, an EMPTY citation set refusing at rc=2 rather than reporting a clean estate over nothing, and a PARTITION CONTROL applied to the tool source itself -- one increment deleted, hash verified CHANGED, rc=2 with the partition not closing, restored and hash verified IDENTICAL. THE NEGATIVE CONTROL IS THE ARM THAT MATTERS: without it an instrument firing on EVERY input passes all five mutants and reads as five successes, which is one method used five times. ITS FIRST FINDING WAS ITSELF -- the run before this row existed reported declared_kind_check.sh as a parity instrument the roster does not carry. COST 730-850ms, n=7, median 770, at loadavg 19.4 over 16 cores, over 334 cited rows -- cheaper than canon_commit_check.sh, view_skew_check.sh and self_provenance_check.sh. It was 930-1070ms until the per-thread jq loop collapsed to one invocation reading input_filename; THE PARTITION CLOSING AT THE SAME 334 IS WHAT SAYS THAT CHANGE WAS BEHAVIOUR-PRESERVING, rather than the timing looking better. REACH: it checks agreement AT THE DECLARED PATH and cannot see an ORPHANED artefact at a path no row cites -- the other end of the same defect, and AC-00.15 says in its own text that the case which MOTIVATED it is not the case this catches. It does not judge an absent citation (AC-01.7 population), a witness outside the parity roster (5 rows: run_v2_suite.bash and two hook templates), or whether a right-kind artefact tests the right thing.
absent_at_check.sh         gated   new 2026-08-30 (dc). THE MIRROR OF stale_at_check.sh: that one catches a to-write row whose cited test EXISTS (understatement); this catches a green or red row -- one asserting a verdict was reached -- whose cited file is ABSENT (overstatement). GATED ON DAY ONE, precedent declared_kind_check.sh: its subject is GREEN on the live estate, measured 386 AT rows across 69 threads, 336 cited, 276 green-or-red-with-a-citation, 51 absent citations and every one of them to-write, ZERO green and ZERO red. A gate that is green on arrival costs nobody a red commit. WHOLE-SET AND NEVER PATH-TRIGGERED, the machine_table_check.sh reason: the failure is a disagreement between a row and an artefact, so a path trigger would fire on whichever of the two the commit does not carry. READS THE COMMITTED CANON EXTRACT, adopting declared_kind_check.sh reasoning rather than re-arguing it -- the extract is what a fresh clone and CI have. AND NOT THE RENDERED acceptance.md, which is the reason this hole survived: 69 threads in canon, 13 with an acceptance.md, so a tree-globbing tool is blind to 56 threads. ST0043, which holds the row that motivated this guard, is one of the 56 and has no directory on disk at all -- stale_at_check.sh scans the tree and could never have seen it in either direction. IT ALSO READS THE TYPED file FIELD RATHER THAN SCRAPING THE ROW. The rendered citation heuristic is the first backticked span before the first double-dash, which on a non-test row picks up whatever the prose backticked: driven over the tree, it reported six n-a rows as citing absent files (INTENT_BIN, ws, wip, doctor --fix and two more) and all six were false. The extract has file null for every one. DRIVEN NINE WAYS ON A PLANTED FIXTURE OUTSIDE THE REPO, all nine matching a prediction written first: green-with-present at rc=0 as the NEGATIVE CONTROL, green-with-absent and red-with-absent at rc=1, to-write-with-absent and non-test-n-a at rc=0 as the two exclusions that must NOT fire, and four refusals at rc=2 -- no thread canon, zero AT rows, zero rows in population, and status renamed to state. THE FOUR REFUSALS ARE THE POINT: this population can legitimately be empty on a young thread, so zero-examined is genuinely ambiguous and the tool refuses rather than taking the reassuring reading. The count is on the ok line either way. COST 250-270ms, n=5, at loadavg 21.05 over 16 cores, over 386 rows. Measured beside declared_kind_check.sh IN THE SAME SITTING at 710-810ms rather than against its recorded figure, because this table already records that a point measurement was never available on a machine four sessions share. REACH, AND IT IS THE LIMIT THAT MATTERS: it covers the ABSENT artefact and NOT the present-and-hollow one -- which is the case that produced it. AT-01.8 cited a bats file that EXISTED while its test passed vacuously, grep on a deleted subject making the fail branch unreachable; 2 of that file 27 tests passed that way and 25 failed loudly. Nothing here would have fired. It catches the bookkeeping consequence of a delete, never the rot before it. COVERS NO AC YET -- an unattached instrument is a real state and recorded rather than papered over; ST0056 canon is vc pen.
partition_closes_check.sh  manual  new 2026-08-30 (vc), covering AC-00.12 via AT-00.13. MANUAL FOR TWO INDEPENDENT REASONS, EITHER SUFFICIENT ON ITS OWN. (1) ITS SUBJECT FAILS TODAY, so a gated row would deliver a permanently-red gate on every node commit -- precedent provenance_fields_check.sh, not guard_home_check.sh. (2) IT COSTS 17.1s BECAUSE IT RUNS THE GATED SET, which is more than twice the entire gate budget. RELEASE CONDITION, BOTH LIMBS REQUIRED: the instruments it names have stated their closures, AND it can read output the gate has ALREADY produced rather than re-running sixteen tools to read their stdout. THE WORK-LIST AS AT 2026-08-30, named because a release condition with no members is not actionable: machine_table_check.sh (GUARD AXIS, 47 agree and 0 disagree and 0 UNMEASURED of 47 rows), rulings_check.sh (41 declared against 39 conform and 1 hv-without-record and 1 PROVISIONAL and 0 dangling and 0 non-conforming), and runner_roster_check.sh, WHICH WAS MINE AND IS FIXED IN THE SAME COMMIT. WHY THE STATEMENT IS THE REQUIREMENT RATHER THAN CORRECTNESS: a partition that is both UNSTATED and BROKEN is invisible to this tool and to every other, because nothing separates three unrelated integers from a failed partition without knowing which were meant to be parts, and only the author knows that. The stated sum is the single thing that makes a partition checkable by anyone but its author. DRIVEN SEVEN WAYS AGAINST A PREDICTION WRITTEN FIRST, all seven matching: a wrong stated closure reds arm A, an unstated closing partition reds arm B, a CORRECT stated closure and a capture with no integers both return rc=0 on different limbs of the partition, an empty capture set refuses at rc=2, and TWO COINCIDENCE CONTROLS THAT WERE LIVE FALSE POSITIVES IN THE FIRST DRIVE AND MUST STAY DEAD: a machine NUMBER matching two counts (4 equals 2 plus 2), and a filesystem path whose digits happen to sum. THE FIRST DRIVE REFUTED THE PREDICTION THREE WAYS AND THAT WAS ITS VALUE: arm A predicted 0 and returned 1 which was THE PARSER (it summed every integer left of the equals, so 510 recorded across 60 plus 9 equals 69 came out 579); arm B predicted 3 and returned a DIFFERENT 3, missing corrected_check.sh whose partition spans two lines, finding rulings_check.sh which nobody predicted, and matching machine_table_check.sh on the WRONG LINE with the real one hidden behind a head -1. REACH: 16 of 64 instruments (the gated set is the only one with a safe bare invocation), PER LINE so a cross-line partition is missed, over OUTPUT rather than source, and at least THREE parts because two is indistinguishable from coincidence.
vocab_adequacy_check.sh    manual  new 2026-08-30 (vc), covering AC-00.13 via AT-00.14. MANUAL BECAUSE ITS SUBJECT FAILS TODAY -- one live finding -- precedent provenance_fields_check.sh and partition_closes_check.sh, not guard_home_check.sh. COST 0.12s, n=5, no spread, one jq pass over surface/dispatch-table.json; cheap enough to gate the day the finding closes, so cost is not why it is manual. RELEASE CONDITION: the authored vocabulary on INV-03 can express vc-ruled-with-hv-outstanding, and the row stops declaring ratified. THE FIX IS NOT TO RE-ANCHOR THE ENUM TO THE PROSE -- AC-00.13 states that the remedy for a DANGLING value is the wrong remedy for an INEXPRESSIBLE one, so making the declaration match the prose would certify the unfinished state as finished and commit the defect inside its own remedy. DRIVEN AGAINST A PREDICTION WRITTEN FIRST AND THE PREDICTION REFUTED IT TWICE, WHICH IS THE WHOLE VALUE. (1) The count and the findings both left on stdout, so the command substitution captured the findings INTO the count and the contradicting fixture scored FINDING then 1 against a wanted 1 -- an arm reporting a miss it had not made. (2) Predicted 1 live and returned 8: seven rows carry the sentence hv ratification is therefore NOT outstanding, and a window wide enough to bind subject to hedge spans the NOT that reverses it, so the instrument read seven rows SAYING IT IS FINE as seven problems -- one boilerplate sentence on seven rows, well-formed and internally consistent at 8. TWO EARLIER PATTERNS ALSO SCORED AND BOTH FAILED IN OPPOSITE DIRECTIONS: a bare outstanding-pending-awaiting over the whole field returned 10 of which ONE was real (seven boilerplate, two HISTORY narrating a state the row has LEFT), and a fixed 70-character leading context then DROPPED the true positive, whose hedge sits 50 characters in. A loose pattern banked nine falsehoods and the tight one banked a clean sweep. FIXTURES DRIVE THREE VERDICTS: agreeing 0, contradicting 1, history 0 -- the history arm exists because without it the two rows narrating pending-hv as now answered come back. REACH, AND IT IS THE POINT RATHER THAN A CAVEAT: AC-00.13 states adequacy is NOT checkable, so this finds ONE SIGNATURE of it -- a declared machine-read state contradicted by the authored prose beside it, which is the estate admitting the vocabulary could not hold the truth. A vocabulary inadequate with NO prose beside it is invisible BY CONSTRUCTION and is the commoner case; so is inadequacy the author never noticed, since the signature IS the author noticing. The hedge vocabulary is OPEN and undercounts, and its false negatives are the rows whose authors phrased it in their own words -- the self_provenance_check.sh finding, inherited rather than escaped.
docs_defect_disposition_check.sh manual  new 2026-08-31 (dc), covering AC-02.3 via AT-02.3. MANUAL BECAUSE ITS POPULATION MOVES WHENEVER ANY NODE FILES AN ISSUE, and it went 103 to 105 to 135 to 136 to 137 in a single session, twice while this row was being written. A gated row would make one node filing an issue refuse every other node commit until somebody dispositioned it -- exactly the 0125 shape, a check whose verdict depends on work nobody committed. IT REFUSES TO REPORT AT ALL IF ITS OWN PATTERN IS BLIND: the non-fix pattern must score 2 of 2 on planted prose before a single exclusion is trusted, because every exclusion below it would otherwise be unearned and a zero from an untested pattern is indistinguishable from a clean corpus. THE POPULATION RULE CARRIES NO JUDGEMENT: a member is excluded only when it was closed in the register AT THE CUT and its body carries no non-fix language, so the narrowing that would otherwise be invisible happens in a DISPOSITION where it is on the record. TWO-SIDED CONTROL FROM THE CRITERION ITSELF: AC-02.3 names five members in its own prose, and they refute both naive rules -- open-at-the-cut-revision drops 4 of 5, open-now drops 0086 whose fix landed four days after the tag. The check refuses if any of the five falls out. NEGATIVE-CONTROLLED ON FOUR PLANTED MANIFEST DEFECTS: an absent quote, an absent page, an empty quote and an empty reason, each caught. THE FIRST ATTEMPT AT THAT CONTROL WAS BLIND and emitted nothing, because it planted a false row for an id that already had one and the reader takes the first match -- a control emitting nothing is indistinguishable from one that passed. A stated claim is verified against the PAGE BYTES, never against a promise, and the docs are the published pages PLUS the binary --help because both are reached by following the docs correctly. REACH: it cannot tell whether a stated defect is stated WELL, only that the quoted bytes are present; and a member whose register record carries no body cannot be dispositioned from its own record at all -- five such members exist and are deliberately left undispositioned rather than judged from a title.
v2_estate_fixture.sh       manual  new 2026-08-31 (dc), serving AC-02.3 via AT-02.3. IT IS A FIXTURE BUILDER, NOT A GATE: it materialises a v2 estate in a mktemp dir and drives the v3.0.0 keg against it, so there is no verdict for a commit to fail on. CAPTURED, NEVER WRITTEN, ON THIS DIRECTORY OWN README GROUND: a hand-authored seed is one degree closer to the thing under test than a fixture may be, and the ingest defects are about what the ingest does to AUTHORED PROSE -- prose written from the issue titles would be shaped by the defect it is meant to detect. The seed is f7434f1c, a real 2.19.0 estate of 194 threads and 14 acceptance files, authored before any of these issues were filed. THE CAPTURE COMMIT MUST DECLARE 2.19.0: an earlier attempt used a 2.18.0 capture and the keg refused it as BELOW THE MIGRATION FLOOR, naming install intent@2 first -- a real refusal, correctly reported, and not the defect under test. IT DRIVES THE KEG AND NEVER THE DEV BINARY, and never in this tree: this repository store is schema 16, the keg speaks 13, and it refuses this tree before reaching any behaviour. EVERY CLAIM IT PRINTS NAMES ITS TREE, because I ran the keg does not say which store it met -- vc caught a claim of mine that was unfalsifiable for exactly that omission. TWO OF ITS PROBES DECLINE TO REPORT A NUMBER AND SAY WHY: the double-dash sequence occurs NATIVELY in v2 row syntax, so counting it in canon cannot show the ingest inserted it; and 0071 concerns the V2 binary own upgrade, which is not installed here. ITS FIRST REAL FINDING NARROWED A REGISTER ROW RATHER THAN CONFIRMING IT: 0080 records every migrated thread carrying no slug and the whole column blank; driven, 21 of 56 lack one and the column renders normally.
v2_prose_survival.sh       manual  new 2026-08-31 (dc), serving AC-02.3 via AT-02.3. MANUAL because it materialises a v2 estate and migrates it with the v3.0.0 keg, which is a build-a-tree operation rather than a verdict a commit can fail on. IT COMPARES EVERY v2 AT ROW AGAINST ITS CANON ROW BYTE FOR BYTE: 176 rows across 14 acceptance files in a keg-migrated capture of f7434f1c, 176 of 176 with prose intact. IT IS NEGATIVE-CONTROLLED FIRST: a one-character change must register as DIFFERS before any identical is believed, because a comparison that cannot detect damage reports a clean estate over anything. ITS FIRST RUN REPORTED 159 OF 176 DAMAGED AND EVERY ONE WAS FORMATTING: v2 wraps a file path in backticks and canon stores the bare path, so stripping them is correct normalisation. ITS SECOND RUN REPORTED 3 DAMAGED AND ALL THREE WERE MY OWN OVER-STRIPPING -- canon PRESERVES backticks inside prose while removing them from a file field, so canon is more faithful than the normalisation I applied to it. BOTH ERRORS RAN THE SAME WAY: the instrument reported the DIFFERENCE BETWEEN TWO SPELLINGS as a difference in content, and only reading the actual pairs distinguished them. A count of differences is not a count of damage. THE CAVEAT IS ON EVERY ROW IT SERVES AND MUST STAY THERE: a clean negative over a corpus whose ability to exhibit the failure is unproven is the honest-and-blind-grep family. This corpus is real v2 output and was NOT constructed to carry prose between two recognised fields or a spliced row, so it says no damage was found and never that none is possible.
...
instrument_reach_census.sh manual  new 2026-08-30 (vc), covering AC-00.16 via AT-00.20. MANUAL BECAUSE IT EXITS 1 BY DESIGN: the row is RED and the instrument says so in its exit code, so a gated row would block every commit in the estate on a criterion nobody has satisfied yet. It measures ONE PRECONDITION -- that an instrument declares what it does NOT cover -- and reports HOW MANY OF THE RUST INTEGRATION-TEST FILES DO SO. THE FIGURES ARE NOT REPEATED HERE AND THAT IS THE POINT: this row is PROSE ABOUT an instrument that DERIVES them, so a copy in the prose is a second home with no witness, drifting in silence the first time the corpus grows. Run the tool for the numbers. Declaring reach is what makes the union computable and is NOT the union, which is what AC-00.16 asks for. RELEASE CONDITION: promote to gated when something owns the union and this can assert a property rather than report a census. ITS OWN REACH IS PRINTED IN ITS OUTPUT RATHER THAN DESCRIBED HERE, because it is subject to the class it measures: it reads native/rust/crates/*/tests/*.rs and NOT the BATS suite, the shell instruments beside it, lib/templates/hooks, the critic rule library, or colocated cfg(test) modules -- and the vc instance of this class was a shipped SHELL surface sitting outside the reach of a Rust scanner. An instrument measuring declared reach while hiding its own would be the fourth instance. PARTITION CLOSES IN THE OUTPUT and the script refuses with exit 2 if it does not, per AC-00.11 and AC-00.12.
instrument_currency_check.sh manual  new 2026-08-31 (vc), covering AC-00.14 via AT-00.15. MANUAL BECAUSE IT EXITS 1 BY DESIGN, the same ground as instrument_reach_census.sh beside it: the row is RED and the instrument says so in its exit code, so gating it would block every commit in the estate on a criterion nobody has satisfied. IT ASKS WHETHER THE ARTEFACT AN INSTRUMENT DRIVES CAN SEE THE CHANGE THAT INSTRUMENT IS ABOUT TO REPORT ON, BY CONTENT. It does NOT read intent-source-commit, deliberately: AC-00.14 rules that currency is a CONTENT test and never a chronological one, a dirty build carries code YOUNGER than any commit stamp so build time fails in both directions, and AT-11.5 has already measured two binaries carrying ONE marker where one wiped the estate and one worked. Provenance is the question self_provenance_check.sh asks and it is a DIFFERENT question -- this is not a copy of it. THE FIGURES ARE NOT REPEATED HERE, per the row above: this is PROSE ABOUT an instrument that DERIVES them, and a copy in the prose is a second home with no witness. Run the tool. THE PROBE IS DERIVED, NOT DECLARED, because a hardcoded probe rots exactly like its subjects -- it is the newest source commit yielding a shipping literal, and the run prints how far behind HEAD that is. THERE IS DELIBERATELY NO DESIGNATED REFERENCE BINARY: requiring the probe to match a known-current artefact is CIRCULAR and fails in the one direction that matters, since a stale reference admits only probes it already carries and so certifies staleness as currency. The control is two-sided instead -- the probe must be shown able to match SOMETHING -- and the tool exits 2 rather than reporting a clean sweep when it matches nothing, because from inside it every-artefact-is-stale and this-literal-never-ships are ONE observation. ITS FIRST DERIVATION COMMITTED THE CLASS THIS ROW IS ABOUT AND IS RECORDED RATHER THAN QUIETLY FIXED: it drew literals from colocated cfg(test) modules, which ship in no binary ever built, so it reported every probe absent and every artefact stale -- a confident, well-formed, internally consistent verdict about a world that never existed, produced by the instrument built to catch that. Caught by DRIVING the derivation before trusting it and by nothing else, which is the conclusion in output-contracts.md holding again: the protection was the instrument, and care was not in short supply. REACH IS PRINTED IN ITS OUTPUT RATHER THAN DESCRIBED HERE. It covers the compiled Intent binaries against ONE derived probe; it does NOT cover the non-binary artefacts an instrument drives -- a generated table, a canon extract, a fixture tree, a vendored manifest -- each of which can be exactly as stale and none of which strings can be pointed at; and it says nothing about whether a verdict is CORRECT, which is a different question from whether it is CURRENT. WEAKER THAN IT LOOKS, and stated in the output: the probe shows an artefact can see ONE commit, not the commit that each instrument makes its own claim about. THIS ROW IS APOSTROPHE-FREE FOR THE REASON THE inert_arg_check.sh ROW STATES, AND I BROKE THE PARSE THE SAME WAY ON MY FIRST ATTEMPT -- the warning was one row above the insertion point and was read only after the failure. RELEASE CONDITION: promote to gated when the report of an instrument is CONDITIONED on a currency demonstration rather than accompanied by one -- ie when a binary-driving instrument REFUSES to report against an artefact that cannot see its subject. Not when the census count rises: adoption is not requirement.
fleet_corpus_conservation.sh manual  new 2026-08-31 (vc), covering AC-10.5 via AT-10.5, shared with AT-00.2. MANUAL BECAUSE IT EXITS 1 BY DESIGN while three of four declared corpus members are unrun, the same ground as instrument_reach_census.sh and instrument_currency_check.sh beside it. IT DOES NOT MEASURE CONSERVATION AND IS NOT A SECOND HOME FOR IT -- conservation_check.sh is the one home and this JOINS its per-member results. It asks the question that tool cannot: has every member AC-10.5 NAMES been run, and did each conserve or name its residue. A per-member verdict is not a fleet verdict and nothing else in the estate joins them. THE MEMBER LIST IS DECLARED BY THE CRITERION, NEVER DERIVED FROM WHAT IS ON DISK: deriving it from present directories would make the denominator a function of the operator machine, which is the failure the file exists to refuse. A member with no recorded verdict is UNRUN, never passing -- absence of a finding is not a finding of absence, and a fleet criterion scored over the one member somebody happened to run is AC-00.11 wrong-M with estates as the population. WHY IT EXISTS IS A REPAIR OF ITS AUTHOR ERROR AND IS RECORDED RATHER THAN TIDIED: AT-10.5 cited a Rust path that has never existed and sat at to-write, where an absent citation is lint-exempt. vc moved the row to red to correct a note whose load-bearing sentence had become false, and red REQUIRES the cited artefact to exist -- so the move turned a lint-exempt row into a live absent_at_check finding that refused EVERY node commit for about fifteen minutes. THERE IS NO VERB THAT SETS A ROW BACK TO to-write: at ships green, red and na, Machine 5 declares the transition and the CLI exposes no spelling, and at new --status refuses a taken id by design. So the move was irreversible through the tool, which is issue 0033 class arriving operationally rather than documentarily. IT IS A SHELL INSTRUMENT ON PURPOSE: every red row in this estate cites a shell tool, because a red Rust test breaks cargo test for everyone and a red shell tool does not. Re-citing to an existing instrument was refused correctly, since the contract requires the cited file to carry the literal row id and conservation_check.sh carries the id of the row that owns it. REACH IS PRINTED IN ITS OUTPUT. It does NOT verify that a recorded verdict was honestly produced -- it reads a file a human or a driver wrote, which is a real hole and a smaller one than it closes, since today the fleet figure is produced by nobody at all. RELEASE CONDITION: promote to gated when all four members have recorded verdicts, so a green means the fleet was measured rather than that nobody looked.
...
suite_vs_table_flags_check.sh  manual  new 2026-09-05 (dc), the suite-versus-table flag seam. MANUAL BECAUSE DIRECTION B IS RED BY DESIGN TODAY and will stay red for the length of the ladder: 47 declared flags have no success-asserting block, which is what a v3 surface ahead of a v2 estate looks like, so a gated row would refuse every node commit for the SHAPE of the work rather than for a defect. Precedent surface_check.sh and implemented_check.sh, both report-only for the same reason, and NOT cost -- this one is static, reads two files and finishes well under a second. IT EXISTS BECAUSE BOTH SIDES WERE ALREADY EXTRACTED CORRECTLY BY TOOLS THAT CANNOT SEE EACH OTHER: extract_flags.sh feeds a generator, the table jq feeds two more, coverage_map.sh joins at FAMILY granularity and surface_check.sh compares the BINARY to the table, so nothing formed a verdict about the pair. 12 CONTROLS, EVERY ONE DRIVING THE SAME EXTRACTOR FUNCTIONS THE VERDICT USES, and the run refuses at rc=2 if any fails to fire. THREE OF THEM CAUGHT REAL DEFECTS BEFORE IT SHIPPED: a root resolved one directory short, which the depth-faithful fixture refused rather than reading the wrong tree; a has() helper whose grep read a flag-shaped pattern as an option, under which three absence controls PASSED FOR THE WRONG REASON while grep was erroring; and a W74a fixture that did not discriminate, whose prose flag the naive grep could not match either, so the control was decoration until the fixture was rebuilt. A TWO-SIDED MEMBERSHIP CONTROL RUNS ON LIVE DATA every invocation: an empty intersection is refused as an extractor mismatch rather than reported as total disagreement. REACH IS PRINTED BEFORE THE VERDICT ON EVERY RUN -- declared population, the complement outside it, the bare-word exclusion by name, and the 30 invocations across 15 files that sit outside any @test block and therefore carry no polarity. MIXED and NOSTATUS blocks are counted and NEVER resolved, on vc ruling. THE ADJACENT AXIS IS NAMED IN THE HEADER AND NOT BUILT: joining a row declared flags to its own declared v2 twin is a second witness over a different population and is a separate instrument that does not exist
...
gen_explorer_shots.sh          not-an-instrument generator; emits the screenshots under docs/images/explorer/ by driving the explorer in a pty on seeded demo projects. It shows what the explorer draws and never judges the estate
...
lib_currency.sh                not-an-instrument sourced, not executed; ships 644 and defines functions only. THE ONE HOME for the currency probe -- can a given artefact see a recent change, by CONTENT. Moved out of instrument_currency_check.sh rather than copied, because that file is a whole-estate CENSUS and a consumer wanting to ask about the ONE artefact it is about to drive would have had to enumerate every binary in the tree. 27 instruments here drive a binary and ONE demonstrated it could see a change; 26 each rolling their own demonstration would be 26 homes for one concern, which is IN-AG-HIGHLANDER-001 arriving through diligence rather than carelessness. THREE STATES AND THE THIRD IS NOT A REFINEMENT: CURRENT carries a probe literal, BLIND links the probe source and carries none, UNREACHABLE does not link the probe source at all. UNREACHABLE exists because intentd does not depend on intent-cli, so a probe derived from a literal in intent-cli cannot be carried by intentd however current it is -- reporting that as BLIND is a confident verdict about a question the probe cannot ask, which is AC-00.14 committed by the tool measuring AC-00.14. The discriminator is DERIVED rather than declared: rustc embeds each source path for panic locations, so an artefact links crates/X/src/f.rs iff that file is in it. currency_require returns 0 on UNREACHABLE DELIBERATELY -- it is not a currency finding and turning cannot-tell into a refusal would block instruments over a question nobody asked -- and rc=2 when the probe cannot be derived, because a silent pass there is the defect itself. THE CENSUS REFACTOR IS CONTROLLED, NOT ASSERTED: pre and post scripts run BACK TO BACK against the same HEAD gave byte-identical output at the same rc. FIRST CONSUMER canon_mandated_verbs_check.sh, chosen because its exposure is demonstrated: it reports a verb UNWIRED when the binary answers the not-implemented phrase, and a verb wired AFTER the build answers exactly that way, so a stale artefact does not make it miss a defect, it makes it INVENT one. Driven to both verdicts -- current binary passes through at rc=0, planted stale binary refuses at rc=2 with ZERO finding lines emitted. NO APOSTROPHES IN THIS ROW BECAUSE ROSTER IS A SINGLE-QUOTED SHELL STRING
...
pipefail_sigpipe_check.sh  gated   new 2026-09-02 (dc) on an hv ruling; it cites no AT and covers no criterion -- precedent canon_race_check.sh -- because its subject is a DEFECT CLASS rather than a criterion. STATIC READ of 86 committed instruments across three declared directories: no binary, no clock, no git, one grep per line. SUBJECT: a pipeline whose final stage is grep carrying -q, which exits on first match and leaves its writer killed by SIGPIPE at 141, which pipefail then promotes to the pipeline status -- so the test reads FALSE on an input that MATCHED. In a guard that is a finding LOST, and where the test is negated it is a finding INVENTED, the same defect wearing the opposite sign. IT IS INVISIBLE BY CONSTRUCTION: a lost race has the same exit code, the same silence and the same green as a clean run, so nothing downstream of the symptom can tell them apart -- which is the argument for a guard rather than a review note. THE SWEEP THAT PRECEDED IT WAS SCOPED TO THE WRONG POPULATION AND THIS TOOL IS HOW WE KNOW: the August report and the ruled sweep both named the printf spelling, so 36 sites were fixed and 13 more were invisible -- reached by git show, by jq, by echo, by a bare intent invocation and by a function call. A DEFECT NAMED AFTER THE SPELLING OF ONE INSTANCE GETS A FIX SHAPED LIKE THAT SPELLING, and the census inherits the blindness rather than reporting it. TWO-SIDED CONTROL ON EVERY RUN rather than once at authoring: three planted defects must be found and a planted fix must be silent, and the tool REFUSES to report on the real population when either arm fails to fire, because an instrument that has only ever produced one of its two answers has not been shown to measure anything. THE CONTROL ALSO PLANTS BOTH FALSE-POSITIVE FAMILIES THIS TOOL HAS ALREADY SHIPPED: a comment describing the idiom, and a double pipe -- which is not a pipeline at all and which the first version read as one, reporting a grep against a FILE and, worse, reporting the REMEDY as the defect, since the fixed herestring form sits behind exactly that operator. A guard whose findings include the remedy it recommends teaches the reader to stop believing it. IT IS A MEMBER OF ITS OWN POPULATION: the detection pattern and the planted fixtures are assembled from a variable so the file does not contain the sequence it looks for, because a checker that must be excluded from its own check carries an untested assumption at the point it is least visible. THE REMEDY IS A HERESTRING, and for a MULTI-STAGE pipeline that is not enough on its own -- the early-exiting reader is still downstream of another writer, so the upstream stage moves into a command substitution, which reads to EOF and cannot be SIGPIPEd; four of the 36 were that shape and the obvious remedy applied uniformly would have left all four still losing. WHERE THE FINAL STAGE IS grep CARRYING -qv THE NAIVE REMEDY CHANGES THE ANSWER: an empty capture becomes ONE EMPTY LINE under a herestring, which -v matches, so emptiness is tested explicitly there. REACH: it reads LINES, so a pipeline broken across a continuation is invisible to it; it does not know whether pipefail is in force AT THE SITE and gates regardless, printing the file-level setting as context, because a file one line away from the defect is not a safe file; and grep carrying -q is the only early-exiting reader it knows -- head, grep carrying -m and an exiting awk have the same shape and are deliberately NOT its subject, since widening a fix quietly is its own defect class and is the thing dc nearly did while sweeping this one. THE ROW IS APOSTROPHE-FREE BECAUSE ROSTER IS A SINGLE-QUOTED SHELL STRING.
contract_check.sh          manual  new 2026-09-12 (ic) under the pen, covering ST0069 AC-16.1 to AC-16.4. MANUAL, AND THE REASON IT GAVE HAS EXPIRED (vc, 2026-09-15): the row said the tool was red by construction until the whiteboard faces landed, and it exits 0 as of 2026-09-15, with board.schema.json among the faces it checks and no map row left pending. The release preflight runs it and prints its three outcomes as three distinct lines -- 0 clean, 1 findings that want an author, anything else no verdict -- and it blocks nothing there or here: making it a gate is a call for hv, and nobody has asked for one. It compares a PROSE DOCUMENT to a SCHEMA and is a SIBLING of drift_check.sh, never an arm of it -- that one compares generated faces to the Rust types, and folding them would put two questions behind one exit code with opposite remedies, a regeneration against an author. Its population is DERIVED at run time from the three published faces and from the documents own field tables; the only list here is the join, which is declared because the two sides share no naming convention (PascalCase JSON against snake_case DB) and a name-match reports Envelope homeless while its table sits at event_log with every row present -- the false finding this author filed before writing the tool. DRIVEN RED IN BOTH DIRECTIONS on planted defects the day it was written, plus the AC-16.3 unparseable-table refusal, with the recipe in its own header and a control showing the planted line absent unplanted; the exit code is not the discriminator while any pending row stands, so an arm is judged by the finding line it adds.
reference_current_check.sh manual  new 2026-09-18 (ic) on hv ruling (vc decision 38, Q5); it cites no AT and covers no criterion. MANUAL BECAUSE ITS CALLER IS THE RELEASE PREFLIGHT, NOT THE COMMIT GATE: docs/reference legitimately lags HEAD between cuts -- every landing that moves the register makes it stale until the cut regenerates it -- so a gated row would refuse ordinary commits for a state that is only wrong at a tag. bin/.devbin/cmd/build.d/release runs it before the version-bump commit and refuses the cut on exit 1 unless --allow-stale-reference is passed, which keeps the diff. Exits 0 current, 1 stale, 2 no verdict (a generator refused or either side came back empty). Driven 2026-09-18 in both directions at efb44044b and through the release dry run in a scratch clone: stale refuses, override warns, regenerated-and-committed passes.
'
...
in_flight() {
  ( unset GIT_INDEX_FILE
    grep -qxF -- "$1" <<<"$(git -C "$ROOT" ls-files -- 'intent/st/*/parity/tools/*.sh' 2>/dev/null \
      | sed 's|.*/||')" )
}
...
total="$(printf '%s\n' "$PRESENT" | grep -c .)"
disp_sum=$((gated_n + manual_n + notinstr_n))

if [ -n "$findings" ]; then
  printf 'roster: %s parity file(s) in this commit; %s gated + %s manual + %s not-an-instrument = %s; the roster and the runner DISAGREE\n' \
    "$total" "$gated_n" "$manual_n" "$notinstr_n" "$disp_sum"
  if [ "$disp_sum" -ne "$total" ]; then
    printf '  AND THE PARTITION DOES NOT CLOSE: %s dispositions against %s file(s), remainder %s. That remainder IS the finding below, stated rather than left to be added up.\n' \
      "$disp_sum" "$total" "$((total - disp_sum))"
  fi
  printf '%s' "$findings"
...
# **THE PARTITION IS STATED AND ASSERTED, NOT LEFT TO THE READER** (AC-00.12,
# found by partition_closes_check.sh on this tool 2026-08-30). The three
# dispositions used to be printed beside the total with nothing saying they
# summed to it -- so a file rostered under a fourth spelling, or counted twice,
# would have shifted one number and closed arithmetically to any reader who did
# not add up. A +1 remainder is as silent as a -1 is loud.
printf 'roster: %s parity file(s) in this commit, all rostered; %s gated + %s manual + %s not-an-instrument = %s; every disposition matches the runner\n' \
  "$total" "$gated_n" "$manual_n" "$notinstr_n" "$disp_sum"
if [ "$disp_sum" -ne "$total" ]; then
  printf 'roster: THE PARTITION DOES NOT CLOSE: %s + %s + %s = %s against %s file(s).\n' \
    "$gated_n" "$manual_n" "$notinstr_n" "$disp_sum" "$total" >&2
  exit 2
fi
exit 0
