...
read -r -d '' JQ_PAGE <<'JQEOF' || true
  ...
  # A RETIRED ROW WHOSE PATH ALSO HAS A LIVE ROW IS NOT RETIRED. `help` and
  # `organize` each carry v2's row, dispositioned `retire`, and v3's own
  # `new-surface` row, so the page listed them under "These do not exist in v3"
  # beside their own reference sections. The live row answers for the path.
  | ($live | map(.path)) as $live_paths
  | ($rows | map(select(.disposition == "retire" and (.path as $p | $live_paths | index($p) | not)))) as $dead
