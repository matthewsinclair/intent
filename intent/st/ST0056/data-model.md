...
| state | tagged | `{is: computed}` · `{is: unsatisfied}` · `{is: satisfied, evidence}` · `{is: descoped, to: STxxxx, by?, reason?}` · `{is: withdrawn, reason, by?}` -- **REQUIRED on every criterion.** `computed` is the in-scope value for a `test` criterion; `satisfied`/`unsatisfied` are refused on one. FIVE values against the ratified four -- see "The fifth state" |
