...
| state | tagged | `{state: computed}` · `{state: unsatisfied}` · `{state: satisfied, evidence}` · `{state: descoped, to: STxxxx, by?, reason?}` · `{state: withdrawn, reason, by?}` -- **REQUIRED on every criterion.** `computed` is the in-scope value for a `test` criterion; `satisfied`/`unsatisfied` are refused on one. FIVE values against the ratified four -- see "The fifth state" |
