...
**THE ENTRY ROW IS DELIBERATELY ABSENT, AND THE ABSENCE IS THE BUILD'S CURRENT STATE RATHER THAN AN OMISSION.** No verb reaches `Fiat`: `ac fc` is unbuilt (measured 2026-08-29 -- zero `ac.fc` tokens anywhere under `native/rust` or `surface/`), and `transitions.rs` declares `fiat` an ORPHAN in its own `orphans:` list for exactly that reason. The exits landed first on purpose, because **an entry edge alone would make `fiat` a trap state** -- the one shape the completeness guard exists to refuse. The entry row lands here in the same change that builds the verb.

### The two lists are a second witness to each other, and only ONE of them is watched

**`transitions.rs` carries the three tables above transcribed into code, and hv's fiat ruling reached that copy in the commit implementing it (`b7a3e771`) while this table sat unchanged.** Nothing detected the gap; dc found it by eye and flagged it in prose. **Measured 2026-08-29: every reference to this document in the entire source tree is a PROSE CITATION inside a comment** -- four of them saying `transcribed` -- and no instrument parses these tables. `mutation_completeness.rs` and its siblings read the Rust. **So the pair is one-way: the copy is machine-guarded and the authority is not, and drift can therefore only run in the direction where the ratified document is the half that goes wrong and nothing can see it.**

**AND THE TWO HALVES DISAGREE ABOUT WHICH IS THE AUTHORITY, which is why the drift had no owner.** This page's own status line says _this document describes, it does not define -- the authored master is the Rust type layer_; `transitions.rs` says _the ratified AC machine (data-model.md "Machine 3"), transcribed_. Each names the other as master. The status line was written about the **schema face**, where it is correct and WP-02 made it so, but nothing on the page limits it to that -- **so a reader landing here reasonably concludes the tables are descriptive and safe to leave behind the code, which is the opposite of what the code believes.** For the MACHINES, this page is the authority; the note above is where that is now written down.

**A DETECTOR IS BUILDABLE AND THE COUNTING RULE IS THE TRAP IN IT.** The two notations do not count the same things: this table writes one row per `(from, to)` pair, while `transitions.rs` writes one `Edge::` per verb-and-landing with a LIST of from-states, so `ac.descope` is three rows here and one edge there. The AC machine is 12 transition rows plus 2 entry rows here, and 10 `Edge::` declarations expanding to 14 `(from, to)` pairs there. **Both counts are correct and they never match, so any comparison that counts before it expands will report a permanent false divergence** -- rule 6's shape, a check that reds forever is one the operator learns to ignore. Expand to pairs first, then compare sets.
