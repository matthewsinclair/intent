...
| P2a  | RECOMMENDATION | Partial: shipped for `tests/run_tests.sh`; rolled back for 6 production scripts (broke 6 bats tests due to non-zero-on-absent checks). Each rolled-back script carries an explanatory comment and a follow-on note. | IN-SH-CODE-003 missing `set -e` in 7 files (`intent_doctor`, `intent_minimal`, `intent_agents`, three `intent_claude_*`, `tests/run_tests.sh`); `intent_config` separate (sourced library -- comment, not flag).            |
| P2b  | RECOMMENDATION | Shipped: canonical `warning()` + new `info()` (QUIET-aware) in `intent_helpers`; pure-shadow duplicates stripped; `show_error`/`show_warning` in `intent_doctor` retained (counter-bumping is a distinct concern).  | IN-SH-CODE-006 `intent_doctor:107,112,124`; `intent_bootstrap:59`; `intent_config:176`; `tests/run_tests.sh:13-31`. Net Highlander pass.                                                                                    |
...
**P0/P1** shipped in commit `a9ee349`. All seven CRITICAL plus three of the WARNING tier resolved.

**P2b** shipped in the follow-up commit (Highlander pass): `warning()` lifted from `intent_config` to `intent_helpers`; new canonical `info()` (QUIET-aware) added. Pure-shadow duplicates stripped from `intent_bootstrap`, `intent_doctor` (the `info()` only -- `show_error`/`show_warning` retained because they bump `ERRORS`/`WARNINGS` counters, a legitimately distinct concern), `intent_config`, and `tests/run_tests.sh` (which now sources `intent_helpers` and gives up its ANSI-coloured locals). Defensive: `((var++))` postfix incrementers converted to `((++var))` prefix in `intent_doctor`, `intent_agents`, `intent_claude_subagents` so they survive future strict-mode adoption.

**P2a partial.** `set -e` was added to all 6 dispatcher/library files plus `tests/run_tests.sh`, then rolled back from the 6 production scripts after the bats suite surfaced regressions. Several scripts use the pattern `[ -d "$path" ] || error "not found"` -- under strict mode the `[ -d ... ]` returning 1 aborts the script before the error branch fires. Six tests went red (`agent_commands.bats:14`, `:48`; `skills_commands.bats:474`, `:488`, `:501`; `test_diogenes.bats:614`). Each script now carries an explicit comment recording the deferral and the reason. `tests/run_tests.sh` keeps `set -e` because its single non-zero-tolerant call (the `bats` invocation) was rewritten as `bats ... || EXIT_STATUS=$?`.

Bats remained 707/707 green at every commit boundary.

### What was queued

**P2a follow-on.** Real `set -e` adoption needs a per-script sweep: every `[ -d "$x" ]` / `find ...` / `command -v ...` early check that signals "absent" via non-zero exit must be wrapped in `if !` or guarded with `|| true` (and the rule's Detection should explicitly recommend the `if !` pattern). Estimated XS-S per script -- tractable but not single-commit. Open a dedicated ST when this surfaces, or fold into the next critic-shell pass.
