...
Recommended sequencing: WP-11 in 3.0.1; this WP with ST0061 (dehydrate) and the preconditions-in-the-tool in the release after -- the ingest-then-prune is the part that must not be rushed into the same cut as the packaging fix. **AND THE AT ACCOUNTING RUNS BEFORE ANY PRUNE SHIPS** (devbin/vc): `at-accounting.sh` compares the v2 bucket's `acceptance.md` against the store and the view, so the bucket is the only source surface it has; prune it and the evidence for hop 2's AT-tranche loss (arca_cli 26 of 55, Utilz 13 of 42, Laksa 9, Cdsync 1, Lamplight 25) is gone with it. The migrator's reach is a filename allowlist -- `info.md` and `acceptance.md` -- and every other filename in a thread directory is left behind entirely; 34 of Devbin's 54 bucket files carry content the store does not hold, ten of them present in both trees with content lost, so the ingest half's acceptance is a per-file CONTENT probe, never presence.
## `unclaimed` is the same population, measured (cc, 2026-08-26)

`organize` previews Devbin at **54 unclaimed** -- the home of the 34 bucket files whose content the store does not hold; same population, seen from the tool's side. Laksa previews **1051**. `Action::Unclaimed` is "report, never remove"; `is_destructive()` is true for exactly one action, `Dehydrate` -- which is what every closed thread's realised files BECOME the moment `--default` writes a file declaring only the open set. The block holds only if the store can put them back, which is why the ingest precedes the prune and the prune runs last: a measured argument, not a sequencing preference. One label, two populations: Intent's own 199 unclaimed are 196 files of `ST0056/parity/tools/tap-baseline/` -- tooling, not thread archive -- so only Laksa's kind is at risk, and the ingest must know the difference (a file under a thread directory is an attachment candidate; a tool tree beside the threads is not).

## Acceptance

Acceptance Criteria for this work package are RENDERED into `ST0057/acceptance.md`, under the `WP-13` heading. THAT FILE IS A GENERATED VIEW -- a row authored there is discarded by the next sync. The contract is canon in the thread's model, and the verbs write it: `intent ac new` and `intent at new` mint a row, `intent ac edit` and `intent at edit` reword or re-cite one, and `intent ac satisfy|unsatisfy|descope|rescope|withdraw|reinstate` and `intent at green|red|na` move its state. This cover never restates them.
