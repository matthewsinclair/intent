---
verblock: "02 Sep 2026:v1.32: vc - globalfold; the delivered pair is REBUILT, 0196 is a defect not an authority, and 0216 is an ACTIVE loss condition"
...
**Current as at `22a4e3e8`. This names a COMMIT, not a date** -- a wip file is read as current and is only ever true of a tree.
...
**THE DELIVERED PAIR WAS REBUILT 2026-09-02 (dc) AND THE CURRENCY ARM IS GREEN.** It read REFUSING for hours before that. **Do not carry that state forward -- re-drive it, because five nodes write this tree.** Run both and compare; no figures here:
...
**When they differ, the pair does not describe this tree.** Two traps. **`bin/devbin --version` prints the same `intent 3.0.0 (<sha>)` rendering for a DIFFERENT quantity** -- what the checkout is at now -- so never diff those two against each other. And **`~/.local/bin/intent` is a bare symlink into `target/release/`, so it passes through nothing**: `bin/devbin cli` runs the staleness verdict and refuses on it, and nobody goes through `bin/devbin cli`.

**AND `0196` IS A DEFECT, NOT AN AUTHORITY CONSTRAINT (corrected 2026-09-02).** This file and four boards carried _a rebuild into the shared path is hv's window_. **It is not: `guarded_release_build` DELETES the shared pair before building and no failure path restores it, so what a rebuild needs is a QUIET TREE, not permission.** The question is _is anyone mid-run_, which is answerable by asking. **Copy the pair aside and `cmp`-verify both halves first -- an unbounded outage is not a window.**
...
- **THE TUI-DIVERGENCE CONVERSATION hv OPENED 2026-08-30 IS STILL OWED.** (The rebuild half is DISCHARGED -- taken 2026-09-02 once `0196` was corrected from an authority constraint to a quiet-tree requirement.) **The TUI conversation gates nothing today and stays owed** -- hv recorded scope (_there's work for IC to do to get towards the desired design_) and it has not started, which is the class that becomes a surprise at the cut.
...
- **`ST0056/WP-15` AND `ST0065/WP-02` ARE ONE JOB.** Same corpus, same verdict vocabulary one synonym apart, different threads, different owners. WP-15 is Not Started and scope L; ST0065/WP-02 is wip, catalogued, retirements already executing. **Menu with options and a recommendation is on `hv/wip.md`.** vc recommends re-scoping WP-15 to EXECUTION rather than cancelling: the triage half is done, the criteria are the part worth keeping, and `AC-15.3` is live -- **canon and `~/.claude/skills/` now BOTH hold 23** -- hv killed the two empty orphans 2026-09-02, so the draft's own step-4 check passes as written. **`0218` stays open against `uninstall`, which still leaves the directory.**
- **`intent claude skills sync` REPORTS `4 need a decision` ON EVERY RUN AND NOBODY KNOWS WHAT THE FOUR ARE.** `list` shows all 23 `installed canon`. **Narrowed 2026-09-02 and NOT resolved: it is not extra installed skills and not orphans** -- the installed set and canon are the same 23, difference empty both ways. **So it is a different population from the one `list` renders**, and seeing the message needs a `sync`, which is a WRITE. **A standing count with no visible subject is the shape that cost this estate a day.**
...
- **`0216` IS AN ACTIVE LOSS CONDITION AND IT IS THE LARGEST UNOWNED THING IN THE ESTATE.** A canon write reports `ok`, lands, and `intentd`'s disk ingest reverts it ~1s later; it fired three times on 2026-09-02 across three nodes. **Full statement, mitigation and reach are in `intent/restart.md` under _Live and unfixed_ -- read it before driving this repo.** **The fix is daemon-side and unbuilt.** cc holds a provocation arm that needs nothing granted but must be FENCED to a scratch entity.
