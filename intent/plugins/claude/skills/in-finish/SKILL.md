...
- **globalfold** -- project-wide tidy before **end of day**, when all workstreams close out. Fold the shared project tracking docs (`intent/wip.md`, `intent/restart.md`, `done.md`) into a coherent snapshot. In a multi-node setup this is typically the coordinating / validation workstream's job, not every node's.
...
- `st done` / `wp done` close a unit whose `## Objective` is still `_(not yet written)_`, and warn when they do. A unit that closed with that warning closed without anyone having said what it was for -- write the sentence with `intent set <ID> objective --from <file>`, ideally before closing.
