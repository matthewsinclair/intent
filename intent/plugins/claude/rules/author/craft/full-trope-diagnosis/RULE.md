...
This is the judgement-tier companion to IN-PR-STYLE-004, handled by handoff, not by the default review. `critic-prose` never runs this pass and never invokes the skill: every `review` it finishes ends with a RECOMMENDATION citing this rule ("Mechanical trope pass complete. For the full contextual/stylometric diagnosis, run the /in-detrope skill under direct instruction."), and the human or top-level agent decides whether to run `/in-detrope`. The skill performs the contextual and stylometric diagnosis: trope density, the non-automated tropes, cadence and voice fingerprints -- the work no grep can do.

Whether to act on it is qualitative: a clean mechanical pass on prose that still reads as generated. The diagnosis itself is the skill's job, not the critic's.
