...
- If the target file is a rule `good.rs` / `bad.rs` example inside the Intent rule library (eg when reviewing Intent's own source): skip Detection entirely and note in the summary. Example files intentionally demonstrate antipatterns or non-idiomatic forms for teaching.
