...
/// and never the time. **The source is a phrase naming the model the view is
/// rendered from, never a file name, so the footer prints it as prose**: every
/// caller passes one, and the backticks that suited `thread.json` made the
/// thread canon read as code.
///
...
fn finish(mut out: String, ctx: &RenderContext<'_>, source: &str) -> String {
  ...
  // **THE BANNER OPENS WITH [`BANNER_MARKER`] ITSELF**, so every reader that
  // keys on the marker keys on the bytes this writes (issue 0439).
  out.push_str(BANNER_MARKER);
  out.push_str(&format!(
    "{} from {source}. Do not edit this file -- it is rendered from the model, and `intent doctor` reports any hand-edit as skew._\n",
    banner_version(ctx.version)
  ));
  out
}

/// The version a generated footer names: the MAJOR alone (hv, 2026-09-25).
///
/// The full version rewrote every realised view at every patch release for one
/// character -- the v3.2.1 release commit changed 102 files, almost all of them
/// only in this line -- and left every view nothing re-renders reading as a
/// stale render. The major names the line the file belongs to, and a real
/// rendering change still shows, because skew compares bytes, not versions.
/// One home for the root files' footer token as well as the views'.
pub fn banner_version(version: &str) -> &str {
...
// ---------------------------------------------------------------------------
// info.md -- reading the authored sections back (hv, 2026-08-29)
// ---------------------------------------------------------------------------

/// The sections of `info.md` a human may edit and `sync --to-store` reads back.
///
/// **ONE DECLARATION, CONSUMED IN BOTH DIRECTIONS.** [`info`] renders these and
/// [`info_read_back`] reads them; a heading renamed in one place and not the
/// other stops round-tripping, and the compiler cannot see that. Keeping the
/// list here -- in the module that owns the format -- is what makes the two
/// halves one fact rather than two that agree today.
///
/// **IT IS CLOSED, AND THAT IS THE WHOLE DESIGN.** `legacy::scan` parses an
/// `info.md` into `preamble`/`objective`/`context`/`body` where `body` is an
/// OPEN catch-all -- everything that is not Objective or Context. Pointed at a
/// v3 view that catch-all swallows `## Work Packages`, `## Acceptance` and
/// `## Related Steel Threads` into authored `body`, and the next render freezes
/// a generated table as authored prose. That is the damage class of issues
/// 0124, 0126, 0127, 0129 and 0133 arriving through the door built to stop it.
pub const INFO_ROUND_TRIP_SECTIONS: &[&str] = &["Objective", "Context"];

/// The sections [`info`] GENERATES for itself when the thread's `body` does not
/// carry one of its own.
///
/// **THESE ARE THE REGIONS A STALE VIEW DIFFERS IN, AND THAT IS WHY THEY ARE
/// DECLARED.** A view is only brought level with the model by `--to-disk`, so
/// between a canon change and the next render these regions disagree with the
/// model through nobody's fault. Treating that disagreement as a hand-edit
/// refused `sync --to-store` on two threads in `sync_scope.rs` the moment a
/// title changed, and would have done it to every estate in the fleet.
///
/// **A HEADING HERE IS ONLY GENERATED WHEN THE BODY DOES NOT CLAIM IT** -- see
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct InfoReadBack {
  pub objective: String,
  pub context: String,
}

/// One region of a rendered view: the text before the first `## ` heading,
/// then one region per `## ` heading, in order.
///
...
/// Read the authored sections of `info.md` back out of an edited file.
///
/// # Why this compares against a render rather than parsing the file
///
...
///
/// Rendering the model and diffing REGION BY REGION sidesteps the question
/// entirely: this never needs to know whether a region is authored, only
/// whether it CHANGED. That also makes the runtime check and the safeguard test
/// the same predicate rather than two that have to be kept in step.
///
/// # Why an out-of-scope edit refuses rather than being dropped
///
/// hv ruled it on 2026-08-29: refuse, name the sections, leave both the store
/// and the file untouched. Carrying the two sections and discarding the rest
/// would be a silent loss on the one path we just opened an editor onto --
/// `IN-AG-NO-SILENT-001`, and the exact shape of the damage class this
/// allow-list exists to prevent.
pub fn info_read_back(
  thread: &Thread,
  ctx: &RenderContext<'_>,
  disk: &str,
) -> Result<InfoReadBack, Vec<String>> {
  // **RENDER AGAINST THE VERSION THE FILE ITSELF DECLARES, NOT OURS.** `finish`
  // stamps `Generated by Intent v{version}` into every view, so a file written
  // by yesterday's binary differs in a byte no human touched.
  let stamped = declared_version(disk);
  let ctx = RenderContext {
    version: stamped.unwrap_or(ctx.version),
    todo_watermark: ctx.todo_watermark.clone(),
  };
  let rendered = info(thread, &ctx);

  let want = authored_regions(thread, &rendered);
  let got = authored_regions(thread, disk);

  // **A STRUCTURAL DIFFERENCE IS REFUSED BEFORE ANY BODY IS COMPARED**, because
  // a heading added or removed by hand makes the region lists non-corresponding
  // and every later comparison would be against the wrong neighbour -- one edit
  // reported as several, none of them where the author actually typed.
  // **TEXT AFTER THE BANNER IS REFUSED HERE RATHER THAN INSIDE
  // `authored_regions`** (issue 0192; placement cc's, vc agreed). That function
  // truncates AND would then also have to refuse, which is two jobs in one
  // home; its contract is "the regions the author wrote", and a region that is
  // outside the view entirely is not one of them.
  //
  // **TRAILING WHITESPACE IS NOT AUTHORED TEXT**, so the comparison is
  // `trim_end`: a file that lost or gained a final newline round-trips
  // unchanged and must not be refused, while `\n## Hand Added\n\nTEXT\n`
  // appended by a script is real content that this view cannot carry.
  //
  // **DRIVEN BOTH WAYS BEFORE IT WAS WRITTEN, ON AN ESTATE THAT COULD EXHIBIT
  // IT.** A fresh project cannot: `resync_inner` builds `touched` from files
  // that are `Changed` AND present in the PREVIOUS index, so an unindexed
  // estate has an empty baseline and the carry is a designed no-op. The drive
  // therefore syncs once to build the baseline, and only then edits.
  let mut said: Vec<String> = Vec::new();
  if after_banner(disk).trim_end() != after_banner(&rendered).trim_end() {
    said.push("text after the trailing banner (added by hand)".to_string());
  }

  let want_headings: Vec<&Option<String>> = want.iter().map(|(h, _)| h).collect();
  let got_headings: Vec<&Option<String>> = got.iter().map(|(h, _)| h).collect();
  if want_headings != got_headings {
    let name = |h: &Option<String>| match h {
      Some(h) => format!("## {h}"),
      None => "the preamble".to_string(),
    };
    said.extend(
      got_headings
        .iter()
        .filter(|h| !want_headings.contains(h))
        .map(|h| format!("{} (added by hand)", name(h))),
    );
    said.extend(
      want_headings
        .iter()
        .filter(|h| !got_headings.contains(h))
        .map(|h| format!("{} (removed by hand)", name(h))),
    );
    if said.is_empty() {
      said.push("the sections were reordered by hand".to_string());
    }
    return Err(said);
  }
  // **BOTH KINDS REPORT IN ONE RUN.** A file carrying an added section AND
  // appended text is one editing session, and naming half of it sends the
  // author back for a second refusal they could have fixed the first time.
  if !said.is_empty() {
    return Err(said);
  }

  let mut carried = InfoReadBack {
    objective: thread.objective.clone(),
    context: thread.context.clone(),
  };
  let mut refused: Vec<String> = Vec::new();

  for ((heading, want_body), (_, got_body)) in want.iter().zip(got.iter()) {
    let Some(heading) = heading else {
      // The preamble is AUTHORED and is not in the allow-list, so an edit to it
      // is real text that cannot be carried -- exactly what hv ruled must be
      // refused rather than dropped.
      if want_body != got_body {
        refused.push("the preamble (authored, but not carried)".to_string());
      }
      continue;
    };
    match INFO_ROUND_TRIP_SECTIONS.contains(&heading.as_str()) {
      ...
      // Reached only by a section carried in `body` -- the generated ones were
      // filtered out above -- so a difference here is authored text that this
      // door cannot take.
      false => {
        if want_body != got_body {
          refused.push(format!("## {heading} (authored, but not carried)"));
        }
      }
    }
  }
...
/// WHOLE CORRECTION.** Comparing a render against a view cannot distinguish a
/// hand-edit from a view the model has simply moved past -- both are "the bytes
/// differ". Asking the MODEL which regions it generates separates them: the
/// frontmatter, the title, the trailing banner and the three generated sections
/// are all regions a stale view legitimately differs in, and none of them is
/// anybody's writing.
///
/// What is left is exactly the author's: the preamble, and every section the
/// thread's `body` carries.
/// The banner every rendered view ends with, as the delimiter two functions key
/// on: [`authored_regions`] takes everything BEFORE it, [`after_banner`] takes
/// everything from it. **One home, because the pair only works while they agree
/// -- text between two different notions of "the banner" is text neither of them
/// examines, which is exactly the hole issue 0192 records.**
const BANNER_MARKER: &str = "\n\n---\n\n_Generated by Intent v";

/// Everything after the trailing banner's own LINE, or `""` when there is none.
///
/// **THE BANNER LINE ITSELF IS EXCLUDED, AND THAT IS AN INVARIANT RATHER THAN
/// A DETAIL.** `finish` appends the banner unconditionally and the sentence
/// inside it says the file is generated, so a REWORDED banner is regenerated
/// rather than refused -- `invariant_a_reworded_banner_does_not_reach_the_model`
/// holds exactly that, and it caught this function comparing from the marker
/// onward, which made an edit to generated bytes refuse. **Text after the line
/// is the author's; the line is the renderer's.**
///
/// **THE COMPLEMENT OF [`authored_regions`]'s TRUNCATION, AND IT EXISTS BECAUSE
/// THE TRUNCATION HAD NO COMPLEMENT** (issue 0192). `authored_regions` cuts the
/// text at the banner and hands back the regions before it; it is called for
/// BOTH sides of the round-trip comparison, so anything after the banner landed
/// in neither `want` nor `got`, the structural guard compared two lists that
/// agreed, and the next projection rewrote the file without it. `>>` is how a
/// script appends, which makes that the likely way to lose text rather than an
/// exotic one.
fn after_banner(text: &str) -> &str {
  let Some(at) = text.rfind(BANNER_MARKER) else {
    return "";
  };
  let line = at + BANNER_MARKER.len();
  match text[line..].find('\n') {
    Some(end) => &text[line + end + 1..],
    // The banner line is the last line, so there is nothing after it.
    None => "",
  }
}

/// A view this renderer wrote, without its banner: the text before the trailing
...
/// 0439). A registered node's board and inboxes are views, and read as
/// hand-authored markdown the banner's rule and sentence were units: a coerced
/// item on the board, and a refused line in every empty inbox.
pub fn view_body(text: &str) -> Option<&str> {
  let at = text.rfind(BANNER_MARKER)?;
  after_banner(text).trim().is_empty().then(|| &text[..at])
}

fn authored_regions(thread: &Thread, text: &str) -> Vec<(Option<String>, String)> {
  let trunk = match text.rfind(BANNER_MARKER) {
    Some(at) => &text[..at],
    None => text,
  };
  regions(trunk)
    ...
    .filter(|(heading, _)| match heading {
      None => true,
      Some(h) => {
        !(INFO_GENERATED_SECTIONS.contains(&h.as_str()) && !carries_heading(&thread.body, h))
      }
    })
    .map(|(heading, body)| match heading {
      // The pre-heading region is frontmatter, then the title, then the
      // preamble. Only the last of those is authored.
      None => (None, preamble_of(&body).to_string()),
      some => (some, body),
    })
    .collect()
}

/// The authored preamble inside the pre-heading region: everything after the
/// generated `# <id>: <title>` line.
///
/// A view with no title line is not one this renderer produced; the whole
/// region is handed back so the mismatch surfaces rather than being trimmed
/// away into an accidental agreement.
fn preamble_of(region: &str) -> &str {
  match region.split_once("\n# ") {
    Some((_, after_marker)) => match after_marker.split_once('\n') {
      Some((_, preamble)) => preamble,
      None => "",
    },
    None => region,
  }
...
/// The Intent version a rendered view says it was generated by.
///
/// Read from the LAST occurrence, because the banner is a suffix and a body
/// that quotes one -- this estate's own docs do -- would otherwise hand back
/// the quoted version instead of the real one.
///
/// `None` when the file carries no banner at all, which is a file that did not
/// come from this renderer; the caller then compares against its own version
/// and the mismatch surfaces as the refusal it should be.
fn declared_version(text: &str) -> Option<&str> {
  let (_, tail) = text.rsplit_once("_Generated by Intent v")?;
  let (version, _) = tail.split_once(" from ")?;
  Some(version)
}
...
/// field as `_(not yet written)_`, so reading that back verbatim would write
/// the placeholder into the model as though a human had typed it, and the next
...
mod tests {
  ...
  /// hv, 2026-09-25: a view's footer names the major alone, so a patch or a
  /// minor release rewrites no view for its version.
  #[test]
  fn a_views_footer_names_the_major_alone() {
    let t = thread("ST0075", "wip");
    let at_patch = info(
      &t,
      &RenderContext {
        version: "3.2.1",
    ...
    assert!(
      at_patch.contains("\n_Generated by Intent v3 from "),
      "{at_patch}"
    );
    assert_eq!(at_patch, at_minor);
  }
