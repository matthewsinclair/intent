...
/// **A PROJECT WITH A WHITEBOARD IS OWED NOTHING, NOW THAT A BUILD CARRIES IT.**
///
/// The whiteboard was this set's one member until `wb register` and `wb migrate`
/// carried it, and upgrade kept reporting every board file as not-yet-carried in
/// every project that had one. The set is empty by delivery, so neither report
/// speaks.
#[test]
fn a_project_with_a_whiteboard_gets_no_not_yet_carried_report() {
  // Issue 0326 and 0363: the whiteboard stayed in NOT_YET_BUILT after the build that carries it landed, so upgrade reported it in every project.
  let fx = crate::common::v2_estate();
  fx.write_file("intent/whiteboard/cc/wip.md", "board\n");
  assert!(
    intentsvcs::sync::migration_not_yet_built().is_none(),
    "no build gap is claimed, so the summary line is not emitted"
  );
  assert!(
    intentsvcs::sync::migration_not_yet_built_artefacts(&fx.project()).is_empty(),
    "and a whiteboard's files are not enumerated as owed by a build"
  );
}
