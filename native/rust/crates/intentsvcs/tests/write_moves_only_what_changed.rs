...
const DECLARED_BUT_UNWIRED: &[(&str, &str)] = &[
  (
    "browse",
    "intent-cli/tests/twin_spellings_agree.rs holds it to its twin; the verb itself answers `rc=2`, `is a known command that is not implemented yet`. **DECLARED AHEAD OF ITS ARM ON PURPOSE, AND AC-17.6 REQUIRES THAT ORDER IN AS MANY WORDS** -- the shape change to `edit` is a surface decision that belongs to the interface owner, so the register row is authored before either realiser is built. **IT IS ALSO HALF OF INV-09, WHICH IS WHY IT CANNOT SIMPLY BE OMITTED UNTIL PIECE 3**: `intent browse <kind> <id>` IS `intent edit <kind> <id> --browser`, and a capability present by one spelling and absent by the other is exactly what `ST0058 AC-00.6` refuses. BOTH HALVES REFUSE TODAY AND THEY REFUSE DIFFERENTLY, WHICH IS THE GAP THIS ROW ACTUALLY RECORDS: the verb answers `rc=2` `is a known command that is not implemented yet`, and the flag arm (`fn browsed`) answers `rc=1` `--browser is not implemented in this build`. **Until 2026-09-04 the flag arm instead blamed a missing daemon, and that message was false in BOTH clauses** -- one WAS answering (`intent daemon status`, rc=0, same minute) and a running one would not have helped, because the serving half is WP-08 and unbuilt. **The two exit codes are left as they are rather than reconciled here**: 1 versus 2 for one capability under two spellings is a surface decision and belongs to the interface owner, so it is recorded rather than taken. Re-buckets the day the realiser lands, which is what this roster is for.",
  ),
  (
    "fc",
