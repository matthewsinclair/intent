...
//! (`intent 3.0.0 (80d8b2ca...)`) every one of the headless languages
...
//! correctly and passes. So the gate across the fleet reports a green
...
//! against the SOURCE tree -- a full, correct library -- elixir, rust and shell
//! each arm some of their rules, while swift and lua arm **none** of theirs.
//! Swift's rules and lua's are all `Undeclared` -- nobody has yet recorded
//! whether they are mechanically checkable -- so **those two languages arm zero
//! in a perfectly healthy install**, and arming is a property of the RULE
//! rather than the run (shell arms the same rules against `README.md` and
//! against a real shell script alike). Refusing on `armed == 0` would therefore
//! block swift and lua on every commit of a correct install: a false refusal
//! with a real population, which is the wrong-population error one layer up.
//!
...
//! breakage, and the module's governing principle is *a gate should fail open
//! on its own breakage and closed on yours*. Blocking every repository carrying
//! the gate because the keg shipped without its rules is issue 0043 rebuilt,
//! which the hook's own comment forbids by name.
//!
...
//! and prints the digest with its denominator. A project's declared languages
//! then read `<n> of <n> declared language(s) went UNENFORCED`, which that comment
//! calls "a gate that is not running at all". Loud, and open.
//!
