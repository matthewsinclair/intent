...
/// **The store is ahead of the file after EVERY mutation, so this must never be
/// a finding.** `history_checks`'s own docstring names what a finding here
/// would cost -- *a finding that fires routinely is the trained-to-be-ignored
/// failure* -- and it would put `doctor` back to a permanent rc=1 on a healthy
/// tree, which is WP-10's defect rebuilt with a different cause hours after it
/// was fixed.
///
/// It is reported anyway for `unattached`'s reason: under D34 the projected
/// file is history's only route off this machine, so a silent gap and a clean
/// bill of health are indistinguishable to a reader, and a number is neither.
///
/// **Both arms, because either alone passes on a bug.** A counter wired to
/// return 0 unconditionally passes the healthy arm; one that never clears
/// passes the exposed arm.
#[test]
fn events_held_only_by_the_store_are_counted_and_never_a_finding() {
  let fx = Fixture::new();
  seed(&fx, &clean_thread("ST0001"));
  let store_path = fx.project().db_path();

  {
    let mut facade = fx.facade_on_disk();
    facade
      .wp_new("ST0001", "the baseline", intentsvcs::model::TShirt::S)
      .expect("a legal mutation that writes an event");
  }

  // **THE BASELINE MUST HAVE ZERO EXPOSURE, AND AN EARLIER VERSION OF THIS TEST
  // DID NOT -- WHICH IS WHY IT FAILED TO CATCH THE MUTANT THAT MATTERS.** With
  // exposure present in BOTH arms, a counter wired as a finding adds one
  // finding to each, the lengths stay equal, and the assertion below passes
  // over the exact defect it exists to refuse. Projecting the file first makes
  // the baseline genuinely unexposed, so the mutant's finding appears in one
  // arm only.
  let store = intentsvcs::store::Store::open(&store_path).expect("store");
  let events = store.events().expect("events");
  fx.write_file(
    "intent/events.jsonl",
    &intentsvcs::event::to_jsonl(&events).expect("jsonl"),
  );
  let before = intentsvcs::doctor::diagnose(&fx.project(), &ctx(), Some(&store));
  drop(store);
  assert_eq!(
    before.unsynced_events, 0,
    "the file now carries every event the store holds, so nothing is exposed"
  );

  // A mutation the file cannot yet carry. Exposure appears; health does not move.
  {
    let mut facade = fx.facade_on_disk();
    facade
      .wp_new("ST0001", "the exposed one", intentsvcs::model::TShirt::S)
      .expect("a second legal mutation");
  }
  let store = intentsvcs::store::Store::open(&store_path).expect("store");
  let after = intentsvcs::doctor::diagnose(&fx.project(), &ctx(), Some(&store));

  assert!(
    after.unsynced_events > 0,
    "history now exists only in this store and must be counted"
  );

  // **THE ARM THAT MATTERS. EXPOSURE IS NOT A DEFECT.** A counter wired as a
  // finding would move this number, put `doctor` back to a permanent rc=1 on a
  // working tree, and rebuild WP-10's defect with a different cause.
  assert_eq!(
    after.findings.len(),
    before.findings.len(),
    "counting exposure must add NO finding: {}",
    details(&after.findings)
  );
  assert_eq!(
    after.exit_code(),
    before.exit_code(),
    "and it must not move the exit code"
  );
}
