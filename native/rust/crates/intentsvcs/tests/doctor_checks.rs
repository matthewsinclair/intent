...
/// **A FOOTER-ONLY DIFFERENCE IS `stale-render`, IT IS NOT `view-skew`, AND IT
/// DOES NOT BLOCK** (issue `0309`).
///
/// The whole of 0309 is in the three assertions: the class, the ABSENCE of the
/// blocking class, and the verdict. Any two of them can hold while the defect
/// stands -- a report that names the new class and keeps the old one beside it
/// still refuses the commit, which is the outage this was filed for.
#[test]
fn a_view_whose_only_difference_is_its_footer_version_is_not_blocking_skew() {
  let fx = Fixture::new();
  seed(&fx, &clean_thread("ST0001"));
  let view = "intent/st/ST0001/info.md";
  let rendered = fx.read(view);
  let older = rendered.replace(
    &format!(
      "_Generated by Intent v{}",
      intentsvcs::views::banner_version(VERSION)
    ),
    "_Generated by Intent v2.9.9-older",
  );
  assert_ne!(
    older, rendered,
    "the fixture must actually change the footer"
  );
  std::fs::write(fx.path(view), &older).expect("write the older render");
  ...
  assert!(
    ...
    "an older footer is a stale render: {}",
    details(&report.findings)
  );
  assert!(
    !report
    ...
    "and it is NOT skew, or the commit is refused anyway: {}",
    details(&report.findings)
  );
  assert_eq!(
    report.exit_code(),
    0,
    "an advisory class leaves the verdict, which is what the pre-commit gate reads: {}",
    details(&report.findings)
  );
}

/// **A BODY EDIT IS STILL SKEW, AND THE DETAIL SAYS WHERE** (issue `0309`).
///
/// The offset is what tells a hand edit from a renderer change without cloning
/// the tree: the two lengths the detail already carried are EQUAL whenever an
/// edit substitutes rather than adds, which is the commonest shape there is.
#[test]
fn a_hand_edited_view_is_skew_and_names_the_first_differing_byte() {
  let fx = Fixture::new();
  seed(&fx, &clean_thread("ST0001"));
  let view = "intent/st/ST0001/info.md";
  let rendered = fx.read(view);
  // A substitution of the same length, so the byte counts in the detail agree
  // and the offset is the only thing that can locate it.
  let edited = rendered.replacen("A clean thread", "A dirty thread", 1);
  assert_ne!(edited, rendered, "the fixture must actually edit the body");
  assert_eq!(
    edited.len(),
    rendered.len(),
    "the edit must not change the length, or the offset is not the only witness"
  );
  std::fs::write(fx.path(view), &edited).expect("write the edited view");
  ...
  assert!(
    report
      ...
      .any(|f| f.class == FindingClass::ViewSkew
        && f
          .detail
          .contains(&format!("first difference at byte {offset}"))),
    "the skew detail names the first differing byte: {}",
    details(&report.findings)
  );
  assert_eq!(
    report.exit_code(),
    1,
    "a hand-edited view is blocking, which is the half of the split that must not move: {}",
    details(&report.findings)
  );
}

/// The version reaches the renderer, so a view is judged against the same
/// banner the tool writes -- not against a default that would make every view
/// look skewed.
///
/// **THE CLASS MOVED AND THE SUBJECT DID NOT** (issue `0309`, vc 2026-09-12).
/// This asserted `ViewSkew`, using the class as the observable proof that the
/// version had arrived; a footer-only difference is now `StaleRender`, and the
/// assertion follows it. The change makes the measurement DIRECT rather than
/// weaker: the detail names both versions, so the test reads the value that
/// reached the renderer instead of inferring from a difference existing that
/// one must have.
#[test]
fn skew_is_judged_against_the_rendering_version() {
  ...
  assert!(
    findings
      .iter()
      .any(|f| f.class == FindingClass::StaleRender),
    "a different version renders a different banner, which is a stale render: {}",
    details(&findings)
  );
  // The detail carries BOTH versions, which is what makes this an observation
  // of the value that reached the renderer rather than of a difference existing.
  assert!(
    findings.iter().any(|f| f.class == FindingClass::StaleRender
      && f.detail.contains("renders v9 ")
      && f.detail.contains(&format!(
        "Intent v{} ",
        intentsvcs::views::banner_version(VERSION)
      ))),
    "the finding names the version on disk and the version this binary renders: {}",
    details(&findings)
  );
  assert_ne!(
    VERSION, "9.9.9-not-the-fixture-version",
