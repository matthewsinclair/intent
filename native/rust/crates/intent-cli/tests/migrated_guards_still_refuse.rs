...
// `output_retrying_busy` moved to `testkit` on 2026-08-24. It was correct
// here and needed in two more files: `embedded_init` and `info_exit_code` copy
// the binary out and exec it the same way, and both had reddened CI or were one
// race from it. The fix did not change; only where it applies.
