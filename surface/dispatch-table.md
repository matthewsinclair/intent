...
- **open:** `-s|--start` today jumps straight to `Wip`. Under the ratified machine that is two edges at once (`Triage -> NotStarted -> Wip`) with no `st triage` decision recorded in between. Left as-is and flagged rather than silently re-pointed: whether the flag should now mean `--start` (to `Wip`, skipping triage) or be replaced is a scope question for vc and hv, not a surface preference for me.
...
- **unknown key refuses:** *** THE LOAD-BEARING DECISION ON THIS ROW: AN UNKNOWN KEY IS REFUSED, NEVER CREATED. *** `intent config set backup.retian.daily 7` must fail and name the unknown key. Silently writing it produces a dead key that nothing reads, a config file that LOOKS configured, and a user who believes they set a retention policy they did not -- and the failure surfaces, if ever, as backups quietly pruned on defaults. This is the whole class this toolchain keeps closing, sitting in the one command whose entire job is to be believed about what is configured.
