...
- **Target:** `as-observed`
- **d42 exposure:** D42 EXPOSURE (ic, audit 2026-08-15). `--flush` is described as advancing the DONE watermark TO NOW, and 'now' is a time the CLI would have to obtain. Under D42 there is no source to obtain it from: not the OS, not the filesystem, and not a read from the store either, because the read and the write are two acts with a gap in them. The v3 watermark must therefore be a REFERENCE TO A RECORD -- the newest done transition already carries the stamp the database wrote -- rather than a computed instant. Flagged, not redesigned: the fix names a record and that is WP-06's to choose.
- **MCP:** exposed as an agent tool -- **mutates**
...
- **d42 exposure:** D42 EXPOSURE (ic, audit 2026-08-15). The backup-staleness obligation above reads 'the newest snapshot AGE against the configured backup.schedule', and an age against NOW requires obtaining a now, which is the prohibited half of the rule. The obligation is right and its wording implies a clock -- and the resolution sits in the same sentence, which cites the clock guard check C: compare two stamps to EACH OTHER. Concretely, the newest snapshot record against the newest event record. Both values are RETURNED by the DB, so a duration computed between them takes nothing and confects nothing. Reword before WP-06 builds it, or the first implementation reaches for a clock because the requirement asked for one.
- **MCP:** exposed as an agent tool -- **mutates**
...
### EXP-06 -- D42 audit of the declared surface: two rows would have to OBTAIN a time, and one only had the wrong source

- **Detail:** The rule is ASYMMETRIC and the audit is only correct once that is stated (hv, 2026-08-15): no CLI or intentsvcs function TAKES a time; functions that RETURN times are fine, because those values went end-to-end through the DB where SQLite set them. So the thing to hunt for is a surface that must OBTAIN an instant or a duration, never one that displays a stamp. Audited the table and the 27 inventories. The inventories are CLEAN -- v2's measured surface declares no time-bearing flag or argument anywhere, so nothing is inherited, and every finding is in the NEW surface: a design exposure rather than a porting one, still cheap to change.
- **Resolution:** Each row carries its own note and none of the fixes is adopted here. The shared shape survives the correction and is worth stating once: EVERY REMAINING CASE WANTS AN INSTANT OR A DURATION, AND EVERY ONE CAN BE ANSWERED BY COMPARING TWO RECORD STAMPS. That is the clock guard check C construction this project has already ratified -- two stamps compared to each other need no clock, and cannot be fabricated by a node that has none.
- **Consequence for the generator:** None today; the generator renders these like any other authored field and the entry-level completeness check proves they reach the view. The standing obligation is on the AUDIT: this table grows new surface every week, and a D42 exposure enters by someone writing plausible help text, never by a check going red. Re-run the word-bounded audit whenever new_surface changes -- and hunt for OBTAINING, not for the word `time`. The first pass matched `manage`, `validate` and `update` on substrings of `age` and `date` and reported 25 rows including every `--help`; word boundaries cut it to five, of which three looked real and two were.
...
- **d42 exposure:** D42 (ic, audit 2026-08-15; CORRECTED after hv's clarification the same day). NOT an exposure for emitting a time -- hv: v3 will have CLI and intentsvcs functions that RETURN times, and that is fine, provided the value went end-to-end through the DB where SQLite SET it. Returning is legal; only obtaining is not. So `--list` showing when each snapshot was taken is a legitimate surface and stays. The defect is its SOURCE: a snapshot is a file under .backup/db/, and a file mtime is a filesystem clock. Taking a snapshot must therefore WRITE A RECORD and `--list` must read that record stamp -- which is a data-model consequence recorded nowhere else. The first draft of this note argued the emission itself was the problem, which would have withdrawn a surface that is allowed.
- **MCP:** not exposed -- **mutates**
- **Wants review:**
  - uncertain on `exposed_on_mcp`
