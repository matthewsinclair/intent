...
- **Target:** `corrected` -- ratified: vc 2026-08-17, relayed after hv ratified Machine 1; this row needed no separate hv call and the reasoning is vc's: Machine 1 already declares `NotStarted -> Wip` and hv ratified the machine, so `st start` is `corrected` on exactly the grounds the other three are. It was `pending-hv`, which was honest but is now answered. THE DEVIATION IS THE FROM-STATE. Machine 1 admits `st.start` from `not-started` ONLY, so v3 REFUSES starting a `hold`, `completed` or `cancelled` thread -- `st resume` and `st reopen` are the edges for the first two -- where v2 had no lifecycle state guard anywhere (vc's issue 0046). **AND IT REFUSES FROM `triage`, WHICH IS THE ENTRY STATE, so a freshly created thread cannot be started directly: `st new` lands on `triage` and `st triage` is the edge to `not-started`.** That is the sharpest user-visible consequence on this row and it is a deliberate consequence of the ratified machine, not an oversight. THE SELF-LOOP IS ACCEPTED AND IS NOT PART OF THE DEVIATION (hv ruled 2026-08-17, relayed by vc; implemented and verified in `facade.rs` by ic before this edit, NOT taken from the relay). `if from == status { return Ok(Outcome::AlreadyThere) }` sits FIRST -- ahead of `check_transition`, ahead of the reason guard, and ahead of the gate -- so re-issuing the verb in the state it targets is a no-op at exit 0. **Its POSITION is the ruling**: a self-loop must not re-run the gate, or a criterion added after a close blocks a unit that is legitimately finished, which happened in this very thread. So a self-loop is PARITY with v2, not deviation. Therefore `st start` on an already-`wip` thread is a no-op at exit 0. Edges re-read off `transitions.rs` and `facade.rs` by ic before this edit.
...
- **Target:** `corrected` -- ratified: vc 2026-08-17 09:39Z under the standing grant; precedent `st cancel`, same mechanism and same author. THE DEVIATION IS THE FROM-STATE AND ONLY THE FROM-STATE. Machine 1 admits `st.done` from `wip`, so v3 REFUSES closing a `triage`, `not-started`, `hold` or `cancelled` thread, where v2 had NO lifecycle state guard anywhere and moved it (vc's issue 0046, twelve undeclared movements). AC-04.6 REQUIRES that deviation, so the previous `as-observed` was asserting parity across one that is mandated. **The `GatePass` guard is PARITY, not part of the correction**: v2 already exited 1 on a BLOCKED acceptance contract (`bin/intent_st:470-471`, in this row's own measured exits), so v3 reproduces the gate and restricts the from-state, and only the second is a deviation. THE SELF-LOOP IS ACCEPTED AND IS NOT PART OF THE DEVIATION (hv ruled 2026-08-17, relayed by vc; implemented and verified in `facade.rs` by ic before this edit, NOT taken from the relay). `if from == status { return Ok(Outcome::AlreadyThere) }` sits FIRST -- ahead of `check_transition`, ahead of the reason guard, and ahead of the gate -- so re-issuing the verb in the state it targets is a no-op at exit 0. **Its POSITION is the ruling**: a self-loop must not re-run the gate, or a criterion added after a close blocks a unit that is legitimately finished, which happened in this very thread. So a self-loop is PARITY with v2, not deviation. Therefore `completed` is NOT in the refused set. **ic wrote this ratification once already, from `transitions.rs` alone, and it over-stated the refusal by exactly the self-loop.** The machine declares the edges; `facade.rs` decides the behaviour, and reading the declaration and calling it the behaviour is the error `target.state` exists to prevent. Measured by cc against the v3 facade and by vc against v2 at 18 cells, fresh project per cell.
...
- **stdout:** confirmation
- **stderr:** `error: ...` on stderr (INV-01)
- **Target:** `corrected` -- ratified: vc 2026-08-17 09:39Z under the standing grant; one ruling covering `st start`, `st done`, `wp start` and `wp done`, all deviating in the SAME single respect -- the from-state. Machine 2 admits `wp.start` from `not-started`, a DIRECT edge with no guard, so v3 REFUSES restarting a `done` package where v2 accepted, having no lifecycle state guard anywhere (vc's issue 0046). This is the cleanest of the four because there is no guard on the edge to confuse with the restriction. THE SELF-LOOP IS ACCEPTED AND IS NOT PART OF THE DEVIATION (hv ruled 2026-08-17, relayed by vc; implemented and verified in `facade.rs` by ic before this edit, NOT taken from the relay). `if from == status { return Ok(Outcome::AlreadyThere) }` sits FIRST -- ahead of `check_transition`, ahead of the reason guard, and ahead of the gate -- so re-issuing the verb in the state it targets is a no-op at exit 0. **Its POSITION is the ruling**: a self-loop must not re-run the gate, or a criterion added after a close blocks a unit that is legitimately finished, which happened in this very thread. So a self-loop is PARITY with v2, not deviation. Therefore re-running `wp start` on a `wip` package is a no-op at exit 0 and is NOT in the refused set. **ic wrote this ratification once already, from `transitions.rs` alone, and it over-stated the refusal by exactly the self-loop.** The machine declares the edges; `facade.rs` decides the behaviour, and reading the declaration and calling it the behaviour is the error `target.state` exists to prevent. AC-04.6 REQUIRES the deviation.
- **ratified in:** vc ruling 2026-08-17, recorded on issue 0046
- **MCP:** exposed as an agent tool -- **mutates**
- **recoverability:** reversible
...
- **Target:** `corrected` -- ratified: vc 2026-08-17 09:39Z under the standing grant; one ruling covering `st start`, `st done`, `wp start` and `wp done`. THE DEVIATION IS THE FROM-STATE AND ONLY THE FROM-STATE. Machine 2 admits `wp.done` from `wip`, so v3 REFUSES closing a `not-started` package where v2 had no lifecycle state guard and moved it. **The `GatePass` guard is PARITY and this row said so before ic touched it** -- the machine note below records that Machine 2's BLOCKED-gate refusal was ALREADY the measured v2 behaviour, and the measured exits carry `1 <- acceptance contract BLOCKED for the WP group`. That note is what caught ic over-claiming the gate as part of the correction, so it has earned its keep twice. THE SELF-LOOP IS ACCEPTED AND IS NOT PART OF THE DEVIATION (hv ruled 2026-08-17, relayed by vc; implemented and verified in `facade.rs` by ic before this edit, NOT taken from the relay). `if from == status { return Ok(Outcome::AlreadyThere) }` sits FIRST -- ahead of `check_transition`, ahead of the reason guard, and ahead of the gate -- so re-issuing the verb in the state it targets is a no-op at exit 0. **Its POSITION is the ruling**: a self-loop must not re-run the gate, or a criterion added after a close blocks a unit that is legitimately finished, which happened in this very thread. So a self-loop is PARITY with v2, not deviation. Therefore re-issuing `wp done` on a `done` package is a no-op at exit 0, and notably does NOT re-run the gate -- so an AC added after the close cannot block it. **ic wrote this ratification once already, from `transitions.rs` alone, and it over-stated the refusal by exactly the self-loop.** The machine declares the edges; `facade.rs` decides the behaviour, and reading the declaration and calling it the behaviour is the error `target.state` exists to prevent. Read beside `wp reopen`, which is `new-surface`: v2 shipped nothing that undid `wp done`, which is what made the absent state guard costly rather than merely permissive.
...
| `backup` | --           | --list                | Snapshot this machine's store for fast local restore -- prunes expired snapshots             | WP-03     | D35 (hv, 2026-08-15): 'a `.backup/` dir in the project that is gitignored could have a per-{day,week,month} rolled-up snapshot of the actual SQLite db file, configurable from `intent config`'.                                                                                                                             |
