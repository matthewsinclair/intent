...
- **Target:** `as-observed`
- **open question:** **IS `list` `as-observed` OR `corrected`? FILED AS 0058 AND DELIBERATELY NOT RULED HERE.** 0058 argues `corrected`: reproducing a list that advertises seven names its sibling cannot deliver launders a v2 defect into a v3 requirement, which `parity.md` forbids in as many words. **But this is a change to what the command ENUMERATES, not to how it speaks, so it is not a voice ruling and not mine to take alone** -- the five voices were ruled by ic because cc was blocked on output strings; this is behaviour. The row stays `as-observed` until it is adjudicated, so v3 building from it today would reproduce the defect: that is the cost of leaving it open and it is stated rather than hidden. The INV-01 and `show` halves need no ruling -- one is an invariant already ratified, the other is a subcommand v2 has.
- **MCP:** exposed as an agent tool -- read-only
