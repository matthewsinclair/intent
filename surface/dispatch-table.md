...
- **Target:** `corrected` -- ratified: vc ruling 2026-08-22, **PROVISIONAL pending hv**, on a fork cc escalated after starting `claude skills` in Rust. **THE RULING: the SKILL.md-only checksum scope is a DEFECT that v3 fixes, not a behaviour v3 reproduces.** **THE WORD `provisional` IS LOAD-BEARING AND STAYS UNTIL hv RULES** -- same footing as `ac gate`, whose field records why: `ratified_in_check.sh` buckets on the word ahead of conformance, so citing a record here does not make the row read as ratified. hv has not been asked and nothing in this row should suggest otherwise. -- behaviour: v3's `sync` detects a change anywhere in a skill's installed content rather than in SKILL.md alone. **THE MECHANISM IS cc's BUILD DECISION AND IS DELIBERATELY NOT SPECIFIED HERE** -- this row records that the defect is corrected, not how. A parity row that dictates an implementation is asserting a second design authority.
...
- **ratified in:** vc ruling 2026-08-22 (provisional pending hv), recorded in this row's `ratification` field, whose text carries the ruling -- it arrived as a cross-session message, which does not survive the session
- **MCP:** exposed as an agent tool -- **mutates**
- **recoverability:** idempotent
