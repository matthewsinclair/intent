...
- **Resolution:** PROPOSED, NOT APPLIED, and deliberately so. The fix is a declared root-help field here plus one line in `spine.rs:26` reading it, and those are two owners: the field is ic, the wiring is cc. **Declaring the field before the spine reads it would create precisely the defect AC-06.8 exists to prevent** -- a declared value nothing renders -- and vc's sharper form of the same rule is that it would read as load-bearing while applying to nothing. So it is recorded here and raised with cc, and the field lands in the same window as the wiring. Until then this entry is the honest statement of what the `about` block overstates.
