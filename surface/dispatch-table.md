...
- **d42 exposure:** D42 EXPOSURE (ic, audit 2026-08-15). The backup-staleness obligation above reads 'the newest snapshot AGE against the configured backup.schedule', and an age needs a now. The obligation is right and its wording implies a clock -- and the resolution is already inside the same sentence, which cites the clock guard check C: compare two stamps to EACH OTHER and no clock is needed. Concretely, the newest snapshot record against the newest event record. Reworded before WP-06 builds it, or the first implementation will reach for a clock because the requirement asked for one.
- **MCP:** exposed as an agent tool -- **mutates**
...
### EXP-06 -- D42 audit of the declared surface: three rows imply a time source, and none of them is a v2 row

- **Detail:** vc's D42 dictate to ic (2026-08-15): the dispatch table and the inventories must not declare or describe any surface that takes or emits a caller-supplied time, and a row that implies one is flagged. Audited both. The 27 command inventories are CLEAN -- v2's measured surface declares no time-bearing flag or argument anywhere, so nothing is inherited. All three findings are in the NEW surface, which is the useful result: this is a design exposure and not a porting one, and every instance is still cheap to change.
- **Resolution:** Each row names its own fix and none is adopted here. The shared shape is worth stating once: EVERY ONE OF THE THREE WANTS A DURATION OR AN INSTANT, AND EACH CAN BE ANSWERED BY COMPARING TWO RECORD STAMPS INSTEAD. That is the clock guard check C construction which this project has already ratified once -- two stamps compared to each other need no clock, and cannot be fabricated by a node that has none.
- **Consequence for the generator:** None today; the generator renders the flags like any other authored field, and the entry-level completeness check already proves they reach the view. The standing obligation is on the AUDIT rather than on the code: this table grows new surface every week, and a D42 exposure enters by someone writing plausible help text, never by a check going red. Re-run the word-bounded audit whenever new_surface changes.
...
- **d42 exposure:** D42 EXPOSURE (ic, audit 2026-08-15). `--list` promises to show WHEN EACH SNAPSHOT WAS TAKEN, and the snapshot is a file under .backup/db/. A file mtime is a filesystem clock and D42 rules it out, so this surface cannot answer its own help text from the artefact it lists. The consequence is a data-model one and it is not recorded anywhere yet: taking a snapshot must WRITE A RECORD, and `--list` reads that record stamp. Otherwise either --list emits a fabricated time or the help text is withdrawn.
- **MCP:** not exposed -- **mutates**
- **Wants review:**
  - uncertain on `exposed_on_mcp`
