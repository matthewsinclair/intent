...
| `surface`  | <subcommand>       | --json                                                 | Query this build's command surface                                                                           | ST0058       | ST0058 AC-00.5. hv ruled 2026-08-25 that retirement is answered OUT OF BAND rather than by a new exit code: `spine.rs` keeps its exit-code decision untouched and a caller branches on MEMBERSHIP in an enumerable roster. hv ruled the SPELLING 2026-08-29, choosing a `surface` root over a bare `retired` leaf and over nesting under `llm` -- `surface/` is already this estate's word for the register, and it gives later surface queries a namespace. The cost was stated and taken: a 14th top-level family holding one leaf today.                          |
...
- **disposition:** new-surface
- **Flags:**
  - `--json` (bool) -- Emit the roster as JSON -- the form a caller branches on
    - **disposition:** keep
