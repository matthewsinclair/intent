...
- **ratified in:** ic, 2026-08-17, on this row's own voice ruling -- NOT parity.md, which does not cite it. Recorded as ratified elsewhere rather than added to parity.md's `Corrected` class unilaterally: that document is ST0056's ratified register and vc stewards it. **The teeth here are `target.stdout_exact`, not the citation** -- this row is asserted against v3 every run, which is a stronger check than membership in a list.
- **stdout exact:**
