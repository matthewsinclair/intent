...
- **Wants review:**
  - uncertain on `exposed_on_mcp`
  - **I LEANED WITHHOLD AND THE DECLARED RULE OVERRODE ME, WHICH IS THE RIGHT WAY ROUND.** My reason was blast radius: this is the only shipped verb that REMOVES committed files, and an agent's mistake here is an absent file rather than a bad edit. The rule this table actually encodes is narrower and is about UNDOABILITY -- MCP withholds a mutation the surface cannot undo -- and `organize` can be undone: every removal is gated on the store reproducing the bytes byte for byte, so re-declaring in `.intentfiles` and re-running restores exactly what was removed. **The same blast-radius argument would withhold `sync --to-store`, whose own row records that a wrong invocation can overwrite this machine's store from a stale extract, and that row ships EXPOSED.** Carving an exception for my own row on the first day it exists, against a rule that already decided a harder case the other way, is how a vocabulary stops meaning anything. Recorded rather than resolved silently, because the generator explicitly refuses a label adjusted to make the number come out and I want it visible that the label was NOT what moved.
