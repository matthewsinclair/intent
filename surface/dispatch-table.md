...
> GENERATED VIEW -- the canon is `dispatch-table.json` beside this file. Regenerate with `intent/st/ST0056/parity/tools/gen_dispatch_table.sh`; do not hand-edit rows. Measured at `9ec1656` on 2026-08-14 by ic.

**Status:** Every v2 family is authored, beside the new-surface rows and the new-surface families. **`families` holds members that are not v2 families, and v2-ness is DERIVED from `v2_source`, never from membership**: the section is the SPINE's unit -- `spine::build` iterates it and a family's leaves are its child rows -- and it was only ever coincidentally also the v2 inventory. `daemon` needed to be there because a `new_surface` row carries subcommands as a bare `values` list, which has no per-verb help, no per-verb flags, and one `recoverability` for verbs whose answers differ (vc's ruling 2026-08-30). Keeping v2-ness separate is deliberate: it is a claim about the PORT and must not silently absorb new surface. Targets marked pending-hv await the usage-convention scope ruling. MEASURED_AT IS A HISTORICAL PIN AND MUST NEVER BE ADVANCED: 9ec1656 precedes the hoist (0ec2ac79) by 1231 commits and names the tree where the v2 surface still existed. This register maps v2 surface to v3 targets, so its stamp is a claim about what was observed, not about how current the file is. Advancing it to HEAD would point a v2-surface measurement at a tree that has no v2 surface -- the same harm the estate_corpus hoist row was carrying until 2026-08-18, where a re-pin-on-HEAD-move rule outlived the migration that expired it. Re-measure only against a tree that still has the v2 surface; there is no such tree after the hoist.
...
- This is the AUTHORED artefact. `dispatch-table.md` beside it is a GENERATED view -- run `intent/st/ST0056/parity/tools/gen_dispatch_table.sh`, never hand-edit the view.
...
| INV-02 | Usage errors exit 1, except in `intent critic`, which exits 2                       | corrected   |
| INV-03 | The project-context gate                                                            | corrected   |
| INV-04 | Exit codes are 0, 1, 2, 3 and 4                                                     | corrected   |
...
| INV-08 | Some commands accept an unknown flag silently at exit 0                             | corrected   |
| INV-09 | Every spelling of one capability agrees about whether it exists                     | new-surface |
| INV-10 | `edit`'s four output modes are mutually exclusive and the default is the terminal's | new-surface |
...
### INV-02 -- Usage errors exit 1, except in `intent critic`, which exits 2
...
- **v2:** bin/intent:206; measured uniform across the commands
...
### INV-04 -- Exit codes are 0, 1, 2, 3 and 4

0 success. 1 the command RAN and the answer is no -- findings, a refused verb, a blocked gate, a usage error. 2 this build could not answer at all, and it carries no verdict about the work -- a declared command that is not implemented yet, `intent critic` rejecting an invocation it cannot act on or finding no rule library, a `--daemon` that cannot be reached; the shipped pre-commit gate fails OPEN on it. 3 REFUSED -- a rule the project armed could not be enforced here (`intent critic`); the gate BLOCKS on it. 4 the estate could not be judged -- `intent doctor` could not read the project it was asked about, so nothing it printed is a verdict; the gate fails open on it. `intent claude hook` propagates the hook's own code by design.
...
- **v2:** `error()` ends in `exit 1`, so every `error "..."; usage` pair has a dead `usage`. Sites in bin/intent_st alone (311, 450, 540, 603, 1048, 1105, 1620).
...
### INV-08 -- Some commands accept an unknown flag silently at exit 0
...
**THE GENERAL FORM OF `ST0058 AC-00.6`, WHICH STATES IT NARROWLY.** That criterion reads _a flag and its subcommand twin agree about whether the capability exists_, and was driven off `intent3 --version` returning rc=0 while `intent3 version` returned the unwired marker. **Separate ROWS can be spellings of one capability (`browse` and `edit --browser`). ARGUMENT SHAPES OF ONE ROW ARE ALSO SPELLINGS**, and that second reading is what the narrow wording misses. **The witness is `fc`, and it is worse than a disagreement -- it is a FALSE STATEMENT**: `intent fc <thread> AC-nn.n` reaches the arm and refuses on the unknown thread at rc=1, while `intent fc <thread> AT-nn.n` answers `fc is a known command that is not implemented yet` with the remedy `nothing in this build provides it`, **for a verb this build provides**. Every other member of the twelve-file marker set is a duplicated TRUE sentence; this is the true sentence emitted where it is false. **A row whose probe answer varies by argument shape declares `varies` and names the axis, and refuses to answer as a boolean** -- a boolean that is genuinely both is not under-specified, it is being asked of the wrong unit, and a `partially_wired` value would encode the confusion rather than resolve it (cc, 2026-08-29).

- **v2:** not-applicable -- v2 had no twin spellings and no unwired marker
- **Target:** `new-surface` -- ratified: vc, 2026-08-29, generalising ST0058 AC-00.6 on ic's `fc` witness and cc's unit argument
- **held by:** `twin_of` names the partner row; a probe result of `varies` names the axis it varies on

### INV-10 -- `edit`'s four output modes are mutually exclusive and the default is the terminal's

**TTY -> TUI, PIPE -> PATH, with three explicit overrides that are mutually exclusive with each other and with the default**: `--editor[=program]`, `--browser`, `--path`. `--editor --path` is already refused and `--browser` joins that set. **`--path` MUST SURVIVE AND IT IS A DOCUMENTED OUTPUT CONTRACT, NOT A CONVENIENCE**: `docs/getting-started.md` carries `$EDITOR "$(intent st edit ST0001 design)"` under test, and command substitution makes stdout a pipe, so that invocation keeps receiving a path. **The bare TTY form alone was rejected by hv (2026-08-29) because it makes behaviour depend on an invisible property of the environment** -- a wrapper, a CI job or an editor plugin gets a different result with nothing in the command saying why. **The register is where the exclusion lives because no single flag can state it**, which is what makes it an invariant rather than a field.

- **v2:** not-applicable -- `st edit` had one output and no modes
- **Target:** `new-surface` -- ratified: hv, 2026-08-29, selecting the TTY form WITH explicit overrides from three options
- **held by:** the four modes are refused pairwise; `--path` is a documented output contract under test in docs/getting-started.md
...
| `st edit`                           | <id> [file]   | --editor, --path                                                         | Open a steel thread file, realising the thread if it is not on disk: in $VISUAL or $EDITOR on a terminal, and as a printed path when stdout is not one | keep        |
...
| `st sync`                           | --            | --write, --width <n>, --format terminal/md                               | Print the steel thread index; with `--write`, write it and each thread's projection to disk                                                            | keep        |
...
- **Flags:**
  - `help`, `--help`, `-h` (bool) -- Print the usage block
    - `help` is a bare word arm, not a flag; grouped here because the spellings share one arm (bin/intent_st:1614)
...
- **Arguments:**
  - `title` (string, arity `1`)
    - Exactly one title; quote it if it has spaces. A second positional is refused as a usage error, exit 1.
- **Flags:**
...
- **Target:** `corrected` -- ratified: hv, 2026-08-15 -- Machine 1 (ThreadStatus) enters at `Triage`, not `NotStarted`. cc landed it at `2aec5f6`. MIGRATED FROM `ratified_in` (ic, 2026-08-22): the machine-read half of the text below is now the declared `rulings` array above; this is the human half and NOTHING PARSES IT. hv, 2026-08-15, ruling applied at commit 2aec5f62
...
- **Flags:**
  ...
  - `--date` `<YYYY-MM-DD>` (string) -- Record this completion date instead of today
    - **disposition:** keep
    - **disposition basis:** hv, 2026-08-28, on issue 0118's fifth limb. The closing verbs stamped TODAY and took no override, so the only open CLI path recorded threads finished in February as finished on the day the estate migrated -- nearly all of Conflab's remaining doctor findings. A COMPLETION DATE IS A CALENDAR FACT IN THE AUTHOR'S LOCAL DAY, NOT A UTC INSTANT (conflab-vc): recovering 64 dates from git scored 64/64 read as local and 63/64 forced to UTC, the miss committed 23:57 +0100. So the flag takes the date the caller states and converts nothing. It is on BOTH closing verbs on the same ground as `--keep` (hv, 2026-08-20): two identical acts with the override on one of them is a surface to be memorised rather than understood.
- **Exit codes:**
  - `0` -- closed
  - `1` -- no id / thread not found
...
- **Arguments:**
  - `id` (st-id, arity `1`)
- **Flags:**
  - `--reason` `<text>` (string) -- Why it is being cancelled. Required: without it the command is refused, and it is recorded as the reason for the new state
    - **disposition:** keep
  - `--keep` (bool) -- Cancel the thread but leave its entry in .intentfiles, so its files stay on disk
    - **disposition:** keep
    - **disposition basis:** **hv RULED THE ASYMMETRY OUT, 2026-08-20, ON ic's QUESTION.** AC-05.2 named `st done --keep` and said nothing about `st cancel`, so the surface shipped with the opt-out on one closing verb and not the other. ic asked whether that was deliberate and guessed it was -- reasoning that cancelling is the stronger statement and an operator is less likely to want the files. **hv ruled the opposite: the asymmetry is not wanted.** The guess was worth recording because it was plausible and wrong: **`--keep` is not about how sure you are that the work is over, it is about whether you still need to READ the files** -- and a cancelled thread is at least as likely to be one you are still mining for what it decided. `st done` and `st cancel` both REMOVE the entry, so both need the same override, and a flag on one of two identical acts is a surface that has to be memorised rather than understood. **Same flag, same word, same meaning: suppress this op's declared edit to the list, and never the close itself.**
  - `--date` `<YYYY-MM-DD>` (string) -- Record this completion date instead of today
    - **disposition:** keep
    - **disposition basis:** hv, 2026-08-28, on issue 0118's fifth limb. The closing verbs stamped TODAY and took no override, so the only open CLI path recorded threads finished in February as finished on the day the estate migrated -- nearly all of Conflab's remaining doctor findings. A COMPLETION DATE IS A CALENDAR FACT IN THE AUTHOR'S LOCAL DAY, NOT A UTC INSTANT (conflab-vc): recovering 64 dates from git scored 64/64 read as local and 63/64 forced to UTC, the miss committed 23:57 +0100. So the flag takes the date the caller states and converts nothing. It is on BOTH closing verbs on the same ground as `--keep` (hv, 2026-08-20): two identical acts with the override on one of them is a surface to be memorised rather than understood.
- **Exit codes:**
...
- **Target:** `corrected` -- ratified: hv, 2026-08-15 -- Machine 1 guards every edge into `Cancelled` with `reason recorded`; cc wired the facade at `2aec5f6` and left the flag for this table to declare. MIGRATED FROM `ratified_in` (ic, 2026-08-22): the machine-read half of the text below is now the declared `rulings` array above; this is the human half and NOTHING PARSES IT. hv, 2026-08-15, ruling applied at commit 2aec5f62
...
- **Arguments:**
  - `id` (st-id, arity `1`)
- **Flags:**
  - `--reason` `<text>` (string) -- Why it is being held. Required: without it the command is refused, and it is recorded as the reason for the new state
    - **disposition:** keep
- **Observed:** nothing to observe -- no v2 antecedent, so there was never anything to run
...
- **Arguments:**
  - `id` (st-id, arity `1`)
- **Flags:**
  - `--reason` `<text>` (string) -- Why it is being reopened. Required: without it the command is refused, and it is recorded as the reason for the new state
    - **disposition:** keep
- **Observed:** nothing to observe -- no v2 antecedent, so there was never anything to run
...
- **Arguments:**
  - `id` (st-id, arity `1`)
- **Flags:**
  - `--reason` `<text>` (string) -- Why it is being reinstated. Required: without it the command is refused, and it is recorded as the reason for the new state
    - **disposition:** keep
- **Observed:** nothing to observe -- no v2 antecedent, so there was never anything to run
...
- **rulings:**
  ...
  - `0.date`: 2026-08-28
  - `0.record`: issue 0121
- **ruling provenance:** WHAT IS FIRST-HAND AND WHAT IS RELAYED, SEPARATED, because the row would otherwise claim more than it can show. FIRST-HAND: issue 0121 is hv's own filing, in writing, against their own tool -- it states the defect, the three-surface reconciliation and three remedy directions, and names direction 3 (`any narrowed render`, not a special case for the default) as the general form. RELAYED BY vc, NOT READ AT SOURCE BY cc: the ruling that chose direction 3 and added the empty-result half -- `when the narrowed result is empty and the store is not, the disclosure IS the output`. The authority is hv either way; what cc has not personally read is the sentence, not the pen.
- **correction:** ISSUE 0121, hv 2026-08-28, filed against their own tool: THE DEFAULT NARROWS TO WIP AND THE OUTPUT SAID SO NOWHERE. v2 does the same, so this moved off `as-observed` -- a filtered view with no filter disclosure is indistinguishable from missing data, and it was read as exactly that (a short list against the thread directories and the store, filed as corruption; the scope reconciles all three surfaces). v3 names the scope, the denominator and the widening flag on every narrowed render, and when the filter empties a NON-empty store the disclosure IS the output. AN EMPTY ESTATE IS UNCHANGED and still prints v2's header -- `0 of 0` would blame a filter for a table that would be empty under any filter at all. THE UNNARROWED PATH IS ALSO UNCHANGED, so `st list --status all` stays byte-identical to `st sync`'s dry run as this row's parity note requires. Not a new convention: `events`, `critic` and `search` already carry it, and `issues list` is the remaining holdout (measured 2026-08-28, `keep`/`as-observed`, its empty case is v2's own string, so it waits on a ruling).
...
- **stderr:** `error: ...`
- **Target:** `as-observed`
- **Note:** `st show` prints the file its `file` positional names, read from the STORE and never from a view on disk (issue 0398, 2026-09-15). `info`, the default, is the cover the MCP resource read also serves; `acceptance` is the contract exactly as `acceptance.md` is hydrated, selected from the one renderer rather than rendered a second time; every other name is the thread's attachment of that name with `.md` appended, so a thread whose files are gone from disk still answers; `all` is the cover and then each file the thread carries under a `-- <file>` separator, in declared order, leaving out the ones it does not carry. A named file the thread does not carry refuses at exit 1 and names `intent st attach`, and a value outside the declared set refuses at exit 1 and names the set, so the row's declared refusals for `file` are reachable, in v3's own words. One refusal is not among the observed codes: an attachment carried as an opaque file whose content this read did not load refuses at exit 2, this build unable to answer rather than a no, because printing nothing at exit 0 would report an empty file. Every BATS test asserting v2's exact `info.md` bytes stays retired under the ratified file-layout class, because `info` is the cover and not the view. **CORRECTED BY THE LANDING THAT MADE IT TRUE.** From 2026-08-17 this note recorded that `st show` never read `file` at all -- `intent st show ST0001 design` and `intent st show ST0001 nonsense` printed the same summary at exit 0, a `keep` row reporting success for a request it discarded (issue 0055, found by cc) -- and before that it claimed `show` read the generated view. v2 cats the file; v3 reads the record the file is rendered from.
- **MCP:** exposed as an agent tool -- read-only
...
- **Arguments:**
  ...
  - `file` (enum, arity `0..1`), default `info` -- one of: `info`, `design`, `impl`, `tasks`, `acceptance`
    - Which file. Unlike `st show`, `all` is refused, because it has no single path.
- **Flags:**
...
- **Target:** `deviate` -- ratified: hv, 2026-08-29, live in vc's session, hv's own selection from three options -- SUPERSEDING two earlier rulings on this question. **The first two were a coordination defect rather than hv changing their mind**: hv chose the TTY form in cc's session, vc then put four options to hv of which NONE was that form, and the result was reported as a ruling. The cause was a false premise -- that a default launch necessarily breaks the documented output contract, which is true of launch-by-default and false of the TTY form. vc returned it to hv with the omission named rather than quietly adding a fifth option. -- behaviour: **TTY-AWARE, WITH AN EXPLICIT SPELLING FOR EACH BRANCH.** v3 opens the file in $VISUAL or $EDITOR when stdout is a terminal, and prints the path when stdout is a pipe or a file. `--editor` forces the open and `--path` forces the print, whatever stdout is; the two together are refused. **THE v2 CONTRACT IS PRESERVED WHERE IT IS ACTUALLY USED**: `$EDITOR "$(intent st edit ST0001 info)"` was the documented form until 46f254484, and command substitution makes stdout a PIPE, so that invocation still receives the path and is covered by a test. The name stops being a misnomer on a terminal and keeps its documented behaviour everywhere a script can see.
...
- **why deviate and not corrected:** `corrected` is a v2 behaviour that is simply WRONG and is fixed; `deviate` is a design consequence of v3. v2's pure emit-path was not a defect -- it was a deliberate, documented, working contract, and the register's own note calls the NAME a historical misnomer the docs work around, not the behaviour a bug. So this is a design change with hv's ratification on the row, which is what `deviate` requires, and it is NOT ratified by class membership.
- **MCP:** exposed as an agent tool -- **mutates**
...
- **Flags:**
  - `--write` (bool) -- Write the index and each thread's projection to disk; without it, print the index and write nothing
    - **disposition:** keep
...
- **v2:** bin/intent_st_zero, reached in v2 by both `intent st_zero` (top level, auto-dispatched) and `intent st zero` (bin/intent_st:1610-1612 execs the binary). Only the second was ever documented by the command itself.
- **Flags:**
  - `--audit-only` (bool) -- Show gap analysis only, no changes
    - **disposition:** retire
    - **disposition basis:** RETIRED 2026-09-11 with its command, `st bootstrap`, by hv's decision 3 (ST0058 AC-00.3). A retired command never reaches clap, so neither can its flags.
...
- **Target:** `retire` -- ratified: hv, 2026-08-15 -- `st_zero` is wrong and the root spelling dies. `zero` was never a verb: it is the NAME of the thing (Steel Thread Zero / ST0000), which is why `intent st zero install` parses noun-then-verb and why the spelling reads as "initialise something to zero" -- not what the command does. It audits which ST0000 deliverables are present, missing or partial in a brownfield project and installs the missing ones. `bootstrap` names that operation and promotes the real verb to the right position. hv considered `initzero` and preferred `bootstrap`. MIGRATED FROM `ratified_in` (ic, 2026-08-22): the machine-read half of the text below is now the declared `rulings` array above; this is the human half and NOTHING PARSES IT. hv, 2026-08-15, ruling applied at commit c1cca8c4
...
- **spelling:** _(declared empty)_
- **retired by:** hv, decision 3 (2026-09-11, ST0058 AC-00.3): STRUCK. It answered `is a known command that is not implemented yet` while canon told agents to run it, and it will not be built. RETIRED RATHER THAN DELETED so the path still refuses at rc=2 with `was retired in Intent v3` (issue 0044) and not clap's generic rc=1; `spelling` is empty because nothing replaces it. The target state it carried before is kept below as history. The ST0000 bootstrap it would have run, and `init --with-st0000` which was refused pending it, are retired with it.
...
- **Cross-reference:** THE surviving face until hv's decision 3 retired it (2026-09-11). The top-level `st_zero` family is the deleted root spelling; see its entry for the divergence cost.
- **recoverability:** one-way
...
- **Flags:**
  - `help`, `--help`, `-h` (bool) -- Print the usage block
    - bare-word arm plus the flag spellings
    - **disposition:** intrinsic
    - **disposition basis:** `intrinsic` is a ratified value in this table's own `flag_dispositions` vocabulary; read the gloss there rather than reproducing it here. Row-specific: clap supplies these spellings and `spine.rs:145-151` already skips them. The spine gets this right by matching on the spelling, which is the inference-from-name that EXP-05 exists to replace with a declaration.
- **Exit codes:**
...
- **Note:** THE URGENT ONE, and it is not urgent in the abstract: its absence is CURRENTLY CORRUPTING this thread's own tracking data. WPs disagree with their own gate (WP-02 Done/BLOCKED, WP-04 Done/BLOCKED, WP-05 Wip/PASS), because adding an AC reopens a WP in the contract while nothing moves the status back. Until this verb exists the ONLY repair is hand-editing the file the CLI exists to own -- which is the same trap `ac satisfy` had before `ac unsatisfy`, in the same tool, found the same way. Second instance of one class; the guard against a third is Machine 2 itself, which now declares the edge whether or not anyone has built it.
- **no op:** `ok: <ID>/NN already WIP`, exit 0 -- **SHIPPED at `d0f345b5`** in the ruled voice. Measured 2026-08-17 by driving the verb twice through the real binary (ic).
- **MCP:** exposed as an agent tool -- **mutates**
- **recoverability:** reversible
- **facade:** wp_reopen
...
- **Note:** THE STATE THE MODEL COULD NOT EXPRESS, AND THE GAP WAS RULED RATHER THAN OVERLOOKED. Machine 2 proposed no Cancelled at WP level -- a WP that stops mattering is a scope change on the thread, not a state on the package -- and flagged it Open for hv if that is wrong. Nothing tracked the question, so the default shipped by silence. A live consumer proved it wrong: scope removed, every AC withdrawn, and wp done refused forever because the gate correctly declines to infer an exemption from an emptied contract (ST0048). The only announced exemption was THREAD-scoped, so closing one unit meant discarding the standing of every one of its ACs. DELIBERATELY NOT GATED: every other close consults the contract; this verb is the announcement that there is no contract to consult, so gating it rebuilds the deadlock it exists to break.
...
- **Arguments:**
  - `stid` (st-id[/NN], arity `1`)
    - The thread whose criteria to list, eg `ST0000`. A work-package suffix such as `ST0000/02` is not read as a filter: it is taken as a thread id and refused.
- **Exit codes:**
  - `0` -- listed
...
- **Target:** `corrected` -- ratified: **RATIFIED 2026-09-15 BY vc UNDER D47** (hv's rulings of 2026-09-15, item 16, `intent/history/20260915-hv-rulings.md`): satisfy-on-first-green stands, and the declared `rulings` entry moved from `provisional` to `ratified` in issue 0334's commit. The record that follows was provisional and is now answered. vc ruling 2026-08-16, provisional pending hv, recorded at commit `d15057e6`, whose diff carries the ruling text, rather than only in an inbox -- filed as issue 0032, which stopped resolving when hv cleared the tracker at `9a9c7799`. **THE WORD `provisional` IS LOAD-BEARING AND STAYS UNTIL hv RULES**: `rulings_check.sh` buckets on it AHEAD of conformance, so this sha does not make the row read as ratified. vc, 2026-08-17, re-anchoring this field while HOLDING `ratified_in`, because a sha there would assert a ratification that has not happened. NOT cited by parity.md -- this row is ratified ELSEWHERE and corrected_check.sh should report it in that bucket. MIGRATED FROM `ratified_in` (ic, 2026-08-22): the machine-read half of the text below is now the declared `rulings` array above; this is the human half and NOTHING PARSES IT. vc ruling 2026-08-16 (provisional pending hv), recorded on issue 0032
...
- **why corrected and not as observed:** The `Corrected` class is defined as `a v2 behaviour that is simply wrong and is fixed rather than faithfully reproduced`, and `as-observed` is for behaviour v3 CHOOSES to reproduce. the wording at `e23a8453` is that the combining rule `was chosen by an early-return rather than by a decision` -- **you cannot faithfully reproduce a decision nobody made**, and reproducing it is what parity.md forbids in its own sentence: laundering a v2 defect into a v3 requirement. **THE ATTRIBUTION TO hv IS STRUCK, AND vc STRUCK IT AGAINST THEMSELVES.** This field said `hv's own wording`. Traced: the sentence is in 0032's FILING text, the issue carries `reporter: matts`, vc's ruling quoted it as hv's, and this field repeated it. **The only thing under the attribution was that metadata field**, and hv's own clearing ruling says what it is worth -- every row carried `reporter: matts` and every one was found by us. Git author settles nothing either: every node commits as `Matthew Sinclair <hello@matthewsinclair.com>` by standing policy. The diff that introduces the sentence is `e23a8453`, a node's own verification commit. **WHY IT MATTERS MORE HERE THAN ANYWHERE ELSE: this row is awaiting an hv ruling, so the field would have manufactured the adjudicator's agreement on the question they have not yet been asked** -- hv arriving to rule and reading that hv already worded the core of the argument. The reasoning is untouched by the strike: the class definition fits verbatim and you cannot faithfully reproduce a decision nobody made. It never needed the name and was weakened by borrowing it. **THE RULE, vc 2026-08-17: an attribution names a person only where an ACT of that person is on the record -- a ruling, a message, a commit body. A metadata field recording who a row was filed UNDER is not evidence of who WROTE it, and in this project it is specifically known not to be.** Sibling of the `AUTHORITY_HV` bug in `ratified_in_check.sh` (renamed `rulings_check.sh` 2026-08-23; that regex is retired): that one matched a name in the wrong POSITION, this one took a name from the wrong FIELD.
- **the conflict ic raised is EMPTY and that is what decides it:** ic recorded this row on 2026-08-15 as two ratified things pointing opposite ways -- AC-04.3 requires v3 to reproduce v2's gate VERDICTS, and correcting 0032 would change verdicts on exactly the contracts AC-04.3 measures against. **It changes none.** vc re-measured every AT row: the multi-AT ACs are AC-00.7 (AT-00.5 red + AT-00.7 to-write -> OR unsatisfied, AND unsatisfied) and AC-03.7 (AT-03.7 green + AT-03.9 green -> OR satisfied, AND satisfied). **Zero verdicts move; AC-04.3 is untouched and there is no parity break to ratify.** OR and AND can only diverge on a multi-AT AC holding a green beside a non-green, and no such row exists.
...
- **Flags:**
  ...
  - `--kind` `test|non-test` (enum) -- Test-backed or not
    ...
    - **disposition:** keep
- **Observed:** nothing to observe -- no v2 antecedent, so there was never anything to run
- **Target:** `new-surface` -- ratified: ic, 2026-08-26, on hv's ruling over issue 0088 -- hv minted AC-08.6 over cheaper remedies, so the thread stops being closed until the verb exists. WP-08's own enumerated-gap table named `Criterion create` as MISSING and no criterion covered it: AC-08.5 covers every writable FIELD, and CREATION IS NOT A FIELD, so the thread's gate PASSed at 51/51 over a stated deliverable. Driven 2026-08-25: `ac --help` listed nine subcommands and `at --help` five, and NOT ONE OF THE FOURTEEN CREATED ANYTHING -- every arm is a transition on a row that already exists. The route until now was a hand-edit of `.canon/st/<ID>.json` plus `sync --to-store`, WHICH IS HOW AC-08.6 ITSELF REACHED CANON. Per WP-08 the id origin is CALLER-ASSIGNED, so the shape is an idempotent PUT to the entity address rather than a POST to the collection. Spelled `new` to match `st new` and `wp new` rather than adding a fifth house word for create; `issues add` is the existing odd one out and is already filed. Builder ic; COVER TAKEN BY vc, who drives the falsifiers independently at HEAD -- ic must not be both builder and verifier on a row blocking ST0057's gate, which is hv's own AC-08.5 precedent on this same WP.
- **spelling:** _(declared empty)_
- **supersession:** **SUPERSEDED IN PART, 2026-08-29, AND THE SUPERSESSION IS RECORDED HERE RATHER THAN LEFT AS A DIVERGENCE BETWEEN THIS ROW AND THE BINARY.** hv ruled 2026-08-28 that a verb named `add`/`new` must FAIL on an existing key rather than replace it (issue 0131). A later first-hand ruling from hv on the same subject supersedes a ratification, so `ac new` is no longer idempotent: a second call on the same id refuses, writes nothing, and names `ac edit`. **The CALLER-ASSIGNED half of ic's ratification below is untouched and still correct** -- what moved is what a second call does, not where the id comes from, so the `POST`-to-a-collection reasoning still holds. The row is amended rather than left declaring `idempotent` over a binary that refuses, which is the `fileindex` shape hv already ruled on: when the table and the binary disagree, the table was the wrong half. **The refusal is at THIS door only** -- `Facade::put` at the entity address is still a PUT, because replace-at-an-address is the contract of the HTTP and GraphQL faces and `ac.put` is its declared op. Amended by cc; ic offered the pen on their own line first. **`recoverability` READ `idempotent` ON BOTH CREATE ROWS AND NOW READS `reversible`, WHICH IS WHAT ITS THREE SIBLINGS ALREADY SAID.** `st new`, `wp new` and `issues add` are all `reversible`; `ac new` and `at new` were the two that claimed idempotence, and hv's refusal ruling makes that claim false rather than merely unfashionable. A first attempt at this amendment minted a fourth value, `refuses-on-collision`, for a closed domain of three -- caught by `recoverability_values` and its enforcement arm in `dispatch.rs`, which is the instrument working. **Refusing on a taken key is not a recoverability class**: it is what the verb does on collision, which the supersession note above is the right home for. The field answers one question -- can this be undone -- and creating a criterion is undone the way creating a thread is. **AND THE SUPERSESSION IS DECLARED IN `target.rulings`, NOT ONLY IN THIS PROSE.** vc required the row to carry authority, date and a resolving record. It was carrying them in a prose `authority` field that `rulings_check.sh` does not read -- a claim about provenance sitting beside the machine-checked field rather than in it, which is the class this table keeps meeting. **These are the FIRST new-surface rows to declare `target.rulings`: none before them did, so the others remain prose-only and that is a population question for vc, not something this commit settles.** The two rulings are declared separately because both were made: ic ratified the shape and hv later ruled its create semantics.
- **rulings:**
  - `0.state`: ratified
  - `0.authority`: ic
  - `0.date`: 2026-08-26
...
- **Arguments:**
  ...
  - `atid` (at-id, arity `1`)
- **Flags:**
  - `--note` `<text>` (string) -- Why the row is in this state, kept as its note. A row that already has a note keeps it: the new text must extend it or the command refuses (`at edit --note` replaces a note). Absent, the note is unchanged
    - **disposition:** keep
- **Exit codes:**
  - `0` -- set
  - `1` -- AT not found
...
- **Target:** `corrected` -- ratified: ic, 2026-08-15, ruling cc's parity question -- KEEP THE GUARD. v2 refuses green unless the AT is currently red (bin/intent_acceptance:1325). v3's at_set takes any status from any status. This is v3 more OPEN and less faithful, not more closed. MIGRATED FROM `ratified_in` (ic, 2026-08-22): the machine-read half of the text below is now the declared `rulings` array above; this is the human half and NOTHING PARSES IT. ic, 2026-08-15, ruling recorded at commit `20e8c4b3` (`st(0056): ac unsatisfy recorded, at green RULED, ...`). **The previous value was `ic, with the `at green` row's own basis` -- the self-citation case, and the precedent I copied when writing my own rows before vc's sweep fixed the grammar's requirement.** A ratification pointing at the row it ratifies is not independent, by exactly the argument that a check is not independent when the same head wrote the needle and the checker: the source has to be something other than the thing under test.
...
- **consequence:** Repeated instances on 2026-08-15 alone of a green that proved nothing, none of which had ever been seen red: four vacuous greps that never opened a file, a normaliser that silently did nothing under BSD sed, and a `touch`ed canary whose empty diff sent the run down the wrong branch. v3 restores the from-red guard.
...
- **MCP:** exposed as an agent tool -- **mutates**
- **recoverability:** one-way
- **recoverability anomaly:** ONE-WAY BY MEASURED BEHAVIOUR, NOT BY DESIGN -- issue 0033. `intent at red|green|na` DESTROYS the row's authored note, so the round trip moves the status back and does NOT restore the prior state. vc measured it on themselves with the issue in working memory: AT-03.12 lost almost all of its authored contract, and `intent at lint` reported every row conforming immediately afterwards. **The transition graph multiplies it: `to-write -> green` is refused and green is reachable only from `red`, so recording a passing test costs a rewrite twice over.** Authored text stands at risk across many rows. **Classified against SHIPPED BEHAVIOUR on vc's ruling** -- a field describing what a command is SUPPOSED to do is the `doctor` failure, where the declaration outlived its subject in silence. This row becomes `reversible` when 0033 is fixed, and that is the field tracking reality rather than drifting from it.
- **facade:** at_set
...
- **voice ruling:** **RULED DRIFT, AND FIXED at `d14cd0b5`.** v2 prints `ok: <AT> -> red`; v3 printed `ok: <AT> red`. It was drift rather than a voice ruling, and v3's own output proved it: `issues close` and `issues open` KEEP the arrow (`ok: issue 0001 -> CLOSED`) and both reproduce v2 exactly, so the arrow is v3's own convention for a state transition and the AT family was the only place it had gone missing. HEAD prints `ok: AT-01.1 -> red` and `ok: AT-01.1 already red`, measured at `67814555`. Restoring it cost nothing and fixed those rows; dropping it everywhere for consistency would have broken two that already passed. **This row was not reported by anyone** -- it was found by measuring the siblings of the row that was, on the rule this row's own family keeps re-teaching: a correction reaches the field that was reported and stops there unless somebody sweeps the rest.
- **MCP:** exposed as an agent tool -- **mutates**
- **recoverability:** one-way
- **recoverability anomaly:** ONE-WAY BY MEASURED BEHAVIOUR, NOT BY DESIGN -- issue 0033. `intent at red|green|na` DESTROYS the row's authored note, so the round trip moves the status back and does NOT restore the prior state. vc measured it on themselves with the issue in working memory: AT-03.12 lost almost all of its authored contract, and `intent at lint` reported every row conforming immediately afterwards. **The transition graph multiplies it: `to-write -> green` is refused and green is reachable only from `red`, so recording a passing test costs a rewrite twice over.** Authored text stands at risk across many rows. **Classified against SHIPPED BEHAVIOUR on vc's ruling** -- a field describing what a command is SUPPOSED to do is the `doctor` failure, where the declaration outlived its subject in silence. This row becomes `reversible` when 0033 is fixed, and that is the field tracking reality rather than drifting from it.
- **facade:** at_set
...
- **Target:** `as-observed`
- **no op:** `ok: <AT> already n/a`, exit 0, stderr empty -- **RE-MEASURED at `67814555`** by driving the verb twice through a binary built from a clean `git archive` extract of HEAD, because files under `native/` were dirty under cc at the time and the worktree binary would have attributed unlanded work to HEAD (ic, 2026-08-17). The previous value read `already n-a` and cited `d0f345b5`. That citation was honest provenance and it still went stale: `d14cd0b5` gave `AtStatus` a `display()` and the self-loop now takes its word from the enum. **A field that cites the commit it was measured at cannot say whether that commit is still HEAD** -- issue 0061, and the `instant` variant of `parity.md`'s class at rest. Re-measured rather than retyped: editing the token by hand would have produced a correct-looking value nothing measured, which is exactly how `n-a` reached the column that records v2.
- **voice ruling:** **RULED A BUG, AND BOTH HALVES FIXED at `d14cd0b5`.** v2 prints `ok: <AT> -> n/a`; v3 printed `ok: <AT> na` on a movement and `ok: <AT> already n-a` on a self-loop -- different spellings from one verb, because the movement arm passed `state`, the word the USER TYPED, while the self-loop read the enum. Neither half was ratifiable: the token was a migration hazard rather than a style choice, since `views.rs` writes it into the generated `acceptance.md` and v2's linter rejects `n-a` at L1 (issue 0056, closed). HEAD prints `ok: AT-01.2 -> n/a` and `ok: AT-01.2 already n/a`, measured at `67814555`. **THE `help` STRING ABOVE OUTLIVED THIS FIX BY A DAY, AND THAT IS THE PART WORTH KEEPING.** Nothing renders help from the enum -- it is an authored literal -- so the single site where the wire spelling still reached a human was the one site no code fix could pass through. Filed as 0061 and corrected here.
- **MCP:** exposed as an agent tool -- **mutates**
- **recoverability:** one-way
- **recoverability anomaly:** ONE-WAY BY MEASURED BEHAVIOUR, NOT BY DESIGN -- issue 0033. `intent at red|green|na` DESTROYS the row's authored note, so the round trip moves the status back and does NOT restore the prior state. vc measured it on themselves with the issue in working memory: AT-03.12 lost almost all of its authored contract, and `intent at lint` reported every row conforming immediately afterwards. **The transition graph multiplies it: `to-write -> green` is refused and green is reachable only from `red`, so recording a passing test costs a rewrite twice over.** Authored text stands at risk across many rows. **Classified against SHIPPED BEHAVIOUR on vc's ruling** -- a field describing what a command is SUPPOSED to do is the `doctor` failure, where the declaration outlived its subject in silence. This row becomes `reversible` when 0033 is fixed, and that is the field tracking reality rather than drifting from it.
- **facade:** at_set
...
- **Flags:**
  ...
  - `--note` `<text>` (string) -- A note for the new row: why it starts in this state
    - **disposition:** keep
- **Observed:** nothing to observe -- no v2 antecedent, so there was never anything to run
...
- **spelling:** _(declared empty)_
- **supersession:** **SUPERSEDED IN PART, 2026-08-29, ON THE SAME hv RULING AND FOR THE SAME REASON AS `ac new`'s.** hv ruled 2026-08-28 that a verb named `add`/`new` must FAIL on an existing key rather than replace it (issue 0131), so this verb is no longer idempotent: a second call on the same id refuses, writes nothing, and names `at edit`. **The whole L2-L5 ratification below is untouched and now runs through one shared check** that `at edit` is held to as well, so a re-cite meets the same grammar a create does. **AND ONE MITIGATION IS RETIRED BY THIS, NOT WEAKENED:** the verb used to READ the stored row and carry its `note` and `legacy` forward, because a create on an existing id destroyed every field it did not set -- six ST0061 notes, measured. With the refusal there is no stored row to carry from, so the carry is unreachable and gone rather than left as dead code reading as live protection; the preservation is now `at edit`'s whole purpose. **AND THE SUPERSESSION IS DECLARED IN `target.rulings`, NOT ONLY IN THIS PROSE.** vc required the row to carry authority, date and a resolving record. It was carrying them in a prose `authority` field that `rulings_check.sh` does not read -- a claim about provenance sitting beside the machine-checked field rather than in it, which is the class this table keeps meeting. **These are the FIRST new-surface rows to declare `target.rulings`: none before them did, so the others remain prose-only and that is a population question for vc, not something this commit settles.** The two rulings are declared separately because both were made: ic ratified the shape and hv later ruled its create semantics.
- **rulings:**
  - `0.state`: ratified
  - `0.authority`: ic
  - `0.date`: 2026-08-26
...
- **Flags:**
  ...
  - `--covers` `<ac-id>` (string) -- The AC id(s) this test covers, replacing the current set; repeatable
    - **arity:** 1..n
    - **disposition:** keep
  - `--note` `<text>` (string) -- Replace the row's note outright; `at green`, `at red` and `at na` can only extend a note with their `--note`
    - **disposition:** keep
- **Observed:** nothing to observe -- no v2 antecedent, so there was never anything to run
- **Target:** `new-surface` -- ratified: cc, 2026-08-29, on hv's 2026-08-28 ruling over issue 0131, sequenced to cc by vc. **NO ACCEPTANCE ROW IS CLAIMED HERE, DELIBERATELY.** `ac new` and `at new` carry AC-08.6 / AC-08.7; these verbs were ruled by hv into an existing package rather than minted with a criterion, and the builder writing their own acceptance criterion for their own build is the shape hv's AC-08.5 precedent exists to prevent. The contract row is OWED and is vc's to place.
- **Note:** **THE RE-CITE DOOR, AND THE MEASURED LOSS THAT ARGUES FOR IT.** `at new` on an existing id was a full replacement, so moving a test to its new file ate the row's `note` in the same call -- six ST0061 notes destroyed by a single re-cite on this repository and recovered from a git blob. The safe route was a hand-edit of canon plus `sync --to-store`, ie the undocumented one; the verb built for the job was the one that lost the work. Here absence is silence: a field nobody names is a field nobody changes, so `note` and `legacy` survive by construction rather than by a carry that had to be remembered. **`--note` ARRIVED 2026-09-02 (issue 0207, vc ruled (c) under hv's pen) AND IS NOT A CONVENIENCE.** `at green` / `at red` / `at na` took a `--note` that REPLACED the field wholesale and silently, on the verbs you reach for at close -- `AT-00.12` lost most of its text and the author did not notice. Those verbs now refuse a note that does not contain the one it replaces, which leaves this as the only deliberate spelling for replacing one. **It carries no such refusal, and that asymmetry is the mechanism: a door that refuses is not a door.** The reasoning in this very entry -- a field nobody names is a field nobody changes -- was applied to the re-cite and stopped one field short of the field it was protecting.
- **no op:** `ok: <AT> already unchanged`, exit 0 -- driven 2026-09-14 through the delivered 3.0.3 pair (ic, issues 0324 and 0337): a row's note set to the note it already carried, then its kind set to the kind it already had, each printed that line at rc 0. `at edit` joins `populations.self_loop` because `AcceptanceTest.kind` became a machine whose single edge is this door (vc).
- **consequence:** Refuses an id the thread does not have (`NoSuchTest`), and refuses a call that names NO field (`NothingToChange`) rather than reporting `unchanged` at exit 0 to a caller who believes they changed something. The prospective row is held to the same L2-L5 grammar `at lint` enforces and `at new` is held to, through the same one check -- so a re-cite to a file that does not exist, or to one not carrying the row's literal id, is refused with nothing written. **`--status` IS DELIBERATELY ABSENT**: `at green` / `at red` / `at na` already own status as a declared state machine with an envelope per movement, and a second writer would be a divergent copy of a transition. **`--kind` IS PRESENT, AND THIS SENTENCE DENIED IT UNTIL ISSUE 0396**: the verb has taken it since issue 0146, and issues 0324 and 0337 made it the single edge of `AcceptanceTest.kind` -- a re-kind re-enters the status at `AtStatus::entry`, and is refused when the row's status cannot hold the new kind.
- **MCP:** exposed as an agent tool -- **mutates**
- **recoverability:** reversible
- **facade:** at_edit
...
- **stderr:** `error: ...` on stderr (INV-01)
- **Observed notes:** One of the commands whose `--help` correctly exits 0 -- it is NOT an INV-07 member.
- **Target:** `as-observed`
- **MCP:** not exposed -- read-only
...
- **Flags:**
  ...
  - `--from` `<file>` (string) -- Read the issue's prose from a file
    ...
    - **exposed on mcp:** false
- **Exit codes:**
  - `0` -- created -- prints `created: <path to the issue's md file>` then `<ID>:<TITLE>` (bin/intent_issues:187-188)
  - `1` -- no title -- `error: Issue title is required`
  - `1` -- outside a project -- `error: not in an Intent project directory` (INV-03)
- **stdout:** `created: <path to the issue's file>` then `<ID>:<TITLE>`. **v2, measured 2026-08-17:** `created: intent/issues/OPEN/0001/0001-probe-two.md` / `0001:Probe two`. **v3 at `34c6a3ae`, measured:** `created: /Users/.../intent/issues/0002.json` / `0002:Probe two`. **THE LINES HAVE DIFFERENT ANSWERS AND THE ROW HAS ONE `target.state` -- see `notes`.**
- **stderr:** `error: ...` on stderr (INV-01)
...
- **Flags:**
  - `--body` `<text>` (string) -- The issue's prose, inline
    - **disposition:** keep
    - **disposition basis:** New with the verb. `issues add` carries the same spellings and the same one-of-them rule, read through the shared `issue_body` helper, so the flags are one declaration honoured twice rather than two that agree today.
  - `--from` `<file>` (string) -- Read the issue's prose from a file
    - narrowed off MCP because it names a local file for the CLI to read -- an MCP agent reads its own files and passes body
    - **disposition:** keep
    - **disposition basis:** New with the verb. `issues add` carries the same spellings and the same one-of-them rule, read through the shared `issue_body` helper, so the flags are one declaration honoured twice rather than two that agree today.
...
| command        | args        | flags                             | help                                                                                                                                                 | disposition |
| -------------- | ----------- | --------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- |
| `todo`         | [command]   | --json, --format terminal/md/json | Show the DOING / TODO / DONE view of threads and work packages, as intent/todo.md holds it; writes nothing, and `intent todo update` writes the file | keep        |
| `todo list`    | --          | --json, --format terminal/md/json | Show the DOING / TODO / DONE view of threads and work packages, as intent/todo.md holds it; writes nothing, and `intent todo update` writes the file | keep        |
...
| `todo notdone` | <specifier> | --                                | Refused in this build: a reopen must record its reason -- use `intent st reopen <id> --reason <text>` or `intent wp reopen <spec> --reason <text>`   | keep        |
| `todo toggle`  | <specifier> | --                                | Refused in this build: close with `intent todo done <spec>`, and reopen with `intent st reopen` or `intent wp reopen` and a `--reason`               | keep        |
...
- **read or mutate correction:** CORRECTED 2026-09-25 (issue 0573). Was `mutate` and `idempotent`, carried from v2, whose bare `todo` and `todo list` generated intent/todo.md when it was absent (`bin/intent_todo:246`); the `mcp_review` on this row records that v2 behaviour. Measured on the v3 binary: with intent/todo.md absent, stale or current, bare `todo`, `todo list` and `todo --json` leave it exactly as found, and `todo update` is the only writer -- as this row's own help says.
- **facade:** todo_buckets
...
- **Flags:**
  - `--flush` (bool) -- Advance the DONE watermark to now, clearing the DONE view
    - **disposition:** keep
    - **disposition basis:** **REINSTATED by hv, 2026-08-26, on seeing what D44 produced: a `todo.md` at Lamplight that was mostly completed items.** hv verbatim: _`The whole point of a todo file is that it shows DOING and TODO work. That is it.`_ and _`I WANT PARITY`_. **The D44 reasoning below was not wrong about the mechanism, only about what to do with it -- and its own quoted sentence argues the other way once read forward: the database holding the complete record is exactly why the FILE does not need to.** What went with the flags was the only BOUND on the DONE bucket, and the display window that replaced it was ruled terminal-only, so the committed view was left carrying every completion ever. The watermark returns as an EVENT rather than a line inside the generated file, which keeps the one defect v2 really had: v2 grepped the instant back out of `todo.md`, so deleting a disposable file resurrected everything ever flushed. Superseded reasoning follows verbatim. RETIRED by D44 (hv, 2026-08-16). Not a deprecation: the operation has no referent in v3. Both flags MUTATED the artefact because in v2 the artefact WAS the record; under D01 the db is the SSOT and `todo.md` is a generated view, so there is nothing to prune and nothing to flush -- only a question of how much of the record to SHOW, which is a display window and is a different flag. hv verbatim: `All of the data is in the db so we can (re)generate whatever we need when we need it.` Reclassified from `keep`, which was correct as a v2 observation and wrong as a v3 target the moment D01 landed.
...
- **v2 help file:** none
- **Owning work package:** WP-06
- **v2 BATS coverage (a record -- the cut deleted that suite):** **HOLE -- nothing in the estate invokes it**

- **This command produces NO OUTPUT AT ALL in a project** -- nothing on either stream, exit 0, measured. `bin/intent_config` is primarily a LIBRARY: `bin/intent:211` sources it for `load_intent_config`, and the executable path is close to vestigial.
...
| command      | args          | flags | help                                                                                  | disposition |
| ------------ | ------------- | ----- | ------------------------------------------------------------------------------------- | ----------- |
| `config`     | [command]     | --    | Display the resolved project configuration -- not implemented in this build (exits 2) | pending     |
| `config get` | <key>         | --    | Print one configuration value -- not implemented in this build (exits 2)              | new-surface |
| `config set` | <key> <value> | --    | Set one configuration value -- not implemented in this build (exits 2)                | new-surface |
...
- **Arguments:**
  - `command` (subcommand, arity `0..1`)
    - Optional: `get` or `set`; bare `intent config` is its own action, not a default verb. Not implemented in this build: every `intent config` invocation exits 2 with `a known command that is not implemented yet`.
- **Exit codes:**
...
- **keys backup:**
  ...
  - `keys.1.meaning`: Base cadence: `hourly`, `daily`, `weekly`, or a whole number of hours or days such as `12h` or `7d`. A snapshot is due once the newest good one is nine tenths of this period old, and intentd's sweep and `intent explore` take it then.
  - `keys.1.note`: DUE AT NINE TENTHS OF THE PERIOD, SO `daily` DOES NOT MEAN EXACTLY 24 HOURS. `intent explore` is opened by habit rather than by a timer, and at exactly 24h a project opened at 09:00 each day after a 09:05 snapshot would be backed up every other day. `doctor` reports a backup as stale only at twice the period, so the two never disagree. THERE IS NO `off` VALUE: `backup.enabled: false` stops scheduled backups, and a second spelling here could disagree with it. `values` lists the three words; a duration is `<N>h` or `<N>d` with N at least 1, and anything else is reported by `doctor`, by intentd and by `intent explore` rather than rounded. ENUMERATED, NOT A CRON EXPRESSION, and that is a decision rather than a simplification. A cron string is a mini-language embedded in a hand-edited config file -- the shape behind the 0012 quoting scar -- and it is SILENTLY wrong when mistyped rather than refused. D35 fixes the retention tiers at day/week/month, so arbitrary cadences have nowhere to land anyway; a schedule coarser than a tier simply leaves that tier unfilled, which needs no special case.
  ...
  - `keys.5.key`: backup.keep
  - `keys.5.type`: integer
  - `keys.5.default`: null
  - `keys.5.meaning`: When set, the whole pruning rule: keep the newest N good snapshots, and do not read `backup.retain`.
  - `keys.5.note`: ONE RULE, NEVER TWO. Absent means the `retain` tiers decide; set means `keep` decides alone, and `doctor` names a config that sets both, because the tiers then look configured and prune nothing. AT LEAST 1: a keep of 0 would remove the snapshot the same cycle had just taken, so it is refused when the config is read. A flat count is the operator's trade: `keep: 20` on an hourly schedule holds less than a day, which is why the tiers are the default. Each snapshot is a full copy of the store, so disk use scales with the store's size times the snapshots kept, and a project with a large store may want a smaller keep.
...
- **Arguments:**
  - `key` (config-key, arity `1`)
    - A dotted path to a nested value, eg `backup.retain.daily`. Not implemented in this build: `intent config get` exits 2 with `a known command that is not implemented yet`.
- **Observed:** nothing to observe -- no v2 antecedent, so there was never anything to run
...
- **unknown key refuses:** AN UNKNOWN KEY EXITS NON-ZERO AND SAYS SO. It must NOT print an empty line at exit 0, because empty is indistinguishable from a key legitimately set to empty -- the same absence-as-meaning collapse that makes an absent retention count different from `0`. A user scripting against this needs the cases separable, and the only place that can be decided is here.
...
- **Flags:**
  - `--with-st0000` (bool) -- Bootstrap all ST0000 deliverables after init
    - **disposition:** retire
    - **disposition basis:** RETIRED 2026-09-11 by hv's decision 3 (ST0058 AC-00.3), in the same commit as `st bootstrap`: it is the same door by a second entrance. It shipped as `keep` with an arm that refused it because the ST0000 bootstrap was not implemented, and that refusal promised the step would work `once the command lands` -- false once the command was struck.
...
| command  | args | flags                                                                                      | help                                        | disposition |
| -------- | ---- | ------------------------------------------------------------------------------------------ | ------------------------------------------- | ----------- |
| `doctor` | --   | --fix/-f, --verbose/-v, --quiet/-q, --format text/json, --scope live/all/closed, --help/-h | Diagnose common Intent configuration issues | keep        |
...
- **Flags:**
  ...
  - `--format` `text|json` (enum) -- Output format
    - **default:** text
    - **disposition:** keep
    - **disposition basis:** NEW IN v3, NO v2 ANTECEDENT, and the request is recorded in the code by the node that hit the need rather than inferred here: `render.rs` says the unattached list stays inline until `doctor` has a machine face to carry it, `which needs a surface row and is not mine to add`. This is that row. SPELLING RULED BY hv 2026-08-21 on a criterion that is not a headcount: `--format` where the verb has formats OTHER than json, `--json` where it does not. The declared surface split evenly (`--json` on `issues show`/`todo`/`todo list`; `--format` on `critic`/`export`/`events`), so counting instances could not decide it -- the spellings encode different ARITIES, which is why the tie was a mis-cut population rather than a close question. CLAP IS DELIBERATELY NOT THE ENFORCER: an unknown value is refused in the renderer at exit 1, never by a `value_parser` at exit 2, because 2 is INV-04 USAGE and the pre-commit gate fails OPEN on it.
    - **exposed on mcp:** false
  - `--scope` `live|all|closed` (enum) -- Which threads to report on
    - **default:** live
    - **disposition:** keep
    - **disposition basis:** NEW IN v3, NO v2 ANTECEDENT. hv RULED IT 2026-09-07 in these words: a narrow default is sensible, `but you should be able to specify the scope with a --scope param`. THE MEASUREMENT THAT PROMPTED IT, taken across the fleet the same day: a large share of the findings sat on CLOSED threads, on Conflab and Lamplight, and not one was work anybody was going to do -- a completed thread whose work packages predate a field is history, and reporting history as a fault buries the live signal under it. `all` is the behaviour every build before this one had and `closed` is the deliberate history audit, so what the default hides stays reachable without re-reading everything. THE DEFAULT IS ONLY SAFE BECAUSE THE NARROWING ANNOUNCES ITSELF: `0 finding(s)` over live threads is byte-identical to `0 finding(s)` over everything, so `Report` carries `out_of_scope` and the summary prints it whenever it is non-zero. Without that count this flag is a denominator attack wearing a flag, which is the error vc committed on 2026-09-07 by reporting estates pristine off a count that could not see pages of output. SCOPE COVERS THE MODEL CHECKS ONLY: view-skew and unattached files are DISK facts and stay always-reported, because a hand-edited generated view is a divergence `sync --to-disk` repairs whatever its thread's status. CLAP IS DELIBERATELY NOT THE ENFORCER, as on `--format` above: an unknown value is refused in the renderer at exit 1, never by a `value_parser` at exit 2, because 2 is INV-04 USAGE and the pre-commit gate fails OPEN on it.
    - **exposed on mcp:** false
  - `--help`, `-h` (bool) -- Print the usage block
    - **disposition:** intrinsic
    - **disposition basis:** `intrinsic` is a ratified value in this table's own `flag_dispositions` vocabulary; read the gloss there rather than reproducing it here. Row-specific: clap supplies these spellings and `spine.rs:145-151` already skips them. The spine gets this right by matching on the spelling, which is the inference-from-name that EXP-05 exists to replace with a declaration.
...
- **new obligations:**
  - STATUS-VS-GATE DISAGREEMENT (hv, 2026-08-15, ratifying the state machines): `doctor` reports any unit whose status disagrees with its gate. Refusal on the way in is not enough on its own -- a status that was TRUE when it was set becomes a false green the moment its contract grows, which is how WPs came to disagree with their own gates while every one of them had been closed legitimately.
...
- **parity.md's table said this family was just `sync`. Measured, its verbs are** init, generate, sync, validate, template.
...
| `agents template` | [subcommand] [name] | --                | Manage AGENTS.md templates                      | retire      |
...
- **Exit codes:**
  - `0` -- regenerated
- **stdout:** **Measured 2026-08-17 against v2.19.0 with the streams split (nothing on stderr):** `Syncing AGENTS.md with latest project state...` / `ok: AGENTS.md updated at project root.` / `    previous version saved to AGENTS.md.bak` (four-space indented continuation). The row previously said `confirmation`, which is true and says nothing a builder could reproduce.
- **stderr:** EMPTY on the success arm, measured. No failure arm was reached.
- **Side effects:**
...
- **Target:** `retire` -- ratified: ic 2026-08-17, on vc's split -- **THE CLASS IS DECIDABLE AND THE REMEDY IS NOT, AND ONLY THE CLASS WAS EVER MINE.** The test for `corrected` is whether the v2 behaviour is simply wrong, and `list` printing `rust` while `show rust` answers `not found` IS wrong -- not as an implementation error but as an OUTPUT MAKING A FALSE CLAIM ABOUT ITS SIBLING. How it came to be wrong is a question about blame; `corrected` is a question about the behaviour. Reproducing it would launder a v2 defect into a v3 requirement, which `parity.md` forbids in as many words. **Ruling the class NOW is the point rather than a formality**: `as-observed` left the row BUILDABLE AS-IS, so the one outcome nobody wants was the one the register was licensing while the question sat open. All three candidate remedies satisfy `corrected`, so nothing waits on the remedy being chosen. **The INV-01 half needs no ruling from anyone** -- errors on stdout, one with no `error:` prefix, is `corrected` on its face. **THE REMEDY IS NOW RULED TOO, BY hv, AND THE TWO RULINGS ARE DELIBERATELY SEPARATE FIELDS.** The class (this field) says v2's behaviour is wrong and v3 must not reproduce it; the remedy (`remedy`) says what v3 does instead. Collapsing them would have made the class contingent on the remedy -- and the class was decidable months before anyone knew which of three answers to pick, which is exactly why it was ruled first. MIGRATED FROM `ratified_in` (ic, 2026-08-22): the machine-read half of the text below is now the declared `rulings` array above; this is the human half and NOTHING PARSES IT. ic, 2026-08-17, CLASS ruled on vc's split; the measurement and argument are at commit `632d9d86`. hv, 2026-08-17, REMEDY ruled in session to ic and recorded verbatim in this commit's message. **The earlier value cited `issue 0058` and that citation no longer resolves**: hv cleared the tracker at `9a9c7799` (findings live in commits; an issue is an external user's report against a release and nothing else), so the sha is now the only record here that leads anywhere. **CORRECTING THE SENTENCE THAT STOOD HERE, WHICH GLUED TWO POPULATIONS INTO ONE NUMBER.** It claimed a number of other `target` fields still cite an issue number with no sha beside it. Fields that carry no sha are a wider set than the ones lacking any record, because the grammar accepts a FILE too; the sentence glued the two sets into one. The denominator rule was ratified an hour earlier at `835bf848` and this is its author's neighbour breaking it. Swept at `b865db12` and the rows re-anchored here.
- **spelling:** _(declared empty)_
- **retired by:** hv, decision 3 (2026-09-11, ST0058 AC-00.3): STRUCK. It answered `is a known command that is not implemented yet` while canon told agents to run it, and it will not be built. RETIRED RATHER THAN DELETED so the path still refuses at rc=2 with `was retired in Intent v3` (issue 0044) and not clap's generic rc=1; `spelling` is empty because nothing replaces it. The target state it carried before is kept below as history.
- **remedy:** **RULED BY hv, 2026-08-17: `list` REPORTS WHAT EACH DIRECTORY ACTUALLY OFFERS, AND `show` SERVES BOTH KINDS.** Of the three remedies put to them -- narrow `list`, widen `show`, split the namespace -- this is the one that stops the two subjects being conflated instead of picking a winner between them. `list` prints the AGENTS.md templates and the ST0035 canon packs as SEPARATE, LABELLED groups; `show <name>` resolves either kind. **So no name `list` prints is a name `show` refuses, which is the whole defect, and nothing becomes undiscoverable to fix it** -- narrowing `list` would have satisfied the class by hiding seven real canon packs from the only command that enumerates them, trading a false claim for a silent omission. That is the trade `guide_refs_check.sh` was built over: a wrong answer earns an error the reader can react to, while a missing one reads as a capability the tool does not have. **The shape of the fix follows the shape of the cause.** `list` broke because ST0035 moved a second population into `templates/` and nothing re-read what `list` meant -- one directory, two kinds, one output. The remedy makes the second kind visible rather than pretending the directory is still homogeneous.
- **rulings:**
  - `0.state`: ratified
  - `0.authority`: ic
  - `0.date`: 2026-08-17
...
- **Flags:**
  - `-v` (bool) -- Show full descriptions in `list`
    - **disposition:** keep
  - `--force`, `-f` (bool) -- Overwrite a subagent that was changed here, and report the checksum of what was discarded
    - **disposition:** keep
    - **measured late and the reason is the finding:** **ADDED BY ic 2026-08-23 ON cc's REPORT, AND ic's VERIFICATION MOVED IT TO THE OTHER CAUSE.** cc reported this row as the sourced-library case -- `intent_claude_subagents` sources `claude_plugin_helpers.sh`, which parses `--force|-f` at `:273`, `:356` and `:471`. **It does source it, and it ALSO parses `--force` in its own file at `:140`**, so a per-command scan had something to find here and did not find it. **That puts this row with `claude upgrade` rather than with `claude skills`: the obvious explanation does not cover it, which is the case cc called the more worrying one -- and it is more than one of the rows.** The likely discriminator is the IDIOM: `:140` spells it `elif [ "$arg" = "--force" ] || [ "$arg" = "-f" ]`, while every row the census DID find uses a `case` arm (`--force|-f)`). **A scan keyed to one parse idiom reports a true answer about the idiom and a false one about the flag.** Stated as the LIKELY cause and not a measured one: ic did not read the census instrument, only the spellings it did and did not catch.
  - `--all` (bool) -- Every unit this install carries (install) or has installed (uninstall)
    - **disposition:** keep
    - **measured late and the reason is the finding:** **RESTORED 2026-09-04 BY dc AS A v2-TO-v3 REGRESSION, NOT ADDED AS A v3 FEATURE (issue 0236, severity high).** v2 shipped it working -- `claude_plugin_helpers.sh` parsed `--all` across `plugin_install`/`plugin_sync`/`plugin_uninstall` -- and `usage-rules.md` names `install --all` FOUR times at :113, :126, :310 and :311. v3 declared it on NO row and clap refused it at **rc=1 `unexpected argument`**, which tells an operator they mistyped when they typed exactly what root canon instructed. **THAT IS THE DISHONEST HALF OF hv's SHIPPING RULE: rc=2 `known command, not implemented yet` is a coherent state and is what `config`/`ext`/`learn` ship as; rc=1 is not.** **AND NO INSTRUMENT COULD SEE IT, WHICH IS THE REAL FINDING.** `implemented_check.sh` draws its population from THIS TABLE, so a flag absent here is not a subject it can probe; `canon_mandated_verbs_check.sh` draws its population from canon and its predicate is `rc==2 AND the marker`, so a clap parse error at rc=1 carries no marker and cannot be classified. **The defect consisted precisely of the DISAGREEMENT between those two populations, so it was invisible to both, and no predicate change reaches it** -- which is why 0236's remedy is a STATIC canon-vs-table arm whose subject is the boundary itself. **`uninstall --all` is the sharper half: canon never names it, so it lives in the suite-versus-table gap and even the static arm would not have caught it** -- found only because the parity BATS suite uses it (cc, 16 invocations, 9 install / 7 uninstall, 14 in `keep`-class files).
  - `--dry-run` (bool) -- Show what would change, no writes
    - **disposition:** keep
    - **measured late and the reason is the finding:** **ADDED BY dc 2026-09-12 UNDER hv's BATCH 4 (silent deletion).** `claude skills` has carried this row since 2026-09-04 and `claude subagents` did not, while BOTH families reach one preview in `payload.rs` through one renderer function whose only difference is `w.verb`. **So the preview was built, and unreachable: `spine.rs:624` declares table flags onto clap, so a family with no row refuses the flag at the parser with `unexpected argument` at rc=1** -- which tells an operator they mistyped a flag the engine implements. **IT MATTERS MORE SINCE BATCH 2:** `sync` and `uninstall` now HOLD a unit edited since it was installed and `--force` discards it, so the preview is how an operator decides whether to type `--force`. That decision was available for skills and blind for subagents -- same risk, same engine, one of them with the lights off. **The help string is taken verbatim from the skills row**, which took it verbatim from `st bootstrap`: a sixth `--dry-run` saying a fifth thing is the drift Highlander is about. **Declared at the family level for the same reason as skills** -- there is no `claude subagents sync` entry to hang a per-verb flag on -- so the renderer REFUSES it on `install` and `uninstall` rather than ignoring it, which it already did for both families before this row existed.
- **Exit codes:**
...
- **Flags:**
  ...
  - `--force`, `-f` (bool) -- Overwrite a skill that was changed here, and report the checksum of what was discarded
    - **disposition:** keep
    - **measured late and the reason is the finding:** **ADDED BY cc 2026-08-23, AND IT WAS ALWAYS A v2 FLAG RATHER THAN A v3 ADDITION.** v2 parses `--force|-f` in `plugin_install` (`claude_plugin_helpers.sh:273`), `plugin_sync` (`:356`) and `plugin_uninstall` (`:471`), and `intent_claude_skills` SOURCES that helper. **IT WAS MISSED BECAUSE IT IS NOT IN THE COMMAND'S OWN FILE** -- `grep -- --force intent_claude_skills` returns nothing, which is a true answer to the wrong question. Every row that DOES declare `--force` (`bootstrap`, `doctor`, `treeindex`) parses it in its own file. **A flag reached through a sourced library is invisible to a per-command scan, and the scan cannot report what it could not look at.** **CORRECTED BY ic 2026-08-23, AND THE CORRECTION NARROWS MY OWN FINDING:** I reported the same gap on `claude subagents` as another instance of the sourced-library cause. It sources the helper AND parses `--force` in its own file at `intent_claude_subagents:140`, so the per-command scan had something to find there and missed it. **That puts subagents with `claude upgrade`, and the cause I called the more worrying one -- because the obvious explanation does not cover it -- is more than one of the rows. The sourced-library finding is true and covers only THIS row.** ic's likely discriminator, recorded by them as likely rather than measured: `:140` spells it as an `elif` string comparison while every row the census DID find uses a `case` arm, so a scan keyed to one parse idiom returns a true answer about the idiom and a false one about the flag. **It does not explain `claude upgrade`, which uses the `case` idiom in its own file and was missed anyway** -- left visible rather than closed by a plausible mechanism. Both rows fixed by ic at `6f95acbc`. **AND THIS ROW ALREADY NAMED THE FLAG IN PROSE WHILE NOT DECLARING IT.** `observed.notes` says a script-only change _needs `install --force`_. cc's renderer then told operators `--force` was _declared in the surface table but not built_ -- false, and taken from that prose. **A row can name a flag in a sentence and omit it from the array that is the declaration, and nothing reads the two against each other.**
  ...
  - `--dry-run` (bool) -- Show what would change, no writes
    - **disposition:** keep
    - **measured late and the reason is the finding:** **ADDED BY dc 2026-09-04 ON hv's COMMISSION** -- hv asked whether `skills sync` had a `--dry-run` and it did not, which gated hv running the sync at all. **THE HELP STRING IS TAKEN VERBATIM FROM `st bootstrap` RATHER THAN AUTHORED** (ic, who drove the precedent): four `--dry-run` rows existed saying three different things, and `st bootstrap` is the only LIVE one -- entry disposition keep, flag disposition keep. A fifth row saying a fourth thing is the drift Highlander is about. **`keep` IS RIGHT ON THE FLAG VOCABULARY'S OWN TERMS AND NOT BY ELIMINATION** (ic): `flag_dispositions` is keep/retire/pending/intrinsic and its axis is DOES-IT-SHIP and WHO-READS-IT, not provenance -- only `retire` mentions v2, and it must, because a retired flag is by definition a v2 artefact. `new-surface` is not a declared flag disposition at all. **DECLARED AT THE FAMILY LEVEL, FOLLOWING `--force`, BECAUSE THERE IS NO `claude skills sync` ENTRY TO HANG IT ON** -- the family has exactly one entry and per-verb applicability would mean minting new entry rows, which is a change to the table's SHAPE and wants its own ruling. The renderer therefore REFUSES it on the verbs that cannot honour it rather than ignoring it: a `--dry-run` silently performing a write is the worst available failure for this flag. **THE ROW AND THE RENDERER ARM LANDED IN ONE COMMIT, DELIBERATELY:** `spine.rs:624` declares table flags onto clap, so a row landing first does not fail -- it PARSES, and the run writes.
  - `--all` (bool) -- Every unit this install carries (install) or has installed (uninstall)
    - **disposition:** keep
    - **measured late and the reason is the finding:** **RESTORED 2026-09-04 BY dc AS A v2-TO-v3 REGRESSION, NOT ADDED AS A v3 FEATURE (issue 0236, severity high).** v2 shipped it working -- `claude_plugin_helpers.sh` parsed `--all` across `plugin_install`/`plugin_sync`/`plugin_uninstall` -- and `usage-rules.md` names `install --all` FOUR times at :113, :126, :310 and :311. v3 declared it on NO row and clap refused it at **rc=1 `unexpected argument`**, which tells an operator they mistyped when they typed exactly what root canon instructed. **THAT IS THE DISHONEST HALF OF hv's SHIPPING RULE: rc=2 `known command, not implemented yet` is a coherent state and is what `config`/`ext`/`learn` ship as; rc=1 is not.** **AND NO INSTRUMENT COULD SEE IT, WHICH IS THE REAL FINDING.** `implemented_check.sh` draws its population from THIS TABLE, so a flag absent here is not a subject it can probe; `canon_mandated_verbs_check.sh` draws its population from canon and its predicate is `rc==2 AND the marker`, so a clap parse error at rc=1 carries no marker and cannot be classified. **The defect consisted precisely of the DISAGREEMENT between those two populations, so it was invisible to both, and no predicate change reaches it** -- which is why 0236's remedy is a STATIC canon-vs-table arm whose subject is the boundary itself. **`uninstall --all` is the sharper half: canon never names it, so it lives in the suite-versus-table gap and even the static arm would not have caught it** -- found only because the parity BATS suite uses it (cc, 16 invocations, 9 install / 7 uninstall, 14 in `keep`-class files).
- **Exit codes:**
...
- **stderr:** `error: ...` on stderr (INV-01)
- **Observed notes:** `sync` checksums SKILL.md ONLY, so a change confined to a skill's scripts/ does not propagate -- it needs `install --force` (or touching SKILL.md). **THE v3 RECOMMENDATION THAT USED TO END THIS NOTE HAS MOVED TO `target` (ic, 2026-08-22, on vc's ruling).** It read _Known trap, worth not reproducing in v3_ -- a v3 intent sitting in a field whose contract is what v2 DOES, while `target.state` just below said `as-observed`, the opposite. **cc could not settle the fork from code because the normative field and the note disagreed, and cc was right not to let a note beat a field.**
- **Target:** `corrected` -- ratified: **hv, 2026-08-22: AC-07.3 SCOPE CONFIRMED.** The SKILL.md-only checksum scope is a DEFECT that v3 fixes, not a behaviour v3 reproduces. **Route: vc ruled it provisionally the same day on a fork cc escalated, and hv confirmed it DIRECTLY TO ic in session -- not relayed.** **THE WORD `provisional` IS DELETED RATHER THAN NEGATED**, per this field's own standing note: `no longer provisional` reads as provisional. **IT WAS HELD FOR ONE COMMIT AND THE HOLD IS WORTH RECORDING.** vc reported the ratification and asked for the marking to come off; ic held the edit, because `ratified_in` ASSERTS an adjudicator's act and `hv/wip.md` carried no entry for the day. **The precedent was on hv's own board in vc's own hand** -- a prior vc session's relay marked _PROVENANCE UNVERIFIED ... treat as advisory until hv confirms_. **A trustworthy report of an act does not discharge a field that certifies the act**, because the row is read by people who cannot see the message. One question to hv settled it, which is what the hold cost. MIGRATED FROM `ratified_in` (ic, 2026-08-22): the machine-read half of the text below is now the declared `rulings` array above; this is the human half and NOTHING PARSES IT. hv, 2026-08-22, confirmed in session to ic; recorded in this row's `ratification` field, whose text carries the ruling and its route -- behaviour: v3's `sync` detects a change anywhere in a skill's installed content rather than in SKILL.md alone. **THE MECHANISM IS cc's BUILD DECISION AND IS DELIBERATELY NOT SPECIFIED HERE** -- this row records that the defect is corrected, not how. A parity row that dictates an implementation is asserting a second design authority.
- **time is a BRACKET AND NOT A POINT:** **vc's own correction, unprompted, and it is why this row carries no clock time.** vc timed three sends by arithmetic from one `date -u` read at 10:05Z, producing 10:07Z / 10:12Z / 10:14Z; the next real read was 10:09Z, so **two of the three stamps were in the FUTURE when written.** The measured bracket is **[10:05Z, 10:09Z], both ends off a clock**; the points inside it are not measurements and their order relative to each other is unrecoverable. **A fabricated stamp is not repaired by a better-looking one**, so the ruling is dated to the DAY, which is measured, and not to a minute, which is not. **The general form vc adopted and I am adopting: a time you cannot read from a command in the same act is written as an INTERVAL, and an interval cannot be produced by arithmetic -- both ends have to come from somewhere.**
- **manifest path is a SEPARATE RULING FROM THE SAME ACT:** **vc, 2026-08-22, recorded here because it is target behaviour of THIS command and would otherwise live only in a message: v3 owns a separate manifest path and never touches v2's.** Reason it is not optional: v2 writes `$HOME/.intent/skills/installed-skills.json` (`intent_claude_skills:28`), cc's proposed v3 manifest was the same path, same `version: "1.0.0"`, same fields -- so once `checksum` denotes a different function, **v2 is shipped and cannot be taught to read a scope field.** It reads v3's value, finds `source != old` unconditionally, falls to the unguarded `elif` at `claude_plugin_helpers.sh:426`, prints `update available` and overwrites; v3 overwrites back. **A mutual clobber in which BOTH binaries print success and neither has anything to report.** Attributed to vc rather than derived by ic: I have not re-driven it.
- **why corrected and not as observed:** The `Corrected` class is _a v2 behaviour that is simply wrong and is fixed rather than faithfully reproduced_; `as-observed` is for behaviour v3 CHOOSES to reproduce. **This scope was never chosen** -- it is what checksumming one file yields, and the v2 note recording it as a `trap` is the observation of a defect, not a specification. Reproducing it is what `parity.md` forbids in its own words: laundering a v2 defect into a v3 requirement.
- **rulings:**
  - `0.state`: ratified
  - `0.authority`: hv
  - `0.date`: 2026-08-22
...
- **Flags:**
  - `--apply` (bool) -- Write the canon; without it the command reports only
    - **disposition:** keep
  - `--force` (bool) -- Overwrite canon files that were edited here: a CLAUDE.md without Intent's marker, a .claude/settings.json whose hooks do not run through `intent claude hook`, and .intent_critic.yml
    - **disposition:** keep
  ...
  - `--skip-settings` (bool) -- Leave the Claude Code harness wiring alone -- .claude/settings.json and .mcp.json -- so no session hooks are wired and no MCP server is declared
    - **disposition:** keep
    - **disposition basis:** Restored 2026-09-11 for issue 0143 (v3.0.1 list #53). v2 shipped it (intent_claude_upgrade:74, :118, :856-860) as the single escape hatch for a project that does not want Claude Code lifecycle hooks, and nothing recorded dropping it at port time as a decision. In v2 it gated .claude/settings.json and the three per-project hook scripts; v3 writes no per-project hook scripts (their bodies are served from the install), so here it declines the settings file only, and reports it as skipped rather than leaving it unmentioned. WIDENED 2026-09-12 (AC-24.1, vc's ruling): it declines .mcp.json too. ONE FLAG FOR THE HARNESS-WIRING CLASS, because both files exist only to wire Claude Code, and a project that declines the lifecycle hooks and then finds an MCP server declared in its editor anyway has been told no by a flag that only half meant it. Seeded-when-absent is not a way to decline either: deleting the file only means the next --apply seeds it again, so without this flag there is no way to say no at all. Both paths are named on the run's skipped line.
- **Exit codes:**
...
- **Target:** `retire`
- **spelling:** _(declared empty)_
- **retired by:** hv, decision 3 (2026-09-11, ST0058 AC-00.3): STRUCK. It answered `is a known command that is not implemented yet` while canon told agents to run it, and it will not be built. RETIRED RATHER THAN DELETED so the path still refuses at rc=2 with `was retired in Intent v3` (issue 0044) and not clap's generic rc=1; `spelling` is empty because nothing replaces it. It carried target state `as-observed`.
- **MCP:** not exposed -- **mutates**
...
- **Target:** `retire` -- ratified: hv, 2026-08-25, carried as ST0069 AC-14.12 and recorded by vc under the pen: the four verbs `new` / `list` / `archive` / `hygiene` are gone and `intent wb` answers instead. The criterion exists to give `AC-12.1`'s cut exception an end that is ENFORCED rather than remembered -- an unsatisfied AC blocks `wp done`, so the exception retires at the moment WP-14 tries to close whether or not anyone recalls why the row is here. AND hv AGAIN 2026-09-12, ruling that ST0069 completes and that WP-14 reifies the whiteboard in 3.0.2 -- the two rulings together are the whole warrant, the first that the family ends and the second that it ends HERE rather than in a later release. Ratified under vc's pen and landed by ic at 2026-09-13 10:44Z; the cutover's board migration landed ahead of this, so this is the landing that follows it rather than the one that opens it.
- **Note:** **WHAT FALSIFIED D14, WHICH THIS ROW CARRIED UNTIL NOW: `D14: the whiteboard stays md-authored through 3.0.0/3.1 and is restructured in the 3.2 bus ST. So this family ports as-is rather than being reified.`** hv ruled 2026-09-12 that ST0069 completes and that WP-14 reifies the whiteboard in 3.0.2 -- so the restructure D14 deferred to a 3.2 bus ST happened two releases early, and `ports as-is` lost its subject rather than its schedule. The four verbs managed the board as FILES; a node is now a ROW and `intent/whiteboard/<id>/` is the rendered view of it. **THE ROW IS RETIRED ON BOTH FIELDS DELIBERATELY**: `Entry::is_shipped()` reads both and fails OPEN, so `retire` on one alone would strike the door while the other field still read `keep`. The spelling names where the capability went -- `intent wb register` creates a node, `intent wb status` lists them, `intent wb show` reads one whole board, and the two whiteboard guards plus `intent doctor` are what `hygiene` was reaching for. It SHIPPED in 3.0.1 and is withdrawn in 3.0.2, which is a user-visible removal and is why the CHANGELOG carries it under Removed.
- **spelling:** intent wb register
- **MCP:** not exposed -- **mutates**
- **recoverability:** idempotent
- **recoverability anomaly:** WITHHELD FOR NO FACADE DOOR (AC-09.6), NOT FOR DANGER -- and the distinction is why this field is filled in rather than the label adjusted. The MCP withhold list is derived from `recoverability` because MCP withholds a mutation the surface cannot undo; this row is recoverable and is withheld anyway, because it has no single facade method to serve it -- AC-09.6, exposed implies servable: a tool that cannot be served must not be published. Narrowed 2026-08-31, hv-directed (narrow the table AND make the drop loud), as the ic-owned table dispositions, with the other exposed-but-unserved rows in one flip; cc lands the loud refusal at `mcp.rs:97` on top. The per-row basis (namespace / facade-bypass / multi-door / the sync fan) is recorded in `parity/ac-09_6-mcp-facade-read.md`. SELF-EXPIRING: when a facade door is built and this row re-exposed, this string must be deleted in the SAME act -- arm 2 of `gen_dispatch_table.sh` refuses a stale anomaly, so a reason for a withhold cannot outlive the withhold.
...
- **parity.md's table said `init, remove, list`. Measured, its verbs are** list, show, init, remove, sync, plus `rm` as an alias of `remove`. Evidence: `intent lang sync` answers `ok: no declared languages; nothing to sync`.
...
- **stderr:** `error: ...` on stderr (INV-01)
- **Target:** `corrected` -- ratified: vc, 2026-08-25, on issue 0068 (HIGH) -- `intent lang init` does NOT reinstate the `RULES-<lang>.md` / `ARCHITECTURE-<lang>.md` fan-out. Measured across the estates: every one byte-identical to its template, none referencing files anywhere in Intent's own tree -- the least favourable case for the conclusion, and it confirmed it. The issue states this close condition in as many words: "If the right answer turns out to be that `lang init` has nothing to install, that is an acceptable close -- the rules it would point at are already served live by `intent claude rules list --lang <lang>`."
- **behaviour changed:** v2 prints "Files installed by 'intent lang init <lang>'" over a pair of paths. v3's `init` installs neither, so a faithful port would print a durable claim that files will appear when they will not. **THIS IS `plugin show`'s DEFECT ARRIVING THROUGH FIDELITY RATHER THAN NEGLECT** -- the same shape as porting a remedy that points at a retired verb, which is why `as-observed` was the wrong answer for a row whose whole output describes a mechanism that has been retired.
- **rulings:**
  - `0.state`: ratified
  - `0.authority`: vc
  - `0.date`: 2026-08-25
...
- **carve out:** THE CARVE-OUT, WHICH IS THE PART MOST LIKELY TO BE GOT WRONG: this does NOT touch the agnostic pair `intent/llm/RULES.md` + `intent/llm/ARCHITECTURE.md`. Same directory, adjacent names, OPPOSITE evidence -- every estate AUTHORED its own. A sweep treating `intent/llm/*` as one family deletes the only files in it with real authors.
- **rulings:**
  - `0.state`: ratified
  - `0.authority`: vc
  - `0.date`: 2026-08-25
...
| `llm guide`       | --           | --              | Print the agent guide: every command this build ships, with what it does and whether it writes, then Intent's workflows and conventions | new-surface |
...
- **Flags:**
  - `--symlink` `[dir]` (string) -- Create a symlink to usage-rules.md in the current or specified directory
    - SUPERSEDED 2026-09-02 AND KEPT INLINE. It read: `the directory argument is OPTIONAL, defaulting to the current directory -- an optional-value flag, which clap models differently from v2's positional peek`. DRIVEN: clap REQUIRES the value -- `intent llm usage_rules --symlink` answers `error: a value is required for '--symlink <dir>' but none was supplied` at rc=1, so the bare form never reaches `llm_usage_rules()` at all. The note described a modelling decision that was not implemented.
    - **disposition:** retire
    - **disposition basis:** WITHDRAWN 2026-09-02 (vc, within the pen granted by hv 2026-08-22; hv reverses with one word). AC-06.8's remedy sanctions wire-or-withdraw, and a WITHDRAW is inside the pen where a WIRE would not be -- declining to build is adjudication, building new behaviour is not. Recorded here in `disposition_basis`, and the first version of this sentence got its reason AND its figure wrong -- both corrected 2026-09-02 20:00Z, superseded text inline. It read: `rather than in a new `ratified_in` key: 78 rows carry the basis in this field and none carries that one`. **`ratified_in` IS NOT A NEW KEY, IT IS A RETIRED ONE.** ic migrated it on 2026-08-22 -- the machine-read half became the declared `rulings` array and the human half became `ratification`, and `ratified_in_check.sh` was renamed `rulings_check.sh` on 2026-08-23. **Sixteen rows still carry the migration sentence, so a BARE-TOKEN pattern counts a retired field's OBITUARY as its use** -- which is how it gets measured as a live one. **THE FIGURE THAT STOOD HERE WAS `returns 30` AND IT WAS STALE BEFORE THE INK DRIED, BY MY OWN HAND: writing this very sentence added four more mentions of the token, 30 -> 34.** A count of a token in the file that documents that token is self-perturbing, so the mechanism is stated here and the number is not. **AND `grep -c` COUNTS LINES, NOT OCCURRENCES** -- several occurrences can share a line, which is why two correct measurements of one pattern disagree. Counted structurally -- over the objects with `jq`, not over lines with `grep -c` -- `disposition_basis`, `ratification` and `rulings` are all in use, and **no object carries `ratified_in`.** **A LINE COUNT WAS ONCE REPORTED HERE AS A ROW COUNT**: `grep -c` and the object count disagreed, because one occurrence sits inside prose rather than being a key. The placement is unchanged and its real ground is the flag-retire precedent -- `agents init --template` records an identical withdrawal in `disposition_basis`, while `ratification` and `rulings` sit at entry and target level. THE DECIDING GROUND IS THAT THE SUBJECT HAS MOVED. v2's flag linked the INSTALL's `$INTENT_HOME/usage-rules.md`; v3's verb is about the PROJECT's root file (`render.rs:6807`), which already sits where every reader looks for it. Retiring a flag whose subject no longer exists in the model is declining to port a pointer to a thing that is not there, not dropping a feature. AND `wire it to what v2 did` IS UNIMPLEMENTABLE AS STATED, which is what closes the alternative. MEASURED BY DRIVING v2 rather than reading it (dc, 2026-09-02), three arms: `--symlink <ABSOLUTE dir>` works at rc=0; `--symlink` BARE does not use the current directory, because `bin/intent:284` cds to PROJECT_ROOT before exec'ing the arm and exports INTENT_ORIG_CWD for exactly that purpose while `bin/intent_llm` never reads it -- a fresh empty directory stays empty under a `created: ./usage-rules.md` line at rc=0; and `--symlink <RELATIVE dir>` refuses a directory that exists (`error: Target directory does not exist: sub`, rc=1) because the validation also runs against the project root. Most of those arms are defects nobody would port deliberately, porting the working arm alone is not parity, and resolving `.` against the real cwd is NEW behaviour -- hv's, by the same rule that made `agents init --template` hv's. A SOURCE READ OF `bin/intent_llm` COULD NOT HAVE SEEN ANY OF IT, BECAUSE THE `cd` IS IN A DIFFERENT FILE, and this entry's own `evidence_class` is `read` -- taken from the v2 source without executing the arm. The population could not contain the defect, which is why it was driven. Same shape as the `agents init --template` ruling, where variation was required over a source read. v2's behaviour, recorded so a later wiring has a target rather than a help string: `--symlink [dir]` created `<dir>/usage-rules.md` as a symlink to the INSTALL's `usage-rules.md`, defaulting `[dir]` to the project root rather than the cwd its help claimed. That capability has no v3 home and this withdrawal does not create one; it stops the surface claiming it has. Issue 0181. Sibling finding, filed separately as 0215: `llm usage_rules` is labelled `v2 parity` and answers about a different file than v2 does.
- **Exit codes:**
...
- **MCP:** not exposed -- read-only
- **Wants review -- the classification disagrees with the verb name:** SUPERSEDED 2026-09-02 BY THE `--symlink` WITHDRAWAL, AND THE OLD TEXT IS KEPT INLINE BECAUSE IT EXPLAINS THE FIGURE THAT MOVED. It read: `Reads as a display command, and its default IS display. The flag is what makes the entry a mutation -- the same shape as `at lint`, `doctor`, and `todo list`, which is why the field is defined over the entry rather than the default.` That was right while the flag stood. `--symlink` was the only thing on this row that ever wrote, so withdrawing it leaves nothing that mutates and the row is now plainly `read`. **THE RECLASSIFICATION IS A CONSEQUENCE OF THE RULING, NOT A SECOND JUDGEMENT** -- and it was found by `gen_dispatch_table.sh` refusing the pair, not by anyone reasoning it out: it declines a row declared `mutate` whose only writer is a RETIRED flag. The generator caught the other half of a landing, which is what 0210 asks of a landing and what a generator is for.
- **MCP classification grounded in:** `bin/intent_llm:65` is `cat "$USAGE_RULES_FILE"` and that is now the whole row. The former grounding also cited `:88-100`, the `--symlink` path that created a symlink and warned when one existed; that arm is withdrawn from the v3 surface (see the flag's `disposition_basis`) and is retained there rather than here.
- **recoverability anomaly:** RECOVERABILITY DROPPED 2026-09-02 WITH THE RECLASSIFICATION, AND THE SUPERSEDED VALUE IS NAMED SO THIS FIELD STILL PARSES AS AN ARGUMENT: it read `idempotent`, which the vocabulary now REFUSES on a `read` row -- `dispatch.rs:811` rejects a read that declares recoverability, because the question is vacuous for a command that changes nothing. **THE WITHHOLD GROUND IS UNTOUCHED BY THAT**, which is the point of keeping this field rather than deleting it with the value it used to explain. WITHHELD FOR NO FACADE DOOR (AC-09.6), NOT FOR DANGER -- and the distinction is why this field is filled in rather than the label adjusted. The MCP withhold list is derived from `recoverability` because MCP withholds a mutation the surface cannot undo; this row is recoverable and is withheld anyway, because it has no single facade method to serve it -- AC-09.6, exposed implies servable: a tool that cannot be served must not be published. Narrowed 2026-08-31, hv-directed (narrow the table AND make the drop loud), as the ic-owned table dispositions, with the other exposed-but-unserved rows in one flip; cc lands the loud refusal at `mcp.rs:97` on top. The per-row basis (namespace / facade-bypass / multi-door / the sync fan) is recorded in `parity/ac-09_6-mcp-facade-read.md`. SELF-EXPIRING: when a facade door is built and this row re-exposed, this string must be deleted in the SAME act -- arm 2 of `gen_dispatch_table.sh` refuses a stale anomaly, so a reason for a withhold cannot outlive the withhold.

### `llm guide`

Print the agent guide: every command this build ships, with what it does and whether it writes, then Intent's workflows and conventions
...
| command | args          | flags                    | help                                                                                                  | disposition |
| ------- | ------------- | ------------------------ | ----------------------------------------------------------------------------------------------------- | ----------- |
| `learn` | [description] | --category <cat>, --list | Capture project-specific learnings for future LLM sessions -- not implemented in this build (exits 2) | keep        |
...
Manage Intent user extensions -- not implemented in this build; nothing reads ~/.local/share/intent/ext/
...
| command        | args      | flags                            | help                                                                                       | disposition |
| -------------- | --------- | -------------------------------- | ------------------------------------------------------------------------------------------ | ----------- |
| `ext`          | [command] | --                               | Manage Intent user extensions -- not implemented in this build (exits 2)                   | keep        |
| `ext list`     | --        | --                               | List installed extensions -- not implemented in this build (exits 2)                       | keep        |
| `ext show`     | <name>    | --                               | Show manifest + contributions for one extension -- not implemented in this build (exits 2) | keep        |
| `ext validate` | [name]    | --                               | Validate extension manifests -- not implemented in this build (exits 2)                    | keep        |
| `ext new`      | <name>    | --skill, --subagent, --rule-pack | Scaffold a new extension -- not implemented in this build (exits 2)                        | keep        |

### `ext`

Manage Intent user extensions -- not implemented in this build (exits 2)
...
List installed extensions -- not implemented in this build (exits 2)
...
Show manifest + contributions for one extension -- not implemented in this build (exits 2)
...
Validate extension manifests -- not implemented in this build (exits 2)
...
Scaffold a new extension -- not implemented in this build (exits 2)
...
- **Target:** `corrected` -- ratified: vc, 2026-08-15 -- and deliberately NOT an hv ruling, because none was required. NOTED ON THE ROW SO NOBODY RE-OPENS IT LOOKING FOR THE RULING THAT NEVER EXISTED: v3 generates help from this table, so a help/implementation mismatch CANNOT arise by construction. That is not a decision anyone has to make; it is a defect the SSOT retires wholesale, and this row was the last place still treating it as open. MIGRATED FROM `ratified_in` (ic, 2026-08-22): the machine-read half of the text below is now the declared `rulings` array above; this is the human half and NOTHING PARSES IT. vc, 2026-08-15, ruling applied at commit 803ff46d -- behaviour: Help text and implementation agree because both derive from this row. The v2 mismatch is not reproduced and needs no per-site correction.
- **rulings:**
  - `0.state`: ratified
  - `0.authority`: vc
  - `0.date`: 2026-08-15
...
- **Every test in `tests/unit/treeindex_commands.bats` execs `bin/intent_treeindex` DIRECTLY, bypassing the dispatcher entirely -- burn ratio zero.** The file reads as CLI-shaped and is not; that discovery is the reason the register classifies by burn measurement rather than by reading assertions.
...
- **Defects observed in v2:**
  ...
  - INV-06 at the unknown-flag path writes to BOTH streams -- a rare case among the probes
...
- **spelling:** _(declared empty)_
- **consequence:** Removes a large block of bash from WP-06's port list, and INV-07 (`--help` exits non-zero here) is moot rather than pending-hv: there is no v3 command to correct.
- **MCP:** not exposed -- **mutates**
...
- **v2 help file:** none
- **Owning work package:** WP-06
- **v2 BATS coverage (a record -- the cut deleted that suite):** **THIN**
...
- The drift is the argument. `lib/help/` held help files for only some of the `bin/intent_*` scripts, so most commands had none. Its `@usage:` / `@options:` / `@arguments:` / `@examples:` grammar is used exactly ONCE each. `stzero.help.md` is named against `bin/intent_st_zero`, and `rules.help.md` documents a `claude` SUBcommand as though it were top-level. At v2.19.0 it still describes `upgrade` as an STP migration.
...
- **Defects observed in v2:**
  ...
  - Most commands have no help file, so `intent help <cmd>` silently falls through to a no-help path for most of the surface.
...
- Reachable as both `intent st_zero` (top level, auto-dispatched) and `intent st zero` (bin/intent_st:1610 execs this binary). Its own usage block says `intent st zero install`, so it documents only the second.
...
- **Flags:**
  - `--audit-only` (bool) -- Show gap analysis only, no changes
    - **disposition:** retire
    - **disposition basis:** The root `st_zero` spelling dies (hv ratified, 2026-08-15) -- these flags reach clap via `st bootstrap`, whose row carries its own live copies. Recorded from v2, never built.
  - `--dry-run` (bool) -- Show what would change, no writes
    - **disposition:** retire
    - **disposition basis:** The root `st_zero` spelling dies (hv ratified, 2026-08-15) -- these flags reach clap via `st bootstrap`, whose row carries its own live copies. Recorded from v2, never built.
  - `--deliverable` `<id>` (string) -- Target a single deliverable (D2-D11)
...
- **Note:** MEASURED 2026-08-15: before this change the v3 binary offered BOTH `intent st_zero` and `intent st bootstrap`, because `is_shipped()` reads `disposition` and the command name comes from `entry.path` -- so `target.spelling` was declared and unread. `target.spelling` STAYS on this row as the human-readable record of the successor; it is simply not machine-read. The successor `st bootstrap` carries the behaviour and its own live copies of these flags.
- **spelling:** _(declared empty)_
- **spelling note:** Named `intent st bootstrap` as the successor until hv's decision 3 (2026-09-11, ST0058 AC-00.3) struck that path too. Since issue 0044 this field is RENDERED in the refusal, and a refusal must never send anyone to a struck door, so nothing replaces this now.
...
- **basis:** design.md:73-74, D07/D08/D19. **DOES NOT EVICT A RUNNING DAEMON** -- `AC-08.12`, landed `6b32b71d`: a kernel file lock decides, not a liveness probe. **THIS ROW SAID _REFUSES_ UNTIL 2026-08-30 AND THAT HAD STOPPED BEING TRUE** (`a402625f`): a second invocation now succeeds and names the running pid -- `ok: intentd is already running (pid <n>)`, exit 0, driven twice through `target/debug/intent` because the delivered pair predates the arm. **THE HALF THAT SURVIVED IS THE INTERESTING ONE, WHICH IS EXACTLY WHAT MADE THE STALE HALF DANGEROUS** -- a reader checking whether it evicts finds that it does not, and has no reason to doubt the clause beside it. A wholly wrong statement gets caught; a half-true one gets believed (cc, 2026-08-30, who found it by reading the row while asking for something else). **THE NO-OP IS STATED HERE RATHER THAN IN `populations.self_loop`, AND THAT WAS TRIED AND REVERTED THE SAME DAY.** `self_loop` is a population of the STATE MACHINE -- every member traces to a ratified verb or a declared delegate, asserted by `self_loop_population.rs` in both directions -- and `daemon start` moves no entity through any machine. Adding it there was a row added on a belief about the SURFACE rather than about the MODEL, which is the test's own wording for what it caught (ic, 2026-08-30). **A structured field that merely LOOKS like the home for a claim is worse than no field**, so the behavioural fact stays in prose with its measurement attached.
- **recoverability:** reversible
- **recoverability anomaly:** REVERSIBLE AND WITHHELD ANYWAY, AND THE WITHHOLD REASON IS NOT IRREVERSIBILITY. The MCP withhold list is derived from `recoverability` because the usual ground for keeping a mutation off the tool tier is that the surface cannot undo it -- and these three ARE undoable, each by a sibling. They are withheld because they are MACHINE-LEVEL PROCESS CONTROL, which is this family's own stated ground and the canonical example behind the exposure field existing at all. **The head row carried `one-way` while it was ONE ROW STANDING FOR SEVERAL VERBS, which is a label bent to make a derived number come out** (cc's words). A representation that must state something false for a mechanism to produce the right answer is the mechanism's defect, not the row's -- and the generator refusing the bent label rather than accepting it is that working. Splitting the family is what made each verb's real recoverability statable.
...
- **Flags:**
  - `--format` `terminal|json` (enum) -- Output format
    - **default:** terminal
    - **disposition:** keep
    - **disposition basis:** **`--format`, NOT `--json`, ON hv's 2026-08-21 SPELLING RULING**: `--format` where the verb has formats other than json, and `--json` kept ONLY as a v2 parity alias. `daemon` is new surface with no v2 ancestor, so there is no obligation to inherit and `--json` would be a second spelling minted rather than kept. **The machine-readable form is why this row exists**: ST0064's menubar app must ask the CLI where the daemon is, so that no port literal and no setting can exist in the app. Geodica reached the same design independently -- _the address comes from the one resolver, never from config or a setting_ -- which is a second measurement of one design under different constraints rather than a citation. **NARROWED TO `terminal` 2026-08-31 (cc, RULED BY vc), AND WIDEN IT AGAIN WHEN A PROJECTION IS BUILT** -- the same correction and the same precedent as `07ad9876`, which narrowed verbs off `json` for want of one. `daemon status` DECLARED `terminal|json`, PROJECTED NEITHER, AND REFUSED NOTHING: `--format json`, `--format terminal` and `--format zzz` were byte-identical at rc=0, because `render.rs`'s `daemon_status()` took no format argument and nothing downstream ever read the flag. **THE PARAGRAPH ABOVE IS WHY THE ROW EXISTS AND IT IS NOT A REASON TO KEEP THE VALUE**: ST0064's menubar app is the named consumer and it is UNBUILT -- no Swift source for it exists, so the contract has no reader, and minting an output contract before a tag owes it forever. **AND THE COUNTER-ARGUMENT IS REAL, RECORDED RATHER THAN BURIED** (vc): unlike the five, this verb returns a small OBJECT -- answering, and where -- not a list of lists, so it is in the shape class that CAN project, and building it is small. Narrowing cannot be wrong, because it makes the declaration true of the binary; widening is a decision anyone can take the day the app needs it. **WIDENED BACK TO `terminal|json` 2026-08-31, ON THIS ROW'S OWN STATED CONDITION.** The narrowing above ended _widen it again when a projection is built_, and its reason for waiting was that the named consumer was UNBUILT with no Swift source. ic is porting it now and `AC-01.2` requires the app reach the answer THROUGH this verb, so a prose-only face forces a parser that is the second predicate that row forbids. **The shape was NAMED BY ic rather than minted here** -- `{"state":"live|stale|absent"}` with `endpoint` iff live and `pid` iff stale, matching the `Health` variant names. **A FOURTH KEY, `url`, LANDED 2026-09-08 (ic, hv's ask), AND IT IS A SECOND KIND OF ADDRESS RATHER THAN A SECOND SPELLING OF `endpoint`.** This row's help promises _the address it answers on_ and a daemon answers on TWO: the unix socket every client routes through, which is what `Health::Live` carries because `route()` prefers it whenever both are up, and the loopback port `D6` has the kernel assign and the daemon publish. Reporting the socket alone left the promise half-kept and left the menubar app with an address no browser can open -- it would have rendered `/Users/.../intentd.sock` as a URL. **`url` IS PRESENT IFF A LOOPBACK ADDRESS IS ANSWERING**, on the same iff-contract as `endpoint` and `pid`, because ic's decoder is per-state optionals and a `null` would decode as present-and-meaningless. **IT IS A ROUND TRIP, NEVER A READ OF `intentd.addr`**: that file outlives a `SIGKILL`ed daemon exactly as the socket file does, so trusting it would publish a page nothing serves. **AND IT IS RENDERED WHOLE, SCHEME AND ALL, BECAUSE THE APP MUST DERIVE NOTHING** (`AC-01.1`) -- the menu item's title IS this string and its action opens this string, so any tidying happens here or it happens twice.
- **Observed:** nothing to observe -- no v2 antecedent, so there was never anything to run
- **Target:** `new-surface`
- **MCP:** not exposed -- read-only
...
- **v2:** new-surface
- **Flags:**
  - `--lines` `<n>` (string) -- How many of the most recent lines to print, from both logs merged in time order
    - **default:** 40
    - **disposition:** keep
...
- **Target:** `new-surface`
- **MCP:** exposed as an agent tool -- **mutates**
- **when to use:** USE IT at the start of a session -- it marks the acting node active, stamps its heartbeat, records the session and focus you name, then prints its board -- the messages still live listed, the handled ones counted in one line that names `--all` -- then `hv`'s live directives, watch-outs and decisions in full (absent when you are `hv` or there is no `hv` board), then every peer's header state with a count, by kind, of the live items it does not show. `--all` lists every message, handled included; it is for reaching the archive, not for starting a session. DO NOT USE IT to read somebody else's board: a peer comes back as its header and those counts, and `wb show <node>` reads one whole board. An unnamed session or focus keeps what the board already holds; `wb touch` is the verb that only stamps the heartbeat.
...
- **recoverability:** idempotent
- **facade:** wb_pickup
- **note:** **A COMPOSITE OF THE READS, WITH ONE WRITE OF ITS OWN** (vc, 2026-09-12 on cc's Highlander question; the write ruled 2026-09-13 on ic's finding against AC-14.7). What it prints is `wb show` for the acting node and `wb status` for its peers, plus `hv`'s standing content and what each peer's header leaves out (issue 0416, below); the alternative rejected was a skill telling a reader to run those in order, which is the hand-kept list this register exists to end. **ITS WRITE IS WHAT A SESSION START STATES**: status active, the heartbeat, and the session and focus when named, which is what makes `wb release`'s paused node active again. **THE WRITE HAPPENS BEFORE THE READ**: this node's own board is part of what comes back, so reading first would hand back a header the same call is about to change. It is `idempotent` because running it again with the same session and focus leaves the same state with a later stamp. **ITS BOARD IS `wb show`'S DEFAULT, THE LIVE SET, AND IT TAKES `--all` FOR THE SAME REASON** (hv, 2026-09-13): a session start asks what still needs an answer, and an inbox that only grows would otherwise answer it with the archive. **`hv`'S STANDING CONTENT COMES BACK IN FULL AND EVERY PEER SAYS WHAT IT LEAVES OUT** (issue 0416, hv decision 18, 2026-09-16): a node read its own watch-outs in full and no peer's at all, so the read looked complete, and it booted past `hv`'s ruling that a flake was known. The standing kinds are `facade::STANDING_KINDS`, in `views::BOARD_SECTIONS` order; archived items are neither shown nor counted.
...
- **v2:** new-surface
- **Arguments:**
  - `moniker` (node, arity `0..1`) -- The node's handle, eg `qa`: its routing key and the name of its board's directory. With no moniker and no flags, the roster is read from each node's own board header
- **Flags:**
...
- **basis:** ST0076 WP-05, vc decision 25 (2026-09-17, under hv's pen) on cc's measurement of rust-analyzer's SCIP export, and vc's rulings the same day on dc's review of the reader contract: level 3 is one core for every language, filled only by this verb. The name `intent index resolve` was agreed by hv, 2026-09-17. **A RESOLVED ROW ALWAYS JOINS A WRITTEN REFERENCE** on path, line and name, against the bytes the tool read, so a reference only the toolchain sees is counted and not stored; matched, unmatched and dropped partition everything the tool emitted, and dropped is counted by reason from a closed roster. **A RUN REPLACES EXACTLY THE FILES IT JOINED, IN ONE TRANSACTION**, purges rows whose path has left the index, and a failure writes only the language's record; a reconcile never deletes a resolved row. **THE INDEX CATCHES UP AFTER THE TOOL HAS READ AND BEFORE THE JOIN**, so an edit before the tool's read joins and an edit after it is dropped as moved and retraced next run. **A RUN IS FULL, WHATEVER WAS ASKED, UNTIL ONE HAS STORED UNDER THIS BUILD'S EXTRACTOR AND THE LAST ONE STORED**, because an incremental run over an existing build cache can trace nothing and report an empty tier as a success. **A DECLARED LANGUAGE WHOSE PROJECT HOLDS NOTHING FOR ITS TOOL IS NAMED NOT APPLICABLE AND ITS RECORD LEFT ALONE**, unless `--lang` asked for it, when it fails. `index status` reports each language's state; the counts and drops by reason of the run that stored, with how many of the files holding the language's rows that run joined (`joined K of F files`), because an incremental run joins only what its tool rebuilt; and the paths gone stale. The toolchain builds into `intent/.cache/resolve/<lang>`, never the project's own build directory, and that directory persists between runs. **ELIXIR COMPILES A MIX PROJECT AT THE PROJECT ROOT WITH THE COMPILER'S OWN TRACER** (ST0076 WP-06, vc decision 27), into `intent/.cache/resolve/elixir/dev`, the `dev` environment whatever `MIX_ENV` the shell holds (vc, 2026-09-17), seeded from the project's `_build/dev/lib` and made again when `mix.lock` changes; a run that makes it, the first run and `--full` force the compile, and every other run is incremental. A call or module reference the compiler gives no column is a macro's expansion, counted and never joined.
- **owner wp:** WP-05
- **acceptance:** AC-05.1, AC-06.1
- **recoverability:** idempotent
- **recoverability anomaly:** IDEMPOTENT AND WITHHELD ANYWAY, AND THE GROUND IS THAT IT RUNS THE PROJECT'S OWN CODE -- recorded here rather than solved by bending the label, which is what this field is for. Running it twice over unchanged files stores the same rows. **What it does is run each language's toolchain over the project**: rust-analyzer's SCIP export runs the workspace's build scripts and proc macros, and no switch it has stops that (measured by cc, 2026-09-17), and a compile runs a project's macros. Where a Rust workspace root holds no `Cargo.lock`, cargo writes one there, as any cargo command would, and Intent leaves it. Where a Mix project's dependency must still be built in Intent's build directory, rebar3 writes its per-dependency cache files under `deps/`, and Intent leaves them. That is work an agent must not start unasked, which is vc decision 25 (1): never intentd, a reconcile or a hook, and not the MCP tool tier in this release.
...
- **Resolution:** AC-07.6 (vc, 2026-08-14): generated views are EXCLUDED from the formatter repo-wide, converged at `init` and migration so consumer repos inherit it -- a local `.prettierignore` would fix this tree and leave the shipped skew check crying wolf in Lamplight, Utilz and Baize forever. Deliberately not landing today: the pre-commit hook rewrites every staged `.md` and `git add`s it, and other nodes hold uncommitted work.
...
### EXP-09 -- The ROOT intrinsics are on the surface and NO row declares them -- and `--version` swallows every argument after it at exit 0

- **Detail:** `-V` / `--version` and the root `--help` reach the surface of the shipped binary and **no entry in this table declares them.** The `version` SUBCOMMAND row is a different thing and declares `flags: []`; the flags carrying `disposition: intrinsic` are all `--help` / `-h` on named commands. The only place the root `-V` is written down at all is a prose `spelling_note` on `schema --versions`, which mentions it to explain why that flag is plural. **So the register describes a binary whose first-typed surface it does not cover.**
- **Resolution:** OPEN, and deliberately not resolved by the node that found it. Three things are entangled and only the first is mechanical. **(1)** The root intrinsics want a home in this table so that something can say what they do; there is no `root` entry today and adding one is a schema question. **(2)** `nothing_reaches_the_surface_that_is_not_in_the_table` passes right now with `-V` on the surface and undeclared, so **that guard's population excludes root flags** -- a hole in the instrument, found by a question about a feature rather than by the instrument. **(3)** Whether the swallow is a DEFECT or an INTRINSIC we accept is a ruling: clap's short-circuit is upstream behaviour and refusing it means intercepting before clap, which buys INV-02 conformance at the cost of reimplementing an intrinsic. **Third instance of the same class** -- `info`'s catch-all `args` slot and `st show`'s discarded `file` positional (AC-05.7) are the other two, and this one is on the command every user types first.
- **Consequence for the generator:** None today: the generator renders the entries it is given and the root intrinsics are not entries. It gains one the moment (1) is answered -- a root row or an `intrinsics` block would need rendering, and until then **the rendered view is silently complete about a surface it does not cover.**

### EXP-10 -- The register has no row for id normalisation because the question was never asked

- **Detail:** **A BEHAVIOUR SPANNING VERBS ACROSS FAMILIES AND BOTH DOORS WAS NEVER ASKED ABOUT IN THIS ARTEFACT.** vc's sweep: seven `normalis` hits in the rendered register, every one about STATUS normalisation or `organise`->`organize`. Id normalisation appears nowhere. It was not recorded-and-deferred, which a reader could act on -- it was unrecorded, and **the register's whole claim is that a row's existence means the question was asked.** The gap was found by an operator typing `intent st edit 59`, not by the artefact. **SO THE FINDING IS ABOUT THE POPULATION AND NOT THIS ROW**: nothing here establishes that id normalisation is the only such question, and the method that missed it was not id-specific.
...
- **Consequence for the generator:** **NOTHING IN THIS GENERATOR WOULD HAVE CAUGHT IT AND NOTHING HERE NOW DOES.** The table's unit is the command and its declared flags; id normalisation is a property of ARGUMENT INTERPRETATION shared across many verbs and owned by none of them. A row per command cannot hold a fact that lives between commands. **So EXP-10 is closed for the instance and open for the class**, and adding a twelfth criterion did not narrow the population of questions never asked.

## Parity holes -- what the v2 BATS suite did NOT cover

**A record of the v2 bats suite, which the cut deleted.** A command family with no burning coverage was a parity hole: v3 could change it freely and that suite stayed green. Produced by `parity/tools/coverage_map.sh`, which joined these families against `burn-baseline.tsv` -- the join mattered, because a naive grep reported `treeindex` as well covered when every one of its tests exec'd `bin/intent_treeindex` directly and the dispatcher never saw them.
...
| `todo`      | covered                                  |
| `info`      | covered                                  |
| `config`    | HOLE -- nothing in the estate invokes it |
| `init`      | covered                                  |
| `bootstrap` | covered                                  |
| `doctor`    | covered                                  |
| `upgrade`   | covered                                  |
...
- **Finding:** a few burning tests. Nearly all of `tests/unit/fileindex_commands.bats` bypasses the dispatcher entirely.
...
- **Action:** Either accept it as a named thin spot in the register, or add dispatcher-level tests at WP-06. Naming it is the minimum; silence would let a whole file of tests read as coverage.

### `*` -- METHOD

- **Finding:** The zero for `config` was calibrated before being believed: the same needle returns files for `doctor` (a known-covered control), and a direct grep for `intent config` / `run_intent config` returns nothing.
...
| ------------- | ------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `search`      | [query]                   | --sql <statement>, --limit <n>, --outline <path>, --context <name>, --tier <tier>, --lang <language>, --kind <kind>, --subkind <subkind>, --in <container>, --target <target>, --path <glob>, --no-reconcile, --json | Find where a name is defined, what a file holds or where a name is called, or search the text of the whole project, with hits grouped by tier and the index's freshness; or run one read-only SQL statement over this store's published schema | WP-06        | design.md:68 -- FTS5 across all bodies, from CLI and MCP. There is no bin/intent_search.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `sync`        | [id]...                   | --to-disk, --to-store, --apply, --yes, --plan <digest>                                                                                                                                                               | Print this clone's plan for bringing its store up to the committed extract (`--apply` applies it), or move the two in one named direction                                                                                                      | WP-06        | hv, 2026-08-14: 'Syncing back-and-forth from disk to db is something that is triggered manually (eg via intent sync) or periodically (eg via intentd running its async synchronisation process).' Daily-driver commands answer from the store and never scan the tree; this is the expensive, infrequent half of that split.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `ingest`      | [path]                    | --from-md                                                                                                                                                                                                            | Check that an estate's markdown would migrate, naming each residue line; writes nothing (`upgrade` migrates)                                                                                                                                   | WP-03        | design.md:66; WP-03 deliverable, shared with the WP-10 migrator                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| `organize`    | --                        | --plan <digest>, --apply, --default, --force, --verbose/-v, --quiet/-q                                                                                                                                               | Reconcile the tree with .intentfiles: realise what is declared, remove what is not (previews unless --apply)                                                                                                                                   | ST0057 WP-04 | D57-3 (hv, 2026-08-18): the reconciliation verb of the sparse-projection model -- hydrate, verify, dehydrate, and refuse. Implemented at `intentsvcs::organize`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `edit`        | [kind] [id] [file]        | --editor <program>, --path, --browser                                                                                                                                                                                | Open an artefact's file, realising it first so the path exists: in $VISUAL or $EDITOR on a terminal, and as a printed path when stdout is not one (`--path` prints, `--editor` opens)                                                          | WP-05        | AC-05.1 / AC-05.3. **`st edit` printed a path and left the operator to find out whether it was there** -- v2's own note says the thread DIRECTORY must exist and the file need not. Under the disk model an artefact may legitimately have no realised form at all, so the same command on a dehydrated thread names a path to nothing. **This verb hydrates first, so the path it prints EXISTS**, and `st edit` becomes a delegate rather than a second implementation: AC-05.3 is explicit that path-printing has ONE home.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `events`      | --                        | --op <op>, --subject <id>, --limit <n>, --format <fmt>                                                                                                                                                               | Show the history this store holds                                                                                                                                                                                                              | WP-09        | The store is the event log's only home, so it is also the only route a person has to read it. Dropping the tracked extract without this verb would trade an unread projection for an unreadable table.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `fc`          | <target> [child]          | --because <text>                                                                                                                                                                                                     | Fiat-close a requirement on authority, recording why -- `--because` is required and its absence refuses                                                                                                                                        | ST0066       | ST0066 AC-00.1, which names this path literally: `intent fc <target> --because "<text>"` fiat-closes any of the four kinds. Top-level rather than per-family because AC-00.3 makes the act CROSS-FAMILY -- an FC on an ST reaches its criteria and tests -- and a family-rooted verb writing into two other families is what family roots exist to prevent.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `surface`     | <subcommand>              | --format terminal/json                                                                                                                                                                                               | Query this build's command surface                                                                                                                                                                                                             | ST0058       | ST0058 AC-00.5. hv ruled 2026-08-25 that retirement is answered OUT OF BAND rather than by a new exit code: `spine.rs` keeps its exit-code decision untouched and a caller branches on MEMBERSHIP in an enumerable roster. hv ruled the SPELLING 2026-08-29, choosing a `surface` root over a bare `retired` leaf and over nesting under `llm` -- `surface/` is already this estate's word for the register, and it gives later surface queries a namespace. The cost was stated and taken: a 14th top-level family holding one leaf today.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `browse`      | <kind> <id>               | --                                                                                                                                                                                                                   | Open an entity in the browser -- the subcommand spelling of `edit --browser`                                                                                                                                                                   | WP-17        | AC-17.6, which names both verbs, and hv's opening ask, which asked for both by name. **THIS ROW EXISTS BECAUSE `ST0058 AC-00.6` REFUSES DISAGREEMENT, NOT DUPLICATION** -- it was driven off `intent3 --version` answering rc=0 while `intent3 version` answered the unwired marker, and spellings that AGREE are what it asks for. An earlier reading in `tui-design.md` section 9 turned _must agree_ into _must not both exist_ and deleted this verb on the strength of it; the bullet is gone rather than struck through (vc, 2026-08-29, on ic's finding).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `explore`     | [address]                 | --                                                                                                                                                                                                                   | Open the TUI at the top of the model, at a view such as `issues`, or near an address if one is given                                                                                                                                           | WP-17        | hv, 2026-08-30, first-hand: _`intent explore` fires up the intent TUI at the top level ... when I do things like `intent st edit 56` I'm deep linking into an entity in the db, but I just wanted to explore it from the top._ **THE NAME IS `explore` AND NOT `browse` ON THE DESIGN'S OWN GROUNDS RATHER THAN TASTE**: `browse <kind> <id>` already ships meaning _open in the BROWSER_, and one word carrying two meanings on one surface is the defect `tui-design.md` section 5 records for `Up`/`Down` and section 4 for `/`-as-both-menu-and-prefix. **IT TAKES AN OPTIONAL ADDRESS, RULED BY hv 2026-08-30**, superseding vc's no-address ruling of the same day, which was made while hv was AFK. **The superseded text is DELETED rather than banner-marked**, per `restart.md`'s own rule about supersedes banners. hv, verbatim: _I see no reason why `intent expl[ore] ...` couldn't take an `intent://...` URL scheme URI or even an ID that it tries to match to something if it can, and if it can't it just opens at the root. That doesn't obviate `intent edit ...` in any way. It's just convenience for the user as they need._ **THE THREE-DOORS OBJECTION DOES NOT REACH IT, AND THE GROUND IS IN THIS TABLE RATHER THAN IN AN ARGUMENT**: `edit` and `browse` ALREADY declare their `id` as `address-or-id` carrying the note _`address::promote` stays the one door_, so `explore` reuses that word and that door and differs only in arity. Deep linking remains `edit`'s job; this is the browser opening NEAR a thing rather than acting ON one, which is why the verbs diverge on the miss: **`edit ST9999` must REFUSE because it was asked to act on a specific thing, and `explore ST9999` opens at the root because it was asked to open the explorer.** **AND THE FALLBACK ANNOUNCES ITSELF.** hv said _it just opens at the root_; the honest form is the root PLUS the reason on the info row, because a browser that silently opens somewhere other than you asked is the answer-confidently-from-partial-evidence class v3 exists to remove. **THERE ARE FIVE OUTCOMES, NOT TWO**, and they must read differently: it resolves and is present; it resolves and is ABSENT (`ST9999` -- `promote` is purely syntactic and never reads the store, so a landing computed from the grammar alone paints a blank form, which is reachable-and-blank reading as data); it is a REAL address whose form has no view (`intent:///nodes/vc` -- six of thirteen `Entity` forms have views); it names TWO things (`1`); it names nothing (`banana`). **`explore` IS NOT A SECOND SUBSYSTEM -- it is the same view stack with a different BOTTOM**, rooted at the entity kinds rather than at one item, which is why the whole addition is a verb rather than a realiser. |
| `discover`    | [fromdir]                 | --depth <n>                                                                                                                                                                                                          | Find Intent projects under a directory and register the ones this build can serve                                                                                                                                                              | ST0074 WP-03 | hv, 2026-09-13, first-hand: _we need an 'intent discover [fromdir]' command that looks for all intent projects from the current dir (by default) or a dir that is passed in as a param. Whenever it finds one, it confirms that it is 'config compatible' with the current intentd, and then ensures that it is registered in the global config for intentd to find._ The walk's bounds and what compatible means are in ST0074's design.md, WP-03.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `help`        | --                        | --                                                                                                                                                                                                                   | Show usage for Intent                                                                                                                                                                                                                          | WP-05        | hv, 2026-08-30, ruling `help` INTO the 3.0.0 cut at the ROOT ONLY. The `help` FAMILY's `target.spelling_note` already scheduled exactly this -- _the v2 face stays retired and the TOKEN is reclaimed by a new program, declared in `new_surface`_ -- so this row lands a ruling the table had been carrying as pending since 2026-08-26. hv's fuller `<cmd> help` shape (man-style `.md` on the WHY/WHAT while `<cmd> --help` keeps the params/HOW) is RECORDED AND POST-TAG; it is not what this row ships.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `graphql`     | <query>                   | --variables <JSON>                                                                                                                                                                                                   | Execute a GraphQL document against this project through intentd -- the read-only escape hatch                                                                                                                                                  | WP-09        | design.md:86 -- the `intent_graphql` escape hatch beside the typed MCP tier; `AC-00.4`'s `bridging to intentd when it is up`. hv ruled it IN before the tag (2026-08-31, first-hand in vc's session, hv's own selection from three options vc authored, over vc's descope recommendation); vc bounded it under the pen to READS ONLY, derived from one roster, mutations NOT BUILT (hv/wip.md, the escape-hatch entry). That bound is ARCHITECTURAL and not a release boundary -- EmptyMutation is what ships, read-first is D37 declared direction, and a GraphQL mutation path would be a second home for the serve() roster. Its wording carried a scope claim until 2026-09-08 and this citation did not follow the source when the source was corrected. No v2 antecedent. DAEMON-ONLY BY RULING: executing a document needs an async runtime and the CLI carries none (`cargo tree -p intent-cli -i tokio` matches no package), so both faces bridge over `wire::ask` and refuse naming `intent daemon start` when no daemon answers -- never an in-process executor. NOT in `SERVED_BY_DAEMON`: it has no in-process twin, so the `AC-08.2` identity claim cannot be made for it; declared at `hatch::DAEMON_ONLY` with that reason.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `set`         | <address> <field> [value] | --from <file>                                                                                                                                                                                                        | Write one field of an addressed entity -- a thread's title or prose, a work package's body -- from a value or a file                                                                                                                           | WP-05        | Issues 0154 (a work package's body has no door) and 0185 (no verb writes a thread's title, objective, context or body), ruled together by vc on 2026-09-11 as ONE door over the existing `Facade::set` -- the generic single-field setter that intentd's `Op::Set` and the TUI's edit handoff already reach, and that no scriptable surface called. The spelling is the name the door already has in both of those layers, so it adds no vocabulary.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `outstanding` | --                        | --show <kinds>, --width <n>, --markdown, --format terminal/md                                                                                                                                                        | Show the outstanding steel threads, work packages and issues in one table                                                                                                                                                                      | ST0079       | hv, 2026-09-21 (ST0079): a snapshot of what is open took three commands -- `intent st list && echo " " && intent issues`, and `intent wp list` per thread -- and takes one. Its rows are the sibling verbs' own defaults, read from the one definition in `intentsvcs::outstanding` that `st list` and `issues` read too, so it cannot disagree with them about what is open.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |

### `search`

- **serving op:** Search
- **when to use:** USE IT FIRST for a code lookup by name, instead of a grep, a glob and several reads: where a function, type or module is defined (`kind` def with the name as `query`), a file's symbols and their line spans (`outline`), or a definition and the places its name is called (`context`). Narrow any of them with `subkind`, the language's own word for what a symbol is (struct, method, defp, call, use), and `in`, the module, impl, trait or type it sits in. With no `query`, `subkind` and `in` are the question on their own: `subkind` method with `in` a type lists that type's methods. Every symbol hit carries its `subkind`, `container`, `qualifier` and `level`. Level 1 is read from the file's syntax and matched by name, never resolved to the definition it names, and level 2 is read from the file's syntax with the qualifier it was written with, a Rust path as written or an Elixir module with the file's own aliases and `__MODULE__` expanded, still never resolved to the definition it names; in Rust, scoped calls, uses of a type and names inside a macro are references, and in Elixir, remote calls with their module, captures, pipes, and use, import, require and alias are. What is not: in Rust, inside a macro invocation a name the tokens do not show as a call or a path is an occurrence of the name rather than a use, and its qualifier is only the one segment before it; a name in a nested use list such as use a::{b::C} has no use row; and a name passed as a value outside a macro, such as map(f), is not a reference; in Elixir, nothing import or use brings in is applied, so a call to an imported function, Kernel's included, is a level-1 reference with no module; and a call through a module held in a variable, such as mod.fun(..), has no qualifier; Swift files give definitions only. Level 3 is resolved to the definition it names by the language's own toolchain, once a person has run `intent index resolve`, which runs the project's build: a reference whose key joins exactly one resolved row carries `level` 3 with `target`, `target_path` and `target_line`, and one joining several keeps its syntax level and lists them as `candidates`. `target` asks for the references resolved to one definition, spelled exactly as a hit's `target` prints it (a callable Rust target ends in `()`, an Elixir one in its `/arity`); it narrows every other question, with no `query` it lists them, and it never lists a call only the toolchain sees, which `intent index status` counts as unmatched. A target no resolved row names is refused with the resolved targets that end the same way, and otherwise its empty answer says a target nothing references and a misspelt one read the same. Every answer's `index.resolution` names each language whose level 3 is not current, by `state`: `missing` (its toolchain is not where Intent can run it, so only what an earlier run stored answers at level 3), `failed` (its last resolution run failed, so only what an earlier run stored answers at level 3), `stale` (the files it lists changed since they were resolved, so their references answer at their syntax level until the next run) or `unresolved` (no resolution run has stored its references here, so they answer at their syntax level), which is named only where the index holds the file the language's reader finds a project by, such as a `Cargo.toml`. USE IT ALSO when the question is WHERE IS THIS WRITTEN across the whole project -- steel threads, work packages, issues, the docs tree and the source, ranked. DO NOT USE IT when you already know the path: read the file. DO NOT USE IT for a regex or for every literal occurrence: grep is exact, always fresh and needs no index. AND WHEN THE ANSWER SAYS `complete: false`, TRUST GREP OVER IT FOR WHAT IT NAMES: a binary file or a symlink is listed under `skipped` and leaves the answer complete, so `complete: false` means text the index has not read, or, for a `def`, `outline`, `context` or filter question, a language whose symbols the index does not hold, listed under `index.unindexed`. The index parses a file by its extension, so a script without one has no symbols.
- **v2:** new-surface
- **acceptance:** AC-06.4 (added by vc, 2026-08-14, on the finding that no AC covered search)
- **disposition:** new-surface
- **facade:** search
- **Flags:**
  - `--sql` `<statement>` (string) -- Run ONE read-only SQL statement over this store instead of a text search
    - WP-17 (AC-17.1): the structured door is a FLAG and nothing is auto-detected from the query's first word, because `select` is also a thing a person searches for. A query and `--sql` together is a usage error, and so is neither.
    - **disposition:** keep
  - `--limit` `<n>` (string) -- Return at most n rows; the answer reports both what matched and what came back
    - **disposition:** keep
  - `--outline` `<path>` (string) -- List one file's symbols with their spans, instead of searching for text
    - WP-24 (AC-24.3): the answer grep cannot give. An agent asks what is in a file and reads the whole file to find out; this replaces read-the-file with read-this-span, which is where the saving is. ONE STRUCTURAL GROUP AND NO LEXICAL ONE: the question names a path rather than words, and an empty lexical group would say a text search ran and found nothing.
    - **disposition:** keep
  - `--context` `<name>` (string) -- Show where a name is defined and where it is called, as source spans, instead of searching for text
    - WP-24 (AC-24.3): the thing an agent does today with a grep, a glob and several reads, in one call. **IT SAYS CALLED RATHER THAN OCCURS, AND THE NARROWING WAS FOUND BY DRIVING IT** (ic, 2026-09-12): the extractor is each grammar's own tags query (AC-20.2), and Rust's tags CALLS rather than type mentions -- so a type used in a signature or a `use` path is not a reference here, and a claim of `where it occurs` was wider than the thing. The references it does report are NAME-MATCHED and never callers in the resolution sense (AC-20.5); the two words are about different questions.
    - **disposition:** keep
  - `--tier` `<tier>` (string) -- Ask only these tiers (lexical, structural, semantic); repeatable
    - WP-24 (AC-24.3's siblings, wired when the second tier existed): AN UNASKED TIER IS ABSENT FROM THE ANSWER, NOT PRESENT AND EMPTY -- an empty group says the tier ran and found nothing, which is a claim nobody made by narrowing. It is also not queried at all, so narrowing the question narrows the work. `semantic` is accepted and answers nothing: the tier is declared and unbuilt (WP-23), and refusing the word would say it does not exist.
    - **arity:** 1..n
    - **disposition:** keep
  - `--lang` `<language>` (string) -- Keep only hits in this language; repeatable
    - WP-24: a hit with no language -- prose, canon -- is excluded by ANY language filter, because it is not of that language. The vocabulary is the project's declared `languages`, which is what the index carries.
    - **arity:** 1..n
    - **disposition:** keep
  - `--kind` `<kind>` (string) -- Keep only hits of this kind (def, ref, file, thread, wp, issue, attachment, project); repeatable
    - WP-19 (AC-19.1): a filter is part of the question, so `matched` counts what survives it. An unknown kind is REFUSED and names the set -- a filter that silently kept nothing would answer a typo with a confident empty result.
    - **arity:** 1..n
    - **disposition:** keep
  - `--subkind` `<subkind>` (string) -- Keep only symbols of this subkind, in the language's own words (struct, method, defp, call); repeatable, and with no query it lists them
    - ST0076 WP-04 (AC-04.1): a flag of its own and not more words for `--kind`, on vc's ruling of 2026-09-17 -- one word carrying two vocabularies meant `--kind def` read as every definition to one caller and as an Elixir public clause to another. The roster is derived from the index's own queries for the languages in scope, and an unknown subkind is refused with that roster.
    - **arity:** 1..n
    - **disposition:** keep
  - `--in` `<container>` (string) -- Keep only symbols inside this container (a module, impl, trait or type), named whole or by its last segment; with no query it lists them
    - ST0076 WP-04 (AC-04.1): matched against the symbol's `container` column, whole or after its last `::` or `.`, so `--in AddressError` finds a method whose container is `errors::AddressError`. A hit with no container is left out.
    - **disposition:** keep
  - `--target` `<target>` (string) -- Keep only references resolved to this definition, spelled exactly as a hit's `target` prints it; with no query it lists them
    - ST0076 WP-07 (AC-07.2): matched exactly against a resolved row's `target`, so it narrows every other door and every other filter narrows it, and a definition is never kept. A target no resolved row names is refused with the resolved targets that end the same way, at a segment boundary and with or without a trailing arity or `()`, and otherwise answers empty with a note that a target nothing references and a misspelt one read the same. An index with no stored resolution is refused with the remedy `intent index resolve` rather than answered as an empty list.
    - **disposition:** keep
  - `--path` `<glob>` (string) -- Keep only hits whose project-relative path matches this glob (`*` within a segment, `**` across)
    - WP-19 (AC-19.1).
    - **disposition:** keep
  - `--no-reconcile` (bool) -- Answer from the index as it stands, without reconciling it against the tree first
    - WP-22 (AC-22.2): WITHOUT `--daemon` THE QUERY RECONCILES FIRST, because nothing else is keeping this process's index current -- an unreconciled answer would report a tree that has moved since, and `complete: true` beside it would say so confidently. WITH `--daemon` the watcher keeps the index current continuously and no query reconciles, so this flag names on the in-process path what the daemon path already does; it is accepted on both rather than refused on one, because _do not reconcile before answering_ is true of both and a refusal would be for a contradiction that is not there. Reach for it when the question is WHAT DOES THE INDEX HOLD rather than what is in the tree, and when a reconcile over a large tree costs more than the staleness does -- the envelope's freshness block says which one you got either way.
    - **disposition:** keep
    - **exposed on mcp:** false
  - `--json` (bool) -- Emit as JSON instead of prose
    - terminal-channel, as `events --format` is: it selects the terminal rendering, and the MCP tool always answers the envelope
    - **disposition:** keep
    - **exposed on mcp:** false
- **MCP:** exposed as an agent tool -- read-only
...
- **v2:** new-surface
- **note:** NOT the same command as `st sync`, and NOT a superset of it either. v2's `st sync` composes `list` and PRINTS the thread table; only `--write` persists `steel_threads.md` (bin/intent_st:1145-1211, verified by ic). Reconciling the store from canon is a different job, so they are separate commands sharing a name and v3 treats them as such. Added by cc at build time (2026-08-14); second clause originally read "v2's job is a strict subset of this reconciliation and both spellings run it", corrected 2026-08-15 after cc found their own test could not catch it -- it was written from the same misreading as the code, asserted the spellings produce identical bytes, and passed.
...
- **Flags:**
  - `--to-disk` (bool) -- Write the store out to the committed extract
    - **disposition:** keep
  - `--to-store` (bool) -- Read the committed extract into the store -- every thread, or only the ids named
    - **disposition:** keep
    - **disposition basis:** RULED `keep` by ic, 2026-08-15, on the boundary declared at `boundary_ruled` below. It was `pending` on the ground that `sync --to-store` and `ingest` might be spellings of one act; they are not, and the flag ships. cc had already implemented it and stopped rather than let the code answer the open question -- which was right, and is the reason this row is ruled rather than discovered.
  - `--apply` (bool) -- Apply this clone's plan, which bare `intent sync` prints: quiet steps run, a reversible step asks y/N and a non-reversible one always asks a person; with no terminal only the quiet steps run and what is left is named
    - ST0078, hv's ruling of 2026-09-18: `intent sync [--apply] [--to-disk|--to-store]`. Bare `sync` prints the plan and writes nothing; `--apply` applies it; `--apply` with a direction is refused. P3 seeds the plan with one quiet step, the daemon's own pass (`Facade::ingest_from_disk` under 0441's hold-unless-moved lock), so it runs beside a daemon rather than refusing like the two directions, and deletes a row only where the store wrote the canon file and the file is gone, never one whose file was never written. P5 adds the other steps. NOT the `ingest` verb, whose input is FOREIGN markdown (`boundary_ruled` below): this reads the extract this tool wrote. The git hooks' template spells this flag, held by a test. P5 (WP-05, AC-05.1 to 05.4) extends the plan to a pull's whole aftermath: a branch behind its upstream (said, never pulled), a canon add/add renumbered to the next id free in the store and the tree, a canon content conflict (a person picks a side), the ingest, unmerged generated views regenerated and staged, stale views, a stale index, and doctor last with its verdict as the exit code. The only git write is staging what it regenerated to resolve a conflict it was asked to resolve; it never pulls, commits or pushes.
    - **disposition:** keep
  - `--yes` (bool) -- Answer yes to every reversible step of `--apply` (a renumber, staging regenerated views, the index); never answers a non-reversible one
    - **disposition:** keep
    - **disposition basis:** ST0078 design, P5, AC-05.3: a reversible step asks y/N and `--yes` answers those; a non-reversible step always asks a person and no flag or environment variable answers for it, the rule `organize --default --force` already enforces.
  - `--plan` `<digest>` (string) -- The digest of a plan you were shown; `--apply` refuses if the tree has moved since
    - **disposition:** keep
    - **disposition basis:** Reused from `organize --plan`, the same digest over the same tree (ST0078 design, P5, AC-05.1): a plan and the `--apply` that follows it are two runs, and naming the digest proves the apply is the plan that was read.
- **MCP:** not exposed -- **mutates**
- **Wants review:**
  - uncertain on `exposed_on_mcp`
...
- **disposition:** new-surface
- **read or mutate correction:** CORRECTED 2026-09-25 (issue 0573). Was `mutate` and `one-way`. `ingest` checks that an estate's markdown would migrate and writes nothing, as its help says; `upgrade` is the verb that migrates. Measured on the v3 binary: the project tree is byte-identical before and after. `exposed_on_mcp` stays false: the row has no facade door, and an exposed row with none is refused.
- **Flags:**
  - `--from-md` (bool)
    - **disposition:** retire
    - **disposition basis:** RULED `retire` BY vc, 2026-09-09, REVERSING THEIR OWN `keep` OF 2026-08-15 BECAUSE ITS PREMISE IS MEASURABLY GONE. That ruling held that withdrawal would put this table in contradiction with a ratified row, and routed ic's objection to AC-10.2/10.3 where its acceptance lands. Driven 2026-09-09: **AC-10.2 and AC-10.3 contain ZERO occurrences of this flag** -- AC-10.2 ratifies that Phase A residue yields BLOCKED with a per-line classed report, AC-10.3 that a clean estate converts in one commit. **Both ratify the ingest capability's BEHAVIOUR; neither ratifies the flag's EXISTENCE**, which is the same two-fields error this table documents elsewhere: `disposition` asks whether the thing survives, `target.state` asks what v3 does. The five citations across four artefacts that carried the other leg are gone from the live tree. **ic's SUBSTANTIVE OBJECTION WAS RIGHT AND WAS OVERRULED ON A PROCEDURAL GROUND THAT HAS SINCE EVAPORATED**: this is a mode flag with ONE mode, because the other thing bare `ingest` could have meant -- rebuilding the store from committed canon -- is already `sync --from-disk`. Wiring the verb turned that judgement into a measurement and it came out ic's way. **NOT `intrinsic`, EXPLICITLY.** That value glosses as ships-and-clap-supplies-it, which is `--help` and `--version`; clap does not supply a domain flag, and relabelling this one would make the row green by describing the flag as something it is not -- the advertised-and-inert lie AC-06.8 exists to stop. **AND NOT WIRED-THEN-IGNORED**: reading a bool that cannot change behaviour satisfies the check in form and commits the defect in substance. **THE GLOSS FITS IMPERFECTLY AND THAT IS RECORDED RATHER THAN PAPERED OVER.** `retire`'s operative half -- never reaches clap -- is exactly right; its gloss also says recorded from v2, and this flag is new-in-v3. vc declined to mint a fifth vocabulary value, because minting one is a ratification act and would risk legitimising a category for flags that ship and are never read. The gloss wording is with hv as tidy-up, not as a blocker.
    - **why not dropped by ic:** THE CITATION COUNT THAT DECIDED THIS IN 2026-08 NO LONGER DESCRIBES THE TREE, AND THAT IS THE LESSON RATHER THAN THE CORRECTION. ic measured five live citations across four artefacts they did not own -- design canon, ratified acceptance, the WP-01 parity register, and a work-package info file -- and proposed to vc rather than dropping a spelling the contract named. **That was correct practice and the measurement was true when taken.** A whole-tree read on 2026-09-09, excluding history and archives, finds none of those four still citing the flag. **A COUNT OF CITATIONS IS A CLAIM ABOUT A CORPUS AT AN INSTANT, AND A RULING THAT RESTS ON ONE INHERITS ITS EXPIRY WITHOUT INHERITING ITS DATE.** Nothing re-checked it for three weeks; the ruling it supported outlived it silently.
...
- **v2:** new-surface
- **config:** Reads `backup.enabled` / `backup.schedule` / `backup.retain.{daily,weekly,monthly}` / `backup.keep` -- named on the `config` entry in this table. `backup.enabled` gates the scheduled backups taken by intentd and `intent explore`, and deliberately does NOT gate this command.
...
- **v2:** new-surface
- **acceptance:** ST0057 AC-04.1 through AC-04.6
- **name reclaimed:** **THE NAME IS RECLAIMED, THE v2 FACE STAYS RETIRED, AND THE TWO SHARE NOTHING BUT THE WORD** (hv, 2026-08-19). The `organize` FAMILY in this table is the v2 command -- `bin/intent_organize` plus `st organize` -- which moved ST directories into status folders, and whose ratified retire of 2026-08-14 is untouched: a strictly structured model cannot hold data in the wrong place, so the disorder it repaired cannot arise. This row is a different program that happens to take the same word: it hydrates and dehydrates artefacts against `.intentfiles`. **ASSERTIONS FORBADE THE TOKEN, NOT THE FACE** -- `dispatch.rs`, `spine.rs` and `dispatch_ssot.rs` each hard-coded the string `organize`, so the ratification was expressed three times and in a form that could not tell a reclaimed name from a resurrected command. All three now key on the retired v2 ANTECEDENT, which is the thing hv actually retired.
- **disposition:** new-surface
- **recoverability:** idempotent
- **flag polarity ruled by ic:** **OPEN, AND DELIBERATELY NOT DECIDED HERE.** The v2 family's own notes record the defect this row must not inherit: the two v2 faces took OPPOSITE polarity for the same operation -- top-level `organize` acted by default with `--dry-run` to preview, `st organize` previewed by default with `--write` to act. That is a command-surface call and the surface is ic's (AC-05.1); hv ruled the NAME, not the flags. Recorded as an open row rather than settled by whoever wired the entry, because a polarity chosen at build time is exactly how the v2 pair came to disagree. NOTE FOR WHOEVER RULES IT: this verb REMOVES FILES, and the removal is gated on the store reproducing them byte for byte -- so the blast radius is bounded, but it is not zero, and `--dry-run`-style act-by-default puts the destructive path on the short spelling. **RULED 2026-08-19 -- see the `--apply` flag's `disposition_basis` above.** The row is kept rather than deleted because it records WHY the polarity was left open by whoever wired the entry, and that reasoning is the reason the ruling was available to be made by the node that owns the surface instead of settled silently in a commit about something else.
- **hidden aliases:**
  - organise
...
- **facade:** organize
- **Flags:**
  - `--plan` `<digest>` (string) -- The digest of a plan you were shown; the run refuses if the tree has moved since
    - **disposition:** keep
    - **disposition basis:** **hv, 2026-09-12, on silent deletion. A PREVIEW AND THE `--apply` THAT FOLLOWS IT ARE TWO RUNS, AND THE SECOND ONE RE-PLANS.** Between them the estate can move -- every read verb materialises the store on access, so a peer running `intent st list` is enough -- and the act would then perform a plan NOBODY WAS SHOWN, with the preview's reassurance standing over it. The terminal face closes that window itself: it previews, renders, asks, and pins the act to the digest it printed. **A NON-INTERACTIVE CALLER HAS NO SUCH MOMENT**, and the MCP tool's `apply: true` removed files with no preview at all -- so on that surface the digest is REQUIRED, and a caller proves it saw a plan by echoing the `plan` a preview returned. On the terminal it is optional, because the interactive flow already pins; given, it pins to the digest the operator names instead.
  - `--apply` (bool) -- Perform the reconciliation. Without it, organize previews and touches nothing
    - **disposition:** keep
    - **disposition basis:** RULED BY ic, 2026-08-19, on AC-05.1, on the open row below. **`intent organize` PREVIEWS BY DEFAULT AND `--apply` PERFORMS.** The ruling rests on precedent rather than taste: AC-03.9 already settled this shape for the estate's other destructive verb -- the destructive direction states what it will overwrite BEFORE it costs it, because a summary afterwards is a receipt for a loss the operator needed one moment earlier. A second destructive verb answering the same question differently is the Highlander problem arriving in the surface. **`--apply` rather than `--dry-run`, and not only because of the default: `--dry-run` makes safety opt-in, and the operator who most needs the preview is the one who does not know to ask for it.** It also matches `organize::Mode::Apply` in the model, so surface and model spell one concept one way.
  ...
  - `--default` (bool) -- Write .intentfiles from status when it is absent: one STEELTHREAD line per WIP thread and one ISSUE line per open issue
    ...
    - **disposition:** keep
    - **disposition basis:** **hv, 2026-08-26, first-hand: `--default` never removes a file -- _"Correct. Unless it is used with --force, which does remove files, after a confirm."_** ST0057 AC-11.1 through AC-11.4 are this flag's contract, and ST0057 WP-11 owns it. **IT EXISTS BECAUSE ABSENT IS NOT EMPTY.** An absent `.intentfiles` means nobody has said, so everything is realised; an empty one declares that NOTHING is, and `organize --apply` against that would remove the estate. So the file must never come into existence as a side effect, and the operator needs one spelling that writes the declaration a project should have: one `STEELTHREAD:<ID>` line per WIP thread and one `ISSUE:<nnnn>` line per OPEN issue. **THIS ROW SAID `per thread that is not Completed and not Cancelled` UNTIL 2026-09-12, WHICH IS THE DEFINITION BY EXCLUSION hv REJECTED IN AS MANY WORDS ON THE DATE THIS BASIS CARRIES.** _"Now it has NOT STARTED STs!??!"_ and _"It should ONLY HAVE WIP STs!!!!!"_, hv 2026-08-26 on a 57-thread realised set. Stated by exclusion the rule reads careful and is really a list of what to leave out, so every status nobody thought about -- Triage, NotStarted, Hold -- was swept IN by default; stated positively it cannot acquire members by accident. The code was corrected that day and the register was not, so the surface has carried the rejected rule as the flag's contract ever since. **AND `WIP` FOR THREADS BESIDE `OPEN` FOR ISSUES IS NOT AN INCONSISTENCY TO TIDY:** each entity's own status vocabulary defines it. A thread has a rich set of which `Wip` is the part somebody is working on; an issue has exactly Open and Closed, so every open issue is being worked on by definition. Forcing one word across both vocabularies would either sweep the Triage threads back in or drop open issues for a symmetry issues do not have. **THE SAME FUNCTION SERVES `init`, THE MIGRATION AND `upgrade` (AC-11.3), which is a criterion rather than a tidiness: callers deriving _the open set_ separately is a chance each to disagree about what open means, and the one that drifts is the one nobody runs.** `intentfiles::default_declaration` is that function. **Bare `--default` is NON-DESTRUCTIVE in every case**: absent, it writes; present, it changes not one byte, reports how many lines it declares, and names `--force`.
    - **exposed on mcp:** false
  - `--force` (bool) -- With --default, regenerate an existing .intentfiles from status. Refuses without a tty; asks for confirmation on one
    ...
    - **exposed on mcp:** false
  - `--verbose`, `-v` (bool) -- List the unclaimed inventory, and each v2 file a prune refusal covers, as well as the summary
    - **disposition:** keep
    - **disposition basis:** **NEW IN v3 WITH NO v2 ANTECEDENT, AND THE ABSENCE IS MEASURED RATHER THAN ASSUMED**: `bin/intent_organize` parses no verbosity flag at all (0 hits for the verbose/quiet pattern, against 43 for the same pattern in `bin/intent_doctor`, which is the positive control that makes the 0 mean something). So nothing is carried across here and the shape is chosen to match the sibling verb rather than to preserve a behaviour. **THE DEFECT IT ANSWERS, MEASURED BY vc across the fleet 2026-09-07**: `organize` prints pages against `doctor`'s screenful, and Lamplight alone is 2100 of which 2072 are `unclaimed:` -- one per directory, already grouped, under a summary line that already carries the file count AND a digest of the membership. **SCOPE OF THE MOVE**: only the unclaimed inventory goes behind `--verbose`. The per-path record of what was hydrated, rewritten, removed or pruned stays at the default, because removals are this verb's subject and `--apply` exists precisely because a build once advertised its destructive half and did not show it. Refusals print at every verbosity. **RATIFICATION**: hv ruled narrow-by-default with a flag to widen for `doctor --scope` on 2026-09-07; this applies that ruled principle to a sibling verb by analogy. It is NOT ruled for `organize` specifically and is reversible in one word. **THE v2 PRUNE REFUSAL JOINED IT, 2026-09-13 (issue 0318, vc under the pen)**: hv met the per-file form on Laksa, where a PREVIEW printed an `error:` line per unheld v2 file with its remedy repeated each time. The refusal now prints once as a class line carrying the count, the tense of the run and one remedy, on stderr at every verbosity; the files it covers are inventory of the same kind as the unclaimed directories, listed with their reasons under this flag and announced as withheld at the default. The summary `refused` count and the MCP per-path list are unchanged.
    - **exposed on mcp:** false
  - `--quiet`, `-q` (bool) -- Only the summary line and refusals
    - **disposition:** keep
    - **disposition basis:** **THE SIBLING HALF OF `--verbose` ON THIS ENTRY; READ ITS BASIS FOR THE MEASUREMENT AND THE RATIFICATION.** Row-specific: **QUIET WINS OVER VERBOSE** when both are given, which is the rule `doctor` already applies (carried there from `bin/intent_doctor:134`). Two sibling verbs answering one question two ways is the surface Highlander problem, so the rule is adopted rather than re-decided. **THE SUMMARY LINE SURVIVES `--quiet`**, deliberately and for `doctor`'s stated reason: a quiet clean run that printed nothing at all would be indistinguishable from the command never having run. **REFUSALS SURVIVE IT TOO** -- they move the exit code on the apply path, and a quiet flag that could hide a refused removal would be hiding the one thing an operator must see.
    - **exposed on mcp:** false
- **MCP:** exposed as an agent tool -- **mutates**
...
- **v2:** new-surface
- **side effect ruled:** hv, 2026-08-19. A generated view is REFUSED and the refusal NAMES THE AUTHORING SURFACE -- `acceptance.md` -> `intent ac` / `intent at`; `info.md` and WP covers -> `intent st` / `intent wp`. **Detection is not prevention**: AC-03.4 catches a hand-edited view AFTER it is written and the work is already gone. An ATTACHMENT is handed over without hesitation, because the file on disk IS the authoring surface. Derived from `Project::edit_disposition`, itself derived from `classify()`, so there is no second answer to what a file is.
- **disposition:** new-surface
- **recoverability:** idempotent
- **recoverability anomaly:** **WITHHELD FROM MCP WHILE BEING RECOVERABLE, AND THE GROUND IS THE RETURN VALUE RATHER THAN THE RISK.** The withhold list is derived from `recoverability` because MCP should not expose a mutation the surface cannot undo -- and this one it can: `edit` delegates to `Facade::hydrate`, idempotent in both of its steps, and adds only a read on top. **It is withheld anyway because what it RETURNS is an absolute filesystem path on this machine**, which is not merely unhelpful to a remote caller but actively misleading: a path that exists here and nowhere else, handed to something with no way to open it. The rule the list encodes is about safety and this exclusion is about meaning, so the number is left disagreeing and the reason recorded rather than the label adjusted to make it come out.
- **arg shape change:** **`edit <ADDRESS> [FILE]` BECOMES `edit <KIND> <ID> [FILE]`, AND THE OLD SHAPE MISPARSES RATHER THAN REFUSING.** `intent edit st 68` today reads `st` as the address and `68` as the file, so it refuses about the wrong thing -- which is the guess-from-shape this row exists to remove. **The terminal branch becoming the TUI is a behaviour change for every interactive user**, taken deliberately under hv's no-3.1.0 ruling: all v3 pain lands under one rubric. The register row is authored BEFORE either realiser is built (AC-17.6), so the surface decision is recorded rather than emerging from whichever realiser landed first. **THE FOUR OUTPUT MODES ARE GOVERNED BY INV-10**, which is where the mutual exclusion lives because no single flag can state it.
- **Flags:**
  - `--editor` `<program>` (enum) -- Open the file in $VISUAL or $EDITOR, or in the named program, whatever stdout is
    - **disposition:** keep
    - **disposition basis:** hv, 2026-08-29, selecting the TTY form WITH explicit overrides from three options, and superseding two earlier rulings on the same question. The bare TTY form makes behaviour depend on an invisible property of the environment, so a wrapper, a CI job or an editor plugin gets a different result with nothing in the command saying why. This is the explicit spelling of the launch branch. **THE VALUE IS OPTIONAL AND NEEDS `=`.** `--editor=code` names a program; bare `--editor` takes the resolved default. An optional-value flag followed by a positional is ambiguous to clap, and this row has three positionals. **`arity` is the register's OWN field, extended to a second position rather than minted**: it carries `0..1` on 29 of the 125 args that declare it, which is exactly this shape, and giving `flags` the same field is Highlander in its cheapest form (vc, 2026-08-29).
    - **arity:** 0..1
  - `--path` (bool) -- Print the path and open nothing, whatever stdout is
    - **disposition:** keep
    - **disposition basis:** hv, 2026-08-29, the other half of the same ruling. Explicit spelling of the print branch, so a caller needing determinism can say which it wants rather than depending on what stdout happens to be. **INV-10 carries the exclusion and the output contract**: `docs/getting-started.md` has `$EDITOR "$(intent st edit ST0001 design)"` under test, and command substitution makes stdout a pipe, so that invocation keeps receiving a path whatever the terminal branch becomes.
  - `--browser` (bool) -- Open the entity in the browser, served by intentd
    - **disposition:** keep
    - **disposition basis:** **hv's opening ask names both doors** -- _the end game here is that can either: `intent edit st 68`, or browse to something like `intent browse st 68`_. **The URL addresses the ENTITY, not a filename** (`http://127.0.0.1:<port>/threads/ST0058?mode=edit`): the TUI and browser edit the model, and only `--editor` opens a file -- mixing the two is what makes the current argument shape misparse. **localhost HTTP rather than a custom `intent://` scheme**: a scheme needs per-platform OS registration, buys nothing, and adds a URL-handler surface anything on the machine can invoke. **It REFUSES when the daemon is not running, naming `intent daemon start`**, rather than spawning a process the operator did not ask for.
- **MCP:** not exposed -- **mutates**
...
- **facade:** events
- **Flags:**
  - `--op` `<op>` (string) -- Show only events with exactly this op, eg `wp.done`
    - **disposition:** keep
  - `--subject` `<id>` (string) -- Show only events about this subject, eg `ST0000/01`
    - **disposition:** keep
  - `--limit` `<n>` (string) -- Show only the most recent n matching events
    - **disposition:** keep
  - `--format` `<fmt>` (string) -- `json` for one JSON document; otherwise one line per event
...
- **Flags:**
  - `--because` `<text>` (string) -- Why this is being closed on authority rather than met -- REQUIRED
    - **required:** true
    - **disposition:** keep
- **no op:** DECLARED BY ic, NOT YET DRIVEN -- dc to confirm against the machine before this row is trusted. `Fiat` is not among the from-states (`computed` and `unsatisfied` only), so a second `fc` on an already-fiat-closed requirement REFUSES at exit 1 and names `ac reinstate` as the exit. Written from dc's stated machine rather than from the binary, and flagged as such: a declaration that has not been driven is a requirement, not a record.
- **kind is derived:** The kind is read from the TARGET and is never declared. One argument matching `ST\d+` is a thread; one containing `/` is a work package; two arguments with an `AC-` second is a criterion, `AT-` a test. There is deliberately NO `--kind` flag: it would be a second home for a fact the address already carries, and that class has already cost this estate a row -- `ac new`'s `--kind` DEFAULT overwrote a stored value because clap materialises a default whether or not the caller typed it (issue 0119).
- **bare form:** **BARE `intent fc` REFUSES AT rc=1, AND rc=1 IS CORRECT -- DO NOT MAKE IT 2.** Measured by dc against a build carrying this row: `error: the following required arguments were not provided: --because <text>, <TARGET>`. A missing required argument is a USAGE error, which is rc=1 by INV-02; rc=2 means `known command, not implemented`, which is a different fact. The property ic asked for -- bare must refuse rather than dispatch to help -- HOLDS, and it holds by clap's required-args refusal rather than by the surface's own arm. That is fine and it stays true after the arm is wired.
- **refusal has no structural home:** The `--because` refusal is stated in `help` because the register has nowhere else to put it. That is issue 0142's structural half, and this row is a NEW member of the population it describes -- a handful of shipped rows assert a precondition in help, one of them (`at green`) measured FALSE, and the detector that found them is a regex over six English words that this row's phrasing would also evade. Recorded here so the membership is declared rather than discovered later.
- **rulings:**
  - `0.state`: ratified
  - `0.authority`: hv
  - `0.date`: 2026-08-29
...
- **Flags:**
  - `--format` `terminal|json` (enum) -- Output format
    - **disposition:** keep
    - **default:** terminal
    - **disposition basis:** **`--format`, NOT `--json`, ON hv's 2026-08-21 SPELLING RULING** -- `--format` where the verb has formats other than json, and `--json` kept ONLY as a v2 parity alias because a parity contract is not a backwards-compatibility shim. **`surface` is new surface with no v2 ancestor**, so there was no alias to inherit and `--json` here was a second spelling MINTED rather than kept. **Filed against itself: ic authored this row and the deviation, found it while authoring `daemon status` under the same ruling, and raised it rather than quietly correcting it** -- the point of raising a deviation is to have it ruled and recorded, and vc ruled it on 2026-08-30. Rows carried `--json`; the others (`issues show`, `todo`, `todo list`) each have v2 parity to point at. This one did not, and it also carried NO `disposition_basis` at all, which is the half that let it sit unexamined: a spelling with no recorded ground is a spelling nobody can check against the ruling.
- **roster definition:** **`surface retired` PUBLISHES `spine::retired_and_unreachable()`, WHICH IS THE SAME PREDICATE THE EXEC PATH CALLS -- not a second implementation of it.** Two doors onto one roster is the design; two answers to `is this retired` is the divergent-copy shape, in the artefact whose whole job is to stop v3 re-deriving its own surface description. **`table.retired()` ALONE IS NOT THE ROSTER and publishing it would be worse than publishing nothing:** it selects on `!is_shipped()`, which reads two declared fields and asks nothing of the built surface, so it names `organize` -- which has a shipped row and a retired one, sits in `populations.shipped` AND `populations.retired`, and RUNS at rc=0. A caller branching on that list removes a working verb from their automation, which is the exact harm `retired_refusal`'s own comment records being measured. Mutation-proved in a detached worktree: dropping the reachability filter reds `the_enumerable_roster_excludes_a_retired_name_this_build_reclaimed` on the `organize` arm specifically.
- **why not an exit code:** The row this serves previously demanded that retired and unbuilt be distinguishable BY EXIT CODE, and `spine.rs:128-131` accepts the opposite in writing. Both positions were coherent and they contradicted. hv ruled that NEITHER gives: the exit-code decision stands and the question is answered by a list. **So this verb must never acquire an exit code that encodes membership** -- that would reinstate the contradiction it was minted to resolve.
- **bare form:** Bare `intent surface` refuses at rc=1 (clap, subcommand required), as `daemon` does. **It therefore inherits the probe hazard recorded on the `fc` row:** an unwired-ness probe that drives this row BARE reads clap's refusal rather than the arm, so it cannot tell unwired from wired. The probe must construct the invocation from this row's own declared `values`, per vc's rule from cc's bare-`daemon` finding.
- **rulings:**
  - `0.state`: ratified
  - `0.authority`: hv
  - `0.date`: 2026-08-25
...
- **v2:** new-surface
- **twin of:** edit --browser
- **disposition:** new-surface
- **recoverability:** idempotent
- **recoverability anomaly:** **WITHHELD FROM MCP FOR THE SAME REASON AS ITS TWIN, AND THE TWO MUST AGREE ON THIS TOO.** What it opens is a URL on THIS machine, which is not merely unhelpful to a remote caller but misleading. `edit` records the same exclusion on the same ground; INV-09 is what stops the rows drifting apart on it.
- **MCP:** not exposed -- **mutates**

### `explore`

- **v2:** new-surface
- **acceptance:** ST0056 AC-17.12 (minted by vc 2026-08-30)
- **disposition:** new-surface
- **recoverability:** reversible
- **recoverability anomaly:** **WITHHELD FROM MCP WHILE BEING REVERSIBLE, AND THE GROUND IS THAT IT IS NOT CALLABLE RATHER THAN THAT IT IS DANGEROUS.** The withhold list is derived from `recoverability` because MCP should not expose a mutation the surface cannot undo -- and this one it can: every edit `explore` makes goes through `intentsvcs` like any other client and is reversible by the same doors. **It is withheld because an interactive TUI blocks on keystrokes and needs a terminal, so an agent calling it receives a request that never returns.** That is a fact about the CHANNEL, not about the risk. **IT IS THE THIRD ROW ON THIS EXACT GROUND AND THE PRECEDENT IS WHY IT IS RECORDED RATHER THAN ARGUED**: `edit` and `browse` are both reversible, both withheld, and both on the ground that what they reach is a HUMAN face on THIS machine -- a path or a URL that exists here and nowhere else. `explore` is the third face in that set and the agent's door onto the same model is the daemon's JSON face (AC-08.9), not any of the three. **Recorded rather than resolved by relabelling: calling it `one-way` would make a derived number come out and would be false**, which is the move the `daemon` head row was making while one row stood for several verbs. Same day, ic ruled `organize` EXPOSED under AC-05.1 on the converse reasoning -- blast radius is not the rule, undoability is -- so bending undoability here to cover a channel problem would have contradicted a ruling made hours earlier.
- **MCP:** not exposed -- **mutates**
- **MCP note:** The browser twin of this reach is `edit --browser` / `browse`, which is also `exposed_on_mcp: false`. Both are HUMAN faces onto the same model; the agent's door onto that model is the daemon's JSON face (AC-08.9), not either realiser.
- **MCP classification grounded in:** Withheld because the channel cannot carry it, not because the mutation is unsafe -- the full argument and its precedent are in `recoverability_anomaly`, which is the field the generator reads and therefore the one home for it. What still mutates, since the row takes no arguments and ships no flags: the TUI writes through the facade on `Enter`, and the surface it offers is the form declaration rather than this command line.
...
- **disposition:** new-surface
- **recoverability:** idempotent
- **recoverability anomaly:** **WITHHELD FROM MCP WHILE BEING IDEMPOTENT, AND THE GROUND IS WHERE IT WRITES RATHER THAN WHETHER IT CAN BE UNDONE.** A second run adds nothing, so the label is honest. What it writes is the operator's per-user project registry, outside every project, and what it reads is any directory the caller names: an agent attached to one project has no business walking the operator's disk and enrolling what it finds. The operator runs it.
- **acceptance:** ST0074 AC-03.3
- **Flags:**
  - `--depth` `<n>` (string) -- How many directory levels below the start directory to search
    - **default:** 4
    - **disposition:** keep
- **MCP:** not exposed -- **mutates**
- **MCP classification grounded in:** Withheld for scope, not for danger: the argument is in `recoverability_anomaly`, which is the field the generator reads.

### `help`

- **v2:** new-surface
- **acceptance:** ST0058 AC-00.6 (driven by vc 2026-08-30). The row's falsifier is _any capability reachable by a flag and refused by its subcommand twin, or the reverse_, and `intent --help` rc=0 against `intent help` rc=2 was its last live instance -- issue 0086.
- **name reclaimed:** **THE NAME IS RECLAIMED, THE v2 FACE STAYS RETIRED, AND THIS FOLLOWS `organize` EXACTLY.** The `help` FAMILY in this table is `bin/intent_help`, whose retire is untouched and whose reasons are unchanged: `lib/help/` held help files for only some commands, its grammar was used once each, and it hand-maintained a command list behind a skip list rather than enumerating the surface. **None of that is ported.** The token is taken by a different program: a spelling of the help clap already renders from the compiled-in dispatch table. **AND `spine.rs` FORBADE THE TOKEN RATHER THAN THE FACE** -- `.disable_help_subcommand(true)` -- which is the same three-assertions shape `organize` hit.
- **disposition:** new-surface
- **one home:** **THIS SHIPS NO HELP TEXT OF ITS OWN, AND THAT IS THE POINT.** The arm rebuilds the clap `Command` from the same compiled-in table `--help` uses and asks IT to render, so the twin AC-00.6 is about cannot drift apart later -- there is one renderer with more than one spelling. The estate already carries several homes for _what is Intent_ (`--help`'s command list, `intent llm`'s generated agent guide, `intent info`'s install-and-project status); **a fourth authored page would have been the Highlander violation this row exists to avoid**, and writing one is what `bin/intent_help` did.
- **root only:** **ROOT ONLY, AND THE ARG IS DELIBERATELY ABSENT** (hv, 2026-08-30). The v2 `help` entry declared `command` at arity `0..1`; this row declares no args, because the live twin-pair AC-00.6 fires on is the ROOT one and per-command help already answers as `intent <cmd> --help`. Declaring the arg here would ship a third spelling of per-command help while hv's post-tag `<cmd> help` is a DIFFERENT surface with different content, and the table would then carry a promise the post-tag ruling has to unmake.
- **recoverability absent:** **DECLARED ABSENT, NOT OMITTED.** `help` is a `read`, and `dispatch.rs`'s vocabulary check refuses a recoverability on one -- _the question is vacuous for a command that changes nothing_. Recorded here so the absence reads as the rule rather than as an oversight, which is the table's own standing objection to absence-as-meaning.
- **MCP:** not exposed -- read-only

### `graphql`

- **v2:** new-surface
- **acceptance:** AC-00.4, AC-09.2 (bounded reading ruled 2026-08-31; `graphql_escape_hatch.rs` is the drive, both faces)
- **disposition:** new-surface
- **facade:** graphql
- **Flags:**
  - `--variables` `<JSON>` (string) -- GraphQL variables as a JSON object, given as text
    - **disposition:** keep
    - **disposition basis:** NEW IN v3, NO v2 ANTECEDENT. JSON TEXT ON BOTH FACES because the row's flag type vocabulary is what the clap surface and the MCP schema both derive from -- one row, one type; an object-typed MCP parameter would need a second vocabulary for one flag. Parsed once, in `hatch::variables`, before any daemon is looked for, so a bad value is refused in one voice at exit 1 (INV-04: values are refused in the renderer, never by a value_parser at 2).
    - **exposed on mcp:** true
- **MCP:** exposed as an agent tool -- read-only
...
- **disposition:** new-surface
- **recoverability:** reversible
- **recoverability anomaly:** REVERSIBLE AND WITHHELD FROM MCP, FOR THE REASON `issues edit` IS: AUTHORSHIP, NOT RECOVERABILITY. The verb can write any value back, including the previous one, but it cannot TELL you the previous one -- the prior prose survives only in the event log -- and what it REPLACES is prose a human wrote: a thread's objective, a work package's body. An agent rewriting a human's authored prose is a different act from an agent moving a status, and no ruling grants it on the machine face. Withholding is the recoverable error: if it is wrong, a flag flips.
- **Flags:**
...
- **rulings:**
  ...
  - `0.authority`: vc
  - `0.date`: 2026-09-11
  - `0.record`: intent/whiteboard/ic/wip.md
- **MCP:** not exposed -- **mutates**
...
- **facade:** outstanding
- **Flags:**
  - `--show` `<kinds>` (string) -- A comma-separated list of st, wp, is, issue, issues -- or `all`, the default
    - **disposition:** keep
  - `--width` `<n>` (integer) -- Render at a fixed column width; 0 or absent means terminal width
    - **disposition:** keep
    - **exposed on mcp:** false
