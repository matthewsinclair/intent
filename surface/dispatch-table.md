...
- **no op:** **NOT IMPLEMENTED AT HEAD, and this is measured rather than inferred**: `intent wp rescope ST0001/01 M` exits 2 with `error: `wp rescope` is a known command that is not implemented yet`. Measured 2026-08-17 by driving the verb twice through the real binary (ic). The row exists so the arm can be wired at all -- the spine builds clap FROM the table -- and issue 0052 is the surface gap. **When it lands its no-op is the only one in the population not decidable from the two states alone**: a rescope to the SAME size is a no-op only when the package carries no `scope_legacy`, because with a carry present the same from and the same to IS a movement -- resolving the carry is the change (`facade.rs:1443-1456`, where the facade already gets this right). So the declaration is `ok: <ST/NN> already <size>` when settled, and the ordinary movement message when a legacy value is being resolved.
- **MCP:** exposed as an agent tool -- **mutates**
- **recoverability:** reversible
