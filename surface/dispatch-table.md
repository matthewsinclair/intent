...
- **Flags:**
  ...
  - `--outline` `<path>` (string) -- List one file's symbols with their spans, instead of searching for text
    ...
    - **disposition:** keep
  - `--context` `<name>` (string) -- Show where a name is defined and where it occurs, as source spans, instead of searching for text
    - WP-24 (AC-24.3): the thing an agent does today with a grep, a glob and several reads, in one call. THE OCCURRENCES ARE NAME-MATCHED AND THE HELP SAYS SO -- nothing here resolves a name to the definition it points at, so a `ref` is never a CALLER (AC-20.5).
    - **disposition:** keep
