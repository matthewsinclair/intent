...
- **input selector gap:** SEPARATE FINDING, true whichever way the flag question goes, ic 2026-08-15. **`intent ingest` has no way to say WHAT to ingest** -- measured against the built binary, `Usage: intent ingest [OPTIONS]`, one bool flag and no positional. The recovery path and the v2 migrator both inherently take a source, and the migrator's source is ANOTHER project's tree, which cannot be implied by the working directory. So the row is short an arg regardless of whether `--from-md` survives, and this is not an argument for the flag: a bool cannot carry a path either. Not declared unilaterally, because the natural shape (`path`, arity `0..1` -- given, ingest that tree; omitted, this project's own) presumes the recovery case defaults rather than refuses, and that is cc's call at WP-03 build time.
- **v2:** new-surface
...
- **Flags:**
  - `--from-md` (bool)
    - **disposition:** pending
    - **disposition basis:** STAYS `pending`, and the reason has CHANGED -- it is no longer the undeclared boundary, which `sync.boundary_ruled` now settles (input domain, not direction: `sync` moves this tool's OWN extract, `ingest` takes foreign markdown, and their error contracts differ). What keeps it `pending` is the narrower thing that survives that ruling: **`--from-md` is a mode flag with ONE mode.** Reading md into the store is what `ingest` IS, so the flag distinguishes nothing, and `intent ingest` currently offers no other way to name its input.
    - **why not dropped by ic:** I RULED IT DROPPED AND THEN REVERSED MYSELF ON EVIDENCE, 2026-08-15, and the evidence is the point. **`--from-md` is cited in SEVEN live places across FOUR artefacts I do not own**: `design.md:67` (design canon), `acceptance.md:298` (ratified -- and it explicitly says _the scaffolding still ships in WP-03_, with acceptance at AC-10.2/10.3), `parity.md:70` (the WP-01 register's new-in-v3 row), `WP/03/info.md:22`, plus `intentsvcs/src/ingest.rs:280` and `tests/prose_ingest_fts.rs:37`. Dropping it here would have made this table disagree with the ratified contract -- **the exact decision-drift class AC-05.5 was landed for an hour earlier, manufactured by me, in my own artefact, on the same day.** The command surface is mine (AC-05.1); a spelling the contract NAMES is not mine alone. Proposed to vc instead.
- **MCP:** not exposed -- **mutates**
- **Wants review:**
  - uncertain on `exposed_on_mcp`
