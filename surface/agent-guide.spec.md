...
**D45 makes that policy stop working, and `read_or_mutate` does not inherit it.** If a skill drives `intent` directly then the flag prevents nothing, and the field an agent now reads first is too coarse to carry the distinction: `st new` and `init` are both `mutate`, 51 of the 64 shipped mutations are exposed, and nothing in the projection separates "writes a steel thread" from "reshapes the estate". The policy still exists, correctly applied to all 13 rows, readable only through a field the ruling just demoted to a routing note.

**The partition is exact, and it only became exact once a defect was removed.** The 26 withheld rows are 13 mutations and 13 reads, and those are precisely the 13 leaves and the 13 roots -- no root is withheld for mutating, no leaf is withheld for reading. It did not read that way when vc went to verify the claim: `config` was declared `mutate` while its own help says it displays and its row carries zero args and zero flags, making the split 14/12 and the policy look approximate. **A single wrong field made a rule look like a tendency.** That is the case for declaring the property rather than leaving it derivable -- a derivation is only as sharp as every field it reads, and nothing was checking this one.
