defmodule Mix.Checks.MissingImplAnnotation do
  ...
  defp traverse({:defmodule, _, _} = ast, {_behaviours, issues}, issue_meta) do
    {behaviours, module_issues} = scan_module(ast, issue_meta)
    {ast, {behaviours, issues ++ module_issues}}
  end
  ...
  defp known_callbacks(behaviours) do
    well_known = %{
      GenServer => ~w(init handle_call handle_cast handle_info terminate code_change handle_continue)a,
      Supervisor => ~w(init)a,
