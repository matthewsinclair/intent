...
**The next action is the release cut** (`bin/release --minor`; interactive; NEVER `--no-confirm`; **if it aborts after the sidecar commit, the `--skip-tests` recovery skips the only full-suite run at HEAD -- run the suite yourself before re-running, or accept a tag certified by per-deck runs only**; it stamps all five sidecars and dates the CHANGELOG at the cut -- VERSION/config are deliberately still 2.18.0 until then).
...
**Suite green as of `2769c40` (hv-run, 2026-08-14); 0021/0022/0023 postdate it and are verified only against their own decks. Nothing pushed; no tag.** `bin/release` re-runs `intent doctor` + the full suite as its own pre-flight, so the cut is self-certifying -- and it refuses to tag if ANYTHING outside its five sidecars is dirty, another node's whiteboard board included.
