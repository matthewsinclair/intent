...
2. **MEASURE HOW FAR THE SHIPPED PAIR IS BEHIND THE TREE, BEFORE BELIEVING ANY BEHAVIOUR.** `intent --version` for the marker, then `git rev-list --count <marker>..HEAD -- native/rust surface`. A node standing on a stale binary sees the OLD behaviour for everything landed since it was built. **Non-zero is the hazard; zero is the ordinary case and is not a reason to skip the check next time.**
