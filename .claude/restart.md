...
**v2.19.0 shipped the morning of 2026-08-14 (tag `071c612`); ST0056 opened the same afternoon.** The architecture is ratified in `intent/st/ST0056/design.md` (D01-D21): schema-as-truth (Rust types generate JSON Schema + DDL + SDL faces, committed and drift-checked), committed JSON canon as durable truth, rebuildable per-project SQLite (`rm` always safe, no DB migrations ever), md as generated views + authored prose, `intentsvcs` as sole owner of DB and files, CLI dual-mode (in-process or GraphQL to one machine-level intentd), intentd IN the 3.0.0 gate, migration floored at v2.19.0.
