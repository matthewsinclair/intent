...
- **All THREE bindings moved**, and the symlink was the weakest of them: `INTENT_HOME=~/Devel/prj/Intentv2`, `~/.local/bin/intent -> Intentv2/bin/intent`, and `$INTENT_HOME/bin` on PATH. **`bin/intent:26` is `if [ -z "$INTENT_HOME" ]`, so the exported var BEATS symlink resolution** -- repointing the symlink alone would have returned 0 and moved nothing.
