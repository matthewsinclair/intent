...
- **The pre-commit gate checks staged bytes against the formatters a project declares, and rewrites nothing** (issue 0505). A project that lists its formatters in `intent/.config/config.json` (any of `markdown`, `elixir` and `rust`, eg `"formatters": ["markdown", "rust"]`) gets a shipped guard that runs each formatter's check over the STAGED bytes of every matching file. It refuses the commit by listing the files and the command that formats them, and it rewrites nothing. A formatter name it does not know is refused too, rather than passed over, and a declared formatter whose tool is not on PATH is reported as unenforced and blocks nothing. Estates had each hand-wired a formatter into their own hook, and a hook that formats the worktree and re-stages it commits bytes nobody staged. `intent/docs/pre-commit-hook.md` describes the declaration. The whiteboard header guard's refusal stops saying that a formatter runs before it.
...
- **A guide to the explorer, `docs/explorer.md`, teaches `intent explore` by task, with screenshots taken from the explorer itself.** It leaves the keys and commands to the explorer's own `/help`, which is built from the keymap and the command table of the build it runs in, so the page cannot fall behind the build. The generator that takes the screenshots is committed beside the reference generator, so each release re-takes them.

### Changed

- **The documentation is re-measured against this release, page by page.** Every claim a page makes about a verb, a flag, a path, an output line or an exit code was checked against the code, and every page that shows commands was run in a scratch project. Where a page and the code disagreed, the page moved to the code; where the code was wrong, the defect is filed as an issue and the page describes what the build does. `docs/getting-started.md` teaches `intent set` for a thread's objective and context, and `docs/concepts/working-in-a-team.md` becomes a walkthrough of a team on GitHub: clones, pull requests under each of GitHub's three merge buttons, the id collisions two clones can make, the whiteboard across clones, and `intent doctor` as a required check. The docs index links the explorer guide and the team guide from Start here.
- **Command help and the reference pages say what each command does, without the maintainers' notes.** Argument notes and help texts had gathered rulings, internal paths and design argument over the v3 port. They are rewritten for a reader of `--help`, of `docs/reference` and of the MCP tool descriptions.
- **Intent's hooks need `jq`, and `docs/install.md` now says so.** Without it the Claude Code session gate blocks every prompt, `/in-session` included, and the pre-commit gate runs no language critic, which it says on each commit.
- **A correction to the 3.1.0 entry.** It said v3 embeds the rule library in the binary. It does not: Intent reads the library from its install at run time, as `docs/install.md` describes.
- **A principle's links to the rules that concretise it agree in all three fields that hold them** (issue 0584). A language rule's `references:`, a principle's `concretised_by:` and a rule's `principles:` each carried the mapping and disagreed in places, so a reader that ranks findings by a principle's `concretised_by:` missed rules that cite it. `references:` is now the home of record, each principle's `concretised_by:` is its exact inverse, and `principles:` names every principle a rule references. `intent claude rules validate` does not yet compare the fields.

### Fixed

- **A `brew upgrade` no longer leaves every gated commit refused** (issue 0527). `intent bootstrap` recorded the install root it resolved, which on a Homebrew install is the versioned keg, `<prefix>/Cellar/intent/<version>/libexec`. `brew upgrade` deletes that keg in its cleanup, so the pointer named nothing and the project shim refused every commit in a gated project until `intent bootstrap` was run again. It now records `<prefix>/opt/intent/libexec`, Homebrew's link to whichever version is current, whenever that link resolves to the same keg, so the pointer survives every upgrade; any other root, a source checkout included, is recorded as before. A machine upgrading from 3.2.0 or earlier still holds the old pointer, and the formula's caveat now tells it to run `intent bootstrap` once, whether or not the old keg is still on disk, and names `intent bootstrap --check` (issue 0540). `intent claude upgrade` compares the pointer with the running install by what the two paths resolve to, so the `opt` path and its keg no longer read as two installs. intentd's LaunchAgent records the `bin` link it was started through, which an upgrade does not delete.
- **`intent/todo.md` opens with its generator marker again, so a tool that keeps its own todo.md can tell Intent's view apart** (issue 0528). v2's `intent todo` opened the view with frontmatter naming it, `generator: intent todo`, and v3 dropped it at the port. Utilz's `todo` refuses to overwrite a todo.md whose frontmatter names another generator and takes one with none for its own, so, run beside Intent's view, it rewrote the view. The view opens with v2's frontmatter again, byte for byte, and `intent todo` prints it too, because the terminal view and the file are one generator. A todo view an older v3 wrote is an advisory stale render in `intent doctor`, named under `--verbose` and blocking nothing, until `intent todo update` rewrites that one view, as the next command that writes it also does; a marker naming any other generator is still skew.
- **A multi-line board item stays one item: in its board view, and under `intent wb show` and `intent wb pickup`** (issue 0532). A board view printed an item's lines after its first at column 0, so they left the list item: a sub-bullet read as a sibling item, and an item holding a fenced block could turn every section below it into code. They are now set in under the item's `- `, and `wb show` and `wb pickup` print them set in under the item's own line; a one-line item prints as it did. A board an older Intent wrote in the old shape is an uncounted stale render in `intent doctor`, named under `--verbose` and blocking no commit, until the next board write by any node, the next thread or issue write, or `intent sync --to-disk` re-renders it. A write that re-renders such a board does not report it as a lost edit (issue 0539).
- **A fresh clone takes its whiteboard on its first open, and no remedy empties a board** (issue 0535). A clone's store was filled from the committed files with every thread and issue and no board, because only `intent sync --to-store` carried a `board.json` in. `intent wb status` then said no nodes were registered and named `intent wb register`. From there, `intent sync --to-disk` or `intent wb migrate` wrote an empty board over every node's `board.json`, each at rc 0. The first open now takes the boards in the same transaction as the threads and issues, and a store holding a board counts as filled. Where a store lacks a board that a `board.json` records as migrated, `wb register`, `wb migrate` and `sync --to-disk` refuse before they write anything. `wb status` and `wb show` name `intent sync --to-store`, and `intent doctor` reports the state.
- **Every remedy that sends someone to register a node prints the whole command** (issue 0541). `intent wb status` with no node registered, and the refusal for a node that is not registered, still named the bare `intent wb register`. That form reads the roster from hand-authored board headers, and registers nothing where there are none. Both now print `intent wb register <moniker> --name "<display name>" --role <role>`, with the moniker filled in where one was typed. They name the bare form only for boards that are still hand-authored.
- **The explorer (`intent explore`) shows what changed under it** (issue 0520). It read every view from the model as it stood when it opened, so a write from another terminal, from another node or from intentd's ingest of a hand edit never reached it, not even on leaving a view and coming back. Neither did its own `/` commands, whose `ok` sat over a list that had not changed. An edit to a record somebody else had changed was refused -- `changed while this command was running -- nothing was written` -- every time until the explorer was restarted. Nothing was lost: that refusal is the store's own check, and it still judges an edit against the record the edit started from. The explorer now catches up with the store before it reads anything, and while nothing is being edited it re-reads the view on screen within half a second of a change once the keys are idle, keeping the cursor on the row it was on. The search pane re-runs when it is entered, as before, because its freshness line says how current its answer is. It follows the store: a file edited by hand under `intent/` shows once intentd's ingest takes it, or, with no daemon running, once `intent sync --to-store` brings it in. That is where every other verb sees such an edit too. The omnibox follows the store as well, so an entity a `/` command or another node has just made can be found in it by name (issue 0543).
...
- **`intent wb edit` in a project without git says that its searches did not run** (issue 0545). It printed that no file under `intent/` held the old text, at HEAD or in the next commit, in a project with no repository, where neither search had run because there is no HEAD and no next commit to search. It now says the project is not a git repository and that neither was searched, and claims nothing about either.
...
- **`intent st done` and `intent wp done` name every step out of an empty contract, and a refused close's `remedy:` line is the refusal's own** (issue 0526). A unit whose thread has no acceptance criteria, or whose criteria are all descoped or withdrawn, is refused, and the refusal named `intent ac new`, or the verbs that bring a criterion back, and stopped there. A criterion added is unsatisfied, and one brought back stays unsatisfied unless it is test-backed and its test is still green, so following it usually led to a second refusal. It now names the step after: satisfy the criterion, a non-test one with `intent ac satisfy --evidence <ref> <STID> <ACID>` and a test-backed one by covering it with `intent at new` and taking that test to red, then green, with `intent at red` and `intent at green`, or cancel the unit. `intent ac gate` and `intent ac status` print the same steps. The `remedy:` line under every refused close read "satisfy or formally descope the remaining criteria, then close again", which under an empty contract named criteria that did not exist. It now comes from the reason the gate refused, and keeps that text only where criteria remain unsatisfied.
...
- **`intent wb claim --help` and `intent wb unclaim --help` say what an `<id>` can be** (issue 0530). Since 0511 a board can claim an issue, and on the command line the forms were named only by the refusal a wrong one got: `--help` printed a bare `<ID>`. It now names a thread as `ST0000`, a work package as `ST0000/01`, or an issue as `ISSUE:0000`, the sentence the refusal prints and the MCP tools give as their `id`'s description, and the whiteboard skill, which still said a claim takes a thread or a work package, names all three.
...
- **Intent's output no longer sends a reader to Intent's own issue numbers** (issue 0542). A finding in `intent doctor`, a refusal on opening the store, two of the build's own warnings, two field descriptions in the schemas `intent schema` prints, one line the pre-commit gate prints, and comments in the hooks, skills and rules Intent installs each pointed at an issue in Intent's own tracker, which a reader in another project cannot open. Each now says its cause in words, and the test that keeps Intent's project-management state out of its output counts an issue number in all of those places.
- **The project shim's `--where` names an empty install pointer ABSENT, as `intent bootstrap --check` and the shim's own commit path already did** (issue 0547). A pointer file that exists and names nothing printed `root: <empty>` and `state: UNUSABLE (no lib/templates under that root)`, which sent a reader looking for a broken install root when none had ever been written. It now prints `state: ABSENT (the pointer file exists and is empty)`, and both answers exit 1 with `intent bootstrap` as the remedy. UNUSABLE is kept for a pointer that names a directory with no `lib/templates` under it.
- **A structural search says when it could not see a language** (issue 0548). `intent search` asked for definitions, a file's outline, a name's context or symbols by filter answered `complete: true` over files the index names no symbols in: a declared language this build has no grammar for, shell among them, and any script without an extension, which the index never parses. An agent told to fall back to grep only when an answer is incomplete therefore read "no definition" as the answer, which is also how a Highlander check concludes a name is free. Such an answer now says `complete: false` and names each language under `index.unindexed` with its reason, `no-grammar`, `no-tags-query` or `not-declared`, and an outline of a file with no language the index parses says `no-language`. The terminal prints the reason on stderr and the explorer's info row carries it. A text search, whose lexical tier reads every file's text, stays complete.
- **Three skills name the file LiveView's usage rules actually ship in.** `in-standards`, `in-elixir-essentials` and `in-phoenix-liveview` sent a reader to `deps/phoenix_live_view/usage-rules.md`, a file LiveView's package does not carry. LiveView's rules ship inside Phoenix's package, at `deps/phoenix/usage-rules/liveview.md`, which the three skills now name, and they say that a dependency's rules can live in `usage-rules.md`, in `usage-rules/*.md`, or in both. Installed copies follow at the next `intent claude skills sync`.
- **The pull-request workflow no longer runs a pull request's description as code** (issue 0585). It substituted the description into its shell script, where `$(...)`, a backticked span or an unbalanced quote ran as code or broke the check; the description now reaches the step through `env:`. The documentation and test-coverage checks now fetch `origin/main` and fail when their diff fails, where before they could never warn, and CI's prettier install fails when it installs no prettier.
- **The explorer's `/help` starts each meaning beside its name** (issue 0550). The name column took its width from the longest section heading, so in a 110-column terminal every meaning started near column 72 and the longer ones were cut. A heading now runs the row's width and no longer sets the column.
- **`intent explore` with an address it cannot open says why** (issue 0552). It landed at the top and wiped its own reason before the first frame; the reason now stays on the status row, as it does when the same address is typed into the explorer's box.
- **The explorer opens a work package by its address** (issue 0553). `intent explore intent:///threads/ST0001/wp/02`, the form Intent's own remedies print, opened an error row because the zero-padded `02` matched no row named `2`; it now opens work package 2.
- **`intent app status` names a route a release install can take** (issue 0560). With no app installed it sent every machine to `bin/devbin`, which only Intent's source tree has; it now gives the same error and remedy as the other `intent app` verbs.
- **The reopen remedy from `todo notdone` and `todo toggle` runs as printed** (issue 0567). It passed the reason positionally, which `st reopen` refuses; it now prints `intent st reopen <ID> --reason "<reason>"`.
- **`intent claude upgrade --apply` seeds `.intent_critic.yml` exactly as the template holds it** (issue 0564). It wrote a Markdown filler line after each of the template's comment lines, under `--force` as well, so a strict YAML reader rejected the file.
- **`intent doctor`'s gate-not-running finding says what happens to a commit and names the verb that repairs it** (issue 0570). It said commits were going through ungated and that no verb repairs the gate. A missing carrier REFUSES every commit and `intent claude upgrade --apply` writes it back; a carrier that runs no guards is the one state that lets commits through, and the same verb replaces it; a shim with no install pointer refuses every commit until `intent bootstrap`. And doctor run from a binary other than the one the pointer names no longer reports a gate that runs every guard as one that cannot run.
- **`intent doctor` names a store that git tracks, and `intent upgrade` names the command that untracks it** (issue 0551). A project made by Intent 3.0 committed its store, `intent/.cache/intent.db`, and the ignore rule a later run writes leaves it in the index; `git rm --cached intent/.cache/intent.db` takes it out and leaves the file on disk.
- **A pull that brings a teammate's board no longer writes this clone's board over it** (issue 0554, in part: a pulled board's rows are not yet carried into the store by the pull itself). The hooks' pass after a pull rewrote each pulled `board.json` from this clone's store, so a message a teammate had sent was deleted from the file with no line saying so. The hooks now keep a pulled board the store holds differently, name it on their `left:` line with `intent sync --to-store`, and every board write refuses until it is carried. A checkout of an older commit is kept and named the same way, and each surface says which version each choice keeps.
- **`intent doctor` names the verb that keeps a hand edit to a thread's Objective or Context** (issue 0556). Its finding named only `intent sync --to-disk`, which regenerates the cover and discards the edit; it now names `intent sync --to-store` first, which carries the edit into the store.
- **The hooks carry a hand edit to a thread's Objective or Context into the store after a pull, a checkout or a rebase** (the hooks' path of issue 0559; an edit to a cover `intent st edit` opened is still overwritten by the next CLI write, so write these sections with `intent set`). The pass read the edited cover, kept the store's copy of the thread because a cover edit moves no canon file, and then rewrote the cover from the store, discarding the edit with no line saying so. The pass now takes the edit and names the thread in the changes it took.
