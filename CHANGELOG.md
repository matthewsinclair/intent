...
- **`intent index rebuild` no longer declares a `--corpus` flag it never took.** The flag was declared from the design and never built, so `intent doctor --verbose` reported it as withheld on every run; the verb rebuilds every corpus, as it always has.
