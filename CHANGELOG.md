...
- **The runtime store migrates from schema 13 to 18 on first open, and neither step (13 -> 17, 17 -> 18) can be undone.** v3.0.0 refuses a migrated store. Take a snapshot with v3.0.0's `intent backup` before upgrading: copied back over `intent/.cache/intent.db` by hand, it is the only way back. See the release notes.
