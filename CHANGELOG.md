...
- **`intent explore <view>` opens the explorer at one of its views**: `threads`, `issues`, `projects`, `outstanding` (or `outs`), `help`, `settings` or `search`. It opens the view exactly as typing `/<view>` in the explorer's palette does, and an `intent://` address or an id still opens what it names.

### Changed

- **A generated view's footer names Intent's major version alone** (`Generated by Intent v3`), and so do the footers of `AGENTS.md` and `CLAUDE.md`. The full version rewrote every generated file in a project at every patch release for one character. A project's views take the new footer once, at their next render.
- **`intent index rebuild` no longer declares a `--corpus` flag it never took.** The flag was declared from the design and never built, so every `intent doctor` run not given `--quiet` reported it as withheld; the verb rebuilds every corpus, as it always has.

### Fixed

- **Carrying a hand edit to a thread's cover back into the store no longer folds the thread's body into its Context** (issue 0568). `intent sync --to-store` and intentd appended the body's opening text to the Context on each carry, so it compounded; an edit to that text, which the cover shows under `## Context` but which belongs to the body, is refused by name rather than carried.
- **`intent critic` no longer reports a rule as not asked when its tool declined only some of the run's files** (issue 0581). A `.zsh` file beside `.sh` files uncounted every shellcheck rule, so the census could read `0 of N rule(s) ASKED` over the findings those rules reported. The declined files are still named, and the JSON census gains `declined`.
- **`intent claude rules validate` refuses a rule library whose principle links disagree** (issue 0584). A rule's `references:`, the principle's `concretised_by:` and the rule's `principles:` must name each other, and each disagreement is reported on the file to edit. Four links whose rule bodies did not argue the principle are dropped.
- **The Claude Code session gate no longer blocks every prompt, `/in-session` included, when `jq` is not on PATH** (issue 0563). It reads whether the prompt opens with `/` from the raw payload, and says `jq` is missing when it blocks.
...
- **The opt-in PostToolUse critic advisory speaks only when the critic reports findings or refuses** (issue 0578). It handed the model a clean run's census on every edit; it now keys on the critic's exit status.
...
- **A pull takes a teammate's whiteboard into the store** (issue 0554). The hooks' `intent sync --apply` now carries a pulled board when the file moved forward and this clone's store has not changed that board since it last wrote or read it. A board both sides moved, and one put on disk by checking out an older commit, are still left and named, with `intent sync --to-store` as the step.
- **Two clones that each add a work package to one thread no longer have to take a side** (issue 0555). `intent sync --apply` renumbers this clone's package to the next free number, and a board claim on it follows. A thread changed in any other way on either side still takes a side.
- **`intent sync --to-store` names a hand edit to a thread's Objective or Context that it carries into the store**, where it used to say it overwrote nothing.
...
- **Migrating from v2 no longer refiles the first line of v2's acceptance preamble into the thread's preamble.** Two later lines of that preamble, which carry ids of Intent's own, are still refiled there and reported among the sections not carried as-is.
...
- **The reference no longer lists the live `help` and `organize` among the retired commands.**
