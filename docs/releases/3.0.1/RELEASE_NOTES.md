...
**The v2 shell implementation is gone from the repository**: `bin/intent` and the 25 `bin/intent_*` scripts. The v2 shell critic and hook launcher under `intent/plugins/claude/` (`lib/critic_runner.sh`, `lib/rules_lib.sh`, `bin/intent_claude_hook`) go with it; v3's critic and its `intent claude hook` launcher are built into the binary. This only affects someone running `bin/intent` from a checkout of the Intent repository. The Homebrew install never shipped any of it, and the installed `intent` is the v3 binary. `bin/int`, `bin/devbin` and `bin/.devbin/` stay. The repository's shell test suite now drives the v3 binary, and the tests whose only subject was the v2 shell went with it.
