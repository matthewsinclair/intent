...
**No status verb returns a row to `to-write`.** The status verbs are `at green`, `at red` and `at na`, and none of them spells `to-write` — a test enters there when `at new` creates it. The one other way in is a change of kind: when a row's status does not hold for its new kind, `at edit --kind` restarts it at that kind's entry state (`to-write` for a test, `n/a` for a non-test), its verdict goes with it, and the verb prints a note naming the row, the old status and the new one. That is recorded rather than fixed: inventing a verb so the diagram looks symmetrical is how a surface grows commands nobody asked for.
...
```
  ...
  lint: ST0001 FAILED -- 1 finding(s) over 1 of 1 AT row(s) examined
...
**Re-citing a row to a different file is `intent at edit ST0001 AT-03.1 --file <path>`**, which changes only the fields you name and keeps the row's note. `at new` on an id that already exists refuses and writes nothing.
