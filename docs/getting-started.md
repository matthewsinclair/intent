...
**`info.md` is a generated view of the thread, not its source.** `intent st edit ST0001 info --path` prints its path, which is where to read what you wrote; without `--path`, and run from a terminal, `st edit` opens the file in `$VISUAL` or `$EDITOR` instead. **Write the objective and the context with `intent set`, not by editing `info.md`.** A hand edit to those two sections is carried back into the store only in some cases, and the next Intent command that renders the file can replace it. `intent doctor` reports an edited view as skew.
