...
**A v3 thread carries `info.md` and `acceptance.md`, and nothing else.** `design`, `impl` and `tasks` were v2 files, and `st edit` refuses them — `intent st edit ST0001 design` answers `not a file this artefact carries` and names what the thread does carry. Creating `design.md` by hand does not change the answer, because the file set is read from the model rather than from the directory. The reasoning goes in `objective` and `context` above. **Be specific about constraints and about what you ruled out** — that is the information nobody can reconstruct later, and it is what an agent reading this file will act on. "Cache because the upstream API limits us to 100 req/min, and we see 10K req/s at peak" tells a reader something. "Add caching" does not.
