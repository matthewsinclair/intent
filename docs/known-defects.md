...
**`intent init` writes an Elixir/Phoenix decision tree into every project, whatever language you work in** (`intent#0224`). Driven on v3.0.0 in an empty directory: `intent init` leaves `intent/llm/DECISION_TREE.md` at 3 KB while the project's declared languages are `[]`. The file routes code placement through Phoenix contexts, LiveViews and Ash resources, so a Rust or Swift project gets guidance for a stack it does not use. Nothing declines it at init time and no flag suppresses it. Delete the file if it is not yours; nothing regenerates it, and **`init` will not put it back** -- it refuses on a project that already has one (`already an Intent project ... init refuses rather than merging`, exit 1), so the deletion stands unless you write the file yourself. Driven 2026-09-09 on both the published-tag-era release pair and a build at HEAD, with a fresh-directory control confirming `init` does write the file into an empty tree: **it is the RE-RUN that is impossible, not the write.** **v3.0.1 closes this**: `init` no longer writes `DECISION_TREE.md` at all, and a project that already has one keeps it.

**The `AGENTS.md` a shell project is given documents a test command that finds no tests** (`intent#0220`). Driven on v3.0.0: with `shell` declared, `intent agents sync` writes an `AGENTS.md` whose line 29 reads `bats tests/`. `bats` is not recursive, so a project whose suites live under `tests/unit/` gets `ERROR: Found no tests. (Try \`--allow-empty-suite\`?)`from the documented command, where`bats -r tests/`runs them.`AGENTS.md`is the first file the project tells an agent to read, and it is generated, so a correction written into it is overwritten at the next`intent agents sync`. No per-project override exists for a template value. Run `bats -r tests/`; the flag is correct for a flat layout too. **v3.0.1 ships `bats -r tests/`** in the template, so `intent agents sync` writes the command that runs a nested suite; this page's note is for v3.0.0 only.
...
**`intent at lint --fix` is advertised and refuses** (`intent#0139`). `at lint --help` documents it as _Migrate the mechanical part of a legacy row_; calling it exits non-zero without doing so. **v3.0.1 closes this by retiring the flag, not by building it**: it is gone from `at lint --help`, and passing it is refused at exit 1 with `unexpected argument '--fix'`. Fix a row `at lint` names by hand with `intent at edit <ST> <AT>`, on either build.
...
**A test file satisfies a citation whenever it contains the id anywhere, including under another thread's name** (`intent#0267`). AT ids are only locally unique -- `AT-01.1` exists in as many threads as you have -- and the check takes a path and a bare id. Driven on v3.0.0: a file whose sole mention is the comment `// ST0002 AT-01.1 -- this test witnesses thread B and nothing else`, cited from **ST0001**, gives `lint: ST0001 ok -- 1 AT row(s) conform` and `gate: ST0001 PASS -- 1/1 satisfied`. Strip the id and the lint goes red naming the row, so the check is running; it is matching a literal that does not belong to the thread asking. Qualify the id with its thread in the test's own text and the collision at least becomes visible to a reader, though not to the tool. **A build after v3.0.0 refuses it**: at least one line carrying the id must not name a different thread, and a line naming no thread at all -- the ordinary form -- still counts.
...
**And a row the check cannot read is counted as conforming** (`intent#0273`). The citation arms run only on a row carrying a structured `file`; a row whose address survived the v2 -> v3 migration in `legacy.raw` has none, so no arm looks at it -- and it is counted anyway, because the row total is incremented before the check is chosen. Driven on v3.0.0: two green rows on one thread, one citing a file that exists and one carrying a `legacy.raw` citation naming `t/ghost_test.rs`, which does not, and `intent at lint` answers `lint: ST0001 ok -- 2 AT row(s) conform` at exit 0. Delete the file the _readable_ row cites and the same run answers `cites a file that does not exist` and exits 1, so the arm is live and it is the population that is wrong. **`N AT row(s) conform` is the count of rows walked, not of rows examined**, and nothing in the output separates the two -- the failure line reports `over 2 AT row(s)` on the same footing. A thread authored under v3 carries structured citations throughout, so this bites migrated estates: read a green lint on a migrated thread as covering only those rows that carry a `file` in `intent/.canon/st/<ST>.json`. **A build after v3.0.0 states the population instead**: `N of M AT row(s) examined and conforming`, with the remainder partitioned by why -- and a row carrying a verdict that nothing examined is named apart from the two exemptions that are correct by design.
...
**A committed attachment whose bytes differ from canon is reported by neither warning surface** (`intent#0276`). One surface does not examine attachments; the other examines GIT STATE rather than content, so a file that has been edited **and committed** is clean by every question it asks. Driven on v3.0.0 with no daemon watching: attach a file, edit it, commit the edit, and `intent sync --to-store` answers `note: the store and the extract agree; this restore overwrites nothing` at exit 0 -- while the file on disk and the bytes in canon plainly disagree. The note is not lying about what it compared; **it compared two of the three places those bytes live**, and the file the attachment names is the one nothing looked at.

What happens to the divergence then depends on something the page cannot see: with `intentd` watching the tree, a disk edit is ingested into canon on its own; without it, as driven here, the disk bytes are simply ignored and canon keeps the old ones. **Neither outcome is announced, and they are opposite.** Read `the store and the extract agree` as a claim about the store and the extract only, and use `intent st attach <ST> <path> --from <file>` to make canon name the bytes you have.
...
## Driven again on 2026-09-09, and two patterns in the remedies themselves

**THE FIVE FORWARD CLAIMS ON THIS PAGE WERE PROMISES WHEN THEY WERE WRITTEN, AND THEY ARE NOW VERIFIED.** Each sentence marked _a build after v3.0.0_ or _v3.0.1 closes this_ was driven, with controls, in an isolated scratch project against a **debug build at `HEAD`** -- **not** against v3.0.0 and **not** against the release pair, which was itself behind the tree at the time. That distinction is the whole reason this section names its subject: a remedy verified against one build is a claim about that build.

- `intent#0119` -- `ac new` on a taken id now refuses at exit 1, and its remedy names `ac edit`.
- `intent#0267` -- a citation whose id appears only under another thread's name is now refused; an unqualified id, the ordinary form, still conforms. Both controls fired, and stripping the id refuses too, so the arm is live.
- `intent#0273` -- `at lint` now states its population: `N of M AT row(s) examined and conforming`, with the remainder partitioned by reason.
- `intent#0270` -- `at red` on an absent cited file is refused at exit 1, naming the file and the consequence.
- The accidental-daemon case -- confirmed from `intentd`'s own argument handling and from `intentd --help` returning exit 0 without serving. **NOT driven with an unrecognised argument, deliberately: doing so means performing the exact action this page warns takes every session on the machine down. A declared exclusion, not an untested claim.**

**AND THREE REMEDIES ON THIS PAGE NAME ACTIONS THE TOOL REFUSES TO PERFORM, WHICH IS THIS PAGE'S OWN SUBJECT TURNED ON ITSELF.** `intent#0224`'s _re-run `init`_ is refused on an initialised project. `intent#0063`'s printed remedy, _satisfy or formally descope the remaining criteria_, cannot be followed precisely because there are none -- which is why it refused. `intent#0227`'s _declare 'acceptance: exempt'_ names a state no verb writes; `exempt` appears nowhere in `ac`, `at` or `st` help. **Three is a class rather than a coincidence: a remedy is a claim, and nothing in this project drives them.** The first is corrected above; the other two are the tool's own output and are recorded as found.

**A REMEDY IS HARDER TO DRIVE THAN TO WRITE, AND THAT IS WHY THEY GO UNCHECKED.** Two of the drives behind this section reached a clean-looking answer without touching their subject at all: one gated a **non-test** criterion, so the gate never asked about the cited file it was supposed to be testing; another hit the work-package state machine before it could reach the descoped-criteria question. **Both were caught only because the output named its own population** -- `0 of 1 examined`, `not a legal transition`. An instrument that printed a verdict without a population would have scored both as passes.
