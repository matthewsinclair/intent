...
**Generated by `intent/st/ST0056/parity/tools/gen_cut_surface.sh` at 2026-09-17 17:49Z. Do not edit -- re-run it.**
...
Commands at `HEAD` that `v3.0.1` does NOT have -- **anything documented from these describes a tool nobody has installed**:
...
Commands `v3.0.1` has that `HEAD` does not -- **a v2-era reader may still type these**:
...
Flags at `HEAD` that `v3.0.1` does NOT have:

- `claude subagents` gains `--dry-run`
- `daemon logs` gains `--follow`
- `daemon logs` gains `--lines`
- `discover` gains `--depth`
- `index rebuild` gains `--corpus`
- `index rebuild` gains `--json`
- `index resolve` gains `--full`
- `index resolve` gains `--json`
- `index resolve` gains `--lang`
