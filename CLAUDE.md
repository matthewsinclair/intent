...
1. **Highlander Rule** -- search `intent/llm/MODULES.md` before creating any new module, helper, or template: `grep -n '<name>' intent/llm/MODULES.md`. **Search it, do not read it** -- the registry runs to hundreds of kilobytes over hundreds of rows, so "read it first" is not an instruction anyone can follow; `wc -c intent/llm/MODULES.md` is the command if you actually need the figure. **THIS LINE CARRIED `~354KB across ~367 rows` UNTIL 2026-09-09 AND WAS WRONG IN BOTH DIRECTIONS AT ONCE** -- driven, the file is 697,867 bytes over 305 table rows, so the byte count had nearly DOUBLED while the row count went DOWN, and no single drift explains both. **A size is a state, and this file is read by every node at every boot** -- the same defect the entry point carried until the same evening, where a stale line is multiplied by the number of nodes rather than divided by it. The sentence below already refuses to pin the VERB spelling here for precisely this reason; a size is no different. (`intent modules find` is the intended verb: **drive it, and fall back to the grep above if it does not answer.** Which spelling works is a property of the tool version you are standing on, and pinning that answer here would make this a second home for a fact the tool already reports about itself.)
